...
- **Web UI**：布局参考 DeepSeek Chat（左侧会话栏 + 居中对话 + 底部圆角输入；**助手**普通回复渲染 **Markdown**，含多级标题、列表、代码块与表格等，经净化后展示）；**首次发送**首条用户消息后，若标题仍为默认「新会话」且未事先重命名，侧栏标题会自动改为该条提问的截断摘要；左侧最近会话可**右键**导出 JSON/Markdown 或删除（与「管理会话」内一致）；聊天（新会话无消息时居中显示操作引导）、工作区浏览/编辑（侧栏可 **`GET /workspace/pick`** 在运行 serve 的机器上系统选目录并填入路径，再 **`POST /workspace`** 应用，须符合 `workspace_allowed_roots`；`GET /workspace/file` 可选 `encoding` 查询参数，语义与 `read_file` 一致，便于 GBK/GB18030 等 legacy 文本）、任务清单（进程内 `/tasks`，重启后清空）、状态栏；Agent 写入文件后，工作区列表会自动刷新。可选 **多角色**：配置 `[[agent_roles]]` 或 `config/agent_roles.toml` 后，新会话首条请求可带 `agent_role`（或 `default_agent_role` / `AGENT_DEFAULT_AGENT_ROLE`）；仓库默认附带 **`companion`（陪聊）**、**`scientist`（科学家向）**、**`engineer`（工程师向）**、**`philosopher`（哲学向）**、**`literary`（文学向）**、**`code_reviewer`（代码审阅）** 等角色，见 `config/agent_roles.toml` 与 `config/prompts/*_system_prompt.md`。详见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「多角色」。
