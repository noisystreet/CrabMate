...
- **Web UI**：聊天、工作区浏览/编辑（`GET /workspace/file` 可选 `encoding` 查询参数，语义与 `read_file` 一致，便于 GBK/GB18030 等 legacy 文本）、任务清单（进程内 `/tasks`，重启后清空）、状态栏；Agent 写入文件后，工作区列表会自动刷新。可选 **多角色**：配置 `[[agent_roles]]` 或 `config/agent_roles.toml` 后，新会话首条请求可带 `agent_role`（或 `default_agent_role` / `AGENT_DEFAULT_AGENT_ROLE`）；仓库默认附带 **`companion`（陪聊）**、**`scientist`（科学家向）**、**`engineer`（工程师向）**、**`philosopher`（哲学向）**、**`literary`（文学向）**、**`code_reviewer`（代码审阅）** 等角色，见 `config/agent_roles.toml` 与 `config/prompts/*_system_prompt.md`。详见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「多角色」。
