...
- **Web UI**：布局参考 DeepSeek Chat（左侧会话栏 + 居中对话 + 底部圆角输入；**助手**普通回复渲染 **Markdown**，含多级标题、列表、代码块与表格等，经净化后展示）；**首次发送**首条用户消息后，若标题仍为默认「新会话」且未事先重命名，侧栏标题会自动改为该条提问的截断摘要；左侧最近会话可**右键**导出 JSON/Markdown 或删除（与「管理会话」内一致）；聊天（新会话无消息时居中显示操作引导）、工作区浏览/编辑（侧栏可 **`GET /workspace/pick`** 在运行 serve 的机器上系统选目录并填入路径，再 **`POST /workspace`** 应用，须符合 `workspace_allowed_roots`；`GET /workspace/file` 可选 `encoding` 查询参数，语义与 `read_file` 一致，便于 GBK/GB18030 等 legacy 文本）、任务清单（进程内 `/tasks`，重启后清空）、状态栏；Agent 写入文件后，工作区列表会自动刷新；工作区卡片底部可打开**变更预览**弹层，经 **`GET /workspace/changelog`** 展示与 **`session_workspace_changelist`** 注入模型同源的 **unified diff** 摘要（弹层打开时若收到 `workspace_changed` 会自动刷新）。侧栏支持按标题**筛选会话**、在全部本地会话中**搜索消息**（内存扫描，最多展示 80 条命中，点击可跳转并滚动到该条）；主区提供**当前会话内查找**与上/下条导航；在消息区等（非输入框 / 查找框焦点）可按 **End** 滚至最新消息并恢复自动跟底；在**聊天消息区右键**打开菜单：可进入多选；主区搜索图标左侧亦有**多选**图标可切换多选。勾选后在右键菜单中**导出已选为 Markdown**（与整会话导出规则一致，按会话内时间顺序）。气泡上支持**复制**、**长回复折叠/展开**；对普通用户消息可选**从此处重试**（需服务端已持久化 `conversation_id` 且 SSE 已下发 `revision` 时同步 `POST /chat/branch`）与**分支对话**（截断本地与会话存储）。可选 **多角色**：配置 `[[agent_roles]]` 或 `config/agent_roles.toml` 后，新会话首条请求可带 `agent_role`（或 `default_agent_role` / `AGENT_DEFAULT_AGENT_ROLE`）；仓库默认附带 **`companion`（陪聊）**、**`scientist`（科学家向）**、**`engineer`（工程师向）**、**`philosopher`（哲学向）**、**`literary`（文学向）**、**`code_reviewer`（代码审阅）** 等角色，见 `config/agent_roles.toml` 与 `config/prompts/*_system_prompt.md`。详见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「多角色」。
