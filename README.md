...
- **调试与排障**：环境变量、日志、`doctor`、HTTP 探针、SSE 对齐等见 **[docs/DEBUG.md](docs/DEBUG.md)**（含 **`AGENT_WEB_DISABLE_MARKDOWN`**、**`AGENT_WEB_RAW_ASSISTANT_OUTPUT`** 与 **`GET /web-ui`**）；变量说明亦在 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「Web 服务」。可选 **`CRABMATE_TURN_REPLAY_DUMP_DIR`**（或 **`AGENT_TURN_REPLAY_DUMP_DIR`**）在每轮对话结束后**后台**写入 `turn-replay-*.json` 会话快照，与 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`** 的 Chrome trace 可同目录对照，详见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「整请求 Chrome Trace」表。
