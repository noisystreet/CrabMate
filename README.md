...
| **MiniMax** | `api_base` : `https://api.minimaxi.com/v1`；**`model`**（已测）：**`MiniMax-M2.7`**、**`MiniMax-M2.7-highspeed`**、**`MiniMax-M2.5`**；其它型号以控制台为准。嵌入配置默认 **`llm_fold_system_into_user = true`**，以避免部分线上接口返回 **`invalid message role: system`**（将系统提示并入 `user`）。若需将思维链与正文分开，可设 **`llm_reasoning_split = true`**（`AGENT_LLM_REASONING_SPLIT`）。说明与示例见 [`docs/CONFIGURATION.md`](docs/CONFIGURATION.md)「MiniMax」与 [MiniMax OpenAI 兼容](https://platform.minimaxi.com/docs/api-reference/text-openai-api)。 |
