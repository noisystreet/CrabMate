...
- **环境变量**：**`API_KEY`**（bearer 时；`serve` / `repl` / `chat` 可先启动，对话前在 Web「设置」或 REPL **`/api-key set`**）；**`models` / `probe`** 在 bearer 下通常仍需环境变量里的 Key。**`AGENT_API_BASE`** / **`AGENT_MODEL`** 覆盖配置。分阶段规划可选 **`AGENT_STAGED_PLAN_TWO_PHASE_NL_DISPLAY`**（或 TOML **`staged_plan_two_phase_nl_display`**）：首轮 JSON 定稿不向用户侧流式输出，再追加一轮自然语言（默认关闭）。完整表见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md#分阶段规划staged_plan_execution)。
