...
**前端**：执行 `cd frontend-leptos && trunk build`（输出 `frontend-leptos/dist`）后启动 `serve`。默认 **`trunk build`** 为开发构建（不跑 `wasm-opt`）；**发布或打包**请用 **`trunk build --release`** 以启用默认 `wasm-opt`、减小 WASM（见 **`frontend-leptos/README.md`** / **`docs/DEVELOPMENT.md`**）。
