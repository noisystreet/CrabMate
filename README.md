...
- **OpenAPI**：`serve` 暴露 **`GET /openapi.json`**（OpenAPI 3.0，`application/json`），汇总主要 HTTP 路径与 JSON 形状，便于 CI / 客户端生成代码；**`/chat/stream` 的 SSE 行协议**仍以 [docs/SSE_PROTOCOL.md](docs/SSE_PROTOCOL.md) 为准。官方 Web 会在 **`POST /chat`** / **`POST /chat/stream`** 请求体附带 **`client_sse_protocol`**，并与首帧 **`sse_capabilities`** 做版本核对（详见该文档「协议版本与协商」）。
...
- **进程内工具统计（可选）**：`agent_tool_stats_enabled`（或 `AGENT_TOOL_STATS_ENABLED`）为真时，在进程内按滑动窗口记录各工具完结的 `ok` / `error_code`（与 SSE 信封语义一致），并在**新会话**首条 `system` 末尾附加短 Markdown 提示；Web、`crabmate chat`、REPL 同源。已加载磁盘会话的延续对话仅用基底 system 刷新，不附加该段。键与范围见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)。
...
**配置**：`config/default_config.toml`、`config/session.toml`、`config/context_inject.toml`、`config/tools.toml`、`config/sandbox.toml`、`config/planning.toml`、`config/memory.toml`（编译嵌入）+ 可选 `config.toml`；默认通过 `system_prompt_file = "config/prompts/default_system_prompt.md"` 从仓库文件加载（**改该文件无需重编**；相对路径会按当前目录、配置文件所在目录、`run_command_working_dir` 依次解析，见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)）。**环境变量与高级项**同见该文档；**PER 终答规划**可选 **`final_plan_require_strict_workflow_node_coverage`**、**`final_plan_semantic_check_*`**（侧向 LLM，默认关，**额外计费**）。**子命令与 Benchmark** 见 [docs/CLI.md](docs/CLI.md)；**release 构建、`cargo deb`、`man` 页**见下文 **[「源码编译与打包」](#源码编译与打包)**。
...
- **Web E2E（可选）**：先 `cd frontend-leptos && trunk build`，再 `cd e2e && npm ci && npx playwright install chromium && npm test`（Playwright 会启动 `cargo run -- serve` 并对 `/chat/stream`、`/workspace` 使用桩响应；见 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md)「E2E」）。
