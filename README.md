...
- **内置工具**：文件、命令、HTTP、联网搜索、多语言开发辅助（含 **JVM**：Maven/Gradle；**容器**：`docker`/`podman`/`docker compose` 最小封装）等；**能力与 JSON 参数示例**见 [docs/TOOLS.md](docs/TOOLS.md)。另有结构化内置工具 **`gh_pr_*`（含 `gh_pr_diff`）、`gh_issue_*`、`gh_run_*`（含 `gh_run_view`）、`gh_release_*`、`gh_search`、`gh_api`**（见 [docs/TOOLS.md](docs/TOOLS.md)）。默认 `run_command` 白名单含 **GitHub CLI `gh`**（需本机安装并完成认证，参数规则同其他白名单命令：不得含 `..`、不得以 `/` 开头）；未安装时 **`GET /health`** 中 **`dep_gh`** 为 degraded，**`crabmate doctor`** 亦会提示。`cargo_test` / `npm run test` 以及部分 `run_command cargo test`：进程内可按「源码指纹 + 参数」复用上一次的截断输出，并标注 **缓存命中**（`test_result_cache_*`，见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)）。
...
- **Web UI**：布局参考 DeepSeek Chat（左侧会话栏 + 居中对话 + 底部圆角输入）；**首次发送**首条用户消息后，若标题仍为默认「新会话」且未事先重命名，侧栏标题会自动改为该条提问的截断摘要；左侧最近会话可**右键**导出 JSON/Markdown 或删除（与「管理会话」内一致）；聊天（新会话无消息时居中显示操作引导）、工作区浏览/编辑（侧栏可 **`GET /workspace/pick`** 在运行 serve 的机器上系统选目录并填入路径，再 **`POST /workspace`** 应用或恢复默认，须符合 `workspace_allowed_roots`；`GET /workspace/file` 可选 `encoding` 查询参数，语义与 `read_file` 一致，便于 GBK/GB18030 等 legacy 文本）、任务清单（进程内 `/tasks`，重启后清空）、状态栏；Agent 写入文件后，工作区列表会自动刷新。可选 **多角色**：配置 `[[agent_roles]]` 或 `config/agent_roles.toml` 后，新会话首条请求可带 `agent_role`（或 `default_agent_role` / `AGENT_DEFAULT_AGENT_ROLE`）；仓库默认附带 **`companion`（陪聊）**、**`scientist`（科学家向）**、**`engineer`（工程师向）**、**`philosopher`（哲学向）**、**`literary`（文学向）**、**`code_reviewer`（代码审阅）** 等角色，见 `config/agent_roles.toml` 与 `config/prompts/*_system_prompt.md`。详见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「多角色」。
...
| **MiniMax**     | `api_base`：`https://api.minimaxi.com/v1`；`model`（已测）：`MiniMax-M2.7`、`MiniMax-M2.7-highspeed`、`MiniMax-M2.5`；其它型号以控制台为准。线上常见 `invalid message role: system`，建议在 `[agent]` 中设 `llm_fold_system_into_user = true`（将系统提示并入 `user`；嵌入默认 `false`，与默认 DeepSeek 模型一致）。**未写 `llm_reasoning_split` 时，MiniMax 网关默认开启思维链分离**（等价 `reasoning_split: true`）；不需要时可显式 `llm_reasoning_split = false` 或 `AGENT_LLM_REASONING_SPLIT=0`。说明与示例见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)「MiniMax」与 [MiniMax OpenAI 兼容](https://platform.minimaxi.com/docs/api-reference/text-openai-api)。 |
...
- **环境变量**：`API_KEY` — 云端 OpenAI 兼容网关的 Bearer 密钥（`llm_http_auth_mode=bearer` 时；`doctor` / `save-session` 等除外）。`AGENT_API_BASE`、`AGENT_MODEL` — 分别覆盖配置中的 `api_base`、`model`，不配或暂不改 `config.toml` 时便于快速切换网关与模型 ID。可选 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`**：每轮对话（`run_agent_turn`）导出 Chrome trace **`turn-*.json`**；若与 **`CRABMATE_WORKFLOW_CHROME_TRACE_DIR`** 同开，工作流事件并入同一文件（详见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)）。完整 `AGENT_*` 表亦见该文档。
...
trunk build
cd ..
cargo run -- serve     # Web，默认 8080（读取 frontend-leptos/dist）
```
...
**前端**：执行 `cd frontend-leptos && trunk build`（输出 `frontend-leptos/dist`）后启动 `serve`。
...
- **Release 版本构建**：`cargo build --release`，二进制位于 `target/release/crabmate`。若以 `serve` 提供 Web UI，先执行 `cd frontend-leptos && trunk build`（输出 `frontend-leptos/dist`），再启动后端。
...
- 生成 **Debian `.deb` 包**：需安装 [cargo-deb](https://github.com/kornelski/cargo-deb)（`cargo install cargo-deb`），先执行 `cd frontend-leptos && trunk build`，再 `cargo build --release` 与 `cargo deb`；生成的包在 `target/debian/`，安装示例 `sudo dpkg -i target/debian/crabmate_*.deb`。包内附带默认配置片段、[README.md](README.md) 与 `/usr/share/man/man1/crabmate.1`。细则与路由表见 [docs/CLI.md](docs/CLI.md)「打包 Debian `.deb`」。
