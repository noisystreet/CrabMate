...
- **Web「设置」**：界面语言 / 主题 / 背景与本机 **`client_llm`**（`api_base` / `model` / `temperature` / 密钥）等修改后，需点击 **「保存全部」** 才写入浏览器 **`localStorage`** 并随请求生效；`api_base` 可从常用供应商预设中选择或手写，`temperature` 支持 `0~2`（留空使用服务端默认），详 [docs/配置说明.md](docs/配置说明.md)「Web 对话队列」。
