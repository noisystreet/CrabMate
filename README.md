...
- **内置多种工具**（工作区文件与命令、HTTP、联网搜索、多语言开发辅助等），由模型按需调用。**完整说明与 Function Calling 参数示例**见 [`docs/TOOLS.md`](docs/TOOLS.md)。
...
- **流式输出与状态栏**：
  ...
  - 终端 CLI（`cargo run` 默认交互/`--query`/`--stdin`）：仍走 SSE 收包；助手正文以**纯文本**随 delta 流式打印到 stdout（`reasoning_content` 与 `content` 与模型下发顺序一致），**不经** `markdown_to_ansi`。在 TTY 且未设置 **`NO_COLOR`** 时，**思考链**（`reasoning_content`）为**灰阶 + 暗淡**，**正文**（`content`，含规划 JSON 与自然语言终答）为终端默认前景色，便于区分。`--no-stream` 时整段到达后同样按字段分色一次写出。无 SSE 下行时（CLI 不传 `out`），**分阶段规划**（`staged_plan_execution`）与各**工具结果**会额外打印到 stdout，便于对照「写代码—编译—运行」等多步任务。
- **会话导出（Web）**：
...
- **新功能**：合并用户可见能力时，请同步更新本 README（功能/命令/配置）与/或 `DEVELOPMENT.md`（架构与协议）；**新增或变更内置工具**时同步 [`docs/TOOLS.md`](docs/TOOLS.md)（工具说明与参数示例）。
...
1. **环境变量**（优先级最高）  
  ...
  - `AGENT_TOOL_CALL_EXPLAIN_ENABLED`、`AGENT_TOOL_CALL_EXPLAIN_MIN_CHARS`、`AGENT_TOOL_CALL_EXPLAIN_MAX_CHARS`：可选「工具调用解释卡」——启用后，**非只读**内置工具（如 `run_command`、`run_executable`、写文件、`http_request`、git 写操作等）须在 JSON 顶层附带字符串 `crabmate_explain_why`（一句人话目的）；服务端执行前会剥离该字段，与审批互补（见 `[agent] tool_call_explain_*`）
   - **长期记忆（默认开启）**：`AGENT_LONG_TERM_MEMORY_ENABLED`（`0`/`false` 等可关闭）；`AGENT_LONG_TERM_MEMORY_SCOPE_MODE`（当前仅 `conversation`）；`AGENT_LONG_TERM_MEMORY_VECTOR_BACKEND`：默认 **`fastembed`**（本地 CPU 嵌入 + 余弦相似度；首次运行可能下载 ONNX 模型），可改为 **`disabled`**（仅按时间取最近片段、不建向量）；`qdrant` / `pgvector` 配置项保留但启动会报错（尚未接入）。另有 `AGENT_LONG_TERM_MEMORY_STORE_SQLITE_PATH`、`AGENT_LONG_TERM_MEMORY_TOP_K`、`AGENT_LONG_TERM_MEMORY_MAX_CHARS_PER_CHUNK`、`AGENT_LONG_TERM_MEMORY_MIN_CHARS_TO_INDEX`、`AGENT_LONG_TERM_MEMORY_ASYNC_INDEX`、`AGENT_LONG_TERM_MEMORY_MAX_ENTRIES`、`AGENT_LONG_TERM_MEMORY_INJECT_MAX_CHARS`。Web：**已配置 `conversation_store_sqlite_path` 时会话库与长期记忆共用同一 SQLite**；若会话仅内存模式，须显式设置 `long_term_memory_store_sqlite_path` 否则不持久化记忆。CLI：默认 `run_command_working_dir/.crabmate/long_term_memory.db`。`GET /status` 返回 `long_term_memory_*` 便于确认是否就绪。多用户无 Bearer 鉴权时，勿依赖 `conversation_id` 作为安全边界。
