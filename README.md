...
- **内置工具**：文件、命令、HTTP、联网搜索、多语言开发辅助（含 **JVM**：Maven/Gradle；**容器**：`docker`/`podman`/`docker compose` 最小封装）等；**能力与 JSON 参数示例**见 [docs/TOOLS.md](docs/TOOLS.md)。默认 `run_command` 白名单含 **GitHub CLI `gh`**（需本机安装并完成认证，参数规则同其他白名单命令：不得含 `..`、不得以 `/` 开头）；未安装时 **`GET /health`** 中 **`dep_gh`** 为 degraded，**`crabmate doctor`** 亦会提示。`cargo_test` / `npm run test` 以及部分 `run_command cargo test`：进程内可按「源码指纹 + 参数」复用上一次的截断输出，并标注 **缓存命中**（`test_result_cache_*`，见 [docs/CONFIGURATION.md](docs/CONFIGURATION.md)）。
