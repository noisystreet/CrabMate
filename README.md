...
- **可选 Cargo features**（根包 **`crabmate`**，默认 **`mcp` + `docker_sandbox`**，与完整产品一致）：裁剪 **`rmcp`**（MCP）或 **`bollard`**（Docker 沙盒）时用 **`cargo build --no-default-features`** 或 **`--no-default-features --features mcp`** 等；关闭 Docker feature 时**勿**在配置里使用 **`sync_default_tool_sandbox_mode = docker`**。维护者说明见 **`docs/DEVELOPMENT.md`** 与 **`docs/BACKEND_CORE_FRAMEWORK_DESIGN.md`**。
