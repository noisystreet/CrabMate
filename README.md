...
| --- | --- |
| **Install to PATH** | **`cargo install crabmate`** (crates.io **stable `0.5.1`**, default feature **`server`**). Git tag **`v0.5.1`** matches this package. Release tarball/`.deb` on GitHub Releases. Does **not** ship **man**; install **[man/crabmate.1](man/crabmate.1)** manually if needed. |
...
- **Single crate**: crates.io **stable** is **`0.5.1`** ([crates.io/crates/crabmate](https://crates.io/crates/crabmate), default **`server`**). **`cargo install crabmate`** installs that. Git tag **`v0.5.1`** matches this package. Official Client should pin **`version = "0.5.1", default-features = false, features = ["protocol"]`** (`crabmate::cm_sse_protocol`, `cm_types`, … — not `types`/`sse` aliases).
