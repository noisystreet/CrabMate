...
//! - **流式节流**：助手仍处于 **`is_loading`** 时，两次写入回答区 DOM 至少相隔约 **40–72ms**（动态检测帧耗时，见 **`adaptive_stream_interval`**），通过 **rAF** 逐帧检查节流条件，无需独立 **Timeout** 定时器；世代门禁防止尾随回调覆盖终态 Markdown。**终态**清空后一次 **`insertAdjacentHTML`** 追加，消除 `innerHTML` 全量重建抖动。
//!
