...
//! - **流式节流**：助手仍处于 **`is_loading`** 时，两次写入回答区 DOM 至少相隔约 **72ms**（常量见 **`md_answer_effect`**），高频 SSE 片段由尾随定时器刷新；世代门禁防止尾随回调覆盖终态 Markdown。**终态**在同一段同步逻辑内 **`innerHTML` 刷新**并清空队列，避免再等一帧或遭陈旧 rAF 干扰。
//!
