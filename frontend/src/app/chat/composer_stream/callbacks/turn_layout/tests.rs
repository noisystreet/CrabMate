...
fn empty_tool(id: &str, text: &str, tool_call_id: &str) -> StoredMessage {
    StoredMessage {
        id: id.into(),
        role: "system".into(),
        text: text.into(),
...
#[test]
fn pin_commentary_rows_moves_stray_assistant_before_tool() {
    let mut msgs = vec![
        empty_tool("t0", "archive", "tc_archive"),
        empty_tool("t1", "unpack", "tc_unpack"),
        StoredMessage {
            id: "c_stray".into(),
            role: "assistant".into(),
            text: "好的，先解压。".into(),
    ...
    pin_commentary_rows_before_anchored_tools(&mut msgs);
    assert_eq!(msgs.len(), 3);
    assert_eq!(msgs[0].id, "t0");
    assert_eq!(msgs[1].id, "c_stray");
    assert_eq!(msgs[2].id, "t1");
}

#[test]
fn sync_commentary_updates_existing_row_before_tool() {
    let mut msgs = vec![
        StoredMessage {
            id: "c".into(),
            role: "assistant".into(),
        ...
        empty_tool("t_new", "new", "tc_a"),
    ];
    sync_commentary_before_tool_in_messages(&mut msgs, "tc_a", "步骤 A。");
    assert_eq!(msgs.len(), 2);
    assert_eq!(msgs[0].text, "步骤 A。");
    assert_eq!(msgs[1].id, "t_new");
}

#[test]
fn loading_tail_empty_when_commentary_row_already_before_tool() {
    let mut msgs = vec![
        StoredMessage {
            id: "c".into(),
            role: "assistant".into(),
    ...
    let anchor = "tc_unpack";
    let mut block = "好的，先解压。".to_string();
    if let Some(tool_idx) = find_tool_index_latest(&msgs, anchor) {
        if find_commentary_before_tool_index(&msgs, anchor, tool_idx).is_some() {
            block.clear();
        }
    }
    sync_loading_tail_block_in_messages(&mut msgs, "a_load", block.as_str());
    assert!(
        msgs[2].text.is_empty(),
        "tail must not duplicate row already before tool"
    );
}
