...
//! # 会话绑定
//!
//! [`ChatStreamCallbackCtx::bound_stream_session_id`] 为 **发起 attach 时** 的快照（见 [`super::make_attach_chat_stream`]），并与 [`crate::chat_session_state::ChatSessionSignals::stream_transport`] 内 Bound 车道的 `session_id` 同步写入。
