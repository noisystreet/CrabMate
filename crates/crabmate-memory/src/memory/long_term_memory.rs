...
use crabmate_types::text_utils::preview_chars;
use crabmate_types::{
...
fn last_user_query_for_memory(messages: &[Message]) -> Option<&str> {
    for m in messages.iter().rev() {
        ...
        let c = crabmate_types::message_content_as_str(&m.content)?.trim();
        if c.is_empty() {
            continue;
        }
        return Some(c);
...
pub fn last_user_assistant_final_pair_for_turn(messages: &[Message]) -> Option<(&str, &str)> {
    ...
    while i > 0 {
        ...
        let ac = crabmate_types::message_content_as_str(&m.content)?.trim();
        if ac.is_empty() {
            continue;
        }
        let mut j = i;
        while j > 0 {
            ...
            let uc = crabmate_types::message_content_as_str(&u.content)?.trim();
            if uc.is_empty() {
                continue;
            }
            return Some((uc, ac));
