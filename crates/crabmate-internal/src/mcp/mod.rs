...
// 注意：不直接再导出 `try_open_session_and_tools`——`crabmate-mcp` 内实现只读 TOML 基础字段
// （`servers` 恒为空）；agent 回合必须走本模块的 user-data 感知版本。
#[cfg(feature = "mcp")]
pub use crabmate_mcp::{
    ...
    probe_mcp_server, server, try_open_turn_handle,
};
...
pub use crabmate_mcp::{
    ...
    try_open_turn_handle,
};

/// 按当前 `AgentConfig` **与 user-data `mcp_servers.json`** 解析并打开 MCP 回合句柄。
///
/// 与 `crabmate_mcp::try_open_session_and_tools` 不同：此处会加载 Web/CLI 设置页落盘的多服务器配置。
