...
pub const TABLE_FTS: &str = "crabmate_codebase_chunks_fts";
/// 供失效逻辑删除文件目录表（与 chunks 同步）。
pub const CODEBASE_SEMANTIC_FILES_TABLE: &str = TABLE_FILES;
...
pub fn open_codebase_semantic_db(path: &Path) -> Result<Connection, String> {
...
pub fn index_path_for_workspace(
    workspace_root: &Path,
    configured: &str,
