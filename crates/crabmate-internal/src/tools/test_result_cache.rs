...
pub fn wrap_cache_hit(fingerprint: &str, body: &str) -> String {
    format!("{CACHE_BANNER}{fingerprint}\n---\n{body}")
}

/// `cargo test` / `rust_test_one` 参数的稳定 JSON 摘要。
pub fn cargo_test_args_fingerprint(v: &serde_json::Value) -> String {
...
pub fn store_cached(enabled: bool, max_entries: usize, key: TestCacheKey, output: String) {
...
pub fn reset_test_result_cache_for_tests() {
    let mut g = CACHE.lock().unwrap_or_else(|e| e.into_inner());
    *g = None;
}
