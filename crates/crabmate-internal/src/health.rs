...
//!
//! 默认**不**请求上游 LLM；可选（配置 [`crabmate_config::AgentConfig::health_llm_models_probe`]）对当前 `api_base` 发起 **GET …/models**（与 `crabmate probe` 同源，无 chat/completions 计费），并带进程内缓存以降低探活频率。
...
///
/// 使用与 **`crabmate models` / `crabmate probe`** 相同的 [`crabmate_llm::fetch_models_report`]；**`bearer` 且无 `API_KEY`** 时跳过探测（检查项 `ok: true`，说明中标注跳过）。
...
async fn probe_llm_models_endpoint(
    ...
    match crabmate_llm::fetch_models_report(client, api_base, api_key, auth_mode).await {
        Ok(rep) => {
