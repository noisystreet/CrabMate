...
//! 在每次调用模型前注入一条 `user` 消息（见 [`crabmate_types::CRABMATE_WORKSPACE_CHANGELIST_NAME`]），减少大仓库下路径猜测。
...
use crabmate_types::CRABMATE_WORKSPACE_CHANGELIST_NAME;
...
pub fn sync_changelist_user_message(
    messages: &mut Vec<crabmate_types::Message>,
    ...
    messages.push(crabmate_types::Message {
        role: "user".to_string(),
...
fn strip_changelist_messages(messages: &mut Vec<crabmate_types::Message>) {
    messages.retain(|m| !crabmate_types::is_workspace_changelist_injection(m));
}

/// 持久化/返回客户端前移除变更集注入条（与长期记忆注入同理，由运行时每次刷新）。
pub fn strip_workspace_changelist_injections(messages: &mut Vec<crabmate_types::Message>) {
    messages.retain(|m| !crabmate_types::is_workspace_changelist_injection(m));
}
...
pub fn record_file_state_after_write(
    ...
    let after = match crate::workspace::path::absolutize_relative_under_root(working_dir, rel) {
...
mod tests {
    ...
    fn sync_appends_changelist_at_end() {
        ...
        let mut msgs = vec![
            crabmate_types::Message::system_only("sys"),
            crabmate_types::Message::user_only("hi"),
        ];
        ...
        assert!(
            crabmate_types::message_content_as_str(&msgs[2].content)
