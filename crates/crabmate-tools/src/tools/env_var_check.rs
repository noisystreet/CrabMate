...
fn format_one_env_var_line(
    raw_name: &str,
    sanitized: &str,
    val: Result<String, std::env::VarError>,
    show_length: bool,
    show_prefix: usize,
) -> String {
    if sanitized.is_empty() {
        return format!("  {}: (无效变量名)", raw_name);
    }
    match val {
        Ok(val) => {
            let mut info = if val.is_empty() {
                "已设置（空值）".to_string()
            } else {
                "已设置".to_string()
            };
...
pub fn run(args_json: &str) -> String {
    ...
    for name in &names {
        ...
        let val = std::env::var(&sanitized);
        if val.is_ok() {
            set_count += 1;
        }
        lines.push(format_one_env_var_line(
            name,
            &sanitized,
            val,
            show_length,
