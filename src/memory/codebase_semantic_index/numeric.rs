...
pub(crate) fn rust_symbol_hints_for_chunk(chunk: &str) -> String {
...
pub(crate) fn default_code_extensions() -> HashSet<&'static str> {
    [
...
pub(crate) fn chunk_text_lines(s: &str, max_chunk: usize) -> Vec<(usize, usize, String)> {
...
pub(crate) fn hash_chunk(rel_path: &str, body: &str) -> String {
...
pub(crate) fn f32_slice_to_bytes(v: &[f32]) -> Vec<u8> {
...
pub(crate) fn bytes_to_f32_slice(blob: &[u8]) -> Option<Vec<f32>> {
...
#[cfg(feature = "fastembed")]
pub(crate) fn ensure_embedder() -> Result<TextEmbedding, String> {
    TextEmbedding::try_new(TextInitOptions::new(EmbeddingModel::AllMiniLML6V2))
        .map_err(|e| format!("fastembed 初始化失败: {}", e))
}

pub(crate) fn rel_path_for_workspace(workspace_root: &Path, file: &Path) -> Option<String> {
...
pub(crate) fn posix_subdir_prefix_for_delete(sub: &str) -> Option<String> {
    let s = sub.replace('\\', "/");
...
pub(crate) fn sqlite_like_escape(s: &str) -> String {
...
pub(crate) fn fts5_match_expression(query: &str) -> Option<String> {
...
pub(crate) fn norm_scores_bm25(scores: &[(i64, f64)]) -> HashMap<i64, f32> {
    if scores.is_empty() {
        return HashMap::new();
    }
