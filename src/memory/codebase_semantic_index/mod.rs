...
pub(super) const TABLE: &str = "crabmate_codebase_chunks";
const TABLE_FILES: &str = "crabmate_codebase_files";
/// FTS5 虚拟表（`content=` 指向 [`TABLE`]，rowid = `id`）。
pub(super) const TABLE_FTS: &str = "crabmate_codebase_chunks_fts";
...
pub(super) fn bytes_to_f32_slice(blob: &[u8]) -> Option<Vec<f32>> {
...
#[cfg(feature = "fastembed")]
pub(super) fn ensure_embedder() -> Result<TextEmbedding, String> {
...
pub(super) fn fts5_match_expression(query: &str) -> Option<String> {
...
pub(super) fn norm_scores_bm25(scores: &[(i64, f64)]) -> HashMap<i64, f32> {
    if scores.is_empty() {
        return HashMap::new();
    }
...
fn scan_rebuild_files(p: ScanRebuildFilesParams<'_>) -> Result<RebuildScanOutcome, String> {
    ...
    for entry in walker {
        ...
        if incremental
            && !subtree
            && let Some((sz, mt, sh)) = catalog.get(&rel)
            && *sz == size
            && *mt == mtime_ns
            && *sh == sha_hex
        {
