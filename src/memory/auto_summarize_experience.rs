...
fn last_non_injection_user_index(messages: &[Message]) -> Option<usize> {
    for (i, m) in messages.iter().enumerate().rev() {
        ...
        if crate::types::is_long_term_memory_injection(m)
            || crate::types::is_workspace_changelist_injection(m)
        {
            continue;
        }
...
mod tests {
    ...
    fn tool_msg(name: &str, body: &str) -> Message {
        Message {
            role: "tool".to_string(),
            content: Some(crate::types::MessageContent::Text(body.to_string())),
