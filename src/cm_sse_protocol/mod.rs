...
//! - **`sse`**：控制面 JSON 协议与编码器；**`server` feature** 才含广播中枢、mpsc 桥与审批桥。
//! - WASM / 外仓分类器：`--no-default-features --features protocol`（不要 `tokio`）。
//!
//! 与 Cargo semver、发版标签的关系：见 **`docs/design/client_contract_versioning.md`**。
//! 单包 crates.io：见 **`docs/design/crates_io_single_package.md`**。
