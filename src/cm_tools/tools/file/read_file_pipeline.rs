...
use crate::cm_tools::text_encoding::{
...
impl ReadFileBodyError {
    pub(super) fn into_tool_error(self) -> crate::cm_tools::tool_result::ToolError {
        match self {
            Self::Utf8Decode(msg) => {
                crate::cm_tools::tool_result::ToolError::external_code("read_file_utf8_decode", msg)
            }
            Self::InvalidRange(msg) => {
                crate::cm_tools::tool_result::ToolError::external_code("read_file_invalid_range", msg)
            }
            Self::Internal(msg) => {
                crate::cm_tools::tool_result::ToolError::internal_code("read_file_internal", msg)
            }
            Self::Io(msg) => crate::cm_tools::tool_result::ToolError::external_code("read_file_io", msg),
        }
    }
}
...
pub(super) fn guard_read_file_count_total_size(
    ...
    if count_total && meta_len > READ_FILE_COUNT_TOTAL_MAX_BYTES {
        return Err(crate::cm_tools::tool_result::ToolError::external_code(
...
pub(super) fn sniff_opened_file_encoding(
    file: File,
    enc_name: TextEncodingName,
) -> Result<(ResolvedTextEncoding, DecodedFileNote, File, Vec<u8>), crate::cm_tools::tool_result::ToolError> {
    let (file, head) = sniff_head_bytes_from(file, enc_name)
        .map_err(|e| crate::cm_tools::tool_result::ToolError::external_code("read_file_io", e))?;
    let (resolved, decode_note) = resolve_text_encoding(&head, enc_name)
        .map_err(|e| crate::cm_tools::tool_result::ToolError::external_code("read_file_encoding", e))?;
...
pub(super) fn maybe_count_total_lines_for_read(
    ...
    target: &Path,
    enc_name: TextEncodingName,
) -> Result<Option<usize>, crate::cm_tools::tool_result::ToolError> {
    if !count_total {
        return Ok(None);
    }
