...
fn is_strict_secret_env(name: &str) -> bool {
    ...
    if matches!(
        u.as_str(),
        "API_KEY" | "CM_WEB_SEARCH_API_KEY" | "CM_SYSTEM_PROMPT"
    ) {
        return true;
    }
...
pub fn capture_trimmed(cmd: &str, args: &[&str]) -> Option<String> {
...
pub fn diagnostic_summary(args_json: &str, working_dir: &Path, extra_lines: &[String]) -> String {
    ...
    if inc_tc {
        ...
        if let Some(s) = capture_trimmed("rustc", &["-V"]) {
            ...
            if let Some(msg) = crate::cm_tools::health_dep_compat::validate_dep_version(
                crate::cm_tools::health_dep_compat::DepCompatKind::Rustc,
                &s,
            ) {
                out.push_str(&format!(
        ...
        if let Some(s) = capture_trimmed("cargo", &["-V"]) {
            ...
            if let Some(msg) = crate::cm_tools::health_dep_compat::validate_dep_version(
                crate::cm_tools::health_dep_compat::DepCompatKind::Cargo,
                &s,
            ) {
                out.push_str(&format!(
