...
use crate::cm_tools::tool_result::ToolFailureCategory;
...
fn test_run_tool_run_command_pwd() {
    crate::cm_tools::tools::command::reset_run_command_rate_limit_for_tests();
    let allowed = test_allowed_commands();
    let ctx = test_ctx(&allowed);
...
fn test_run_tool_call_graph_sketch_smoke() {
    ...
    let out = run_tool(
        "call_graph_sketch",
        r#"{"symbols":["run_tool"],"path":"src/cm_tools/tools","max_edges":80}"#,
        &ctx,
    );
    assert!(
    ...
    assert!(
        ...
        "应对 src/cm_tools/tools 下 run_tool 有命中或明细，得到: {}",
        out.chars().take(400).collect::<String>()
    );
}
...
fn test_build_tools_names() {
    ...
    assert!(names.contains(&"gh_pr_merge"));
    assert!(names.contains(&"gh_pr_review"));
    assert!(names.contains(&"gh_pr_comment"));
    assert!(names.contains(&"gh_pr_body_draft"));
    assert!(names.contains(&"gh_issue_list"));
    assert!(names.contains(&"gh_issue_view"));
    assert!(names.contains(&"gh_issue_create"));
    ...
    assert!(names.contains(&"gh_run_rerun"));
    assert!(names.contains(&"gh_run_failure_summary"));
    assert!(names.contains(&"gh_release_list"));
    assert!(names.contains(&"gh_release_view"));
    assert!(names.contains(&"gh_release_create"));
...
fn test_summarize_read_file_range_normalizes_inverted_order() {
    let s = summarize_tool_call(
        "read_file",
        r#"{"path":"frontend/src/api/mod.rs","start_line":419,"end_line":150}"#,
    )
    .expect("summary");
    assert!(
        s.contains("[150-419]"),
