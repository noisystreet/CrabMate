...
ToolSpec {
        ...
        ToolSpec {
            name: "http_fetch",
            description: "对 **http/https** URL 发起 **GET**（默认）或 **HEAD**。GET 返回状态、Content-Type、**重定向链**与正文（按配置截断）；HEAD 不下载 body，仅元数据与重定向链，省流量。可选 **`text_format: html_text`**：将 HTML 文档页转为纯文本（去 script/style，优先 main/article，**scraper/html5ever**）；非 HTML 仍返回解码原文。`charset` / meta / 嗅探解码见工具说明。**Web**：URL 匹配 `http_fetch_allowed_prefixes` 的同源 + 路径前缀边界时直接执行；未匹配时经 SSE 审批（`approval_session_id`）。运维 CLI 无同进程终端审批。GET/HEAD 共用白名单键 `http_fetch:<归一化URL>`。勿在 URL 中放真实密钥。`workflow_execute` 节点内仅白名单 URL 可成功。",
            category: ToolCategory::Basic,
        ...
        ToolSpec {
            name: "http_request",
            description: "对 **http/https** URL 发起 **POST/PUT/PATCH/DELETE**（可选 JSON body）。响应正文可选 **`text_format: html_text`**（与 `http_fetch` 相同）。URL 匹配 `http_fetch_allowed_prefixes` 的同源 + 路径前缀边界时直接执行；**Web（流式 + 审批会话）**下未匹配时可经 SSE 人工审批（与 `http_fetch` 同套；运维 CLI 无同进程终端审批；**永久允许**键为 `http_request:<METHOD>:<归一化URL>`，与 GET/HEAD 的 `http_fetch:` 键区分）。响应含状态、Content-Type、重定向链与正文（按配置截断）。**`workflow_execute` 节点内**仍仅白名单 URL。默认建议 dry-run 先验证，勿在 body 中放真实密钥。",
            category: ToolCategory::Basic,
