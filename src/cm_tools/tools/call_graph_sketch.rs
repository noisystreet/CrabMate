...
use crate::cm_tools::tools::tool_param_types::CallGraphSketchArgs;
...
fn rel_path_under_workspace(workspace_root: &Path, abs: &Path) -> String {
    let Ok(base) = crate::cm_tools::tools::file::canonical_workspace_root(workspace_root) else {
...
fn push_first_reference_edge(
    ...
    for (sym, re) in symbol_res {
        ...
        if crate::cm_tools::tools::symbol::line_looks_like_rust_definition(trimmed, sym, false) {
            continue;
        }
        edges.push(Edge {
            file_rel: rel_path.to_string(),
...
mod tests {
    ...
    fn use_clause_as_rename() {
        assert!(use_clause_touches_symbol("crate::cm_tools::api::Thing as T", "Thing"));
    }
}
