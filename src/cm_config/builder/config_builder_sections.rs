//! [`super::ConfigBuilder`] 的组合式子结构（与 [`crate::cm_config::config::types::AgentConfig`] 域对齐，仍为 `Option`/原始累加形态）。

use std::collections::HashMap;

use crate::cm_config::agent_roles;
use crate::cm_config::source::ScheduledAgentTaskRow;
...
pub(crate) struct ConfigBuilderRolesPrompts {
    ...
    pub(crate) default_session_mode: Option<String>,
}
...
pub(crate) struct ConfigBuilder {
    ...
    /// `id -> 未合并条目`；在 [`crate::cm_config::config::finalize`] 中与全局 cursor rules 设置一并落成 `AgentConfig.roles_prompts.agent_roles`。
