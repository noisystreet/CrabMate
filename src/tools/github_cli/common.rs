...
pub(crate) const DEFAULT_LIST_LIMIT: u32 = 30;
pub(crate) const MAX_SEARCH_LIMIT: u32 = 100;
pub(crate) const MAX_SEARCH_QUERY_BYTES: usize = 400;
pub(crate) const MAX_RELEASE_TAG_LEN: usize = 200;
pub(crate) const MAX_JOB_NAME_LEN: usize = 128;
pub(crate) const MAX_PR_TITLE_BYTES: usize = 240;
pub(crate) const MAX_PR_BODY_BYTES: usize = 65_536;
pub(crate) const MAX_PR_REF_TOKEN_BYTES: usize = 200;
...
pub(crate) fn validate_extra_args(args: &[String]) -> Result<(), String> {
    for a in args {
        if !is_safe_token(a) {
            return Err(format!(
...
pub(crate) fn gh_allowed(allowed: &[String]) -> Result<(), String> {
...
pub(crate) fn join_json_fields(fields: &[String]) -> Result<String, String> {
    if fields.is_empty() {
...
pub(crate) fn clamp_limit(n: Option<u32>) -> u32 {
    n.unwrap_or(DEFAULT_LIST_LIMIT).clamp(1, MAX_LIMIT)
}

pub(crate) fn try_pretty_json(stdout: &str) -> Option<String> {
    let t = stdout.trim();
    if t.is_empty() {
        return None;
    }
...
pub(crate) fn wrap_with_parsed(raw: String, stdout: &str) -> String {
...
pub(crate) fn extract_stdout_from_formatted(out: &str) -> &str {
...
pub(crate) fn attach_json_if_exit_zero(formatted: String, stdout_raw: &str) -> String {
...
pub(crate) fn validate_search_query(q: &str) -> Result<(), String> {
    let t = q.trim();
    if t.is_empty() {
...
pub(crate) fn validate_pr_title(title: &str) -> Result<(), String> {
    let t = title.trim();
    if t.is_empty() {
...
pub(crate) fn validate_pr_body(body: &str) -> Result<(), String> {
...
pub(crate) fn validate_pr_ref_token(token: &str) -> Result<(), String> {
    let t = token.trim();
    if t.is_empty() {
