...
subtasks：若用户输入包含多个独立子任务，列出各子任务描述（如 ["重构 auth 模块", "添加单元测试"]）；单任务或无法拆分时为空数组。
relation：subtasks 非空时必填。"parallel"=子任务可并行执行，"sequential"=子任务有先后顺序。
...
{{{{
  ...
  "abstain": false,
  "subtasks": ["string"],
  "relation": "parallel|sequential|null"
}}}}
"#,
...
fn parse_l2_response_json(raw: &str) -> Option<L2IntentCandidate> {
    ...
    let relation = match parsed.relation.as_deref() {
        Some("parallel") => Some(IntentRelation::Parallel),
        Some("sequential") => Some(IntentRelation::Sequential),
        _ => None,
    };
