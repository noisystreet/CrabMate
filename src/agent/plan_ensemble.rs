...
pub(crate) fn ensemble_secondary_planner_user_body(
    ...
    format!(
        "{coach}\n\
         ...
         每一步须对齐用户**原始**表述的意图与粒度，勿静默把宽泛任务替换成单文件深层 bugfix（除非用户原文已限定）。\n\
...
pub(crate) fn ensemble_merge_planner_user_body(plans: &[AgentReplyPlanV1]) -> String {
    ...
    format!(
        "{}\n\
         ...
         - 合并结果须与用户**原始**诉求对齐：若用户仅要概览/只读分析，勿合并成「单文件深层修复」式路径，除非各草案与用户授权一致。\n\
         - 覆盖全部必要子目标，去掉明显重复；理清依赖：**先读后写**、**先分析后改**。\n\
         - 可合并彼此无依赖的只读探查到更少步（一步内可多次工具调用）。\n\
         - 为每步分配**新的**非空 `id`（可带合理前缀如 `m1-`）；若某来源方案为步设置了 `executor_kind`（子代理分角色），合并后的对应步**应保留**该字段（勿无故清空）。\n\
