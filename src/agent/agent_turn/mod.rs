...
//!
//! 子模块：`intent`、`plan`（P）、`reflect`（R）、`execute`（E，实现见 **`execute/tools`**）、`messages`、`staged_sse`（实现见 **`staged/sse`**）、`params`、`outer_loop`、`staged`（含 **`staged/orchestrator`**、**`staged/patch_planner`**、**`staged/ensemble_fsm`** 等）。
//!
