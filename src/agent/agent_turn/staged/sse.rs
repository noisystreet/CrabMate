...
pub(crate) fn staged_plan_phase_instruction_default() -> String {
    format!(
         ...
         **`review_readonly` 步**：`acceptance` 仅用于文档/源码等只读路径的 `expect_file_exists`；**禁止**在本步验收可执行二进制（如 `bin/<可执行体>`、`target/release/<crate>`）或 `expect_exit_code`——构建/运行产物须放在后续 `test_runner` 步。\n\
