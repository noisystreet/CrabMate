...
//!
//! **命名**：此处相位为 **`StagedRoundOrchestratorPhase`**，与 **`turn_fsm::StagedTurnPhase`**（滚动视界外层：`PreStepExecutionRound` / `AfterStepExecutionRound`）**刻意区分**，避免阅读源码与设计稿时混淆。
...
/// 定稿后的规划已进入 **步执行队列**：已向 Web/终端发出 `staged_plan_started` 与首条 `notice`。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum StagedRoundOrchestratorPhase {
    StepsExecuting,
}
...
pub(crate) async fn enter_steps_executing(
    ...
    .await;
    StagedRoundOrchestratorPhase::StepsExecuting
}
