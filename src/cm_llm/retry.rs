...
use crate::cm_types::{ChatRequest, LlmConfig, Message};
use log::{debug, error, info};

use crate::cm_llm::backend::ChatCompletionsBackend;
use crate::cm_llm::call_error::{llm_call_error_http_status, llm_call_error_retryable};
use crate::cm_llm::chat_params::StreamChatParams;
use crate::cm_llm::complete_error::LlmCompleteError;
use crate::cm_llm::retry_hooks::{LlmRetryDecisionPoint, LlmRetryHooks};
...
fn on_llm_attempt_ok(
    ...
    info!(
        ...
        request.model,
        obs.total_t0.elapsed().as_millis(),
        obs.attempt
    );
    ...
    hooks.append_turn_replay_json(
        ...
        request.model.as_str(),
        Some(serde_json::json!({
            "llm_call_id": obs.llm_call_id,
            "model": request.model,
            "finish_reason": finish_reason,
            "attempt": obs.attempt,
            "max_attempts": obs.max_attempts,
            "attempt_elapsed_ms": obs.attempt_t0.elapsed().as_millis(),
            "total_elapsed_ms": obs.total_t0.elapsed().as_millis(),
...
enum RetryOutcome {
    ...
    Stop(LlmCompleteError),
}

fn on_llm_attempt_err(
    hooks: &dyn LlmRetryHooks,
    cfg: &LlmConfig,
    obs: &AttemptObs<'_>,
    ...
    let can_backoff = obs.attempt < obs.max_attempts && retryable;
    let backoff_ms = if can_backoff {
        cfg.http_retry
            .api_retry_delay_secs
            .saturating_mul(2_u64.saturating_pow(obs.attempt.saturating_sub(1)))
            .saturating_mul(1000)
    } else {
        0
    };
    ...
    if can_backoff {
        ...
        RetryOutcome::Stop(LlmCompleteError::from_boxed(e))
    }
}
    ...
    for attempt in 0..max_attempts {
        ...
        match p.llm_backend.stream_chat(&p.stream, &mut req).await {
            Ok((msg, finish_reason)) => {
                last_ok = Some(on_llm_attempt_ok(hooks, request, &obs, msg, finish_reason));
                break;
            }
            Err(e) => match on_llm_attempt_err(hooks, p.cfg, &obs, e) {
                RetryOutcome::Retry { delay_secs } => {
                    info!(
                        target: "crabmate",
                ...
                RetryOutcome::Stop(err) => return Err(err),
            },
        }
    }
    last_ok.ok_or_else(|| {
