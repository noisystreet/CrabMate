...
use crate::cm_types::{StreamChoice, StreamDelta};
...
pub(super) struct IngestSseState<'a> {
    ...
    /// SSE 末尾帧提取的 usage。
    pub(super) usage: &'a mut Option<crate::cm_types::Usage>,
}
...
pub(super) async fn ingest_sse_data_payload(
    ...
    let Ok(chunk) = serde_json::from_slice::<crate::cm_types::StreamChunk>(payload.as_bytes())
    else {
        return Ok(());
    };
    // 提取 choices（可能有）
...
pub(super) struct SseStreamAccum {
    ...
    pub(super) usage: Option<crate::cm_types::Usage>,
}
