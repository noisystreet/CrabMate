...
impl Completer for ReplSlashCompleter {
    fn complete(&mut self, line: &str, pos: usize) -> Vec<Suggestion> {
        ...
        match tail.split_once(char::is_whitespace) {
            None => repl_slash_complete::complete_slash_no_whitespace_tail(span, tail),
            Some((cmd, after_ws)) => {
                repl_slash_complete::complete_slash_after_whitespace(span, cmd.trim(), after_ws)
            }
        }
    }
}
