...
/// 单次从正文中解析出的工具调用（与 [`crate::types::ToolCall`] 解耦，便于单测）。
...
pub(crate) fn text_looks_like_dsml(content: &str, reasoning: &str) -> bool {
    [content, reasoning]
        .iter()
        .any(|s| s.contains("DSML") || s.contains("dsml") || s.contains(DSML_OPEN_FW))
}

pub(crate) fn combine_assistant_text_fields(content: &str, reasoning: &str) -> String {
    if content.is_empty() {
        reasoning.to_string()
...
/// 纯函数：从 `content` + `reasoning` 拼接文本解析 DSML，不写 [`crate::types::Message`]。
