...
pub struct AgentConfig {
    ...
    /// 为 true 时：`POST /chat/stream` 经控制面下发结构化 **`thinking_trace`**（推理增量、终答阶段、工具前后上下文摘要等），供 Web「调试台」展示。**默认 true**；**不**从 `[agent]` TOML 读入，可用 **`CM_THINKING_TRACE_ENABLED=0`** 关闭。
    ...
    /// Web 写副作用工具审计：`target=crabmate::audit_write_tool`（无密钥明文）。
    pub web_audit_log_write_tools: bool,
    /// 审计 `client_ip` 是否信任 `X-Forwarded-For` 首跳（仅可信反向代理后启用）。
    ...
    pub tool_registry_sub_agent_patch_write_extra_tools:
        Option<std::sync::Arc<std::collections::HashSet<String>>>,
    /// `executor_kind: test_runner` 在默认测试运行器与 `run_command` 之外额外允许的工具。
