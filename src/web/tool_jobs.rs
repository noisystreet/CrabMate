...
use crate::cm_internal::tool_jobs::registry::{CancelOutcome, GetOutcome};
use crate::cm_internal::tool_jobs::types::{JobRecord, JobStatus};
use crate::web::http_types::chat::ApiError;
use crate::web::http_types::tool_jobs::{ToolJobCancelResponseBody, ToolJobStatusResponseBody};
...
fn check_workspace_ownership(rec: &JobRecord, headers: &HeaderMap) -> Result<(), ApiErr> {
    ...
    let rec_root = normalize_workspace_root(&rec.workspace.to_string_lossy());
...
pub(crate) async fn tool_job_status_handler(
    ...
    check_workspace_ownership(&rec, &headers)?;
...
pub(crate) async fn tool_job_cancel_handler(
    ...
    if let Err((status, Json(api))) = check_workspace_ownership(&rec, &headers) {
        return Err((
            status,
...
mod tests {
    ...
    fn ownership_passes_without_header_or_matching_root() {
        ...
        assert!(check_workspace_ownership(&rec, &HeaderMap::new()).is_ok());
        assert!(check_workspace_ownership(&rec, &headers_with("/home/user/project")).is_ok());
        // 尾斜杠与空白容忍。
        assert!(check_workspace_ownership(&rec, &headers_with(" /home/user/project/ ")).is_ok());
        // 空头等同未提供。
        assert!(check_workspace_ownership(&rec, &headers_with("  ")).is_ok());
    }

    #[test]
    fn ownership_rejects_mismatched_root() {
        let rec = record("/home/user/project");
        assert!(check_workspace_ownership(&rec, &headers_with("/home/user/other")).is_err());
        assert!(check_workspace_ownership(&rec, &headers_with("/home/user/project/sub")).is_err());
    }
