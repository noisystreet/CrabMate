...
pub(super) fn openapi_paths_fragment_tool_jobs() -> Value {
    json!({
        ...
        "/tools/jobs/{tool_job_id}/output": {
            "get": {
                "tags": ["tool_jobs"],
                "summary": "轮询后台工具任务实时输出（增量游标；tail -f 语义）",
                "security": [{ "bearerAuth": [] }, { "apiKeyAuth": [] }],
                "parameters": [
                    {
                        "name": "tool_job_id",
                    ...
                    },
                    {
                        ...
                        "in": "query",
                        "required": false,
                        "description": "上次响应返回的游标；省略/非法值从最早可用起（可能 truncated）",
                ...
                "responses": {
                    "200": {
                        "description": "输出增量快照（含 status/cursor/truncated/eof）",
                        "content": {
                            "application/json": {
                                "schema": { "$ref": "#/components/schemas/ToolJobOutputResponseBody" }
                            }
                        }
                    },
                    "403": { "description": "X-Workspace-Root 与任务归属不符（JOB_OWNERSHIP_MISMATCH）" },
