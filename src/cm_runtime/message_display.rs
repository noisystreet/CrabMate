...
use crate::cm_runtime::latex_unicode::latex_math_to_unicode;
use crate::cm_runtime::message_display_parts::tool_display_from_normalized_envelope;
use crate::cm_agent::plan_artifact::{
    format_agent_reply_plan_for_display, strip_agent_reply_plan_fence_blocks_for_display,
};
use crate::cm_tools::tool_result::{ToolResult, normalize_tool_message_content};
use crate::cm_types::Message;

/// 工具结果中「原始输出」块的 Markdown 小标题（与 Web `ChatPanel`、CLI 完整回显一致）。
pub use crate::cm_runtime::message_display_parts::TOOL_OUTPUT_SECTION_HEADLINE;
...
pub fn tool_content_for_display_for_message(
    ...
    crate::cm_tools::tools::summarize_tool_call(&name, &args)
...
pub fn assistant_streaming_plain_concat(m: &Message) -> String {
    ...
    if let Some(c) = crate::cm_types::message_content_as_str(&m.content) {
        s.push_str(c);
    }
    s
}
...
pub fn assistant_markdown_source_for_message(m: &Message) -> String {
    let raw = assistant_raw_markdown_body_from_parts(
        ...
        crate::cm_types::message_content_as_str(&m.content).unwrap_or(""),
    );
...
fn preprocess_unfenced_assistant_prose_dedup(stripped: &str) -> String {
    ...
    if let Some(idx) = stripped.find("```") {
        ...
        let pre_deduped = crate::cm_agent::text_sanitize::dedupe_plain_assistant_preamble(pre);
        format!("{pre_deduped}{from_fence}")
    } else {
        crate::cm_agent::text_sanitize::dedupe_plain_assistant_preamble(stripped)
    }
}
...
mod tests {
    ...
    fn tool_for_message_prepends_summary_from_assistant_tool_calls() {
        ...
        let raw = crate::cm_types::message_content_as_str(&messages[2].content).unwrap();
