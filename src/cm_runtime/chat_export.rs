//! 会话导出落盘：复用 [`crate::cm_chat_export`] 的 schema / Markdown；本模块写
...
/// CLI/TUI 默认中文标题；助手正文经 [`crate::cm_runtime::message_display::assistant_raw_markdown_body_for_message`]。
fn messages_to_markdown(messages: &[Message]) -> String {
    crate::cm_chat_export::messages_to_markdown_with_body(messages, ExportMdLocale::ZhHans, |m| {
        if m.role == "assistant" {
            crate::cm_runtime::message_display::assistant_raw_markdown_body_for_message(m)
        } else {
...
pub fn messages_to_display_export(messages: &[Message]) -> Vec<DisplayExportMessage> {
    ...
    for (idx, m) in messages.iter().enumerate() {
        ...
        let content = match m.role.as_str() {
            "assistant" => {
                let body = crate::cm_runtime::message_display::assistant_markdown_source_for_message(m);
                let t = body.trim();
                if t.is_empty() {
                    None
                } else {
            ...
            "user" => {
                ...
                let shown = crate::cm_runtime::message_display::user_message_for_chat_display(&plain);
                let t = shown.trim();
                if t.is_empty() {
                    None
                } else {
            ...
            "tool" => {
                let body = if let Some(raw) = message_content_as_str(&m.content) {
                    crate::cm_runtime::message_display::tool_content_for_display_for_message(raw, messages, idx)
                } else {
...
mod tests {
    use super::*;
    use crate::cm_chat_export::messages_to_markdown_with_body;
