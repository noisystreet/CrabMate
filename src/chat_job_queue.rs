...
struct StreamJobOutcomeCtx<'a> {
    r: Result<(), crate::agent::agent_turn::RunAgentTurnError>,
    cancelled_by_signal: bool,
    queue_deps: &'a WebChatQueueDeps,
    sse_tx: &'a mpsc::Sender<String>,
    job_id: u64,
    messages: &'a mut Vec<Message>,
    cfg_snap: &'a Arc<AgentConfig>,
    app: &'a AppState,
    conversation_id: &'a str,
    expected_revision: Option<u64>,
    request_agent_role: Option<&'a str>,
    persisted_active_agent_role: Option<&'a str>,
    stream_ended_sent: &'a mut bool,
}

async fn stream_job_outcome_after_agent_turn(
