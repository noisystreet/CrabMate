...
use crate::cm_internal::process_handles::TurnProcessHandles;
use crate::cm_internal::read_file_turn_cache::ReadFileTurnCache;
use crate::cm_internal::workspace::changelist::WorkspaceChangelist;
use crate::cm_config::AgentConfig;
use crate::cm_types::Tool;

pub struct ToolsForTurnPrepared {
    pub tools_for_turn: Vec<Tool>,
    pub mcp_turn: Option<crate::cm_internal::mcp::McpTurnHandle>,
    /// 本轮尝试连接但失败的 MCP 服务器（供终端/SSE 提示）。
    pub mcp_skipped: Vec<crate::cm_internal::mcp::McpServerSkipInfo>,
}
...
pub fn resolve_read_file_turn_cache_for_turn(
    ...
    match read_file_turn_cache {
        ...
        None if cfg.chat_queues_cache.read_file_turn_cache_max_entries > 0 => {
            Some(crate::cm_internal::read_file_turn_cache::new_turn_cache_handle(
...
pub async fn prepare_tools_for_turn(
    ...
    tools_for_turn = crate::cm_internal::mcp::merge_tool_lists(
        tools_for_turn,
        crate::cm_internal::dynamic_tools::load_dynamic_tools(effective_working_dir),
    );
    let open = crate::cm_internal::mcp::try_open_session_and_tools(cfg.as_ref()).await;
    let mcp_skipped = open.skipped;
    let mcp_turn = match (open.handle, open.tools) {
        (Some(handle), extra) => {
            tools_for_turn = crate::cm_internal::mcp::merge_tool_lists(tools_for_turn, extra);
            Some(handle)
        }
        (None, _) => None,
    };
    ...
    if let Some(allow) = turn_allowed_tool_names {
        tools_for_turn.retain(|t| {
            crate::cm_tools::tool_naming::tool_name_allowed_by_turn_allowlist(
                t.function.name.as_str(),
                Some(allow),
            )
        });
