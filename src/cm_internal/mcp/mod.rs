...
pub use crate::cm_mcp::resolve::classify_mcp_connect_error;

use crate::cm_config::AgentConfig;

// `crabmate-mcp` 在 `mcp` feature 关闭时只导出 stub 类型/函数；开启后才有完整实现。
// 此处显式列出导出项，避免 `pub use crate::cm_mcp::*` 的 resolve 模块与外层冲突。
// MCP 回合打开入口在本模块（加载 user-data）；勿使用已删除的 `crate::cm_mcp` 恒空桩。
// agent 回合必须走本模块的 user-data 感知版本。
#[cfg(feature = "mcp")]
pub use crate::cm_mcp::{
