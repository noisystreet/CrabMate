...
- **Cargo features / slim binaries**: defaults **`web` + `repl` + `tui`**; **`mcp`**, **`fastembed`**, and **`docker_sandbox`** are opt-in. Examples: `cargo build --features mcp`, `cargo build --features fastembed`, or `cargo build --all-features`. See root **`Cargo.toml`** **`[features]`** and **`AGENTS.md`**.
...
- **Web API**: embedded default **`web_api_require_bearer = false`**—**`serve`** may start without a shared secret; with **`true`**, a non-empty **`CM_WEB_API_BEARER_TOKEN`** (or TOML **`web_api_bearer_token`**) is required before start. When the token is non-empty, the Bearer layer is mounted; send **`Authorization: Bearer …`** or **`X-API-Key: …`**. **The browser does not pick up the env var / XDG automatically**: open Web **Settings → “Web API shared secret (Bearer)”**, enter the **same** value as the server, and save (stored in-page and in **`localStorage`** key **`crabmate-api-bearer-token`**). **Do not** confuse this with the LLM **`API_KEY`**. If the first screen still shows 401 after saving, refresh and retry. **Temporary skip for local testing**: `unset CM_WEB_API_BEARER_TOKEN` (and clear the TOML key), bind **`127.0.0.1`**; or clear the secret and set **`CM_ALLOW_INSECURE_NO_AUTH_FOR_NON_LOOPBACK=true`** before **`0.0.0.0`** (trusted networks only). For exposed or untrusted networks, prefer **`web_api_require_bearer = true`** plus a configured secret. See [docs/en/CONFIGURATION.md](docs/en/CONFIGURATION.md).
- **Other**: Web sidebar **Settings** needs **Save all** to persist via **`/user-data`**; workspace must stay under allowed roots (path checks: [docs/en/CONFIGURATION.md](docs/en/CONFIGURATION.md)). Debug env vars and **`GET /web-ui`**: [docs/en/DEBUG.md](docs/en/DEBUG.md).
