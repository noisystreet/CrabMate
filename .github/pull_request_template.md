...
- [ ] 若改动 **`config/default_config.toml`**、**`config/session.toml`**、**`config/context_inject.toml`**、**`config/tools.toml`**、**`config/sandbox.toml`**、**`config/planning.toml`**、**`config/memory.toml`**、**`config.toml.example`**、**`CM_*` 环境变量** 或 Axum 路由契约：已更新 **`README.md`** 与 **`docs/DEVELOPMENT.md`**
