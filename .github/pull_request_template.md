...
- [ ] 若改动 **`frontend/`**：已运行 **`cd frontend && npx tsc -b --noEmit`**（大改时 **`npm run build`**）
- [ ] 若改动聊天 / SSE / **`frontend/src/api.ts`**：已核对双端协议一致（见 **`.cursor/rules/api-sse-chat-protocol.mdc`**）
- [ ] 若改动 **`default_config.toml`**、**`config.toml.example`**、**`AGENT_*` 环境变量** 或 Axum 路由契约：已更新 **`README.md`** 与 **`docs/DEVELOPMENT.md`**
