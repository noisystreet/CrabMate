...
struct TextWriter<'a, I, S: StyleSheet> {
    ...
    /// Syntect theme name to use for fenced code blocks.
    #[cfg(feature = "highlight-code")]
    code_theme: String,
    ...
    S: StyleSheet,
{
    ...
    fn text(&mut self, text: CowStr<'a>) {
        if self.in_table {
            // Keep tables compact: collapse newlines into single spaces.
            let s = text.replace('\n', " ");
    ...
    fn code(&mut self, code: CowStr<'a>) {
        if self.in_table {
            self.table_cell_buf.push_str(code.as_ref());
            return;
        }
    ...
    fn set_code_highlighter(&mut self, lang: &str) {
        if let Some(syntax) = SYNTAX_SET.find_syntax_by_token(lang) {
            ...
            let theme = THEME_SET
                ...
                .expect("default theme exists in syntect ThemeSet");
    ...
    fn end_table_cell(&mut self) {
        if self.in_table {
            let cell = self.table_cell_buf.trim().to_string();
            self.table_row.push(cell);
            self.table_cell_buf.clear();
        }
    }

    fn render_compact_table(&mut self) {
        ...
        // Build output lines first to avoid mutable/immutable borrow conflicts.
        let rows = self.table_rows.clone();
        let alignments = self.table_alignments.clone();
        let mut rendered: Vec<String> = Vec::with_capacity(rows.len().saturating_mul(2));
        ...
        for (ri, row) in rows.iter().enumerate() {
            ...
            if ri == 0 {
                ...
                for ci in 0..cols {
                    let w = widths[ci].max(1);
                    sep.push(' ');
                    sep.extend(std::iter::repeat('-').take(w));
                    sep.push(' ');
                    sep.push('|');
                }
                rendered.push(sep);
