...
- **Backend + Web UI**: `cargo run -- serve` is **API-only by default** (no SPA) and can start **without** `API_KEY` when using default `bearer`. Host Client UI with **`serve --with-web`** + **`CM_WEB_STATIC_DIR`**, then set the key in the Web sidebar (**API 密钥** → `client_llm`) or export `API_KEY`. Example: `API_KEY="..." cargo run -- serve` (default port 8080, binds **127.0.0.1** only). For LAN access use `serve --host 0.0.0.0` (see README). Legacy `cargo run -- --serve` still works. Optional global `--log /path/to.log` appends logs and mirrors to stderr. Without `RUST_LOG`, `serve` defaults to **info**; `bench` / `config` / `save-session` (alias `export-session`) and other non-serve subcommands default to **warn** unless you set `RUST_LOG` or `--log`. **`POST /config/reload`** (same auth as protected APIs) hot-reloads most `AgentConfig` fields without restarting `serve`; see **`docs/配置说明.md`** (**`API_KEY` 不因热重载从环境重读**；Web 密钥仍走 `client_llm`)。
...
- Before `serve` with UI: build Client UI (`cd ../crabmate-client && make frontend`; clone [crabmate-client](https://github.com/noisystreet/crabmate-client) as a sibling first), set `CM_WEB_STATIC_DIR` to that `frontend/dist`, and run **`serve --with-web`**. Default **`serve`** is API-only (no SPA).
