...
- Optional hardening (now default via embedded config): **`web_api_require_bearer`** defaults to **`true`**; set **`CM_WEB_API_BEARER_TOKEN`** / **`web_api_bearer_token`** before **`serve`** or disable **`web_api_require_bearer`** only for trusted local debugging. The Web UI can send the same secret from `localStorage["crabmate-api-bearer-token"]` (see README).
