...
- Optional hardening: set **`web_api_require_bearer=true`** and a non-empty **`CM_WEB_API_BEARER_TOKEN`** / **`web_api_bearer_token`** before **`serve`** so startup fails without a shared secret; embedded default is **`web_api_require_bearer=false`** (UI can load without server-side token, but protected routes stay open until you configure a secret—see auth middleware). The Web UI can load the same secret from the system keyring account **`web_api_bearer`** (via `/user-data/secrets`; see README).
