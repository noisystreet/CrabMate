...
- Before `serve` with UI: build Client UI (`cd ../crabmate-client && make frontend`) and set `CM_WEB_STATIC_DIR` to that `frontend/dist`, or use `serve --no-web` for API-only.
