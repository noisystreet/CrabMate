...
- **E2E tests** (Playwright): the system may have `http_proxy` / `https_proxy` set, which interferes with the local backend at `127.0.0.1:8080`. Run Playwright tests with `no_proxy=127.0.0.1,localhost` or start the E2E runner in an environment without proxy vars: `cd e2e && no_proxy=127.0.0.1,localhost npx playwright test`.
...
- Optional hardening: set **`web_api_require_bearer=true`** and a non-empty **`CM_WEB_API_BEARER_TOKEN`** / **`web_api_bearer_token`** before **`serve`** so startup fails without a shared secret; embedded default is **`web_api_require_bearer=false`** (UI can load without server-side token, but protected routes stay open until you configure a secret—see auth middleware). The browser must also save the **same** shared secret under Web **Settings → “Web API shared secret (Bearer)”** (in-page + **`localStorage`** key **`crabmate-api-bearer-token`**); this is **not** the LLM **`API_KEY`**. See **`README.md`** / **`docs/配置说明.md`**.
