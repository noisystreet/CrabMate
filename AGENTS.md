...
- **思维链少复述系统提示**：默认开启（**`thinking_avoid_echo_system_prompt`** / **`AGENT_THINKING_AVOID_ECHO_SYSTEM_PROMPT`**）；附录正文默认来自 **`config/prompts/thinking_avoid_echo_appendix.md`**（可改盘内 Markdown），亦可 TOML 内联 **`thinking_avoid_echo_appendix`** 或环境变量 **`AGENT_THINKING_AVOID_ECHO_APPENDIX*`**。见 **`docs/CONFIGURATION.md`**。
