...
- **Default (`llm_http_auth_mode = bearer` in config)**：`API_KEY` is the cloud vendor Bearer token. **`serve` / `repl` / `chat` / `tui` / `bench` 可在未设置 `API_KEY` 时启动**；实际调用 `chat/completions` 前须有关密钥，优先级：**环境变量 `API_KEY`** → **系统钥匙串**（与 Web/桌面侧栏保存同源）→ **REPL/TUI `/api-key set <密钥>`**。默认 `/api-key set` 持久化到钥匙串；`--no-persist` 才仅本进程。旧 `$XDG_DATA_HOME/crabmate/secrets/client_llm` / `executor_llm` 与保存模型中的明文 key 在钥匙串写入成功后自动迁移删除。交互式 CLI 另与桌面对齐：无 **`--config`** 且 cwd 无本地覆盖时采用 **`$XDG_CONFIG_HOME/crabmate/config.toml`**（可种子/回退 `/etc`），并合并 **`llm_overrides.json`** 与 **`prefs.json`** 的 **`cm_role`**（**不**自动打开 **`last_workspace_root`**；须 **`--workspace`** 或手动选择）。**`crabmate models` / `crabmate probe`** 在 `bearer` 下仍要求环境变量 **`API_KEY`** 非空。 **`config`（`cargo run -- config` / `--dry-run`）、`doctor`、`web-bearer` 与 `save-session`（别名 `export-session`）不要求 `API_KEY`**。
...
- **Backend + Web UI**: `cargo run -- serve` is **API-only by default** (no SPA) and can start **without** `API_KEY` when using default `bearer`. Host Client UI with **`serve --with-web`** + **`CM_WEB_STATIC_DIR`**, then set the key in the Web sidebar (**API 密钥** → `client_llm`) or export `API_KEY`. Example: `API_KEY="..." cargo run -- serve` (default port 8080, binds **127.0.0.1** only). For LAN access use `serve --host 0.0.0.0` (see README). Legacy `cargo run -- --serve` still works. Optional global `--log /path/to.log` appends logs and mirrors to stderr. Without `RUST_LOG`, `serve` defaults to **info**; `repl` / `chat` / `tui` / `bench` / `config` / `save-session` (alias `export-session`) default to **warn** unless you set `RUST_LOG` or `--log`. **`POST /config/reload`** (same auth as protected APIs) hot-reloads most `AgentConfig` fields without restarting `serve`; see **`docs/配置说明.md`** (**`API_KEY` 不因热重载从环境重读**；Web 密钥仍走 `client_llm`)。
