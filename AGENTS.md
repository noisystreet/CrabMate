...
- Before `serve` with UI: build Client UI (`cd ../crabmate-client && make frontend`), set `CM_WEB_STATIC_DIR` to that `frontend/dist`, and run **`serve --with-web`**. Default **`serve`** is API-only (no SPA).
