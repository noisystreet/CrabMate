...
- **Default (`llm_http_auth_mode = bearer` in config)**：`API_KEY` is the cloud vendor Bearer token (optional process fallback). **`serve` / `bench` 可在未设置 `API_KEY` 时启动**；实际调用 `chat/completions` 前须有关密钥，优先级：**环境变量 `API_KEY`** → **系统钥匙串**（与 Web/桌面侧栏保存同源）→ 请求体 **`client_llm.api_key`**（官方 Client）。同进程 **`repl` / `chat` / `tui` 命令入口已移除**（D2.1；用 Client **`crabmate-tui`**）。旧 `$XDG_DATA_HOME/crabmate/secrets/client_llm` / `executor_llm` 与保存模型中的明文 key 在钥匙串写入成功后自动迁移删除。无 **`--config`** 且 cwd 无本地覆盖时采用 **`$XDG_CONFIG_HOME/crabmate/config.toml`**（可种子/回退 `/etc`），并合并 **`llm_overrides.json`** 与 **`prefs.json`** 的 **`cm_role`**（**不**自动打开 **`last_workspace_root`**；须 **`--workspace`**）。**`crabmate models` / `crabmate probe`** 在 `bearer` 下仍要求环境变量 **`API_KEY`** 非空。 **`config`（`cargo run -- config` / `--dry-run`）、`doctor`、`web-bearer` 与 `save-session`（别名 `export-session`）不要求 `API_KEY`**。
...
- Before `serve` with UI: build Client UI (`cd ../crabmate-client && make frontend`), set `CM_WEB_STATIC_DIR` to that `frontend/dist`, and run **`serve --with-web`**. Default **`serve`** is API-only (no SPA; **`--no-web`** is a compat no-op).
- **E2E tests** (Playwright): owned by Client repo `../crabmate-client/e2e/`（`./scripts/e2e-playwright.sh` 转发）。Proxy tip: `no_proxy=127.0.0.1,localhost`.
