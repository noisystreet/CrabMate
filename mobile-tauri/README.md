...
> **权威维护**：同级 Client 仓 [`../crabmate-client/mobile-tauri`](../../crabmate-client/mobile-tauri/)。本目录为路径 A **Phase 4 前双轨副本**。

Tauri 2 壳 + 连接页，**不**拉起本机 `crabmate serve`。  
包名 **`edu.crabmate`**，桌面显示名 **`crabmate`**。  
定位：**远程薄客户端**——只连已启动的 `serve`（LAN/VPS），业务 UI 与编排均在服务端（与桌面壳同模型）。

连接页与桌面壳共用 **`crabmate-connect`**（Client 仓 `crates/crabmate-connect`；探测、`#cm_web_api_bearer=` 交接、首次 Bearer 写钥匙串）。静态页源文件为 **`crates/crabmate-connect/assets/connect.html`**；同步到本目录：`bash scripts/sync-tauri-connect-page.sh`（写入 **`mobile-tauri/dist/index.html`**）。契约钉版本见 Client 仓 **`docs/design/contract_pin.md`** 与主仓 **`docs/design/client_contract_versioning.md`**。
