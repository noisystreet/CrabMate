...
若你只关心功能与使用方式，请看 `README.md`；**工具 JSON 参数示例**见 `docs/TOOL_EXAMPLES.md`。

## TODOLIST 与功能文档约定

- **`docs/TODOLIST.md`**：只保留**未完成**项；**上半**为全局优先级（P0–P5），**下半**为「按模块的优先选项」（中长期方向，每域若干条）。实现某条后**从文件中删除该条目**（不要用 `[x]` 长期占位）；空的小节可删掉标题。历史追溯用 Git。
- **新功能 / 用户可见变更**（新 CLI 标志、HTTP 接口、配置键、工具名、Web/CLI 行为等）：合并代码时同步更新 **`README.md`**（面向使用者：功能、命令、配置、安全提示）和/或 **`docs/DEVELOPMENT.md`**（面向维护者：模块、协议、扩展点）。**工具调用 JSON 示例**集中在 **`docs/TOOL_EXAMPLES.md`**，与 README 长列表演进时优先改该文件。纯内部重构且无行为变化时，可只改 `DEVELOPMENT` 或注释。
...
| `sse/` | **`protocol`**：`SsePayload` / `encode_message`（根再导出）；**`line`**：`classify_agent_sse_line` 等（与 `frontend/src/api.ts` 语义对齐；当前无 crate 根再导出）；**`mpsc_send`**：`send_string_logged`（`mpsc::Sender<String>` 失败时 `target=crabmate::sse_mpsc` 的 **debug**）。 |
| `tool_registry.rs` | 按工具名选择 Workflow / 命令超时 / 天气与联网搜索超时 / 默认同步等策略；**`is_readonly_tool`** / **`tool_calls_allow_parallel_sync_batch`** 供同轮安全并行判定（**`mcp__*`** 代理名视为非只读、且不参与并行 `SyncDefault` 批）。**`CliToolRuntime`**：`run_command` CLI 路径的审批、**`--yes`/`--approve-commands`** 自动批准与 **`CliCommandTurnStats`**（`agent_turn` 每回合开头 **`reset_command_stats`**）。 |
...
| `web/` | Web（HTTP）专用 axum 模块：`app_state`（`AppState`、`ConversationBacking`：内存或 SQLite、可选 **`long_term_memory`**、**`web_tasks_by_workspace`**：侧栏任务清单按工作区键入的进程内表）、`tasks_types`（`TasksData` / `TaskItem`）、`chat_handlers`（`/chat*`、`/upload*`、`/health`、`/status`）、`server`（Router 组装）、`workspace`、`task`（`/tasks` 读写内存表）。`open_conversation_sqlite` 会 **`LongTermMemoryRuntime::migrate_on_connection`**；`AppState` 由 `lib.rs::run` 装配；`SaveConversationOutcome` 在 **`conversation_store`**，crate 根再导出供 `chat_job_queue` 等使用。 |
