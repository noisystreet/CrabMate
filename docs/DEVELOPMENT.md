...
**`llm::LlmRetryingTransportOpts`** 与 **`CompleteChatRetryingParams::new`** 收敛重复样板；**`agent_turn::agent_llm_call::AgentLlmCall`** 在持有 **`RunLoopParams`** 时封装 **`request_chrome_trace`** 与 `complete_chat_retrying` 调用，仍须**经 **`complete_chat_retrying`** 出口**，不改变上表「唯一入口」。
