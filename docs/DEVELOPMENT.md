...
| `turn_replay_dump.rs` | 可选 **`CRABMATE_TURN_REPLAY_DUMP_DIR`** / **`AGENT_TURN_REPLAY_DUMP_DIR`**：每轮 **`run_agent_turn`** 返回前写入 **`turn-replay-{wall_ms}.json`**（`wall_ms` 与 Chrome 文件名用同一**回合起点**时间戳，可与 **`turn-*.json`** 并列对照）；**不**经 TOML。 |
