...
- **根包 `crabmate` 的 Cargo features（可选依赖）**：**`default = ["mcp", "docker_sandbox"]`**（与历史发行行为一致）。**`mcp`** 控制 **`rmcp`**（MCP 客户端与 `mcp serve`）；**`docker_sandbox`** 控制 **`bollard`**（`sync_default_tool_sandbox_mode = docker`）。嵌入或裁剪链接时可 **`cargo build --no-default-features`** 或按需 **`--features mcp`** 等组合；关闭 **`docker_sandbox`** 且配置为 docker 时 **`finalize`** 会报错。详见 **`docs/BACKEND_CORE_FRAMEWORK_DESIGN.md`**。
