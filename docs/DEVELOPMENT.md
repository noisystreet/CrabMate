...
- **`runtime`**：CLI 运行时逻辑，负责 REPL、单次问答与调用 `run_agent_turn`。
  ...
  - **`runtime/cli_mcp`**：**`mcp list`** 只读输出进程内 MCP 缓存（`mcp::cached_mcp_status`）；可选 **`--probe`** 调用 `try_open_session_and_tools` 刷新缓存。REPL **`/mcp`**、**`/mcp list`**、**`/mcp probe`** 走同一实现（`run_mcp_list`，`repl_context=true` 时提示语指向 **`/mcp probe`**）。
