...
| `types.rs` | `Message`、`Tool`、流式 chunk 等 OpenAI 兼容类型；`Message::system_only` / `user_only`、`messages_chat_seed` 供 Web 首轮与 CLI 共用。**`is_message_excluded_from_llm_context_except_memory`**：合并 `is_chat_ui_separator` 与 `is_chat_timeline_marker`，供上下文摘要 / 分阶段规划等过滤。**`messages_for_api_stripping_reasoning_skip_ui_separators`**：单次遍历跳过 UI 占位并剥离 `reasoning_content`，供 `llm::tool_chat_request` 拼装请求。 |
