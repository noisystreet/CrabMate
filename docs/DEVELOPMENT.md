...
| `agent_memory.rs` | 工作区相对路径备忘文件读取（`load_memory_snippet`）；与 **项目画像**、**依赖结构摘要** 合并后首轮消息组装在 **`project_profile::build_first_turn_user_context_markdown`**（Web **`build_messages_for_turn`**、CLI **`prepend_cli_first_turn_injection`**、REPL **`workspace_session::initial_workspace_messages`**）。 |
