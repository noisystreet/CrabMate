...
| `codebase_semantic_index.rs` | **`codebase_semantic_search`**：工作区内文本分块 + **fastembed** 向量写入 SQLite（与长期记忆分库）；按规范 **`workspace_root`** 键隔离；索引路径相对工作区（默认 **`.crabmate/codebase_semantic.sqlite`**），**`rebuild_index`** 会写盘。`run_agent_turn` 在 **`codebase_semantic_search_enabled=false`** 时从当轮工具列表移除该工具。 |
