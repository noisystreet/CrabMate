...
- **MCP（可选）**：`[agent] mcp_enabled` + 非空 `mcp_command` 时，`run_agent_turn` 在回合开头尝试连接 stdio MCP server；成功则 **`RunLoopParams::mcp_session`** 持有 `Arc<Mutex<RunningService<…>>>`，并将合并后的工具表作为 **`tools_defs`**。失败则打 `warn` 并仅使用内建工具。配置键与环境变量见 **`README.md`**（`CM_MCP_*`）。
