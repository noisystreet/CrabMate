...
| `tools/` | 全部 Function Calling 定义、`ToolContext`、`run_tool`；`resolve_workspace_path_for_read`、`resolve_repl_workspace_switch_path`（REPL **`/workspace`**：相对路径走 `resolve_for_read` 语义，绝对路径委托 `path_workspace::validate_workspace_set_path`）；`tools/mod.rs` 与 `tools/markdown_links.rs` 的测试已外移到同名子目录 `tests.rs`，并把工具调用摘要逻辑拆到 `tools/tool_summary.rs`，降低主文件长度；子模块见下表。 |
