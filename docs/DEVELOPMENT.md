...
**`agent` 侧当前调用 `complete_chat_retrying` 的路径**（新增调用点时应保持同一模式）：**`agent/context_window.rs`**（可选上下文摘要；**`LlmRetryingTransportOpts::headless_no_stream`**）、**`agent/agent_turn/plan/plan_call.rs`**（主循环 **P** 步）、**`agent/agent_turn/staged.rs`**（**`AgentLlmCall`** + **`RunLoopParams::llm_transport_opts`**，规划轮可覆写 **`out`/`render_to_terminal`**；定稿后 **`staged_plan_started` / 首条 notice** 由 **`agent_turn/staged_orchestrator.rs`** 统一发送）、**`agent/per_plan_semantic_check.rs`**（终答规划侧向一致性 **LLM**）。其它子模块（如 **`per_coord`**、**`outer_loop`**）通过上述路径**间接**调模型，避免再开 HTTP。
