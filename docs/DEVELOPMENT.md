...
| `tool_approval.rs` | 敏感工具 **Web / CLI 审批单一真源**：**`SensitiveCapability`**（能力等级，供日志与后续按能力配置策略）、**`ApprovalRequestSpec`**（与 SSE **`CommandApprovalBody`** 同形字段 + CLI 文案）、**`WebApprovalSink`** / **`CliApprovalInput`**（避免与 **`tool_registry`** 循环依赖）。**`run_web_tool_approval`**（**Strict**：`send` 失败即 Err，供 `tool_registry`；**Lenient**：与历史工作流一致仍等待 receiver）、**`request_tool_interactive_approval`**（Web 优先，否则 CLI **`--yes`** 与 **`prompt_tool_approval_cli`**）。由 **`tool_registry`**（`run_command` / `http_fetch` / `http_request` 与 **`prefetch_http_fetch_parallel_approvals`**）及 **`agent/workflow/execute`** 调用。 |
