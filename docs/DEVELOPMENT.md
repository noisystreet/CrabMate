...
| `sse/` | **`protocol`**：`SsePayload` / `encode_message`（根再导出）；**`stream_hub`**：`SseStreamHub`（`broadcast` + 环形缓冲；`/chat/stream` 断线重连与 `Last-Event-ID`）；**`line`**：`classify_agent_sse_line` 等（与 `frontend/src/sse_control_dispatch.ts` 语义对齐；当前无 crate 根再导出）；**`control_dispatch_mirror`**（`#[cfg(test)]`）：与前端 `classifySseControlPayloadParsed` 同序，金样 **`fixtures/sse_control_golden.jsonl`**；**`mpsc_send`**：`send_string_logged`；**`web_approval`**：Web 审批决策中文标签与 `timeline_log` 下发（`tool_registry` / 工作流共用）。 |
