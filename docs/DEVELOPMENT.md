...
| `codebase_semantic_invalidation.rs` | 串行工具批中：写工具成功或 **`workspace_changed`** 后按工具参数删除索引行或整表清空（与 **`read_file`** 缓存清空时机对齐）；见 **`codebase_semantic_invalidate_on_workspace_change`**。 |
