...
| **`app/chat/`** | 聊天主路径：列表、输入、滚动、查找、流式接线 | **`wire_chat_session_lifecycle`**（**`wire_chat_session_lifecycle_effects`**：首启会话、**`GET /web-ui`**、**`session_hydrate`**、持久化、上下文估算等 **`wire_*` 顺序入口**）/ **`wire_chat_domain`**（**`wire_chat_domain_effects`**：从 **`App`** 迁出的聊天主列域 **`wire_*` 注册入口**）/ **`column`** / **`composer`** / **`composer_wires`**（**`wire_chat_composer_streams`**：发送 / 重试 / 取消 / 新会话；**`stream_user_abort`**：**`apply_user_abort_of_inflight_stream`** 收口用户停止）/ **`composer_stream/`**（**`mod` / `context` / `callbacks`**：流式句柄、共享 ctx、**`ChatStreamCallbacks`** 装配）/ **`handles`**（含 **`ChatComposerWires`**）/ **`scroll`** / **`find`** / **`find_bar`** / **`message_chunks`** / **`message_row`** / **`message_row_actions`**（用户行分支/再生与跳转滚动的副作用）/ **`message_group_views`** / **`timeline`**（见 **`app/chat/mod.rs`** 再导出）；**`ChatSessionSignals`** 定义在 **`src/chat_session_state.rs`**（crate 根），供 **`app/`** 与 **`session_modal_row`** 共用；流式与壳层共享的 **`ComposerStreamShell`**（**`handles.rs`**）聚合 `status_*` / 审批 / 中止 / 工作区刷新 / 变更集 / 澄清等句柄，供 **`WireComposerStreamsArgs`**、**`composer_stream/context`**（**`ChatStreamCallbackCtx`**）与 **`ChatColumnShell`**（主列视图）复用，避免 `App` 向流式与主列重复传入同一组 `RwSignal` |
