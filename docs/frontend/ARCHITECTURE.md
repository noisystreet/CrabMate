...
| **`app/chat/`** | 聊天主路径：列表、输入、滚动、查找、流式接线 | **`wire_chat_session_lifecycle`**（**`wire_chat_session_lifecycle_effects`**：首启会话、**`GET /web-ui`**、**`session_hydrate`**、持久化、上下文估算等 **`wire_*` 顺序入口**）/ **`wire_chat_domain`**（**`wire_chat_domain_effects`**：从 **`App`** 迁出的聊天主列域 **`wire_*` 注册入口**）/ **`column`** / **`composer`** / **`composer_follow_up`**（**`ComposerStreamFollowUp`**：截断再生与失败助手重试的待发队列）/ **`composer_wires`**（**`wire_chat_composer_streams`**：发送 / 取消 / 新会话及消费待发队列；**`stream_user_abort`**：**`apply_user_abort_of_inflight_stream`** 收口用户停止）/ **`composer_stream/`**（**`mod` / `context` / `callbacks`**（含 **`delta_apply` / `done_session` / `error_session`** 等子模块）：流式句柄、共享 ctx、**`ChatStreamCallbacks`** 装配）/ **`handles`**（含 **`ChatComposerWires`**）/ **`scroll`** / **`find`** / **`find_bar`** / **`message_chunks`** / **`message_row`** / **`message_row_actions`**（用户行分支/再生与跳转滚动的副作用）/ **`message_group_views`** / **`timeline`**（见 **`app/chat/mod.rs`** 再导出）；**`ChatSessionSignals`** 定义在 **`src/chat_session_state.rs`**（crate 根），供 **`app/`** 与 **`session_modal_row`** 共用；流式与壳层共享的 **`ComposerStreamShell`**（**`handles.rs`**）聚合 `status_*` / 审批 / 中止 / 工作区刷新 / 变更集 / 澄清等句柄，供 **`WireComposerStreamsArgs`**、**`composer_stream/context`**（**`ChatStreamCallbackCtx`**）与 **`ChatColumnShell`**（主列视图）复用，避免 `App` 向流式与主列重复传入同一组 `RwSignal` |
...
- **会话 + 流式**：已与 **`ChatSessionSignals`** 对齐；**`stream_transport`**（[`ChatStreamTransport`]）在 Bound 车道内持有 attach 时快照的 `session_id`，与 UI **`active_id`** 可暂时不一致（SSE 仍写绑定会话）。**`composer_stream/callbacks/stream_session_access`** 的 **`append_stream_assistant_chunk`** 为 **`on_delta`** 正文/思维链追加的单一入口。后续新增字段（如只读展示标志）应优先加在聚合体上，而非再增加平行的 6 个参数。
