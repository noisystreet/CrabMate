...
├─────────────────────────────────────────────────────────┤
│  领域与工具（session_ops、session_sync、storage、i18n）  │  尽量不依赖具体组件路径；用户可见文案集中在 **`i18n/`**（如 **`stream`**、**`tool_cards`**、**`diag_sections`**、**`export_download`**、**`settings`** 等分片），SSE 控制面用户错误经 **`SseCallbacks.user_locale`** 与流式 **`Locale`** 对齐
├─────────────────────────────────────────────────────────┤
...
| 区域 | 职责 | 典型内容（现有或目标） |
|------|------|------------------------|
| **`app/`** | 应用壳：布局、路由级组合、全局快捷键、将子视图串起 | `mod.rs`、**`app_shell_effects`**（壳级 `wire_*` / `Escape`）、**`wire_workspace_domain`**（**`wire_workspace_domain_effects`**：变更集 fetch / `innerHTML` / Workspace 侧栏可见刷新）、侧栏/主列/状态栏视图 |
| **`app/chat/`** | 聊天主路径：列表、输入、滚动、查找、流式接线 | **`wire_chat_session_lifecycle`**（**`wire_chat_session_lifecycle_effects`**：首启会话、**`GET /web-ui`**、**`session_hydrate`**、持久化、上下文估算等 **`wire_*` 顺序入口**）/ **`wire_chat_domain`**（**`wire_chat_domain_effects`**：从 **`App`** 迁出的聊天主列域 **`wire_*` 注册入口**）/ **`column`** / **`composer`** / **`composer_wires`**（**`wire_chat_composer_streams`**：发送 / 重试 / 取消 / 新会话接线）/ **`composer_stream/`**（**`mod` / `context` / `callbacks`**：流式句柄、共享 ctx、**`ChatStreamCallbacks`** 装配）/ **`handles`**（含 **`ChatComposerWires`**）/ **`scroll`** / **`find`** / **`find_bar`** / **`message_chunks`** / **`message_row`** / **`message_row_actions`**（用户行分支/再生与跳转滚动的副作用）/ **`message_group_views`** / **`timeline`**（见 **`app/chat/mod.rs`** 再导出）；**`ChatSessionSignals`** 定义在 **`src/chat_session_state.rs`**（crate 根），供 **`app/`** 与 **`session_modal_row`** 共用；流式与壳层共享的 **`ComposerStreamShell`**（**`handles.rs`**）聚合 `status_*` / 审批 / 中止 / 工作区刷新 / 变更集 / 澄清等句柄，供 **`WireComposerStreamsArgs`**、**`composer_stream/context`**（**`ChatStreamCallbackCtx`**）与 **`ChatColumnShell`**（主列视图）复用，避免 `App` 向流式与主列重复传入同一组 `RwSignal` |
...
**B. 壳与域分离** | `app/mod.rs` 仅保留布局组合 + 全局 `Effect`，会话/工作区/任务各自的 `Effect` 块可迁到对应子模块的 `wire_*`；**`init_app_shell()`** 直接返回 **`AppShellCtx`**（不再向 `App` 暴露 **`AppSignals`**）；**长 `view!` 实参**收拢到 **`AppShellCtx`** 的组装方法（如 **`sidebar_nav_signals()`**、**`side_column_view_signals()`**、**`mobile_shell_header_signals()`**、**`changelist_modal_signals()`**、**`settings_modal_signals()`**、**`session_list_modal_signals()`**、**`status_bar_footer_signals()`**、**`chat_find_bar_signals()`**、**`approval_modal_signals()`**、**`settings_page_view_input()`**、**`settings_page_form_signals()`**） | `mod.rs` 行数持续下降或由脚本统计不再增长
