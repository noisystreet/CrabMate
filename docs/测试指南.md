...
- **`lizard-rust`**：Rust 圈复杂度（需 **`pip install lizard`**；**`scripts/lizard-rust.sh`** / **`scripts/lizard_rust_metrics.py`**：单函数上限 **`LIZARD_CCN`**（默认 40）、棘轮基线 **`scripts/lizard_max_ccn_baseline.txt`** 与 **`scripts/lizard_top10_ccn_sum.txt`**；CI 见 **`.github/workflows/code-complexity.yml`**）
