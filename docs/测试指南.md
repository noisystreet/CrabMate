...
- **`fn-param-ratchet`**：Rust 函数形参个数（**`scripts/fn-param-ratchet.sh`** / **`scripts/fn_param_rust_metrics.py`**，lizard 的 `parameter_count`）+ **`#[allow(clippy::too_many_arguments)]`** 行数；棘轮基线 **`scripts/fn_param_max_baseline.txt`**、**`scripts/fn_param_top10_sum_baseline.txt`**、**`scripts/fn_param_allow_count_baseline.txt`**；硬上限 **`PARAM_COUNT_CAP`**（默认 32）；与 **`lizard-rust`** 同在 CI **`.github/workflows/code-complexity.yml`**
