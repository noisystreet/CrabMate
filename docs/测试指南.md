...
- **`fn-nloc-ratchet`**：Rust 函数体行数（lizard **`nloc`**；**`scripts/fn-nloc-ratchet.sh`** / **`scripts/fn_nloc_rust_metrics.py`**）；棘轮基线 **`scripts/fn_nloc_max_baseline.txt`**、**`scripts/fn_nloc_top10_sum_baseline.txt`**；可选硬上限 **`FN_NLOC_CAP`**；与 **`lizard-rust`** 同在 CI **`.github/workflows/code-complexity.yml`**
