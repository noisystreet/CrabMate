...
本文汇总在仓库根目录（或注明子目录）执行的**自动化测试与常用检查命令**，覆盖 Rust 后端、Leptos 前端、浏览器 E2E 与依赖审计。更细的模块与协议说明见 [`开发文档.md`](开发文档.md)。**`crabmate bench` 测评子系统的功能规划与专用测试策略**见 [`基准测试规划.md`](基准测试规划.md)（与本文通用测试清单分离）。HumanEval 外挂判分脚本的**本地冒烟**（不调用 LLM）：`python3 scripts/humaneval_score_benchmark_results.py --tasks fixtures/benchmark/humaneval_tiny_tasks.jsonl --results fixtures/benchmark/humaneval_tiny_results.jsonl --output /tmp/tiny_scores.jsonl`（会执行 fixture 中的补全代码，见 `基准测试规划.md` §5.3）。
