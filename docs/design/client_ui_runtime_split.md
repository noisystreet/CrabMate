...
- **`serve` 默认即为纯 API**。
- 同机托管静态 UI 的 **`--with-web`** / **`--web`** 与 **`CM_WEB_STATIC_DIR`** **随后已整体移除**：`serve` **永不**托管 SPA，UI 一律由 Client 仓构建与托管。
- **`--no-web` / `--cli-only` 亦已删除**（曾为与默认等价的无操作旗标）。
...
静态 UI:   不配公网 A；壳包内 / Tailscale 私有 Origin（均由 Client 托管）
```
...
- [`../配置说明.md`](../配置说明.md) — CORS、Bearer  
- [`../命令行与路由.md`](../命令行与路由.md) — `serve`  
