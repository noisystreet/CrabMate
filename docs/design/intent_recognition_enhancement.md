...
**现行 L2（Phase C）**：仅在 **`intent_at_turn_start_enabled=true`** 且 **`session_mode=act`** 时调用；JSON 契约为 **`suggested_mode`**（`ask`/`plan`/`act`）+ **`confidence`**，供时间线/UI 提示，**不**自动改用户会话 mode；不可用或低置信 **fail-open**（门控开时临时只读）。下文 §4.3 旧 schema（`primary_intent` / `slots` 等）为历史设想。
...
- 管线内占位：`qa.readonly`（及 `qa.codebase`）等走 `intent_router::qa_direct_reply_for_primary` 生成 `IntentAction::DirectReply` 正文；`qa.explain` / `qa.meta*` / 寒暄同理仅作占位。默认 L2 结果由 `intent_pipeline` 直接映射为 `IntentDecision`；旧 L1 规则仅在 L2 不可用时兜底。
...
- **`qa.readonly` / `qa.codebase`**：意图门控对 **`Qa` + `DirectReply` + `qa.readonly*`** 进入主循环，设置 `RunLoopParams::turn.turn_planner_hints.step_executor_constraint = ReviewReadonly`，并注入一轮 ephemeral **`system_intent_gate_hint`**（首轮 P 后从会话剔除）；**不再**要求 `l2_applied`。分层模式下此类问句与澄清/确认一致，**不**进 Manager，直接 **`run_agent_outer_loop`**。
