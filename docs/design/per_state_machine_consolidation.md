...
**状态**：**子 FSM + reduce/driver 体系已落地（阶段 A–H 及步队列/终答 Context 收拢）**；全局单表驱动与 `staged/mod.rs` 剩余 IO 瘦身仍属后续。**受众**：维护 `agent_turn`、`per_coord`、分阶段规划与分层 Agent 的开发者。  
**语言**：中文。  
**关联文档**：
...
| 2026-07-03 | **阶段 I（步队列 reduce + Gate Context + 外循环 mock）**：**`steps_loop_reduce`**、**`prepared_stagnation_reduce`**、**`completed_replanning_reduce`**；**`final_plan_gate_context`**；外循环 **`OuterLoopDriver`** 三条集成 mock；文档状态更新。 |
| 2026-07-03 | **阶段 H（滚动 advance reduce + 外循环 driver）**：**`rolling_horizon_advance_reduce`**；**`outer_loop_driver`** + **`outer_loop_iteration_reduce`**；金样 +8。 |
