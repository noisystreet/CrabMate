...
**状态**：主要已实现（持续完善中）  
**目标读者**：维护者；实现前须与 **`docs/开发文档.md`**（分阶段规划）、**`docs/配置说明.md`**、**`docs/SSE协议.md`** 及 **`crates/crabmate-sse-protocol`** 变更策略对齐。
...
- 已与 `docs/配置说明.md` / `docs/开发文档.md` 对齐“固定单步、无新增配置项”的决策。
...
仓库内**「单智能体 + 工具循环」**的既有实现，主要指非分层下 **`staged_plan_intent_gate`** 拒绝、走 **`Freeform`** 时的 **`run_agent_outer_loop`**：同一对话链路、由模型在循环中反复选择工具直至终答。

**本设计所服务的演进目标**是：在仍使用 **分阶段规划**（门控放行 → **`PlannedStep`** 滚动视界）与 **`agent_reply_plan` v1** 的前提下，让**运行时形态**更接近上述模式——
...
- **现状**：门控放行时，无工具规划轮可产出 **`agent_reply_plan` v1**，`steps` 常为**多条**；执行器按 `steps` 顺序逐次进入外层循环（每步仍含 P/R/E 与工具调用）。
...
- **非目标**：不替代 **`planner_executor_mode = hierarchical`**。  
  门控拒绝、走 **`Freeform`** 时的 **`run_agent_outer_loop`** **已是**单智能体 + 工具循环；本设计**不是**重复实现该路径，而是让 **PlannedStep** 路径通过单步 + 步后 replan **向该形态收敛**（仍保留结构化规划与相关 SSE/审计能力）。
...
- 现有逻辑在 **`steps.len() >= 2`** 时才进入部分优化路径（见 `开发文档.md` 描述）。**`steps.len() == 1`** 时自然跳过；**无行为变更需求**，但须在 UI/日志中避免「误报优化未运行」为故障。

### 4.5 规划提示词（`PLAN_V1_SCHEMA_RULES` / `staged_plan_phase_instruction`）

- Schema 规则文本须明确：**在单步模式下**「除 `no_task` 外 **`steps` 仅含一项**」。
- 默认规划 **system**（`staged_plan_phase_instruction_default`）与内置 **`PLAN_V1_EXAMPLE_JSON`** 应引导：滚动多轮完成读→写→测；**`test_runner`** 步优先带 **`acceptance`**（退出码 + 可选 stdout/stderr 短锚点），与 **`docs/工具说明.md`** 步级验收字段一致。
...
- **`docs/配置说明.md`**：新键、默认值、环境变量、与 `workflow_validate_only` 的优先级。
- **`docs/开发文档.md`**：在「分阶段规划」小节增加指向本文的链接及一句行为摘要（实现合并时做）。
- **`README.md`**：若对用户可见「推荐配置场景」，可加一句（可选）。
- **`docs/基准测试规划.md`**：无需引用；本特性与 benchmark 无强耦合。
