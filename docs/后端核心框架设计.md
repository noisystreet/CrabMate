...
**已落地（依赖裁剪）**：根 `Cargo.toml` 提供可选特性 **`mcp`**（`rmcp`）、**`docker_sandbox`**（`bollard`）、**`fastembed`**（本地 ONNX 嵌入）；**默认** `default = ["web", "mcp"]`（不含 **`fastembed`** / **`docker_sandbox`**；同进程 **`repl`/`tui` feature 已于 D2.2 移除**）。按需：`cargo build --features fastembed` 或 `cargo build --all-features`。关闭示例：`cargo build --no-default-features` 或子集（如 `--features web`）。**无 `mcp`** 时：`mcp list` / `mcp serve` 不可用，进程内 MCP 工具代理不链接。**无 `fastembed`** 时：`finalize` 将 **`long_term_memory_vector_backend=fastembed`** 降为 **`disabled`**（仍可用 SQLite 长期记忆）；**`codebase_semantic_search`** 从工具列表移除且 **`codebase_semantic_search_enabled`** 强制为 **`false`**；**`hybrid`** 检索退化为仅 FTS。
