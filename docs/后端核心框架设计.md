...
- **工作区已有拆分**：`frontend`（Web UI）、`crates/crabmate-sse-protocol`（SSE 控制面协议）、**`crates/crabmate-types`**（OpenAI 兼容消息与 chat API 类型；根包经 `pub use crabmate_types as types` 再导出）。**协议、类型与前端**已部分外置，**「无 UI 的服务端核心」**仍集中在根包。
