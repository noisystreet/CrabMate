...
- **工作区已有拆分**：`frontend`（Web UI）、`crates/crabmate-sse-protocol`（SSE 控制面协议）、**`crates/crabmate-types`**、**`crates/crabmate-config`**、**`crates/crabmate-llm`**（根包分别再导出为 **`types`** / **`config`** / **`llm`**）。
