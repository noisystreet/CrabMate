...
- `workflow_execute`（DAG 工作流执行：并行/串行、审批、SLA、失败补偿）：
  ...
  - **结果 JSON**（`workflow_execute_result` / `workflow_validate_result`）含 **`workflow_run_id`**（与日志 `workflow_run_id=` 对齐）、**`trace`**（`dag_start` / **`node_run_start`/`node_run_end`**（整节点墙钟）/ `node_attempt_*` / `node_retry_backoff` / **`compensation_phase_*`**（启用补偿时）/ `dag_end` 等；带 **`node_id`**、**`tool_name`**、**`phase`**）、**`completion_order`**（成功节点完成顺序，供补偿逆序对照）；**`nodes[].attempt`** 为最终计入结果的尝试次数。可选：设置 **`CRABMATE_WORKFLOW_CHROME_TRACE_DIR`**（或 **`AGENT_WORKFLOW_CHROME_TRACE_DIR`**）后，每次 DAG 执行结束写入 Chrome trace JSON，且 **`workflow_execute_result`** 含 **`chrome_trace_path`**，详见 **`docs/CONFIGURATION.md`**「工作流（Chrome Trace 导出）」。
  ```json
  {"workflow":{
    "max_parallelism":2,
    "fail_fast":true,
