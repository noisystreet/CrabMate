...
**长输出进模型上下文**（`tool_result_envelope_v1` 默认开启）：写入历史的 `role: tool` 为 **`crabmate_tool`** JSON 信封（`summary`、`output` 等）。另含 **`tool_call_id`**、**`execution_mode`**（`serial` / `parallel_readonly_batch`）、**`parallel_batch_id`**（同批并行只读工具共享）；失败时含 **`error_code`**、**`failure_category`**（与 **`tool_result::ToolFailureCategory`** 蛇形字符串同源，由 `error_code` 推导，见 **`docs/SSE_PROTOCOL.md`**）、**`retryable`**（与 `error_code` 配套的**启发式**，非保证）。**并行只读批**内各工具使用与串行 **`dispatch_tool`** 对齐的**墙上时钟**（`tool_registry::parallel_tool_wall_timeout_secs`，如 **`http_fetch`** 取 **`http_fetch_timeout_secs` 与 `command_timeout_secs` 的较大者**，**`get_weather` / `web_search`** 用各自超时，其余多为 **`command_timeout_secs`**）；超时时输出含「超时」文案，**`tool_result.error_code`** 为 **`timeout`**。每次请求模型前若超过 **`tool_message_max_chars`**，服务端对 **`output`** 做**首尾采样**并设置 **`output_truncated`**、**`output_original_chars`**、**`output_kept_head_chars`**、**`output_kept_tail_chars`**，避免单次 grep/构建日志撑满上下文；完整原文仍可通过 SSE/导出等在会话中查看（视 UI 设置）。SSE **`tool_result`** 事件含相同关联字段，与信封对齐。执行前 SSE **`tool_call`** 另含 **`arguments_preview`**（与 `execute_tools` 日志同源的单行截断）；配置 **`sse_tool_call_include_arguments`**（**`AGENT_SSE_TOOL_CALL_INCLUDE_ARGUMENTS`**）为真时再下发启发式脱敏后的 **`arguments`**（更长上限，仍非全文保证）。详见 **`docs/DEVELOPMENT.md`**、**`docs/SSE_PROTOCOL.md`** 与 **`docs/CONFIGURATION.md`**。

**外部 CLI 未找到**：依赖本机可执行文件的工具在子进程 **`ENOENT`（未找到）** 时，除「请确认 PATH」外，还会在输出末尾附带**简短安装/验证示例**（常见为 `apt`/`brew`/`pip` 或官方文档链接）；覆盖范围包括 **`run_command`**（白名单内命令名）、`calc`（`bc`）、**`gh_api`** 的 `gh`、`format_*`（如 `rustfmt`/`clang-format`/`npx`/`gofmt`/`shfmt`/`xmllint`）、**`python_snippet_run`**（`python3`/`uv`）、**`port_check`/`process_list`**（`ss`/`lsof`/`ps`）、**`changelog_draft`** 与 **`git_*`** 前置探测（`git`）及 **`cargo audit`/`cargo deny`** 启动失败等；未内置映射的程序仍仅保留通用说明。

**会话工作区变更集**（默认开启，`session_workspace_changelist_*`）：对 **`create_file` / `modify_file` / `copy_file` / `move_file` / `delete_file` / `append_file` / `search_replace` / `apply_patch` / `structured_patch`**（实际写入成功）按会话作用域累积**相对路径**与相对「本会话首次触碰」的 **unified diff**；在**每次**向模型发请求前注入一条 **`user`**（**`name=crabmate_workspace_changelist`**），便于后续轮次优先引用真实改动路径。Web 保存会话前会**剥离**该条（与长期记忆注入同理）。**`workflow_execute` DAG 内**工具不在此表（独立 `ToolContext`）。

**终答 `agent_reply_plan` v1**（在工作流反思等路径下由服务端校验）：`steps[].id` 须唯一且符合稳定字符规则（见 **`docs/DEVELOPMENT.md`**）；可选 **`workflow_node_id`** 用于与最近一次 **`workflow_execute`** 工具结果里的 **`nodes[].id`** 对齐（子集校验），便于规划与 DAG 节点一一对应。分阶段规划下还可选 **`steps[].executor_kind`**（`review_readonly` / `patch_write` / `test_runner`）以步级收窄可见工具并拒绝越权调用（越权时工具结果含本步允许名摘要）；**`[tool_registry]`** 可用 **`sub_agent_*`** 键扩展默认补丁/测试名单或只读步拒绝表，详见 **`docs/CONFIGURATION.md`**（分阶段规划与 `tool_registry`）。
...
- **内置多种工具，由模型按需调用**：
  ...
  - `http_fetch`：对给定 URL 发起 **GET**（默认）或 **HEAD**。GET 返回状态、Content-Type、**重定向链**与正文（有超时与体长上限）；**HEAD** 不下载 body，仅状态码、Content-Type、Content-Length 与重定向链。正文解码：`Content-Type` 的 **`charset`**、HTML **`<meta charset>`** / **http-equiv=Content-Type**、`<?xml encoding=…?>`，否则 **`chardetng`** 嗅探；`application/json` 等优先按 UTF-8；`image/*` 等二进制类型仍按 UTF-8 有损预览。可选 **`text_format: html_text`**：用 **`scraper`（html5ever）** 将 **HTML 文档**转为纯文本（跳过 `script`/`style`/`noscript`，优先 `main` / `article` / `[role=main]`，否则 `body`）；非 HTML 或 JSON/二进制仍输出解码原文，并在「正文解码」说明中注明。请求使用 **`User-Agent: crabmate/<版本>`**（与模型 API 客户端一致）。URL 匹配 `http_fetch_allowed_prefixes` 的**同源 + 路径前缀边界**规则时直接执行；不匹配时，Web（`/chat/stream` 携带 `approval_session_id`）或 **CLI** 可人工审批 **拒绝 / 本次允许 / 永久允许**（GET/HEAD 共用同一归一化白名单键；CLI 见 `tool_approval::cli_terminal`）。
  - `http_request`：对给定 URL 发起 **POST / PUT / PATCH / DELETE**（可选 `json_body`）。受 `http_fetch_allowed_prefixes` 约束（同源 + 路径前缀边界）；匹配则直接执行，**未匹配**时 Web（`/chat/stream` + `approval_session_id`）与 **CLI** 可与 `http_fetch` 一样走 **拒绝 / 本次允许 / 永久允许**（永久键为 `http_request:<METHOD>:<URL>`，与 `http_fetch:` 键区分）。**`workflow_execute` 节点**内仍仅白名单前缀（同步路径无审批）。返回状态、Content-Type、重定向链与正文（解码与可选 **`text_format: html_text`** 与 `http_fetch` 相同；默认建议先 dry-run，不在 body 中放真实密钥）。
  ...
  - `repo_overview_sweep`：只读**聚合**：默认可选附加 **项目画像**（与侧栏 / `GET /workspace/profile` 及首轮注入同源：`Cargo.toml` / workspace、`package.json`、顶层目录、tokei、可选 `cargo metadata --no-deps`；由 `include_project_profile` 与 `project_profile_max_chars` 控制，默认开启且上限 6000 字符）；预览主要文档（默认 `README.md`、`AGENTS.md`、`docs/DEVELOPMENT.md` 等，缺则跳过）、对 `src`（可改 `source_roots`）做 `list_tree`、用 glob 汇总 `Cargo.toml` / `package.json` / CI 等工作流相关路径；输出末尾附**结论撰写提纲**。**不调用 LLM**，结构化分析结论由模型根据本工具输出撰写。可选 `doc_paths`、`doc_preview_max_lines`、`list_tree_*`、`build_globs` 等。
  - `docs_health_sweep`：只读**文档与健康聚合**：主文档头预览（默认路径与 `repo_overview_sweep` 主文档列表一致）+ `typos_check` + `codespell_check` + `markdown_check_links`。缺 `typos`/`codespell` CLI 时对应步记为 **skipped**。**Markdown 外链**：仅当传入 **`md_allowed_external_prefixes`**（非空）时，`markdown_check_links` 会用**内置 HTTP 客户端**对匹配前缀的 URL 发 HEAD；**不经过** `http_fetch` / `http_request` 工具、**不受** `http_fetch_allowed_prefixes` 约束，也**无** Web/CLI 审批流程。前缀留空则外链仅统计、不联网。可选 `fail_fast`、`summary_only`、`spell_paths`、各子步骤开关等。
  ...
  - `long_term_remember` / `long_term_forget` / `long_term_memory_list`：与会话长期记忆 SQLite 交互的显式工具（须 **`long_term_memory_enabled`** 且本会话已挂载记忆库）。`remember` 写入带可选 `tags` 与 `ttl_secs`（0=不过期）；`forget` 按 `memory_id` 或正文精确匹配删除；`list` 列出最近条目。**勿写入密钥或敏感原文**。
  ...
  - **容器 CLI**：`docker_build`（`docker build`，**会写本地镜像**；可选相对 `context`/`dockerfile`/`tag`/`no_cache`）、`docker_compose_ps`（`docker compose ps`，只读）、`podman_images`（`podman images`，只读）。依赖宿主已安装对应 CLI；`run_command` 白名单默认含 `docker` / `podman` / `mvn` / `gradle`（见 `config/tools.toml`）。
...
- `gh_pr_diff`：
  ...
  ```
- `gh_run_view`（结构化摘要）与 **日志**（体积大，易截断）：
  ```json
...
- `cargo_audit`（依赖安全扫描）：
  ...
  {"deny_warnings":true}
  ```
- `ci_pipeline_local`（本地 CI 关键检查；可选前端 `npm run build`：`run_frontend_build`；可选 Python：`run_ruff_check` / `run_pytest` / `run_mypy`）：
  ```json
...
- `workflow_execute`（DAG 工作流执行：并行/串行、审批、SLA、失败补偿）：
  - **架构扩展（设计稿）**：状态机式配置、条件分支、有界循环等与当前 DAG 的关系及推荐演进阶段见 **`docs/WORKFLOW_ORCHESTRATION_ARCHITECTURE.md`**（**不**改变本节已约定的 JSON 语义，仅供维护者对齐路线图）。
  ...
  - **结果 JSON**（`workflow_execute_result` / `workflow_validate_result`）含 **`workflow_run_id`**（与日志 `workflow_run_id=` 对齐）、**`trace`**（`dag_start` / **`node_run_start`/`node_run_end`**（整节点墙钟）/ `node_attempt_*` / `node_retry_backoff` / **`compensation_phase_*`**（启用补偿时）/ `dag_end` 等；带 **`node_id`**、**`tool_name`**、**`phase`**）、**`completion_order`**（成功节点完成顺序，供补偿逆序对照）；**`nodes[].attempt`** 为最终计入结果的尝试次数。可选：设置 **`CRABMATE_WORKFLOW_CHROME_TRACE_DIR`**（或 **`AGENT_WORKFLOW_CHROME_TRACE_DIR`**）后，每次 DAG 执行结束写入 Chrome trace JSON，且 **`workflow_execute_result`** 含 **`chrome_trace_path`**；若同时设置 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`**，工作流事件并入回合 **`turn-*.json`**，**`chrome_trace_path` 为 null**，详见 **`docs/CONFIGURATION.md`**。
  ```json
  {"workflow":{
    "max_parallelism":2,
    "fail_fast":true,
...
另外，已支持的 Rust/前端开发辅助工具还包括：`cargo_check`、`cargo_test`（与 `rust_test_one` 在 `test_result_cache_enabled` 下可对**相同指纹**复用上次截断输出）、`cargo_clippy`、`cargo_metadata`、`cargo_machete`、`cargo_udeps`、`cargo_publish_dry_run`、`rust_compiler_json`、`rust_rustc`（工作区根执行 `rustc`，参数同 `run_command` 安全规则）、`rust_analyzer_goto_definition`、`rust_analyzer_find_references`、`rust_analyzer_hover`、`rust_analyzer_document_symbol`、`read_binary_meta`、`frontend_lint`、`find_references`、`call_graph_sketch`、`rust_file_outline`、`format_check_file`、`quality_workspace`、`markdown_check_links`、`structured_validate`、`structured_query`、`structured_diff`、`structured_patch`、`table_text`、`text_diff`、`ast_grep_rewrite`、`present_clarification_questionnaire`、`diagnostic_summary`、`error_output_playbook`、`playbook_run_commands`、`package_query`。
以及：`cargo_tree`、`cargo_clean`、`cargo_doc`。

**Python / uv / pre-commit**：`ruff_check`、`pytest_run`、`mypy_check`、`python_install_editable`、`uv_sync`、`uv_run`、`python_snippet_run`、`pre_commit_run`；聚合类还有 `run_lints`（Rust 侧可选先 `cargo check` 再 `clippy`；可选 `npm run lint` / `run_frontend_build`；可选 ruff）、`quality_workspace`（可选 `run_cargo_check` / 前端 `run_frontend_build` / ruff / pytest / mypy 等）。
...
- `diagnostic_summary`（脱敏排障：工具链、`target/`、关键 env 是否设置，**不输出任何 env 取值**）：
  ...
  ```
- `present_clarification_questionnaire`（**Web 澄清问卷**：参数合法且工具成功时，流式路径在 **`tool_result` SSE 之后**再下发控制面 **`clarification_questionnaire`**；用户下一条 **`POST /chat`** / **`POST /chat/stream`** 可带 **`clarify_questionnaire_answers`**；CLI/TUI 仅见工具正文）：
  ```json
  {
    "questionnaire_id": "req-scope-v1",
    "intro": "在动手前请确认以下信息：",
    "questions": [
      {"id": "goal", "label": "主要目标是什么？", "required": true, "kind": "text"},
      {"id": "deadline", "label": "期望完成时间？", "hint": "例如：本周五", "kind": "text"}
    ]
  }
  ```
...
- `format_check_file`（单文件格式检查，不写盘：`rustfmt --check` / `clang-format --dry-run --Werror` / `prettier --check` / `ruff format --check`）：
  ...
  {"path":"src/main.rs"}
  ```
- `quality_workspace`（组合质量检查；默认 `fmt --check` + `clippy`；可选 `run_cargo_check` 单独跑 `cargo check --all-targets`；可按开关加 `test` / 前端 lint / `run_frontend_build` / prettier check）：
  ```json
  ...
  ```
- `rust_rustc`（工作区根目录执行 `rustc`，不经 shell；`args` 为字符串数组，不得含 `..` 或绝对路径；**不要求** `Cargo.toml`）：
  ```json
  {"args":["--explain","E0382"]}
  ```
