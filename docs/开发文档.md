...
| **`crabmate-agent`**（`crates/crabmate-agent`） | 规划产物 **`plan_artifact`**、意图 **`intent_*`** / **`intent_routing`**、**`agent_turn::tool_execution`**（批模式、`ToolExecutionHost`、early-deny）、**`agent_turn::outer_loop_*`**（外循环 FSM / reduce / driver / pre-gate reason）、**`step_executor_policy`**、**`turn_budget`**、工作流反思控制器等**领域层**；根包 **`agent`** 再导出并承载 **`agent_turn` 执行面**（**`tool_execution_host`**、SSE **`emit`**、**`TurnControlSink` / `TurnTerminalIo`**、**`outer_loop` IO**）、**`hierarchy`** / **`workflow` 执行** 等编排。拆分进度见 **`docs/design/agent_turn_split.md`**。 |
