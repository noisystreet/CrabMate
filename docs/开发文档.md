...
- **前端**：**`localStorage`** 存主题、部分 LLM 草稿等；**会话列表**按 **`GET /workspace` 返回的根路径**分桶（键派生见 **`frontend/src/storage.rs`** 的 **`sessions_json_storage_key`**，与旧版默认键 **`agent-demo-sessions-v1`** 并存）；切换工作区时换桶加载（见 **`session_workspace_partition.rs`**）。**计划中的本机用户数据目录**（`~/.local/share/crabmate`，三端共用、含 LLM 非机密覆盖与 `secrets/` 密钥分级）见 **`docs/design/user_data_dir.md`**（**未**实现）。
- **Tauri 桌面**：WebView 的 WebKit 存储目前在 **`~/.local/share/com.crabmate.desktop/localstorage/`**（按 `identifier` 与页面来源分文件）；与上项设计稿收敛后，业务数据应改走 **`/user-data`** 与 **`~/.local/share/crabmate`**。
...
**本机用户数据目录（`~/.local/share/crabmate`）**：见 **`docs/design/user_data_dir.md`**（Web/Tauri/CLI 共用 prefs、按工作区分桶的侧栏会话、`llm_overrides` 与 `secrets/` 分级；**未**承诺实现时间表）。
...
- **`chat/session_hydrate.rs`**：**`wire_session_hydration`**：订阅水合 nonce，拉取 **`GET /conversation/messages`** 并与本地 **`ChatSession`** 对齐；Effect 同步段用 **`try_hydration_wire_snapshot`** → **`HydrationWireSnapshot`**，异步段为 **`run_conversation_hydration_cycle`**；合并前守卫为 **`try_hydration_merge_precheck`**（`Result` 表达「可合并 / 跳过原因」），结果以 **`SessionHydrationMergeOutcome`** 命名；由 **`chat::wire_chat_session_lifecycle_effects`** 在首启 / Web UI 配置之后注册。
