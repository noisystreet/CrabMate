...
| **L0b 编程工作台** | **`coding_workbench_increment.md`**（Git / 代码 / 开发工具；默认全局与工程向角色） | **`finalize`**（叠加在 L0 之后、L1 之前） | **`config/finalize.rs`** · **`config/agent_roles.rs`** |
...
- **强动态策略的统一入口**：若需「工作区快照 + 用户 query → 附加 **system** / **user** 块」的通用扩展，宜在 **`context_bootstrap/prompt_compose.rs`** 扩展（已收敛 L3+L4+L5），并由 Web **`build_messages_for_turn`** 与 CLI **首条 system** 路径调用；内置失败降级与可观测性（参见上文「按工作区与用户提问的动态组装」）。
