...
- **`tui_stream_view.rs`**：TUI 风格投影（当前默认聊天视图）；将当前会话消息与 **`stream_text_overlay`** 合并后经 **`tui_line_markdown`** 按行轻量 Markdown 渲染（流式半行纯文本、闭合行/落定后解析；未闭合围栏保持纯文本）。向 OpenCode/OpenClaw 式 append-only + 简单跟底演进见 **`docs/design/web_tui_stream_to_opencode_style.md`**。
- **`tui_line_markdown.rs`**：按行流式 Markdown → 安全 HTML（复用 **`markdown::to_safe_html` / `plaintext_to_safe_html`**）。
- **`find_bar.rs`**：主区内查找条。
