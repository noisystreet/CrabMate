...
| `dsml/` | DeepSeek **DSML** 解析、物化与展示剥离：**`DsmlToolCallAdapter`** / **`materialize_deepseek_dsml_tool_calls_in_message`**（执行轮兜底）、**`strip_deepseek_dsml_for_display`**、**`parse_combined_assistant_text`**；分阶段无工具轮经 **`staged_policy`**（**`strip_staged_planner_message_tool_calls`**、**`staged_first_planner_tool_call_total_after_materialize`** 等）。**`llm::complete_chat_retrying`** 在每次成功 `stream_chat` 后按 **`materialize_deepseek_dsml_tool_calls`** 物化。子模块：**`normalizer`**、**`parser`**、**`strip`** / **`strip_scan`**、**`adapter`**、**`types`**、**`staged_policy`**。 |
| `text_sanitize/` | 用户可见正文轻量清洗（规划步骤描述自然化、相邻行去重等）；DSML 展示剥离与物化 re-export 自 **`dsml/`**（**`strip_deepseek_dsml_for_display`**、**`materialize_deepseek_dsml_tool_calls_in_message`**）。**`assistant_tail.rs`** 等；**`#[cfg(test)]`** **`tests.rs`**。 |
