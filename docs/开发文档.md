...
核心流程在 **`src/run_agent_turn.rs`** 的 **`run_agent_turn`**（入参 **`RunAgentTurnParams`**，类型定义在 **`lib.rs`**）：内部组装 **`RunLoopParams`** 后调用 **`run_agent_turn_common`**（实现见 **`src/agent/agent_turn/mod.rs`**；薄分发见 **`run_dispatch.rs`**；统一 driver 见 **`non_hierarchical_turn.rs`**；回合阶段见 **`turn_orchestration.rs`**（**`TurnOrchestrationMode`**：`intent_at_turn_start_finished` / `react`）；路由决议见 **`turn_route_decision`**，结构化日志目标 **`crabmate::agent_turn`**、字段 **`turn_orchestration_mode`**）。
