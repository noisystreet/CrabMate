...
| **活跃证据窗口** | `task_level_evidence::check_active_user_goal_completion_evidence` | 自最后一条真实 user 起至末尾；用于 L2 外循环冗余 **tool_calls** 抑制、分阶段步后重复规划抑制、滚动视界早停。 |
| **步级验收** | `step_verifier.rs` | 分阶段**单步**内 `acceptance` 与最后一条 `tool` 比对；**不**替代会话级完成证据 heuristic。 |
