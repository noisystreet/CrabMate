...
若你只关心功能与使用方式，请看 **`README.md`**；**环境变量与配置细节**见 **`docs/配置说明.md`**，**子命令与 HTTP 路由**见 **`docs/命令行与路由.md`**；**`chat` 退出码与 `--output json` 稳定行**见 **`docs/命令行契约.md`**（与 **`docs/SSE协议.md`** 流错误码交叉引用）。**测试与质量检查命令汇总**见 **`docs/测试指南.md`**（英文 **`docs/en/TESTING.md`**）。

## TODOLIST 与功能文档约定

- **`docs/待办清单.md`**：只保留**未完成**项；**全局优先级（跨模块）**为 P0–P5 小节；**按源码模块**各独立成章（`agent/`、`llm/`、`tools/` 等）。实现某条后**从文件中删除该条目**（不要用 `[x]` 长期占位）；空的小节可删掉标题。历史追溯用 Git。
- **新功能 / 用户可见变更**（新 CLI 标志、HTTP 接口、配置键、工具名、Web/CLI 行为等）：合并代码时同步更新 **`README.md`**（面向使用者：功能、命令、配置、安全提示）和/或 **`docs/开发文档.md`**（面向维护者：模块、协议、扩展点）。**内置工具**的能力说明与 Function Calling 示例见 **`docs/工具说明.md`**，增删改工具时同步该文件。纯内部重构且无行为变化时，可只改 `DEVELOPMENT` 或注释。
...
- **提交前检查**：根目录 `.pre-commit-config.yaml`（含 **`cargo fmt --all`**、**`cargo clippy --all-targets -- -D warnings`**、**`lizard-rust`**：对 `src/`、`crates/`、`frontend-leptos/src/` 做 **Rust 圈复杂度** 检查——**单函数 CCN 上限** 默认 **`40`**（**`LIZARD_CCN`** 可覆盖）；另有两份**棘轮基线**（只降不升，本地跑脚本时若指标下降会写回文件，**`CI=true` 不写回**）：**`scripts/lizard_max_ccn_baseline.txt`**（全局最大 CCN）、**`scripts/lizard_top10_ccn_sum.txt`**（最高 10 个函数 CCN 之和）。实现见 **`scripts/lizard_rust_metrics.py`** / **`scripts/lizard-rust.sh`**；**函数形参个数**棘轮见 **`scripts/fn_param_rust_metrics.py`** / **`scripts/fn-param-ratchet.sh`**（基线 **`scripts/fn_param_*_baseline.txt`**）；**`nloc` 与源文件行数**棘轮见 **`scripts/fn_nloc_rust_metrics.py`** / **`scripts/fn-nloc-ratchet.sh`**（函数：**`scripts/fn_nloc_*_baseline.txt`**、可选 **`FN_NLOC_CAP`**；按文件物理行数：**`scripts/rust_file_max_lines_baseline.txt`**、**`scripts/rust_file_top10_lines_sum_baseline.txt`**、可选 **`RUST_FILE_LINES_MAX_CAP`**）。与 **`code-complexity.yml`** 一致；**`code-coverage.yml`**（llvm-cov + 覆盖率棘轮）默认仅 **手动 `workflow_dispatch`**，不在 push/PR 自动跑。；需 **`pip install lizard`**）、**commit-msg：Conventional Commits 校验**）。安装：`pip install pre-commit && pre-commit install`（配置里含 `default_install_hook_types: [pre-commit, commit-msg]` 时会一并安装 **pre-commit** 与 **commit-msg** 钩子；若本机是旧配置下已装过，请补跑 **`pre-commit install --hook-type commit-msg`**）。手动执行文件检查：`pre-commit run --all-files`（**不**会跑 commit-msg；说明校验仅在 **`git commit`** 时触发）。Cursor 规则 **`.cursor/rules/pre-commit-before-commit.mdc`** 要求 Agent 在 `git commit` 前先跑 pre-commit（无则至少 `fmt` + `clippy`）。改 `src/` 时另见 **`.cursor/rules/rust-clippy-and-tests.mdc`**（测试范围）、**`.cursor/rules/rust-error-handling.mdc`**（unwrap/unsafe）。配置/路由/环境变量与文档的对应关系见 **`.cursor/rules/todolist-and-documentation.mdc`** 中「Rust：默认配置、环境变量与 HTTP 路由」。
...
### `AgentConfig` 组合结构与热更边界

运行态 **`AgentConfig`**（**`src/config/types/mod.rs`**）由多个领域子结构组合而成，字段分组定义在 **`src/config/types/agent_config_sections.rs`**（如 **`llm`**、**`session_ui`**、**`command_exec`**、**`staged_planning`**、**`conversation_persistence`** 等）。业务代码应统一使用 **`cfg.<section>.<field>`**，勿假设仍存在历史扁平顶层字段。

- **装载与装配**：**`config/builder.rs`** 的 **`ConfigBuilder`** 仍按 TOML/环境变量合并为扁平中间态；**`config/finalize.rs`**（含 **`config/finalize_parts/`**）校验、clamp 并组装为组合式 **`AgentConfig`**。
- **热更（`apply_hot_reload_config_subset`，`config/hot_reload.rs`）**：对 **`AgentConfig`** 中除 **`conversation_persistence`** 全文替换以外的各 **子结构** 执行 **`clone_from`**（整段替换，等同于从新快照拷贝该域）。**`conversation_store_sqlite_path`** 刻意**不**随热更改写（会话库路径在进程启动时打开）；**`scheduled_agent_tasks`** 则从 **`src`** 拷贝到运行中配置（与 **`hot_reload.rs`** 顶部注释一致：cron 注册仍在启动时完成）。其余边界（**`API_KEY`**、MCP 缓存清理等）见 **`hot_reload.rs`** 文档注释。
...
4. **工具与工作流**：工具表驱动执行（`tools/mod.rs`）、按名分发与 Web 侧阻塞超时（`tool_registry`）、DAG 工作流（**`agent::workflow`**）。**`workflow_execute`** 在 **`parse.rs`** 中可选 **`workflow.workflow_template`**（当前 **`rust_ci_light`**：内置 **`cargo_fmt_check` → `cargo_check` → `cargo_clippy` → `cargo_test`** 串行 DAG），再与用户 **`workflow`** 其余字段合并；解析节点时校验 **`tool_name`** 为内置工具名，并对 **`tool_args`** 做基于各工具 JSON Schema 的 **必填键**粗校验（递归进入数组元素中的对象；完整类型校验仍由各 `runner_*` 负责，见 **`tools/schema_check.rs`**）。可选 **`nodes[].node_tool_role`**（兼容 **`executor_kind`**）与分阶段 **`executor_kind`** 共用 **`agent::step_executor_policy`**，在 DAG 节点执行前收窄 **`tool_name`** 语义（见 **`docs/工具说明.md`**）。**`workflow_execute`** 执行结果 JSON 含 **`workflow_run_id`**（与 `crabmate` 日志对齐）、**`trace`**（调度/节点尝试/重试退避事件）、**`completion_order`**；节点 **`max_retries`** 仅对超时 / `spawn_blocking` 汇合失败等**可重试**错误自动重跑（见 **`docs/工具说明.md`**）。可选 **`CRABMATE_WORKFLOW_CHROME_TRACE_DIR`** / **`CM_WORKFLOW_CHROME_TRACE_DIR`** 将 **`trace`** 导出为 Chrome Trace JSON（**`agent/workflow/chrome_trace.rs`**）；若与 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`** 同时启用，经 **`agent::workflow_tool_dispatch::dispatch_workflow_execute_tool`** 将 **`RequestTurnTrace`** 并入 **`request_chrome_trace.rs`** 的回合缓冲，见 **`docs/配置说明.md`**。
...
状态机式配置、条件分支与有界循环等**能力边界与推荐演进方向**（与当前 **`workflow_execute` DAG**、**分阶段规划**、**`agent_reply_plan` / 工作流反思** 的关系）见 **`docs/工作流编排架构.md`**。  
**结构化规划—执行—验证（P-E-V）闭环**（显式子任务、确定性验收、与 **`plan_rewrite` / 工作流反思 / `final_plan_semantic_check`** 的边界）见 **`docs/规划执行验证架构.md`**。可选「观众」侧向点评的设计草案见 **`docs/design/audience_critic_role.md`**（**`docs/未来规划功能.md`** 有索引）。  
**后端核心库化与跨语言嵌入**（单体 crate 与公开 API 现状、Python 等宿主的可行边界与演进注意点）见 **`docs/后端核心框架设计.md`**。  
若实现涉及 **`src/agent/workflow/`** 或 **`plan_artifact` / `per_coord` / `staged`** 语义扩展，须同步更新上述设计稿、**`docs/工具说明.md`** 相关契约与（若变更模块边界）本节「架构设计」/「代码模块索引」；触及 **SSE** 时遵守 **`.cursor/rules/api-sse-chat-protocol.mdc`**。
...
**`agent` 侧当前调用 `complete_chat_retrying` 的路径**（新增调用点时应保持同一模式）：**`agent/context_window.rs`**（可选上下文摘要；**`LlmRetryingTransportOpts::headless_no_stream`**）、**`agent/turn_budget.rs`**（单轮墙钟 **`max_turn_duration_seconds`**；与 **`context_window`** 正交）、**`agent/agent_turn/plan/plan_call.rs`**（主循环 **P** 步）、**`agent/agent_turn/staged/rolling_horizon_facade.rs`**（**`AgentLlmCall`** + **`RunLoopParams::llm_transport_opts`**，规划轮可覆写 **`out`/`render_to_terminal`**；回合级滚动视界外层循环驱动见同模块 **`run_staged_rolling_horizon_outer_loop`**（每次迭代经 **`staged_rolling_horizon_apply_advance`** 集中 **advance + rewrite 计数 + tracing 摘要**；转移表见 **`advance_staged_turn_after_sub_call`**；相位/事件类型见 **`staged/turn_fsm.rs`**）；首轮解析后的 ensemble / 优化轮门控见 **`staged/planner_round_fsm.rs`**；解析成功后 ensemble/优化轮 **调用与日志** 表驱动见 **`staged/post_parse_pipeline_fsm.rs`**；多规划员串行草案采纳与合并轮步列表应用见 **`staged/ensemble_fsm.rs`**（**`EnsembleDriverPhase` / 辅助轮次数与合并门控**见 **`staged/ensemble_schedule_fsm.rs`**；优化轮复用同一 **`AppliedSteps` / `KeepPriorPlan`** 判定）；首轮解析 **`Result` → 静默结束 / 降级 outer_loop / 继续规划** 见 **`staged/prepared_parse_fsm.rs`**（**`resolve_parse_with_assistant`**、**`PreparedPlannerRoute` / `resolve_prepared_planner_route`**；**`run_staged_plan_with_prepared_request`** 打 **`staged_fsm=prepared_request`**）；**`no_task` 与完整流水线（ensemble/优化/分步）** 分叉见 **`staged/prepared_post_parse_fsm.rs`**（粗粒度 **`PreparedPostParseSchedule`**；结构化路径上 ensemble/优化/两阶段 NL 门控快照见 **`PreparedFullPipelineSchedule`** / **`prepared_full_pipeline_schedule`**）；**`run_staged_plan_with_prepared_request`** 将首轮无工具重试、`tool_calls` 剥离与解析分流拆为私有辅助，**full-pipeline** 主体在 **`advance_full_pipeline_phases_after_parse_inner`**、优化轮在 **`maybe_run_optimizer_round_and_apply_steps_inner`**；**首轮解析后、分步循环前** 的显式线性相位与 `staged_fsm=full_pipeline` 观测见 **`staged/full_pipeline_fsm.rs`**（**`StagedFullPipelinePhase`**）；**`omit_no_task_planner_from_history`** 等细则仍见 **`staged/planner_parse_fsm.rs`**；步执行循环内补丁尝试预算与是否启用 patch 规划见 **`staged/staged_step_fsm.rs`**；**transitions 跳转 + 注入 user 正文**见 **`staged/step_loop_fsm.rs`**；**步内子阶段枚举 `StagedStepRunningSub`、outer 后与工具健康检查路由、墙钟超限纯函数**见 **`staged/step_iteration_fsm.rs`**（**`run_staged_plan_steps_loop`**（**`staged/steps_loop.rs`**）的单次迭代为 **`run_one_staged_plan_step_iteration`** → **`staged_step_run_outer_half`** / **`staged_step_run_after_outer_half`**）；定稿后 **`staged_plan_started` / 首条 notice** 由 **`staged/orchestrator.rs`**（**`StagedRoundOrchestratorPhase`**，与滚动视界 **`turn_fsm::StagedTurnPhase`** 区分）统一发送；**`run_staged_plan_steps_loop`** 打 **`target: crabmate::staged`**（**`staged_fsm=steps_loop`**、**`steps_loop_phase`**）；**`prepare_messages_for_model` + 规划请求拼装**的 fixture 测试在同目录 **`#[cfg(test)] staged_plan_prepare_fixture_tests`**）、**`agent/per_plan_semantic_check.rs`**（终答规划侧向一致性 **LLM**）。其它子模块（如 **`per_coord`**、**`outer_loop`**）通过上述路径**间接**调模型，避免再开 HTTP。**`per_coord/final_plan_gate`**：`after_final_assistant` **始终**经 **`run_final_plan_gate`**（`NoRequirement` 时不读 `workflow_validate` 层数缓存）。**`agent_turn/outer_loop`**：`run_agent_outer_loop` 的迭代守卫、上下文准备、反思分支（**`ReflectBranchCtl`**）与工具执行轮拆为私有辅助。
...
- **R**：**`per_reflect_after_assistant`**（**`agent_turn/reflect/reflect_impl.rs`**）；其中 **`StopTurnPendingPlanConsistencyLlm`** 再进入 **`per_plan_semantic_check::evaluate_plan_consistency_with_recent_tools_llm`** → 又一次 **`complete_chat_retrying`**（无工具侧向调用）；侧向输出后经 **`per_coord/final_plan_gate::run_final_plan_gate_semantic_completed`** → **`final_plan_gate::apply_plan_rewrite_count_from_gate`**（与静态终答路径同一计数写入）→ **`reflect_finish_from_after_final_assistant`** 映射为外环 **`ReflectOnAssistantOutcome`**。
...
- **`agent_turn`** 将上述失败与编排早停映射为 **`RunAgentTurnError`**（**`agent_turn::errors`**），维护当前 **`sub_phase`**（**`planner` / `executor` / `reflect`**，与 P/R/E 心智模型对齐）；Web 经 **`TracingChatTurn::job_id`** 作为 **`turn_id`**（与 **`x-stream-job-id`** 一致）。**编排观测**：`target: crabmate::agent_turn` 下结构化字段 **`turn_orchestration_mode`**（见 **`turn_orchestration::TurnOrchestrationMode`**）；非分层主路径另有 **`non_hierarchical_main_route`**（**`turn_orchestration::NonHierarchicalMainRoute`**）；若主路径为 **`single_agent_outer_loop`**，另有 **`single_agent_outer_loop_because`**（**`turn_orchestration::SingleAgentOuterLoopBecause`**：`staged_gate_*` 拒绝，或 **`config_fallback_single_agent_outer_loop`**）。**`planner_executor_mode = hierarchical`** 时在同 target 下另有 **`hierarchical_phase`**（**`agent_turn/hierarchy.rs`** 内 `run_hierarchical_agent` / `handle_execution_result`；**任务级验收与「关键证据」Markdown** 纯逻辑在 **`agent_turn/task_level_evidence/`**）；话语型回落外层循环时另有 **`hierarchical_post_intent_route`** / **`hierarchical_discourse_fallback_reason`**（**`hierarchical_intent_route`**），与顶层 `turn_orchestration_mode=hierarchical` 正交，便于对照「分层内走到哪一步」。
- **`agent_turn::outer_loop`**：**`run_agent_outer_loop`** 逐次调用 **`run_outer_loop_single_iteration`**；单次迭代结束去向由 **`OuterLoopIterationExit`** 显式表达（**`continue_next_iteration`** / **`stop_outer_loop`**），对应 **`tracing`** 字段 **`outer_loop_iteration_exit`**（与同 target 下的 **`outer_loop_step`**、**`reflect_branch`**、**`outer_loop_plan_model_role`**（[`params::OuterLoopPlanCallModelRole`]：`planner_round` / `executor_round`）并列）。首轮 planner 模型对应 **`iteration_count == 1`**（**`use_executor_model=false`**），第二轮起走 executor 模型；迭代→角色的映射集中在 **`OuterLoopPlanCallModelRole::from_outer_loop_iteration`**。
- **`chat_job_queue`** 将 **`RunAgentTurnError`** 编码为 SSE **`SsePayload::Error`**（**`code`**、可选 **`turn_id` / `sub_phase`**），详见 **`docs/SSE协议.md`**（如 **`LLM_RATE_LIMIT`**、**`turn_aborted`**）。**`AgentTurnJobOutcomeKind`**（**`agent_turn::errors`**）统一流式/JSON 队列 worker 对 **`Err`** 的分流（取消 / 历史 **`staged_plan_invalid:`** / 常规失败），避免 worker 内散落字符串前缀判断。
...
- **Execution / DAG**：分层按依赖拓扑调度与子目标执行的核心编排见 **`execution.rs`**（**`execute_with_result`** 与 DAG 调度），子目标顺序/并行与单步验证重试见 **`execution_impl.rs`**（由 **`execution.rs`** 引入）；同层 **`execute_parallel`** 见 **`execution_parallel.rs`**，单个子目标的 spawn 任务体与 **`ParallelSubgoalTask`** 见 **`execution_parallel_spawn.rs`**；**DAG 构建与拓扑分层、`trim_for_detail` / 证据摘要** 等纯逻辑见 **`execution_helpers.rs`**；**`supplement_subgoal_required_tools`** 见 **`subgoal_required_tools.rs`**（由 **`hierarchy/mod.rs`** 再导出）；**`ExecutionError`** 独立为 **`execution_error.rs`**（供 `execution` 与 `execution_helpers` 共用，避免与 `execution.rs` 形成模块依赖环）。原 `include!("execution_body.inc.rs")` 已移除，改为显式子模块以改善导航与可维护性。
...
**D. Operator 实现细节**
- **目录**: `src/agent/hierarchy/operator/`（`mod.rs` 汇总；**`react_loop`**：`execute_with_tools`；**`agent_impl`**：`execute` / LLM 与产物上下文；**`operator_tool_analysis`**：工具执行结果启发式（任务完成判定）；**`prompt`**：系统提示与按任务类型的执行指导；**`compile`**：编译错误分类与收敛指标；**`inject`**：占位符注入与工作目录探测；**`text`**：轨迹截断；**`state`**：ReAct 内部状态；**`types`**：`OperatorAgent` / `OperatorConfig` / `OperatorError` 等）
...
| `command.rs` | `run_command`：`RunCommandError`（JSON/白名单/参数/限流/启动失败等可判别）、`run`（兼容字符串输出）、`run_checked`（`Result`）；**`prepare_run_command_invocation`** / **`run_cargo_test_with_optional_cache`** / **`run_command_execute_workspace_binary`** 拆分执行路径；可选 `cargo test …` 路径经 **`test_result_cache`**；**`error_playbook::playbook_run_commands`** 对校验失败打 **`warn`**（含 `kind`）；全局限流可在 **`cargo test`** 下经 crate 根 **`reset_process_tool_globals_for_tests`** 清空 |
...
| `tool_param_types.rs` | 可选：内置工具入参的 **`Deserialize` + `schemars::JsonSchema`** 真源，与 `runner_*` 及发给模型的 `parameters` 对齐（见 `tool_json_schema`） |
| `tool_json_schema.rs` | **`tool_parameters_schema_value<T>()`**：由类型生成 OpenAI 兼容 `parameters` JSON（`schemars`）；`tool_params` 中可用手写 `json!` 或调用本模块与手写并存 |
...
| `structured_preview.rs` | **`merge_sse_structured_preview`** / **`structured_preview_for_tool_sse`**：`read_*` / `list_tree` 首行 **`crabmate_tool_output`**；**`run_command`** 等与 **`crabmate_tool.structured_payload`** 合并，供 SSE **`tool_result.structured_preview`** |
| `file/` | 工作区文件工具目录：`mod.rs` 再导出各 `pub fn`；`path`（`resolve_for_read`/`resolve_for_write` 等）、`write_ops`、**`replace_lines_stream`**（`modify_file` 的 **`replace_lines`** 流式实现）、**`read_tool`**（`read_file` / **`read_file_try`**；成功路径在正文前加一行 **`crabmate_tool_output` v1**）、`directory`（**`read_dir`** 成功路径同样首行 **`crabmate_tool_output`**）、`tree_glob`（**`list_tree`** 成功路径首行 **`crabmate_tool_output`**）、`inspect`、`extract`、**`mutate`**（**`search_replace`** 与 **`apply_search_replace_inner`** / **`search_replace_dry_run_preview`**）、`perm`、`symlink`、`display_fmt`；单元测试 `tests.rs` |
...
| `github_cli/` | GitHub CLI（`gh`）封装：`mod.rs` + **`api.rs`**（`gh_api`、**`validate_gh_api_json`** / **`format_gh_api_process_output`** 等）；`gh_pr_*`（含 **`gh_pr_checks`**、**`gh_pr_create`**）/ `gh_issue_*` / `gh_run_*` / `gh_release_*` / `gh_search` 等列表类经 **`command::run`** |
...
| `contract_map.rs` | `crate_contract_map`：只读 **契约地图**（`src/lib.rs`、`docs/SSE协议.md`、前端 `api.rs`/`sse_dispatch.rs`、`control_classify.rs`、金样、`config/default_config.toml`、配置/路由文档等锚点的短预览 + 关键字行节选；可选 `extra_paths`） |
| `docs_health_sweep.rs` | `docs_health_sweep`：文档预览 + typos + codespell + `markdown_check_links`（外链 HEAD 不经 http_fetch 审批） |
| `rust_ide.rs` | 编译器 JSON、rust-analyzer LSP（goto/references/hover/documentSymbol 等）；**`lsp_rust_analyzer_handshake`** / **`ra_lsp_request_method_params`** / **`lsp_rust_analyzer_run`** 拆分 stdio 会话 |
...
核心流程在 `src/lib.rs` 的 `run_agent_turn(RunAgentTurnParams { … })`：内部组装 **`RunLoopParams`** 后调用 **`run_agent_turn_common`**（实现见 **`src/agent/agent_turn/mod.rs`**；模式分发见 **`run_dispatch.rs`**；顶层编排形态枚举与 **`NonHierarchicalEntryResolution`**（门控+配置→主路径及单 Agent 外循环根因）见 **`turn_orchestration.rs`**（**`NonHierarchicalMainRoute`**）；分层意图后路由见 **`hierarchical_intent_route.rs`**，结构化日志目标 **`crabmate::agent_turn`**、字段 **`turn_orchestration_mode`**）。
...
- **对外文档**：版本号、`error`/`code` 与 `tool_result.error_code` 枚举、控制面变体表、与 Leptos 的对齐清单见 **`docs/SSE协议.md`**（修改协议时须同步 **`crates/crabmate-sse-protocol`** 与 **`frontend-leptos/src/sse_dispatch.rs`**）。
...
- **扩展内置工具的推荐顺序**（按依赖关系自上而下；多数工具落在 **`SyncDefault`**，无需改 **`tool_registry`**）：
  ...
  3. **JSON Schema**：在 **`tools/tool_params*.rs`**（或既有 `params_*` 模块）中增加与运行时解析一致的 **`parameters_*`** 构建函数；**推荐**在 **`tools/tool_param_types.rs`** 定义 `Deserialize + JsonSchema` 后由 **`tools/tool_json_schema::tool_parameters_schema_value`** 生成 schema，与 **`runner_*`** 共用同一结构体，减少与 `jsonschema` 校验漂移。
  ...
  10. **配置 / 环境变量**（若有新键）：**`config/`** 分片、**`config.toml.example`**、**`docs/配置说明.md`**，以及规则要求的 **README / DEVELOPMENT** 摘要。
...
- 样式加载：**`frontend-leptos/index.html`** 内多条 **`data-trunk` `rel="css"`** 指向 **`frontend-leptos/styles/`** 下 `tokens.css`（**色板 + 主题覆层变量**，如 `--selection-bg`、`--page-bg-decor-*`、`--nav-rail-bg`、`--btn-primary-*`、`--modal-backdrop-bg`、`--composer-bar-box-shadow` 等；**多套浅色预设时优先只增改 `tokens.css` 中各 `data-theme` 块**）、`base.css`、`components.css`、`layout-chat.css`、`shell-ds.css`（DeepSeek 式左栏 + 居中线程 + 底栏一体输入；**`shell-main-toolbar`** 在 **`side-column`** 顶部，不占聊天列；工作区与任务均关时 **`side-column-rail-only`** 保留窄轨宽度；**`session-ctx-*`** 为左栏会话右键菜单）、`sidebar.css`、`status.css`、`approval.css`、`modal.css`、`motion.css`（顺序即层叠顺序；Trunk 不会跟随根目录 `styles.css` 的 `@import` 复制子文件，故须在此声明）。设计 token 与模块边界见各文件头注释。
- **多套预设主题（扩展 `data-theme`）**：当前 Web 仅 **`light` / `dark`**；若增加多套色板预设，见 **`docs/design/web_theme_presets.md`**（`localStorage` **`crabmate-theme`**、`<html data-theme>`、CSS 分叉与 Rust 白名单约定）。
...
- **`composer_stream/`**：**`/chat/stream`** 接线拆分——**`mod.rs`**（**`ComposerStreamHandles`**、**`make_attach_chat_stream`** 与 **`spawn_local`**）、**`context.rs`**（**`ChatStreamCallbackCtx`**）、**`callbacks/`**（**`mod.rs`** 再导出 **`build_chat_stream_callbacks`**；**`helpers`** / **`builders`** / **`assemble`** 分担各 **`on_*`** 装配为 **`ChatStreamCallbacks`**，与 **`api::send_chat_stream`** 对齐）。
...
### `frontend-leptos/src/message_format/`
...
- **SSE 协议演进**：载荷形状以 **`sse::protocol::SseMessage` / `SsePayload`**（及 `sse/mod.rs` 再导出）为事实来源；**版本号**以 **`crates/crabmate-sse-protocol`** 与 Leptos 共享。Rust 侧行分类见 **`sse/line.rs`**；浏览器侧在 **`frontend-leptos/src/sse_dispatch.rs`**（由 `frontend-leptos/src/api.rs` 调用）。**人读契约与错误码表**：**`docs/SSE协议.md`**。**控制面分类契约测试**：`fixtures/sse_control_golden.jsonl` + `cargo test golden_sse_control`（**`crabmate-sse-protocol`** + **frontend-leptos** 对齐）。
