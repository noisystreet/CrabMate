...
- **`chat/scroll_shell.rs`** / **`chat/scroll_follow.rs`** / **`chat/messages_list.rs`**：消息列滚动壳（**`ChatScrollShellSignals`**：StickToBottom pin/`on:scroll` 近底 re-pin / `on:wheel` unpin、prepend 锚点补偿；**`arm_programmatic_stick`** 抑制工具 `ReplaceAll` 等夹低 `scrollTop` 的误 unpin）；内容侧 **`wire_content_follow_scroll`**（含 `tool_output_chunks` 指纹）+ **`engage_follow_and_scroll_bottom`** + ResizeObserver（内容根 `.chat-tui-transcript` / `.chat-thread`）；全量 **`ChatChunk`** 渲染（气泡路径现默认不挂载）；更早历史仅经顶部 **「加载更早」** 按钮触发（**`try_load_older_messages_for_active_session`**）。
...
- 流式消息与自动跟底策略（用户上滚时禁用、回到底部附近恢复；程序化贴底见 **`chat/scroll_shell::arm_programmatic_stick`**）。
...
- **`tui_stream_view.rs`** / **`tui_transcript_sync.rs`** / **`tui_actions_bar.rs`** / **`tui_tool_process.rs`**：TUI 默认聊天视图；每回合 `wrap`（`section` + 下方 **`msg-action-icon-btn` 图标行**，对齐气泡）；工具为一行摘要 + 可折叠详情；流式只对 live 按行 patch；会话切换全量重建。演进见 **`docs/design/web_tui_stream_to_opencode_style.md`**。
