...
| **活跃证据窗口** | `task_level_evidence::check_active_user_goal_completion_evidence`；早停门控见 **`agent_turn/turn_completion.rs`**（`turn_early_stop_allowed` / `turn_redundant_tools_after_completion_allowed`） | 自最后一条真实 user 起至末尾；用于 L2 外循环冗余 **tool_calls** 抑制、分阶段步后重复规划抑制、滚动视界早停。混合「构建/编写 + 分析」意图时须**各实现维度**均有证据，只读 shortcut 不可单独满足。 |
