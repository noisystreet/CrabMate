...
| **`crabmate-types`**（`crates/crabmate-types`） | OpenAI 兼容 **`Message`** / **`Tool`** / **`ChatRequest`**、会话变换 helpers、服务端注入 user 识别；根包 **`pub use crabmate_types as types`**。 |
...
| `dsml/` | DeepSeek **DSML** 解析、物化与展示剥离：**`DsmlToolCallAdapter`** / **`materialize_deepseek_dsml_tool_calls_in_message`**（执行轮兜底）、**`strip_deepseek_dsml_for_display`**、**`StreamingDsmlContentFilter`**（流式 SSE/CLI/TUI 下发前剥离）、**`parse_combined_assistant_text`**。**`llm::complete_chat_retrying`** 在每次成功 `stream_chat` 后按 **`materialize_deepseek_dsml_tool_calls`** 物化；**`llm::api::sse_parser`** 在流式正文增量经 **`dsml_stream_strip_enabled`** 过滤后下发。子模块：**`normalizer`**、**`parser`**、**`strip`** / **`strip_scan`**、**`stream`**、**`adapter`**、**`types`**。 |
| `text_sanitize/` | 用户可见正文轻量清洗（规划步骤描述自然化、相邻行去重等）；DSML 见 **`dsml/`**。**`assistant_tail.rs`** 等；**`#[cfg(test)]`** **`tests.rs`**。 |
