...
- **`chat/message_virtual_viewport.rs`** / **`chat/messages_scroll_compensate.rs`**：消息列 **chunk 数 > 48** 时仅挂载视口附近 **`ChatChunk`**；prepend 更早页后用 **`scrollHeight` 差值**补偿 **`scrollTop`**（**`messages_scroll_compensate`**）。
