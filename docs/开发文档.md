...
Leptos CSR：**`api/`**（`fetch`、JSON API、**`chat_stream`**）与 **`sse_dispatch`** 消费 SSE；**`app/`** 聊天与工作区 UI；**`message_format`** 与渲染管线。组件拆分与依赖约定见 **`docs/frontend/ARCHITECTURE.md`**（若仓库中存在）。构建：**`cd frontend && trunk build`**。
