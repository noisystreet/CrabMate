...
**设计稿（渐进落地）**：见 **`docs/design/per_state_machine_consolidation.md`**（终答 Gate、**`staged/turn_driver`** 运行时顶层相位、**`staged_parse_terminal`** 两层解析路由、**`step_iteration_reduce`** 步循环表驱动；全局单表 PER 仍属后续）。
