...
>>>>>>> c0023697 (refactor(agent): 抽出 TurnControlSink/TurnTerminalIo 收拢 RunLoopIo / nest RunLoopIo behind TurnControlSink and TurnTerminalIo)
...
**完成判定金样**：`fixtures/turn_completion_golden.jsonl` + **`agent_turn/turn_completion_golden.rs`**（`cargo test golden_turn_completion`）；与 **`turn_completion_decision.rs`** 的 `evaluate_turn_*` 对齐。
