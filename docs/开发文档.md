...
- **`chat_session_state.rs`**（`frontend/src/`，**非**仅 `app/`）：**`ChatSessionSignals`**——会话列表、活动 id、**`SessionSyncState`**、水合 nonce、流式 job / SSE 序、**`stream_bound_session_id`**（与 **`make_attach_chat_stream`** 内 **`ChatStreamCallbackCtx::active_session_id`** 同步，便于与侧栏 **`active_id` 对比）等 **`RwSignal`** 聚合（**`Clone`/`Copy`**）；**`bind_stream_to_session`** 在 attach 时设绑定；**`clear_stream_resume_handles`** 统一重置 **`stream_job_id` / `stream_last_event_seq` / `stream_bound_session_id`**（会话切换、流结束、错误、换角等）；**`update_sessions_hydration` / `update_sessions_composer` / `update_sessions_stream_sse` / `update_sessions_branch` / `update_sessions_message_row`** 为 **`sessions` 向量**更新提供按来源命名的薄封装（便于检索竞态，语义约束依赖调用约定）。置于 crate 根以便 **`session_modal_row`** 与 **`app/`** 共用，避免 `app` ↔ 根模块循环依赖。
...
- **`composer_stream/`**：**`/chat/stream`** 接线拆分——**`mod.rs`**（**`ComposerStreamHandles`**、**`make_attach_chat_stream`** 与 **`spawn_local`**）、**`context.rs`**（**`ChatStreamCallbackCtx`**）、**`callbacks/`**（**`mod.rs`** 再导出 **`build_chat_stream_callbacks`**；**`stream_session_access`** 含 **`append_stream_assistant_chunk`**（**`on_delta`** 写会话唯一热路径）、**`helpers`** / **`builders`** / **`assemble`** 分担各 **`on_*`** 装配为 **`ChatStreamCallbacks`**，与 **`api::send_chat_stream`** 对齐）。
