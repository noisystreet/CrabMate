...
- **`chat/session_hydrate.rs`**：**`wire_session_hydration`**：订阅水合 nonce，拉取 **`GET /conversation/messages`** 并与本地 **`ChatSession`** 对齐（首屏 **`limit=80` 尾部页**；更早历史经 **`before_index`** prepend，**`history_*`** 元数据写回 **`ChatSession`**）；Effect 同步段用 **`try_hydration_wire_snapshot`** → **`HydrationWireSnapshot`**，异步段为 **`run_conversation_hydration_cycle`**；合并前守卫为 **`try_hydration_merge_precheck`**（`Result` 表达「可合并 / 跳过原因」），结果以 **`SessionHydrationMergeOutcome`** 命名；由 **`chat::wire_chat_session_lifecycle_effects`** 在首启 / Web UI 配置之后注册。服务端窗口切片见 **`web/conversation_messages_window.rs`**。
