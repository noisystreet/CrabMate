...
| **`crabmate-agent`**（`crates/crabmate-agent`） | 规划产物 **`plan_artifact`**、意图 **`intent_*`** / **`intent_routing`**、**`agent_turn::tool_execution`**（批模式、`ToolExecutionHost`、early-deny）、**`step_executor_policy`**、**`turn_budget`**、工作流反思控制器等**领域层**；根包 **`agent`** 再导出并承载 **`agent_turn` 执行面**（**`tool_execution_host`**、SSE **`emit`**）、**`hierarchy`** / **`workflow` 执行** 等编排。 |
