...
维护 **`task_level_evidence/`**、**`completion_suppression.rs`**、**`plan_optimizer`**、**`crabmate-types::real_user_message`** 时，请区分三层语义：
...
| **活跃证据窗口** | `task_level_evidence::check_active_user_goal_completion_evidence` | 自最后一条真实 user 起至末尾；用于 L2 外循环冗余 **tool_calls** 抑制、分阶段步后重复规划抑制、滚动视界早停。混合「构建/编写 + 分析」意图时须**各实现维度**均有证据，只读 shortcut 不可单独满足。 |
...
**顶层分支**（非分层：**`staged_plan_intent_gate`** 放行 → **`PlannedStep`** 滚动视界；拒绝 → **`Freeform`** 外循环。`**staged_plan_execution**` 不再参与顶层路由。`logical_dual_agent` 在门控放行时走逻辑双代理规划消息构造）：
