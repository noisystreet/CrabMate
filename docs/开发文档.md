...
**单轮工具回合的消息展示顺序**（旁注在对应工具之前、post-tool 尾泡、晚到 plain delta）见 **`docs/Turn布局设计.md`**（共享 **`crates/crabmate-turn-layout`** + Web **`TurnLayout`** / **`TurnCanonicalState`**；终端 TUI 本轮经 **`runtime/tui/run_session/turn_project`** 消费同一投影）。
