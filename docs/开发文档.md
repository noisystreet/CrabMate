...
- **范围**：**`smoke`** / **`user-data`** / **`conversation-messages-pagination`**（含 **`chat-load-older`** 点击）/ **`sse-control`** / **`prefs-theme`** / **`session-list`**；依赖 **`CM_E2E_FIXTURES=1`** 与 **`POST /e2e/fixtures/conversation`**。公共逻辑 **`e2e/tests/helpers/`**；CI job **`e2e`** 见 **`.github/workflows/ci.yml`**。关键 DOM：**`data-testid`**（**`chat-load-older`**、**`session-list-modal`**、**`approval-modal`** 等）。
