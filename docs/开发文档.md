...
- **流式布局 v2 不变量**：`crabmate-turn-layout::project_turn_web_v2` 将每条已关闭工具前旁注按 `tool_call_id` 投影为独立行；`callbacks/turn_layout/bubble_queue.rs` 只执行 insert-if-absent，ready 行不得改文、删除或移动。当前唯一可变正文是 loading 尾泡的 `stream_text_overlay`；v1 `turn-batch-narration` 仅保留历史会话兼容识别。
