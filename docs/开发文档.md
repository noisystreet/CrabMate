...
- **架构级变更**：若增删 `src/lib.rs` 顶层模块或调整分层职责，须更新本文「主要模块」与 **Mermaid** 图，并遵守 **`.trae/rules/architecture-docs-sync.mdc`**。
- **提交与质量**：根目录 **`.pre-commit-config.yaml`**（`cargo fmt`、根与 **`frontend/`** 的 **`cargo clippy -D warnings`**（暂存含 `frontend/` 时）、复杂度棘轮等）；提交说明 **Conventional Commits**，见 **`.trae/rules/conventional-commits.mdc`**。
- **依赖与许可证**：改动 **`Cargo.toml`** / **`Cargo.lock`** 时核对 **`deny.toml`** 与 CI，见 **`.trae/rules/dependencies-licenses.mdc`**。
...
2. **编排**：对话排队（**`chat_job_queue`**）、Agent 回合（**`agent/`**：**`agent_turn`**、上下文与消息管道、**`per_coord`**、工作流 **`workflow_execute`** / 反思控制器）。
...
5. **契约**：**`crabmate-types`**、**`crabmate-config`**、**`crabmate-llm`**、**`crabmate-agent`**（根包再导出）、**sse**（**`crabmate-sse-protocol`**）、**config**（再导出别名）。
...
- **P / R / E**：**P** = 向模型要一轮输出；**R** = 助手消息后的反思/终答门控（**`reflect`**、**`per_coord`**）；**E** = **工具执行**（**`execute_tools`** → **`ToolExecutionHost`** 注入 **`tool_registry`** / **`workflow_execute`**）。
- **消息变换**：出站供应商正文前的 strip/normalize 在 **`message_pipeline`** / **`context_window`**，与 **`llm::api`** 出站兜底一致；**tiktoken** prompt 粗估见 **`agent::tiktoken_prompt_tokens`**（`GET /conversation/messages`、debug 日志与 **`GET /status`** 的 **`tiktoken_prompt_counting_model`**）。

### Web 流式对话（概要）

`POST /chat/stream` → **`ChatJobQueue`** → **`TurnRunner`**（默认 **`run_agent_turn`**）→ **`llm`** SSE → 若有 **`tool_calls`** 则 **串行或并行只读批** 执行工具 → 结果以 **`role: tool`** 写回 → 控制面事件经 **`sse::protocol`** 下发。详细事件键与错误码以 **`docs/SSE协议.md`** 为准；前后端字段变更须同步 **`frontend`** 与 **`crabmate-sse-protocol`**。

**单轮工具回合的消息展示顺序**（旁注在对应工具之前、post-tool 尾泡、晚到 plain delta）见 **`docs/Turn布局设计.md`**（共享 **`crates/crabmate-turn-layout`** + Web **`TurnLayout`** / **`TurnCanonicalState`**；终端 TUI 本轮经 **`runtime/tui/run_session/turn_project`** 消费同一投影）。终端 TUI 后续对齐阶段见 **`docs/design/tui_align_tauri_display.md`**；中区展示所有权见 **`docs/design/tui_chat_display_ownership.md`**。
...
| 区域 | 职责 |
|------|------|
| **`agent/`** | 单轮与多轮编排：**`agent_turn`**（意图门控 → ReAct 外循环、工具执行 **execute_tools**）、**`context_window`** / **`message_pipeline`**、**`per_coord`**（终答与工作流协调）、**`workflow_tool_dispatch`**（`workflow_execute`）。 |
...
| **`crabmate-types`**（`crates/crabmate-types`） | OpenAI 兼容 **`Message`** / **`Tool`** / **`ChatRequest`**、会话变换 helpers、服务端注入 user 识别、**`llm_gateway_presets`**（网关 URL 预设表，供 Web / REPL/TUI）；根包 **`pub use crabmate_types as types`**。 |
| **`crabmate-chat-export`**（`crates/crabmate-chat-export`） | 会话导出信封：**`schema` / `schema_version` / `projection`（`raw`｜`display`）**、**`ChatSessionFile`**（完整 `Message`）与 **`DisplayChatSessionFile`**（展示投影）、Markdown 标题/组装；**`crabmate-runtime::chat_export`** 落盘 raw；Web/Tauri **`session_export`** 写 display。 |
| **`crabmate-approval`**（`crates/crabmate-approval`） | 敏感工具 **Web** 审批类型与 SSE 往返（**`run_web_tool_approval`**）；供 **`crabmate-workflow`** 与 **`crabmate-internal::tool_approval`** 共用，避免 workflow → internal。CLI/TUI 终端实现仍在 internal。依赖自检：**`scripts/check-crate-deps.sh`**。 |
| **`crabmate-web-host`**（`crates/crabmate-web-host`） | Axum Web **宿主**面（与 Leptos 包 **`crabmate-web`** / `frontend/` 不同）：HTTP DTO、`limits`/`chat_keys`、**`GET /web-ui`**、**`serve`** 体积分层与静态挂载；**禁止**依赖 **`crabmate-internal`**。根包 `build_app` 装配 `AppState` 与域路由后调用本 crate。路线见 **`docs/design/web_host_extract.md`**。 |
| **`crabmate-config`**（`crates/crabmate-config`） | **`AgentConfig`** 加载、`finalize`、热重载、CLI 定义、Skills/Cursor rules 合并、**`skills_slash`**（`/<skill-id>` 解析与强制选用）；根包 **`pub use crabmate_config as config`**。 |
| **`crabmate-llm`**（`crates/crabmate-llm`） | 厂商 **`vendor`**、出站 **`vendor_messages`**、**`requests`**（`ChatRequest` 构造）、共享 **`http_client`**、**`ChatCompletionsBackend`** trait、错误类型；根包 **`llm`** 再导出并承载 **`api::stream_chat`** / **`complete_chat_retrying`**。 |
| **`crabmate-agent`**（`crates/crabmate-agent`） | 规划产物 **`plan_artifact`**、意图 **`intent_*`** / **`intent_routing`**、**`agent_turn::tool_execution`**（批模式、early-deny 等；**不含**根包 **`ToolExecutionHost` trait**）、**`agent_turn::phase_vocabulary`**（编排相位对照真源）、**`agent_turn::outer_loop_*`**（外循环 FSM / reduce / driver / pre-gate reason；相位只经 **`OuterLoopDriver::record_*`**）、**`agent_turn::turn_route_decision`**（**`assess_turn_routing`** 为门控后唯一非早退出口）、**`agent_turn::turn_completion_decision`** / **`completion_suppression`** / **`task_level_evidence`** / **`run_command_dedupe`**（完成判定核）、**`step_executor_policy`**、**`turn_budget`**、工作流反思控制器等**领域层**；根包 **`agent`** 再导出并承载 **`agent_turn` 执行面**（**`ToolExecutionHost`** / **`tool_execution_host`**、SSE **`emit`**、**`TurnControlSink` / `TurnTerminalIo`**、**`outer_loop` IO**、**`turn_completion` 纠偏文案**）、**`workflow` 执行接合** 等编排。拆分进度见 **`docs/design/agent_turn_split.md`**；相位机设计见 **`docs/design/per_state_machine_consolidation.md`**；执行面解耦见 **`docs/design/turn_host_decouple.md`**。 |
| **`crabmate-turn-layout`**（`crates/crabmate-turn-layout`） | 单轮 canonical **`Turn`** + **`TurnEvent`** reducer；**`project_turn`** / **`project_turn_web_v2`** / **`project_turn_projection`**（`TurnProjection { finalized_rows, active_row }`）；金样 **`fixtures/turn_project_golden.jsonl`**；Web **`TurnLayout`** / **`projection_reconciler`** 与终端 TUI **`turn_project`** 消费见 **`docs/Turn布局设计.md`**。 |
| **`types`（根再导出）** | 与 `crabmate-types` 同一实现；仓库内仍常用 **`crate::types::`** 路径。 |
| **`observability.rs`** | tracing 初始化与 **`TracingChatTurn`**。 |

**细分源码路径**（如 **`tools/file`**）请在仓库内按模块浏览或检索；本文不再维护「逐文件索引」表，以免与 **`lib.rs`** 的 `mod` 列表重复且易过时。

---

## 前端（`frontend/`）

Leptos CSR：**`api/`**（`fetch`、JSON API、**`chat_stream`**）与 **`sse_dispatch`** 消费 SSE；**`app/`** 聊天与工作区 UI；**`message_format`** 与渲染管线。组件拆分与依赖约定见 **`docs/frontend/ARCHITECTURE.md`**（若仓库中存在）。**单轮流式消息顺序（Turn 布局）**见 **`docs/Turn布局设计.md`**。构建：**`cd frontend && trunk build`**。

**流式助手正文的只读真源**：SSE 热路径将尾条 `loading` 助手的增量写在 **`ChatSessionSignals::stream_text_overlay`**，与 **`sessions`** 中的 `StoredMessage` 可能短暂不一致。凡需与用户气泡一致的全文（侧栏全局消息搜索、会话内查找、复制、Markdown 快照等）应调用 **`stream_text_overlay::message_text_for_display_including_stream_overlay`**，并把 `parent_session_id` 设为承载该消息的 **`ChatSession::id`**；勿仅对 `StoredMessage` 调 **`message_format::message_text_for_display_ex`**。落盘前合并见 **`sessions_snapshot_with_stream_overlay_merged`**。

**客户端状态「谁写权威」**（便于新功能选对存储，避免再开隐式第三份）：

| 数据 | 权威来源 | 说明 |
|------|----------|------|
| 壳层偏好（主题、侧栏宽度、字体等） | **`GET`/`PUT` `/user-data/prefs`** | 由 **`user_prefs_sync`** 防抖同步；与 UI 信号对齐 |
| 会话消息列表与草稿 | 内存 **`ChatSessionSignals::sessions`** + 防抖 **`session_storage`** | 持久化到工作区 **`web_sessions.json`**（按 workspace 根分桶） |
| 流式尾段缓冲 | **`stream_text_overlay`** | 仅进行中回合；收尾 **`stream_overlay_take_into_stored_message`** 合并回 **`sessions`** |
| Web 侧 LLM 覆盖（`client_llm` 等） | 进程内信号 + **`client_llm_storage`** | 浏览器侧存储策略见 **`api/client_llm_storage.rs`**；与 **`/user-data`** 无自动合并 |
| 服务端模型 / 状态栏 | **`GET /status`** 等 | 以后端响应为准；前端 **`StatusData`** 为展示副本 |

---

## 数据与持久化（概要）

- **会话**：嵌入默认启用 SQLite（**`conversation_store_sqlite_path`** → **`.crabmate/conversations.db`**，**`conversation_store`**）；可在配置中清空该路径退回纯内存；可选会话条目不写入供应商请求的规则见 **`message_pipeline`**。
- **工作区**：工具与 **`POST /workspace`** 共用当前工作目录；**`.crabmate/`** 下可有提醒、导出等（详见 **`README`** / **`docs/配置说明.md`**）。
- **前端**：用户级状态经 **`GET`/`PUT` `/user-data/*`** 读写 **`~/.local/share/crabmate`**（`src/user_data/`、`frontend/src/api/user_data.rs`、`frontend/src/user_prefs_sync.rs`）；**不再**使用浏览器 **`localStorage`** 存会话/偏好/LLM 覆盖（无 migrate 端点）。会话列表按 **`GET /workspace` 根路径**分桶写入服务端 `web_sessions.json`；切换工作区见 **`session_workspace_partition.rs`**。设计见 **`docs/design/user_data_dir.md`**。
- **Tauri 桌面**：业务数据应走本机 **`serve`** 的 **`/user-data`**（不再依赖 **`com.crabmate.desktop/localstorage/`** 分叉）；WebKit 目录仅作过渡缓存。

---

## 常见扩展点

- **新工具**：在 **tools** 表驱动注册 + Schema + **`docs/工具说明.md`**；复杂执行策略时在 **tool_registry** 登记；遵守 **`.trae/rules/security-sensitive-surface.mdc`**。
- **SSE / API 变更**：同步 **Rust 路由**、**`crabmate-sse-protocol`**、**前端 dispatch**、**`docs/SSE协议.md`**（见 **`.trae/rules/api-sse-chat-protocol.mdc`**）。
- **新 HTTP 路由或配置键**：同步 **`README.md`**、**`docs/配置说明.md`** 与本节若涉及架构叙述。
- **编排侧向能力**（观众点评、分角色预算等）：挂现有 P/R 钩子，**不**新开并行阶段机；可变状态归属见 **`docs/design/run_loop_state_ownership.md`**。侧向点评须经 **`llm::complete_chat_retrying`**，默认不写上送 `messages`；共享 **`TurnBudgetCounter`** 墙钟（观测类 **`LlmCallBudgetClass`**）。观众设计见 **`docs/design/audience_critic_role.md`**（现行锚点 C/D）。

---

## 上下文管道（观测）

同步消息管道（**`message_pipeline::apply_session_sync_pipeline`**）在每轮 **P** 步前改写会话侧 `messages`；**阶段顺序**与实现细节见 **`src/agent/message_pipeline/mod.rs`** 模块文档及下文「**上下文窗口策略**」条目。

- **`GET /status`**：返回 **`message_pipeline_trim_count_hits`**、**`message_pipeline_trim_char_budget_hits`**、**`message_pipeline_tool_compress_hits`**、**`message_pipeline_orphan_tool_drops`**，均为**自进程启动以来**的累计命中次数（**非**单会话），用于对照配置是否实际触发了按条数/字符裁剪、工具正文压缩、孤立 `tool` 丢弃等逻辑；另返回 **`tiktoken_prompt_counting_model`**（**tiktoken-rs** 用于 prompt 计数时采用的 OpenAI 模型 id，未知 `model` 时回落 **`gpt-4` / `gpt-4o`**）。
- **`GET /conversation/messages`**：可选返回 **`tiktoken_prompt_tokens`**（`prompt_tokens` + `tiktoken_model`），与会话**落盘全量**消息经 **`conversation_messages_to_vendor_body`** 后的切片对齐，**不含**本轮 `tools` JSON；与网关真实计费可能仍有偏差。
- **SSE**：成功回合的 **`stream_ended`** / **`conversation_saved`** 可附带同形 **`tiktoken_prompt_tokens`**（Web 底栏即时更新，不必等水合）；规则与上条一致。
- **日志**：**`RUST_LOG` 含 debug**（如 **`crabmate=debug`**）时 **`context_window`** 在每轮 P 步前打一行汇总 **`message_pipeline session_sync: …`**（末尾可含 **`tiktoken_prompt_tokens≈`**，见 **`agent::tiktoken_prompt_tokens`**）；更细可设 **`RUST_LOG=crabmate::message_pipeline=trace`**，每步一行 **`session_sync_step`**（`stage=`、`message_count=`、`non_system_chars_est=` 等），无需把全局日志开到 trace。
...
2. **`system` 与「注入消息」分流**：**长期稳定**的角色准则、项目规则 → 尽量落在**会话首条或持久 `system`**。**易与用户口语混淆、或仅本轮有效**的约束 → 用 **服务端注入**的独立消息承载（专用 `user` 正文前缀、`system_intent_gate_hint` 等），与用户可见对话区分。
...
| 块 | 典型来源 | 组装时机 | 代码入口（锚点） |
|----|----------|----------|------------------|
| **L0 全局基底** | **`system_prompt` / `system_prompt_file`**（默认 **`config/prompts/base_system_prompt.md`**，领域无关） | **`config::finalize`** | **`config/finalize.rs`** |
| **L0b 编程工作台** | **`coding_workbench_increment_file`**（Git / 代码 / 开发工具；**`coding_workbench_enabled`** 全局开关；每角色 **`prepend_coding_workbench`**） | **`finalize`**（叠加在 L0 之后、L1 之前） | **`config/finalize.rs`** · **`config/agent_roles.rs`** |
| **L1 项目规则** | **`.trae/rules/*.mdc`**、可选 **`AGENTS.md`** | **`finalize`**：**`merge_system_prompt_with_cursor_rules`** | **`config/cursor_rules.rs`** |
| **L2 全局 Skills** | **`.crabmate/skills`**（目录扫描后仅注入轻量索引） | **`finalize`**：**`merge_system_prompt_with_skills_index`** | **`config/skills.rs`** |
| **L3 命名角色** | **`config/agent_roles.toml`** → 每角色 **`system_prompt_file`** / 内联正文 | **`finalize_agent_role_catalog`**：每角色再跑 L1+L2（Skills 仅注入索引，不注入正文） | **`config/agent_roles.rs`** |
| **L4 运行时附录** | **`thinking_avoid_echo_appendix`**（或内置附录）；**`agent_tool_stats`** 窗口统计 | **进程运行中**：首条 `system` 构建路径调用 **`ToolOutcomeRecorder::augment_system_prompt`** | **`context_bootstrap/prompt_compose.rs`** · **`tool_stats.rs`** |
| **L5 按轮 Skills** | 工作区 Skills + **当前用户消息** top-k；或用户 **`/<skill-id>`** 强制选用（**`skills_slash`** 解析后跳过 Top-K） | Web **`build_messages_for_turn`** 与 CLI `chat`/REPL 刷新首条 `system` 时 | **`config/skills.rs`** · **`config/skills_slash.rs`** · **`context_bootstrap/prompt_compose.rs`** · **`web/chat_handlers/chat/turn_build.rs`** · **`runtime/cli/chat.rs`** · **`runtime/cli/repl.rs`** |
...
| **L8 记忆与其它注入** | 长期记忆 strip、变更集等 | **`prepare_messages_for_model` / 管道前后** | **`memory`**、**`agent_turn`**、**`message_pipeline`**（各子路径） |
...
1. **配置装载**：L0 → L0b → L1 → L2（Skills 索引）→（每条角色）L3。  
2. **新会话首条消息**：**`system`** = `system_prompt_for_new_conversation(role)` + **L4**（+ Web/CLI **L5**）；随后可选 **L6** `user`；再跟真实用户 `user`。  
...
5. **每一轮 Agent**：**L7**（如有）→ **`prepare_turn_messages_for_model`** → **L8** 按配置插入。
...
- **增强 grounding**：在 token 预算内注入与工作区相关的约定（构建方式、目录与命令习惯等），有助于减少路径/命令类胡编（仍须与 **`tool_registry` 白名单**及 **工作区安全边界**一致，见 **`.trae/rules/security-sensitive-surface.mdc`**）。
...
- **已具备的工作区 + 用户语境驱动**：Web **L5**（**`skills_top_k`** 按**当前用户消息**从 **`.crabmate/skills`** 选材，或 **`/<skill-id>`** 强制注入，**`web/chat_handlers/chat/turn_build`** · **`builtin_skills`** · **`config/skills_slash`**）；**L6** 首轮将项目画像、living docs、依赖摘要等以**专用 `user`** 注入（与首条 **system** 分工，**`context_bootstrap/conversation_turn_bootstrap`**）；**L1** 在 **finalize** 时并入 **`.cursor/rules`**（及可选 **`AGENTS.md`**）。配置变更可走 **`POST /config/reload`**（不写盘的密钥等除外，见 **`docs/配置说明.md`**）。
...
- **强动态策略的统一入口**：若需「工作区快照 + 用户 query → 附加 **system** / **user** 块」的通用扩展，宜在 **`context_bootstrap/prompt_compose.rs`** 扩展（**`compose_first_system_for_turn`** 收敛 L3+L4+L5；角色切换见 **`agent_role_turn.rs`**），并由 Web **`build_messages_for_turn`**、CLI REPL 续聊与 **`/agent set`** 调用；内置失败降级与可观测性（参见上文「按工作区与用户提问的动态组装」）。

## `src/` 代码模块索引

> **维护约定**：增删 `lib.rs` 顶层 `mod`、调整目录/文件职责边界、或改变工具/路由/工作流的调用关系时，应同步更新**本节表格**与上文**架构设计**（含 Mermaid 是否与现状一致）。Cursor 规则见 **`.trae/rules/architecture-docs-sync.mdc`**。

### 顶层模块（与 `src/lib.rs` 中 `mod` 声明一致）

| 路径 | 职责摘要 |
|------|----------|
| `agent/` | **`agent_turn/`**：主循环（Web + CLI 经 `run_agent_turn`；目录分组 **`loop/`→`turn_loop`** / **`plan_reflect/`** / **`host/`**，见 **`docs/design/agent_turn_split.md`**）；**`run_dispatch`**（薄分发 → **`intent/at_turn_start`** → **`non_hierarchical_turn`** → ReAct 外循环）；**`turn_orchestration`** / **`turn_route_decision`**（`TurnOrchestrationMode`：`react` / `intent_at_turn_start_finished`）；**`errors`**（**`RunAgentTurnError`** / **`AgentTurnSubPhase`**）；**`plan`**（P）、**`reflect`**（R）、**`execute_tools`**（E）；**`outer_loop`**（**`outer_loop_fsm`** / **`outer_loop_reflect`** / **`outer_loop_step`**）；**`params`**（**`RunLoopParams`** / **`TurnPlannerHints`**）；**`sub_agent_policy`**（步级 `executor_kind` 工具收窄）；**`message_pipeline`** / **`context_window`**；**`reflection/plan_rewrite`**；**`per_coord`**（**`final_plan_gate`**）；**`per_plan_semantic_check`**；**`plan_artifact`** / **`workflow_reflection_controller`**；**`workflow_tool_dispatch`**（`workflow_execute` 接合）；**`acceptance/`**（`AcceptanceSpec` / 共用判定内核）。 原 **`hierarchy/`** 与分阶段 **`staged/`** 编排入口已移除。 |
...
| `chat_job_queue/` | **目录聚合（顶层 `mod chat_job_queue`）**：Web `/chat`、`/chat/stream` 有界队列与并发上限；**`mod.rs`** 为队列句柄与入队参数类型；**`stream_finish.rs`** 为流式回合收尾（**`post_turn_web_prepare_and_save`**、**`stream_job_outcome_after_agent_turn`**、`final_response` 兜底等）；**`worker/`**（**`mod.rs`** 薄分发 + **`stream_job.rs`** / **`json_job.rs`** 实现 **`run_stream_queued_job`** / **`run_json_queued_job`**）；**`tests.rs`** 为单元测试。运行中任务的 `PerTurnFlight` 注册供 `GET /status` 的 `per_active_jobs`；入队参数为 **`StreamSubmitParams`** / **`JsonSubmitParams`**：**`WebChatJobEnvelope`**（**`job_id`**、`Arc<WebChatQueueDeps>`、**`WebChatJobAppFacet`**、会话消息与 revision、角色、覆盖与审计等 Web/JSON 共用字段）+ 路径专有 **`reply_tx`**（JSON）或 **`stream_event_tx`** / **`web_approval_session`**（SSE）。**`Arc<WebChatQueueDeps>`**（`cfg` / `api_key` / `client` / `tools` / `chat_queue` / `long_term_memory` / `sse_stream_hub` / **`turn_runner`**）与 **`WebChatJobAppFacet`**（会话落盘、审批会话表、**`Arc<TurnProcessHandles>`**；由 **`AppState::chat_job_app_facet`** 克隆，**不**持整包 **`AppState`**）。另含 **`request_agent_role`** / **`persisted_active_agent_role`**、可选 **`WebChatLlmOverride`**（来自 JSON **`client_llm`**），在执行 **`run_agent_turn`** 前与快照 **`AgentConfig`** / 进程 **`api_key`** 合并（仅该任务，不写盘）。流式与 JSON 成功路径共用 **`post_turn_web_prepare_and_save`**；JSON 失败经 **`ChatJsonJobFailure`** 回传 **`chat_handlers`**，与 SSE 同源使用 **`RunAgentTurnError::http_api_error`** / **`sse_error_payload`**（稳定 **`code`**、用户可见 **`message`** / SSE **`error`**；SSE 的 **`reason_code`** 较宽，JSON **`ApiError.reason_code`** 仅 **`INTERNAL_ERROR`**；worker 侧 **`tracing`** 拼接 **`RunAgentTurnError::diag_log_kv()`**）。 |
| `clarification_questionnaire/` | **目录聚合**：**澄清问卷**协议与校验（**`mod.rs`** 再导出子模块符号）；内置工具 **`present_clarification_questionnaire`** 成功后，**`execute_tools`** 经 **`emit_tool_result_sse_and_append`**：有 SSE **`out`** 时在 **`tool_result` SSE** 之后补发 **`clarification_questionnaire`**；无 SSE 时可调用 **`TurnControlSink::clarification_questionnaire_hook`**（经 **`RunLoopCtx::io.control`**；**`crabmate tui`** 打开全屏表单并入队）。Web **`POST /chat*`** 可选 **`clarify_questionnaire_answers`**；TUI 经 **`repl_dispatch_chat_round`** 的 **`clarify_answers_for_next_user_message`** 与 **`expand_at_file_refs_in_user_message`** 后的正文合并（**`merge_user_text_with_clarification_answers`**）。 |
...
| `run_agent_turn.rs` | **`run_agent_turn`**：装配 **`RunLoopParams`** 并调用 **`agent::agent_turn::run_agent_turn_common`**（含 Chrome trace / **`tracing::Instrument`** 包裹）；**`RunAgentTurnParams`**、**`AgentTurnTransport`**、**`RunAgentTurnSharedInputs`** 等类型仍在 **`lib.rs`**，crate 根 **`pub use run_agent_turn::run_agent_turn`** 保持对外路径不变。 |
| `turn_runner.rs` | **`TurnRunner`** / **`DefaultTurnRunner`**：Web **`chat_job_queue`** worker 经 **`WebChatQueueDeps.turn_runner`** 注入；默认转发 **`run_agent_turn`**（见 **`docs/design/turn_host_decouple.md`** P2）。 |
...
| `text_sanitize/` | 用户可见正文轻量清洗（规划步骤描述自然化、相邻行去重等）。**`assistant_tail.rs`** 等；**`#[cfg(test)]`** **`tests.rs`**。 |
...
| `mcp/mod.rs` | **MCP**：**客户端**（stdio 与 **Streamable HTTP**）`try_open_turn_handle` → `McpTurnOpenResult` / `McpTurnSessions`（`rmcp`：`TokioChildProcess` / `StreamableHttpClientTransport`），按 **user-data `mcp_servers.json`** 多 server（指纹含 `id`+stdio 字段或 `url`+`headers`）**进程内复用**；`slug` 由 **`name` 自动生成**；远端 `tools/list` → OpenAI `Tool`（`mcp__{slug}__{remote}`，`inputSchema` 经 `sanitize_mcp_json_schema`）合并；连接失败写入 `skipped`/`last_error`（分类见 `classify_mcp_connect_error` → status **`last_error_kind`**）。远程 Bearer 可存系统钥匙串 **`mcp_bearer_{id}`**（`PUT …/remote-auth`；GET 仅 `has_bearer`）。执行经 `tool_registry::dispatch_tool` → `tools/call`。**服务端**（`mcp/server.rs`）：**`crabmate mcp serve`**（stdio/TCP）。**安全**：`command` 等价启动子进程；`url` 仅 HTTPS 或 loopback HTTP；GET 公开体不回传启动明文。OAuth / 延迟 schema / Resources 等待办。 |
...
| `context_bootstrap/project_profile.rs` | **项目画像**：只读扫描 `Cargo.toml` / `package.json` / 顶层目录 / 代码行数占比（**`project_metrics`** feature 启用 **tokei**，否则内置扩展名 walk）/ 可选 **`cargo metadata --no-deps`**，生成 Markdown；Web **`GET /workspace/profile`**；首轮与备忘、**`project_dependency_brief`** 合并见 **`build_first_turn_user_context_markdown`**（**`project_profile_inject_*`**）。 |
...
### 真实 user、完成证据抑制与步级验收（`agent_turn`）

> **说明**：原 `src/agent/hierarchy/`（Manager/Operator）与分阶段 `staged/` 编排入口已从树中移除；运行时固定为 **意图门控 → ReAct 外循环**。前端分层子目标 UI（`hierarchical_subgoal` 气泡/横幅）亦已拆除；历史 `timeline_log` kind 在 Web 侧显式忽略。

维护 **`task_level_evidence/`**、**`turn_completion.rs`**、**`completion_suppression.rs`**、**`crabmate-types::real_user_message`** 时，请区分三层语义：

| 概念 | 实现入口 | 含义 |
|------|----------|------|
| **trigger / 不变层** | `TurnPlannerHints::staged_immutable_user_goal` | 缓冲内**最新真实 user**（跳过 `is_server_injected_user_message`）；多轮会话须对齐**当前**人类诉求，勿用会话首条 user。 |
| **活跃证据窗口** | `task_level_evidence::check_active_user_goal_completion_evidence`；早停门控见 **`agent_turn/loop/turn_completion.rs`**（再导出 **`agent_turn::turn_completion`**）（`turn_early_stop_allowed` / `turn_redundant_tools_after_completion_allowed`）；结构化判定与日志见 **`turn_completion_decision.rs`**（`TurnCompletionDecision` + `tracing` 字段 `turn_completion_decision`） | 自最后一条真实 user 起至末尾；用于 L2 外循环冗余 **tool_calls** 抑制、滚动视界早停。混合「构建/编写 + 分析」意图时须**各实现维度**均有证据，只读 shortcut 不可单独满足。 |
| **步级窗口** | `crabmate-types::staged_step_window`（含 **`tool_messages_in_staged_step_window`**） | 自分步注入 `user` 至下一**真实 user**或**下一条分步注入**；步内外循环纠偏等 orchestration user **不**截断。`step_verifier` / `empty_execution` / 补丁观测共用。 |
| **步级验收** | `step_verifier.rs` | **单步**内 `acceptance` 与最后一条 `tool` 比对（步界同上）；**不**替代会话级完成证据 heuristic。 |

**编排注入 user**（外循环纠偏、重复规划拦截、状态机流转、全局重规划等）须在 **`crabmate-display-rules`** 登记为 server-injected，构造时优先 **`Message::user_staged_orchestration_injection`**。**真实 user** 解析共用 **`crabmate-types::real_user_message`**。**完成后冗余探针**判定（只读工具 / `run_command` 验证命令 / verify 步）统一见 **`agent_turn/completion_suppression.rs`**。

抑制触发时 **`tracing`**（目标：`crabmate::agent_turn`）记录 `goal_preview`（截断）与冗余 tool 名列表，便于排障。

**完成判定金样**：`fixtures/turn_completion_golden.jsonl` + **`agent_turn/loop/turn_completion_golden.rs`**（`cargo test golden_turn_completion`）；与 **`turn_completion_decision.rs`** 的 `evaluate_turn_*` 对齐。
...
- **`AppState`**：定义于 **`web::app_state`**，由 **`AppStateHttpCore`**（**`SharedAgentConfig`**、**`config_path_for_reload`**、`client` / `api_key` / `tools`、工作区覆盖、上传目录）、**`AppStateChatRuntime`**（**`chat_queue`**、**`Arc<WebChatQueueDeps>`**，与队列 worker 共享 `cfg` / `client` / `tools` / `sse_stream_hub` 等）、**`AppStateConversationRuntime`**（**`conversation_backing`**：`Arc<tokio::sync::RwLock<ConversationBacking>>`、**`conversation_id_counter`**；load/save/truncate/delete/count 实现落在此结构上，**`AppState`** 薄委托）、**`AppStateWebAux`**（审批会话表、**`long_term_memory`**、模型探测缓存、**`sse_stream_hub`**、**`process_handles`**（含 **`workspace_tasks_by_path`** 侧栏任务表）、**`async_chat_jobs`**）组合而成；队列入队经 **`chat_job_app_facet()`** 得到 **`WebChatJobAppFacet`**（会话 + 审批表 + **`Arc<TurnProcessHandles>`**），**不**把整包 **`AppState`** / 完整 **`ProcessHandles`** 塞进 **`WebChatJobEnvelope`**。窄 HTTP handler 经 **`web/app_state_facets`** 的 axum **`FromRef`** 投影（**`ConfigReloadFacet`** / **`UploadsFacet`** / **`WebTasksAppFacet`** / **`WebHealthAppFacet`** / **`WebStatusAppFacet`** / **`WebChangelogAppFacet`**，以及 **`AppStateHttpCore`** 直接投影供 workspace / skills / github / user_data / auth）；工作区路径解析落在 **`AppStateHttpCore`**（**`AppState`** 仍薄委托）。**`GET /skills`** / **`/workspace*`** / GitHub 响应信封 / **`ApiError`**·upload·config / **`chat` 请求体与 `chat_keys`** 等 JSON 类型已下沉 **`crabmate-web-host::http_types`**（**`ConversationMessagesResponseBody`** 因依赖运行时快照仍留根包）。Web 宿主提取路线见 **`docs/design/web_host_extract.md`**。**`effective_workspace_path`** 在尚未 **`POST /workspace`** 时为**空串**（**不**把 **`run_command_working_dir`** 当作已选 Web 工作区）；**`chat_handlers`** 与 **`tool_registry`**（**`workspace_is_set`**、**SyncDefault** 门禁、**`@`** 展开 **`WORKSPACE_NOT_SET`**）及 **`GET /health`**（空时依赖探测用 **`run_command_working_dir`**）与之对齐。**`chat_job_queue`** 在每轮 **`run_agent_turn`** 前 **`read`+`clone`** 得快照 `Arc<AgentConfig>`，避免长跑任务与热重载互相撕裂；crate 根 `pub(crate) use` 保持 `chat_job_queue` / `web/workspace` 等路径不变。
- **`RunAgentTurnParams`**：库根 `run_agent_turn` 的唯一入参（Web / CLI / benchmark 共用）。**`shared: RunAgentTurnSharedInputs`** 聚合 **`client` / `api_key` / `cfg` / `tools`**；其余字段：**`messages`**、**`effective_working_dir`**、**`workspace_is_set`**、**`transport`**（**`AgentTurnTransport`**：`out` / `cancel` / `per_flight` / Web·CLI 审批上下文 / `plain_terminal_stream` / **`llm_backend`**）、**`llm`**（**`AgentTurnLlmOverrides`**：`temperature_override`、planner/executor **model** 与 **api_base** / **api_key** 覆盖、`seed_override`）、记忆作用域、可选 **`read_file_turn_cache`**、**`turn_allowed_tool_names`**、**`tracing_chat_turn`**、可选 **`request_audit`**（Web/异步/流式/定时入队为 **`WebRequestAudit`**；CLI/bench 为 **`None`**）、**`process_handles`**（**`Arc<process_handles::TurnProcessHandles>`**：工作区变更集注册表、工具结果统计、**`HandlerLookupTable`**、**`sync_default_sandbox_backend`**、只读 TTL；**不含**侧栏任务 / CLI LTM——完整袋仍为 **`ProcessHandles`**，经 **`turn_handles_arc()`** 注入；**`benchmark_batch`** 与部分测试用 **`TurnProcessHandles::default_arc()`**，**非**进程级 `static` 单例）。**`WebChatStreamBuildArgs` / `WebChatJsonBuildArgs` / `CliTerminalChatBuildArgs`** 复用同一 **`RunAgentTurnSharedInputs`**。关联函数 **`web_chat_stream` / `web_chat_json` / `cli_terminal_chat` / `benchmark_batch`** 仍封装各入口默认组合。可选 **`llm_backend`**（`None` 时与历史一致，**`llm::default_chat_completions_backend()`** / **`OPENAI_COMPAT_BACKEND`**）。**`turn_allowed_tool_names`**：多角色白名单；**`run_agent_turn`** 在合并 MCP 后裁剪 **`tools_for_turn`** 并写入 **`RunLoopParams::ctx.tools_defs`**。**`cli_tool_ctx`** 在 **`transport`** 内，**`run_command`** 非白名单审批等语义不变。`agent_turn` 入口对 CLI 上下文 **`reset_command_stats`** 不变。
...
| `github_cli/` | GitHub CLI（`gh`）封装：**`mod.rs`**；**`api`**；**`common`**；**`pr_issue`** / **`pr_workflow`** / **`pr_mutate`** / **`pr_body`** / **`issue_create`** / **`run_ci`** / **`release_create`** / **`run_release_search`**；列表类经 **`command::run`** |
...
| `contract_map.rs` | `crate_contract_map`：只读 **契约地图**（`src/lib.rs`、`docs/SSE协议.md`、前端 `api/mod.rs` / `chat_stream/parser_v2.rs` / `sse_dispatch/types.rs`、`control_classify.rs`、AG-UI/V1 金样、`config/default_config.toml`、配置/路由文档等锚点的短预览 + 关键字行节选；可选 `extra_paths`） |
| `docs_health_sweep.rs` | `docs_health_sweep`：文档预览 + typos + codespell + `markdown_check_links`（外链 HEAD 不经 http_fetch 审批） |
| `rust_ide.rs`（`rust_ide_lsp_request.rs` / **`rust_ide_lsp_response_format.rs`**：单次 JSON-RPC、响应格式化） | 编译器 JSON、rust-analyzer LSP（definition/references/hover/documentSymbol/**implementation**/**typeDefinition**/**documentHighlight**/**workspace/symbol** 等）；**`lsp_rust_analyzer_handshake`** / **`ra_lsp_request_method_params`** / **`lsp_rust_analyzer_run`** 拆分 stdio 会话 |
...
| `http_fetch/` | `http_fetch`（GET/HEAD）与 `http_request`（POST/PUT/PATCH/DELETE + 可选 JSON body）；共享重定向记录、体长上限、`encoding_rs`+`chardetng` 正文解码、可选 **`text_format: html_text`**（**`scraper`/html5ever** 抽纯文本）、**`User-Agent: crabmate/<版本>`** 与 `http_fetch_allowed_prefixes` 的**同源 + 路径前缀边界**校验；**Web 流式 + 审批会话**与 **CLI** 下二者未匹配前缀均可审批（`http_request` 永久键含 **METHOD**）；**`workflow_execute` 同步节点**仍仅白名单。实现子模块：**`args`**（入参/解析/方法枚举）、**`decode`**（正文解码与 HTML→文本）、**`policy`**（白名单键与审批展示）、**`sync_fetch`**（阻塞客户端与 `run_direct`）。 |

## 核心机制：Agent 主循环与工具调用

核心流程在 **`src/run_agent_turn.rs`** 的 **`run_agent_turn`**（入参 **`RunAgentTurnParams`**，类型定义在 **`lib.rs`**）：内部组装 **`RunLoopParams`** 后调用 **`run_agent_turn_common`**（实现见 **`src/agent/agent_turn/mod.rs`**；薄分发见 **`run_dispatch.rs`**：意图门控 → **`assess_turn_routing`**（唯一非早退决议）→ 统一 driver；见 **`non_hierarchical_turn.rs`**；回合阶段见 **`turn_orchestration.rs`**（**`TurnOrchestrationMode`**：`intent_at_turn_start_finished` / `react`）；路由决议与相位词汇见 **`turn_route_decision`** / **`phase_vocabulary`**，结构化日志目标 **`crabmate::agent_turn`**、字段 **`turn_orchestration_mode`** / **`outer_loop_step`**）。路由金样：**`fixtures/turn_route_decision_golden.jsonl`**（`cargo test golden_turn_route_decision`）；外循环相位金样：**`fixtures/outer_loop_phase_golden.jsonl`**（`cargo test golden_outer_loop_phase`）。
...
- **调用模型**：默认经 **`llm::OpenAiCompatBackend`** 调用 **`src/llm/api/mod.rs`** 的 `stream_chat`（`POST {api_base}/chat/completions`）；实现按职责拆分为同级 **`sse_parser`**（SSE 行与 delta）、**`terminal_render`**（CLI 输出）、**`error_handler`**（HTTP/JSON 错误与请求体日志）；`stream: true`（SSE 增量）。CLI `--no-stream` 或 `RunAgentTurnParams { transport: AgentTurnTransport { no_stream: true, … }, … }` 时为 `stream: false`，按 OpenAI 兼容 `ChatResponse` 解析 `choices[0].message`（有正文则经 `out` 整段下发）。工具调用仅认 API 结构化 **`tool_calls`**（JSON function calling），不再从正文 DSML 物化。**自定义后端**：实现 **`llm::ChatCompletionsBackend`** 并在 **`RunAgentTurnParams { transport: AgentTurnTransport { llm_backend: Some(&your_backend), … }, … }`** 中传入；须保持与现有 `Message` / `tool_calls` / SSE `out` 语义一致。其它协议形态可在该 trait 内适配。**CLI 终端输出**：`RunAgentTurnParams { transport: AgentTurnTransport { plain_terminal_stream: true, … }, … }`（仅 `runtime::cli`）时 `render_to_terminal && out.is_none()` 下助手为纯文本流式/整段（**`reasoning_content`** 与 **`content`** 分色，不经 `message_display` 剥规划 JSON）；Web 队列等传 `plain_terminal_stream: false`，`out.is_none()` 时仍可用 `markdown_to_ansi`（避免污染服务端 stdout 的误用）。
- **步级子代理（规划 JSON）**：`steps[].executor_kind` 可选 `review_readonly` / `patch_write` / `test_runner`（`test_runner` 默认在可见工具中含 `run_command`，实际可执行命令仍仅限 `allowed_commands` 白名单，便于 `cargo build` / `cargo check` 等）；该步内 P 步仅下发收窄后的 `tools` 定义，E 步对越权 `tool_calls` 返回拒绝说明（仍记入 `role: tool`）；与 **`tool_registry::is_readonly_tool`** / **`write_effect_tools`** 一致，**不**替代 `run_command` 白名单或 MCP 审批语义。
- **规划器/执行器模式**（`[agent] planner_executor_mode` / `CM_PLANNER_EXECUTOR_MODE`）：运行时**固定** `single_agent`（ReAct 外循环）。`finalize` 与解析均将 `logical_dual_agent` / `hierarchical` 映射为 `single_agent`（兼容旧 TOML）；`doctor` / `GET /status` 的 **`effective_orchestration_path`** 恒为 `non_hierarchical: react outer loop`。
- **意图门控（默认 L2，弃用 L0/L1 兜底，共用）**：
  - 管线构成：默认 **`intent_l2_enabled=true`** 时经 `intent_l2_classifier` 无工具 LLM（合并路由 + 当前句）直接产出 `IntentDecision`；`intent_l2_min_confidence` 仅作为日志/时间线观测阈值，低于阈值仍采纳 L2，不再回退 L1。`intent_l0` 仍在澄清/确认流中做**路由文本**与特征（可合并前序 user + 当前短续接、含 `role: tool` 近期失败位），旧 L1 规则与 L0 提级（`intent_l0_routing_boost_enabled`）只在 L2 关闭、调用失败或解析失败时作为弃用兜底；可将 **`intent_l2_enabled=false`** 临时关闭以省一次调用。**`IntentContext`** 由 **`crabmate-agent::agent_turn::intent::context`** 的 **`build_intent_routing_context`** 装配；根包 **`IntentL2ClassifierHost`**（**`l2_classifier_host.rs`**）注入 L2 LLM；**非分层**主路径在 **`intent/at_turn_start`** 共用 **`assess_intent_routing_full_pipeline`**。**L2 缓存**：`intent_at_turn_start` 写入 **`turn_planner_hints.intent_routing_cache`**，同 effective task 下复用判定、跳过第二次 LLM。**简单编译 fast path**（`simple_execute_fast_path`）对单步 cmake/构建类 Execute 走 freeform。工具失败后用户短句「继续」在 L0 合并前序 user 并在 **`extract_effective_user_task`** 还原原任务。
  - 观测：`[INTENT_PIPELINE]` 与 SSE `intent_analysis`（`IntentMergeMeta` 含 `used_merged_continuation`、L0 字段、`tool_fail`，以及 L2 不可用时的脱敏原因，如 `api_key_missing` / `transport_error` / `json_parse_failed`）。
  - **`intent_at_turn_start`**：在 `intent_at_turn_start_enabled=true` 时于主循环前跑意图管线；阈值优先使用 `intent_non_hier_execute_low/high_threshold`（缺省回退 `intent_execute_low/high_threshold`）。寒暄 / 澄清 / 确认 / 只读 QA 等可早退或写入 **`intent_turn_gate_hint`**（由 **`outer_loop`** 首轮 P 前插入 **`Message::system_intent_gate_hint`**，P 结束后剔除）。放行后进入 **`run_agent_outer_loop`**（ReAct）。
  - **编排 mock 集成测**：`tests/run_agent_turn_orchestration_mock.rs`（`run_agent_turn` + mock LLM）。

- **工具批执行（E，`ToolExecutionHost`）**：
  - **领域层**（**`crabmate-agent::agent_turn::tool_execution`**）：`resolve_tool_batch_execution_mode`、`tool_policy_early_deny_message`、`ExecuteToolsBatchOutcome`；**`ToolExecutionHost`** trait（`dispatch_tool_call`、`prefetch_parallel_approval_failures`）。
  - **根包宿主**（**`agent_turn/execute/tool_execution_host.rs`**）：**`CrabmateToolExecutionHost`**（串行 + **`workflow_execute`**）、**`CrabmateParallelToolDispatch`** / **`CrabmateRegistryToolDispatch`**（并行只读 / registry 分发）；并行 **`http_fetch`** 经 **`dispatch_parallel_http_fetch`**（`spawn_blocking` + **`run_direct`**）。
  - **仍留根包**：**`execute/tools/emit.rs`**（SSE / 终端）、TTL 缓存、**`run_command`** 预检、审计与 replay。

#### 配置与 P/R/E 路径对照（`run_agent_turn_common`）

**P/R/E 含义**（与 `agent_turn/mod.rs` 注释一致）：**P** = 一次 `complete_chat_retrying`（向模型要本轮输出）；**R** = `per_reflect_after_assistant`（`async`）→ 终答时 `per_coord::after_final_assistant`（可选 `StopTurnPendingPlanConsistencyLlm` 再经 `per_plan_semantic_check` 侧向 `complete_chat_retrying`）；**E** = `per_execute_tools_web`。**`final_plan_requirement`** / **`plan_rewrite_max_attempts`** 只作用于 **R**（模型以非 `tool_calls` 结束的轮次）。**工作流反思**：`prepare_workflow_execute` 在注入 `instruction_type == workflow_reflection_plan_next` 时置位 `plan_requirement_source`；`append_tool_result_and_reflection` 对 **`workflow_reflection_next`** 同样置位，使后续轮次终答仍与 `final_plan_requirement = workflow_reflection` 一致。

**R 三条轨（正交，勿混用名词）**：（1）**终答 Gate** — `per_coord/final_plan_gate.rs`，决策原因见 **`FinalPlanGateDecisionReason`**；（2）**工作流反思** — `workflow_reflection_controller` + `prepare_workflow_execute`，仅 `workflow_execute` 路径，经 **`PlanRequirementSource`** 与 Gate 衔接（金样 **`fixtures/workflow_to_plan_requirement_golden.jsonl`** / `cargo test golden_workflow_to_plan_requirement`）；（3）**外循环 Gate 前纠偏** — `outer_loop_reflect.rs`（build-idle / 终答缺失），计数在 **`OuterLoopReflectMemo`**（暂住 `PerTurnCounters`），原因见 **`OuterLoopReflectPreGateReason`**，**不**改写 Gate 判定。已删除未接线的 **`plan_rewrite_controller`**（策略与计数以 **`PerCoordinator`** + **`final_plan_gate`** 为准）。顶层形态见 **`turn_orchestration_mode`**（`react` / `intent_at_turn_start_finished`）。状态归属见 **`docs/design/run_loop_state_ownership.md`**。

**设计稿（现状备忘）**：见 **`docs/design/per_state_machine_consolidation.md`**（终答 Gate / 外循环 FSM / 工作流反思；**勿**再按已删的 staged/hierarchy 全局单表落地）。
...
**官方 VS Code 扩展（设计备忘）**：见 **`docs/design/vscode_extension.md`**（Webview + HTTP/SSE 连 `serve`、分期能力、Remote/WSL 拓扑、与 MCP / `plugins/*.json` 分工、安全与交付；**未**承诺实现时间表）。

**Web / 桌面内置 IDE 模式（完善规划）**：见 **`docs/design/ide_mode_roadmap.md`**（对话↔编辑器互斥基线、边聊边改 / 变更审阅 / 深链 / 诊断分期、与 VS Code 扩展分工；**未**承诺实现时间表）。

**本机用户数据目录（`~/.local/share/crabmate`）**：见 **`docs/design/user_data_dir.md`**（`CM_CRABMATE_USER_DATA_DIR`、`/user-data/*`、`cargo run -- doctor` 路径摘要）。

**顶层分支**（意图门控 → ReAct 外循环）：

| 路径 | 顶层入口 | 行为 |
|-----|-----|-----|
| 意图早退 | `intent_at_turn_start` 写入终答 | `turn_orchestration_mode=intent_at_turn_start_finished`，本回合结束 |
| 放行后主执行 | `run_agent_outer_loop`（经 **`non_hierarchical_turn`** **`ReAct`**） | 每圈 **P（带 tools）→ R → E** 直至本轮结束 |

**终答规划策略**（只要某路径进入 `run_agent_outer_loop`，每步结束时的 R 均受下表约束）：
...
- **`plan_artifact::format_plan_steps_markdown` / `format_agent_reply_plan_for_display`**：对合法 v1 规划生成**简单 Markdown 有序列表**（后者另含围栏前自然语言段落）；前端 `agentPlanDisplay` / `ChatPanel` 展示用，**不**改写 `Message.content`。解析与 **`strip_agent_reply_plan_fence_blocks_for_display`** 将围栏首行 **`json` / `markdown` / `md`**（忽略大小写，可含前导空行）视为等价语言标签；**无**语言行的裸 \`\`\` 围栏内即使以 `{` 开头也不当作规划 JSON 流式缓冲（与 `message_display` / 前端 `agentPlanDisplay` 对齐）。
...
- **`GET /status`** 返回 `final_plan_requirement`、`plan_rewrite_max_attempts`、`final_plan_require_strict_workflow_node_coverage`、`final_plan_semantic_check_enabled`、`final_plan_semantic_check_max_non_readonly_tools`、`final_plan_semantic_check_max_tokens`，便于与 `reflection_default_max_rounds` 一起核对运行态；另返回 **`per_active_jobs`**（仅队列内**正在执行**的 `/chat`、`/chat/stream` 任务）：每项含 `job_id`、`awaiting_plan_rewrite_model`（已追加**终答**规划重写 user 消息、等待下一轮模型输出）、`plan_rewrite_attempts`（**终答** `plan_rewrite` 已用次数）、`require_plan_in_final_content`。与前端「会话」无稳定 id 对应；若需按会话展示，需扩展请求体/存储后再关联 `job_id` 或自建会话字段。
...
- **`lib.rs`**：crate 根模块；**`RunAgentTurnParams`** / **`AgentTurnTransport`** 等与入口相关的类型，以及 **`pub use run_agent_turn::run_agent_turn`**（实现体在 **`run_agent_turn.rs`**）；Axum Web 路由与 handler 等。**`run()`** 薄转发至 **`cli_run::run`**（CLI 编排体）。**对外再导出** `run`、`load_config`、`AgentConfig`、`Message`、`Tool`、`build_tools`、`build_tools_filtered`、`build_tools_with_options`、`ToolsBuildOptions`、`dev_tag`、**`tool_sandbox`** 等，供集成测试与其它二进制复用；**`pub mod agent`** 供测试等工作流相关集成调用（如 **`agent::workflow::run_workflow_execute_tool`**）。来自 **`crabmate-internal`** 的多数模块（`tools`、`mcp`、`health`、`workspace` 等）以 **`pub(crate) use`** 挂在根上，包内仍写 **`crate::…`**，**不**再作为公共 API 暴露（策略见 **`docs/design/crate_dep_policy.md`**）。
- **`main.rs`**：薄入口；普通路径 `crabmate::run()`；**`tool-runner-internal`** 走 **`crabmate::tool_sandbox::tool_runner_internal_main`**；**`CliExitError`** 映射约定退出码（契约见 **`tests/cli_contract.rs`**）。
...
- **Web 服务**：使用 axum 路由，核心接口包括：
  ...
  - `GET /status`：状态栏数据（模型、`api_base`、**`llm_http_auth_mode`**（`bearer` / `none`）、`max_tokens`、**`llm_context_tokens`**、**`effective_context_char_budget`**、**`tiktoken_prompt_counting_model`**（tiktoken-rs 计数回落 id）、`temperature`、**`llm_seed`**（默认 seed，未配置为 `null`）、**`tool_count` / `tool_names` / `tool_dispatch_registry`**、`reflection_default_max_rounds`、**`final_plan_requirement` / `plan_rewrite_max_attempts`**、**`max_message_history` / `tool_message_max_chars` / `context_char_budget` / `context_summary_trigger_chars`**、**`message_pipeline_trim_count_hits` / `message_pipeline_trim_char_budget_hits` / `message_pipeline_tool_compress_hits` / `message_pipeline_orphan_tool_drops`**（自进程启动以来同步管道**实际**触发次数，累计非会话级）、**`chat_queue_*` / `parallel_readonly_tools_max` / `chat_queue_recent_jobs` / `per_active_jobs`**、`conversation_store_entries`、**`conversation_store_sqlite_path_configured` / `conversation_store_sqlite_active`**、**`long_term_memory_enabled` / `long_term_memory_vector_backend` / `long_term_memory_store_ready` / `long_term_memory_index_errors`**）
  - `POST /config/session/conversation-store`：在**当前进程**内切换 Web 会话存储（**`{"sqlite":true|false}`**；**不**写配置文件；与 **`POST /config/reload`** 相同鉴权）
  - `GET /health`：健康检查（API_KEY/静态目录/工作区可写/依赖命令）；实现见 `health.rs`。
  - `GET|POST /workspace` + `GET|POST|DELETE /workspace/file` + `POST|DELETE /workspace/dir`：工作区浏览、读写删文件与创建/删除目录（`POST /workspace/dir` 在 JSON 中带 **`delete=true`** 时删除目录，供旧后端或 **`DELETE`** 不可用时的前端回退；`GET /workspace/file` 仅读取不超过 1 MiB；正文解码与 `read_file` 一致，可选查询参数 **`encoding`**，默认 UTF-8 严格，非法序列返回错误而非有损替换；Web UI 工作区树右键菜单调用上述写删接口）。`POST /workspace` 对非空路径执行目录存在性、`workspace_allowed_roots` 白名单与敏感系统目录黑名单校验，避免把运行时工作区切到 `/proc`、`/sys`、`/dev`、`/etc`、`/usr` 等区域。
...
- **扩展内置工具的推荐顺序**（按依赖关系自上而下；多数工具落在 **`SyncDefault`**，无需改 **`tool_registry`**）：
  1. **设计与安全**：是否子进程、出网、写盘；与 **`run_command` 白名单**、**`http_fetch_allowed_prefixes`** / **`http_request`** 审批、工作区路径策略、**`tool_call_explain_*`** / **`[tool_registry] write_effect_tools`**（只读判定）是否一致；见 **`.trae/rules/security-sensitive-surface.mdc`**。
  ...
  11. **用户可见文档与索引**：**`docs/工具说明.md`**（能力说明与 JSON 示例）；若增删 **`tools/mod.rs` 的 `mod`**，更新本节 **「`src/tools/` 子文件」**表（**`.trae/rules/architecture-docs-sync.mdc`**）；**`README.md`** 仅在行为或能力对用户可见时同步。
...
- **`http_fetch/`**：`http_fetch`（阻塞 GET/HEAD）与 `http_request`（阻塞 POST/PUT/PATCH/DELETE + 可选 JSON body）；共用 `redirect::Policy` 记录重定向跳数与响应截断。二者在 **`tool_registry` 异步路径**（Web 流式 + 审批会话、CLI）下未匹配 `http_fetch_allowed_prefixes` 时均可走审批（`http_request` 白名单键为 **`http_request:<METHOD>:<URL>`**）；**`run_tool` / workflow 节点**仍仅白名单前缀。
- **`command.rs`**：命令白名单 + 超时 + 输出截断；配合 `allowed_commands` 与工作区路径限制。
- **`github_cli/`**：GitHub CLI 封装（`gh_pr_*` 含 **`gh_pr_checks`**（**`structured`** 失败摘要）、**`gh_pr_create`**（**`auto_body`** + `--body-file`）、**`gh_pr_merge`/`review`/`comment`**、**`gh_pr_body_draft`**；`gh_issue_*` 含 **`gh_issue_create`**；`gh_run_*` 含 **`gh_run_failure_summary`**、**`gh_run_rerun`**；`gh_release_*` 含 **`gh_release_create`**（**`auto_notes`**）；`gh_search` / `gh_api`）：结构化参数；JSON 成功时附加格式化块。写远端工具（含 **`gh_api`** 变更类 HTTP）列入写副作用。依赖 **`allowed_commands` 含 `gh`**。
...
- **`runtime`**：CLI 运行时逻辑，负责交互式 CLI、**`chat` 单次**与调用 `run_agent_turn`。
  ...
  - **`runtime/cli_mcp`**：**`mcp list`** 只读输出进程内 MCP 多服务器缓存（`mcp::mcp_servers_runtime_status`；配置真源 **`user_data` `mcp_servers.json`**）；行内含 **`transport=stdio|remote`** 与失败时的 **`last_error_kind`**；可选 **`--probe`** 调用 `try_open_turn_handle` 刷新缓存（失败时打印 `skipped`/`last_error`）。**`mcp serve`** 支持 stdio（默认）与 **TCP**（`--port N`）两种传输模式，均通过 `rmcp` 的 `TransportAdapterAsyncCombinedRW` 将 `TcpStream` 直接用作 transport。TCP 模式可配合 SSH 隧道实现远程开发。交互式 CLI **`/mcp`**、**`/mcp list`**、**`/mcp probe`** 走同一实现（`run_mcp_list`，`repl_context=true` 时提示语指向 **`/mcp probe`**）。Web：**`GET/PUT /user-data/mcp-servers`**（GET 仅 `has_*`、不返回启动明文）、**`PUT …/{id}/remote-auth`**（Bearer → 系统钥匙串 **`mcp_bearer_{id}`**）、**`POST …/import`**（结构化 `command`/`args`/`env`/`cwd` 或 `url`）、**`…/status`**（含 `transport`/`last_error`/`last_error_kind`）、**`POST …/probe`**；设置页 **`#settings/mcp`**（JSON 解析在服务端；草稿上提至设置页壳层，删除/改服务器计入顶栏 **「保存全部」** dirty，亦可点页内 **「保存 MCP 配置」**；远程行可单独保存 Bearer）。
  ...
  - **`runtime/tool_replay`**：从 **`ChatSessionFile`**（须 **`projection=raw`**；display 导出会被拒绝）消息序列提取 `assistant.tool_calls` 与对应 `tool` 消息，写出 **`ToolReplayFile`** fixture；**`tool-replay run`** 按步调用 **`tools::run_tool`**（与 Agent 路径相同，**无** LLM、**无** CLI 审批交互）。**`--compare-recorded`** 与 `recorded_output` 全等比较失败时退出码 **`EXIT_TOOL_REPLAY_MISMATCH`（6）**。
...
- **多套预设主题（扩展 `data-theme`）**：当前 Web 为 **`dark`**（默认 `:root`）、**`light`**、**`material`**、**`high-contrast`**（经 **`/user-data/prefs`** 持久化 **`theme`**、`<html data-theme>`、**`app_prefs::THEME_SLUGS`** 白名单）；扩展约定见 **`docs/design/web_theme_presets.md`**。
...
### E2E（`desktop-tauri/src-tauri/tests/`，Victauri）

- **框架**：基于 [Victauri](https://github.com/runyourempire/victauri)（`victauri-plugin` + `victauri-test`），在 Tauri WebView 内通过 `eval_js` / `Locator` 运行端到端测试（`desktop-tauri/src-tauri/tests/`）。
- **测试目录**：`desktop-tauri/src-tauri/tests/`，**50 个测试用例**覆盖 UI 交互、API 播种、SSE 存根、IDE 布局、真实 LLM。
- **运行**：`cargo test`（无 `VICTAURI_E2E` 时自动跳过）；真机需先 `cargo tauri dev` + `VICTAURI_E2E=1 CM_E2E_FIXTURES=1 cargo test`。
- **CI**：`check-clippy-test` job 在 `desktop-tauri/src-tauri` 中运行 `cargo check --tests` + `cargo clippy` + `cargo test`。

### `frontend/src/api/`（`mod.rs` 聚合）

- **`http.rs`**：JSON `fetch`（工作区、任务、上传、会话分支、审批等）。
- **`chat_stream/`**：**`/chat/stream`** SSE 读取与 **`send_chat_stream`**；UTF-8 分块拼接、`data:` 行分发、`[DONE]`；**`upload_files_multipart`**（`POST /upload`）与请求体 **`image_urls`**（与后端多模态 `user` 对齐）。
...
- 纯文本 / AG-UI 正文增量经 **`parser_v2`** 回调；控制面 JSON 分发见 **`frontend/src/api/chat_stream/parser_v2.rs`**（回调形状 **`frontend/src/sse_dispatch/types.rs`**）。
...
- **`types.rs`**：控制面载荷形状（**`ToolResultInfo`** 等）与 **`SseControlSink`** / **`SseTurnPhaseHooks`** 回调分组；由 **`parser_v2`** 填充并调用。
- AG-UI 金样：**`fixtures/sse_ag_ui_golden.jsonl`** + **`golden_ag_ui_v2_parser_matches_expected`**。V1/`classify_sse_control_outcome` 参考向量见 **`fixtures/sse_control_golden.jsonl`**（IM bridge 等）。
...
- **目标架构与重构方向**（页面分层、依赖规则、`wire_*` 约定）：[`docs/frontend/ARCHITECTURE.md`](frontend/ARCHITECTURE.md)。

### `frontend/src/client_llm_presets.rs`

- 薄再导出 **`crabmate_types::llm_gateway_presets`**（**`CLIENT_LLM_API_BASE_PRESETS`** / **`api_base_select_value_for_draft`**）；与 REPL/TUI **`/api-base set <预设id>`** 同源。

### `frontend/src/i18n.rs`

- **`Locale`**：**`zh-Hans`** / **`en`**；经 **`/user-data/prefs`** 的 **`locale`**（与 **`app_prefs::LOCALE_KEY`** 语义一致）。设置弹窗可切换语言；**`<html lang>`** 由 **`app_shell_effects::wire_sync_locale_html_lang`** 同步。
...
- **`app_signals/`**（**`mod.rs`** 聚合 **`AppSignals`** / **`ChatDomainWiringSignals`**；子文件 **`shell_ui`**、**`chat_composer`**、**`approval`**、**`stream_control`**、**`sidebar`**、**`modal`**、**`llm_settings`**、**`resize`**、**`workspace`**、**`status`**）：**`RwSignal`** 按域拆分，减少单文件体量。
- **`mod.rs`**：单根 **`App`**；**`RwSignal`** 声明、调用 **`app_shell_effects`** 与各域聚合 **`wire_*`**（**`chat::wire_chat_session_lifecycle_effects`**、**`wire_workspace_domain_effects`**、**`wire_status_tasks_domain_effects`**、**`chat::wire_chat_domain_effects`** 等）、主 `view!` 组合子视图；**`/status`、侧栏 `/tasks`** 的拉取闭包与「初始化后补拉 / 切到任务侧栏再拉」由 **`status_tasks_wiring::wire_status_tasks_domain_effects`** 注册。与 **`GET /conversation/messages`** 对齐的会话水合仍由 **`chat::session_hydrate::wire_session_hydration`** 实现，安装顺序由 **`chat::wire_chat_session_lifecycle`** 与原先在 **`App` 内一致**；**`ChatSessionSignals`**（见 **`chat_session_state`**）在侧栏、聊天列、会话管理模态、状态栏等处整包下传；**`StatusTasksSignals`**（见 **`status_tasks_state`**）供底栏与右栏任务区。聊天主路径（流式、滚动、查找、主列 UI）见 **`chat`** 子模块；Workspace 树与变更集侧栏级 **`Effect`** 见 **`wire_workspace_domain`**（内部再调 **`workspace_panel`**、**`changelist_modal`** 的细粒度 **`wire_*`**）。
- **`app_shell_effects/`**（**`mod.rs`** 再导出；实现按职责分子文件）：**`ShellEscapeSignals`** 与壳级 **`wire_*`**——**`persist_prefs`**（侧栏视图 / 底栏 / 角色 / 宽度 / 左侧栏收起等）、**`sync_dom`**（**`data-theme`** / **`lang`** / **`data-bg-decor`**）、**`approval_follow`**（待审批会话切换时审批条展开态）、**`settings_llm_open`**（打开设置时填充 LLM 草稿；**`status_snapshot_for_llm_drafts`** 刻意 **`get_untracked`**）、**`session_delete_hotkey`**、**`escape`**（**全局 `Escape`**，在 **`input`/`textarea`/`contenteditable` 外**按层级关闭菜单 / 查找 / 模态——**`wire_escape_key_layered_dismiss`**）。首启 **`/user-data`** 拉取、**`GET /web-ui`**、会话列表防抖落盘见 **`chat::session_storage`**，由 **`chat::wire_chat_session_lifecycle_effects`** 注册。**`mod.rs`** 文档说明各块与「勿隐式订阅」边界；注册顺序见 **`app_shell_bootstrap::bootstrap_app_shell`**（别名 **`app_shell_init::init_app_shell`**）。
- **`shell_prefs_storage.rs`**：首屏壳 UI 初始快照 **`read_shell_ui_initial_snapshot`**（供 **`ShellUISignals::new`**；持久化真源为 **`/user-data/prefs`**）；主题 **`data-theme`** / 经纪人角色键等与 DOM 同步写回仍在本模块或 **`app_shell_effects`**。
...
- **`chat/session_hydrate.rs`**：**`wire_session_hydration`**：订阅水合 nonce，拉取 **`GET /conversation/messages`** 并与本地 **`ChatSession`** 对齐（首屏 **`limit=80` 尾部页**；更早历史经 **`before_index`** prepend，**`history_*`** 元数据写回 **`ChatSession`**）；Effect 同步段用 **`try_hydration_wire_snapshot`** → **`HydrationWireSnapshot`**，异步段为 **`run_conversation_hydration_cycle`**；合并前守卫为 **`try_hydration_merge_precheck`**（`Result` 表达「可合并 / 跳过原因」），结果以 **`SessionHydrationMergeOutcome`** 命名。非空 v2 finalized 投影不进入 legacy pool 重排；较新服务端 revision 含新增 user 回合时，从首个服务端独有 user 起追加 canonical 后缀。该水合由 **`chat::wire_chat_session_lifecycle_effects`** 在首启 / Web UI 配置之后注册。服务端窗口切片见 **`web/conversation_messages_window.rs`**。
- **`chat/scroll_shell.rs`** / **`chat/scroll_follow.rs`** / **`chat/messages_list.rs`**：消息列滚动壳（**`ChatScrollShellSignals`**：StickToBottom pin/`on:scroll` 近底 re-pin / **`on:wheel` 上滑 unpin** / 指针拖离底 unpin、prepend 锚点补偿；**不再**用 `scrollTop` 下降判上滑）；内容侧 **`wire_content_follow_scroll`**（含 `tool_output_chunks` 指纹）+ **`engage_follow_and_scroll_bottom`** + ResizeObserver（内容根 `.chat-tui-transcript` / `.chat-thread`）；全量 **`ChatChunk`** 渲染（气泡路径现默认不挂载）；更早历史仅经顶部 **「加载更早」** 按钮触发（**`try_load_older_messages_for_active_session`**）。
...
- **`workspace_panel_state.rs`**：**`WorkspacePanelSignals`**：工作区目录树、根路径草稿、设置/选目录忙状态、**`recent_workspace_roots`** 等 **`RwSignal`** 聚合，供 **`make_refresh_workspace`**、**`side_column_view`** 传递。**`workspace_root_actions.rs`**：顶栏正中只读路径标题与 **「文件 → 选择工作区目录 / 最近的工作区」（级联子菜单）**（Tauri 原生选夹 / 浏览器 `prompt` 手输绝对路径 / 一键打开已记录路径）共用提交逻辑；成功后写入 **`prefs.last_workspace_root`** 与 **`prefs.recent_workspace_roots`**（最多 10 项）。
...
- **`sidebar_nav/`**：左侧导航与会话列表（**`mod.rs`** 组装；**`debounce`** / **`context_menus`** / **`search_panel`** / **`session_rail`**）；会话项 **右键菜单**含收藏/置顶切换；列表顺序见 **`session_sort`**。**`debounce_signal_to_effect`** 对会话标题筛选与跨会话消息搜索做 **`TimeoutFuture`** 防抖（规则见 **`debounce_schedule`**）。**`mobile_shell_header.rs`**：统一壳顶栏（对话模式含 **文件** 菜单；编辑器模式为文件/编辑/视图）。
...
- 流式消息与自动跟底策略（用户滚轮上滑 / 指针拖离底暂停；近底恢复；工具 live 用 **`TuiBodyPatch::ToolRow`** 改文案）。

### `frontend/src/app/chat/`

- **`mod.rs`**：聊天域 **`pub`** 再导出（供 **`app::App`**）；与 [`docs/frontend/ARCHITECTURE.md`](frontend/ARCHITECTURE.md) **「`app/chat_*`」** 对齐。
- **`wire_chat_session_lifecycle.rs`**：**`wire_chat_session_lifecycle_effects`**——按顺序调用 **`chat::session_storage`** 的 **`wire_initial_sessions_from_storage`**、**`wire_web_ui_config_once_after_init`**、**`chat::session_hydrate::wire_session_hydration`**、**`wire_persist_chat_sessions`**（顺序与原先一致；会话写盘带防抖）。
...
- **`composer_wires/`**：**`mod.rs`** 中 **`wire_chat_composer_streams`**；**`helpers`**（澄清折叠、推用户行 + Loading、**`begin_stream_shell_turn`**）；**`follow_up`**（**`StreamFollowUpWiring`** + **`wire_stream_follow_up_effect`**：唯一 **`Effect`** 消费 **`ComposerStreamFollowUp`**）。
- **`composer_follow_up.rs`**：**`ComposerStreamFollowUp`**——截断再生与失败助手重试的**单一待发队列**（单 **`RwSignal`**，取代原先两个 `Option` 信号）；由 **`composer_wires::follow_up`** 侧唯一 **`Effect`** 在门闩通过后清空并 **`attach`**。
- **`stream_follow_up_gates.rs`**：**`RegenAttachGate`**（busy / 工具忙 / abort 槽 / 冲突 Loading 快照）与 **`compose_user_send_allowed`**（初始化 + 回合不忙 + 非空输入），由 **`composer_wires`** 在主发送与截断再生路径复用。
...
- **`handles.rs`**：**`ChatColumnShell`**（内嵌 **`ComposerStreamShell`**，与流式接线共用 `status_busy` / `status_err` / `pending_clarification` 等）、**`WireComposerStreamsArgs`**、**`ComposerStreamShell`**（流式与壳层共享的 `RwSignal`/中止/工作区刷新等聚合）；**`ChatMessagesPaneSignals`** / **`ChatComposerPaneSignals`** 与 **`ChatColumnShell::messages_pane_signals` / `composer_pane_signals`** 单点组装主列 **`view!`** 参数；**`ComposerStreamApprovalSignals`** 提供 **`clear_pending_user_interactions` / `replace_with_pending_approval` / `replace_with_pending_clarification`**，在发起新流与 SSE 回调路径上保证**待审批与待澄清互斥**— 压缩 **`App` → `chat_column_view` / `wire_chat_composer_streams`** 的参数面，并与 **`composer_stream/context`**（**`ChatStreamCallbackCtx`**）成组使用。
- **`composer_stream/`**：**`/chat/stream`** 接线拆分——**`mod.rs`**（**`ComposerStreamHandles`**、**`make_attach_chat_stream`** 将 bump/bind/Abort/`ChatStreamCallbackCtx` 同步阶段委托 **`stream_attach_lifecycle::prepare_stream_attach`** 后与 **`spawn_local`**）、**`stream_attach_lifecycle.rs`**（**`StreamAttachPrepared`**；**`approval_session_id`** 与回调上下文单次配对，避免 HTTP 与 SSE 侧 id 漂移）、**`context.rs`**（**`ChatStreamCallbackCtx`**）、**`stream_turn_scratch_state`**（单轮流 **lane + 尾泡 id + post_tool + 工具 FIFO** 收进 **一处 `RefCell`**，与 Leptos 信号解耦）、**`stream_sse_scratch`**（**`StreamSseScratch`** 委托 **`stream_turn_scratch_state`**，并维护 **`stream_control_reducer`**（**`StreamControlPhase`** 粗粒度归约））、**`stream_turn_state`**（**`StreamModelOutputLane`** 及 `lane_*` 的 `Cell` 薄封装）、**`callbacks/`**（**`mod.rs`** 再导出 **`build_chat_stream_callbacks`**；**`stream_session_access`** 含 **`append_stream_assistant_chunk`** / **`ChatStreamCallbackCtx::append_assistant_chunk`**、**`with_stream_write_session_*`** 与 **`ChatStreamCallbackCtx::update_bound_session` / `read_bound_session`**；**`assemble`** 内 **`push_timeline_system_bubble_with_tail`** 收口分步时间线系统气泡 + **`ensure_streaming_assistant_tail_last`**；**`helpers/`**（**`indices` / `text_format` / `timeline_tail` / `stream_diag`** 经 **`helpers/mod`** 再导出）、**`builders`**（含 **`stream_phase_hooks`** 闭包工厂以降低 **`assemble` nloc**）/ **`assemble`** 分担各 **`on_*`** 装配为 **`ChatStreamCallbacks`**，与 **`api::send_chat_stream`** 对齐）。
- **流式布局 v2 不变量**：`project_turn_projection` 分离定稿/active；Web **`projection_reconciler`** 落盘；正文写入点见 **`text_ownership`**。金样：`fixtures/turn_project_projection_golden.jsonl`（`cargo test -p crabmate-turn-layout golden_turn_project_projection`）。过渡债见 **`docs/Turn布局设计.md` §15**；终态协议 / hydration（Phase E）见 **§16**。
- **`composer.rs`**：草稿缓冲与 textarea 同步、发送 / 停止 / 重试 / 截断再生、新会话（流式细节见上）；**`tool_result`** 写入消息时附带 **`timeline_scan`** 的 **`cm_tl`** JSON（**`StoredMessage.state`**，仅本机 UI）；**`pending_images`** 与 **`column`** 内隐藏 file input 上传附图（最多 6 张预览），发送时写入 **`StoredMessage.image_urls`**。
- **`scroll.rs`**：消息列表指纹变化时的自动跟底、**`focus_message_id_after_nav`** 滚入视图。
- **`find.rs`**：主区查找匹配 id、光标与首条 **`scroll_message_into_view`**。
- **`column.rs`**：中部终端流消息区与输入区（接收 **`ChatColumnShell`**）；默认以 **`tui_stream_view`** 展示，复用同一 **`ChatSessionSignals`**、composer 与滚动壳。
- **`tui_stream_view.rs`** / **`tui_transcript_sync.rs`** / **`tui_actions_bar.rs`** / **`tui_tool_process.rs`**：TUI 默认聊天视图；每回合 `wrap`（`section` + 下方 **`msg-action-icon-btn` 图标行**）；工具折叠定高，live 用 **`TuiBodyPatch::ToolRow`** 改文案；流式助手按行 Incremental；会话切换全量重建。演进见 **`docs/design/web_tui_stream_to_opencode_style.md`**。
- **`tui_line_markdown.rs`**：按行解析为 `closed` HTML chunks + `open_plain`；提供增量 `TuiBodyPatch`。
- **`find_bar.rs`**：主区内查找条。
- **`message_chunks.rs`**：将 **`StoredMessage`** 列表折叠为 **`ChatChunk`**（单条 / 连续工具组），供 **`column`** 迭代。
- **`message_row/`**：单条消息气泡与操作条（复制 / 重试 / 分支等）；**`mod.rs`**（**`ChatMessageRowSignals`**）、**`helpers.rs`**（样式类与工具抽屉辅助）、**`views.rs`**（元信息 / 正文 / 工具抽屉 / 操作条）、**`row.rs`**（**`chat_message_row`** 根视图）；与 **`POST /chat/branch`** 及本地截断再生的副作用在 **`message_row_actions.rs`**。
...
- **`message_group_views.rs`**：连续工具输出分组折叠视图（供 **`column`**）。
- **`timeline.rs`**：可折叠的「规划 / 工具时间线」索引（**`timeline_scan`** 扫描消息；点击跳转到 **`#msg-{id}`**）。

### `frontend/src/app_prefs.rs`

- 壳层偏好键与侧栏视图枚举（**`SidePanelView`**）、时间线面板展开态、桌面端左侧会话栏收起等经 **`prefs.json`**；状态栏用的本机/服务端 **`api_base` / `model`** 合并展示、侧栏宽度钳制与读写。

### `frontend/src/timeline_scan.rs`

- 从 **`StoredMessage`** 抽取时间线条目：流式写入的 **`state`** JSON（键 **`cm_tl`**，**仅**本机 UI，**不**发往模型）与无前缀数据的**旧会话**回退（`is_tool` 工具卡）。供 **`app/chat/timeline.rs`** 与 **`app/chat/composer_stream/`**（SSE 回调写入 `state`）共用。
...
- **`WorkspaceFilesystemTree`**：右侧工作区文件列表的树形 UI；目录默认折叠，展开时对子路径调用 **`GET /workspace?path=`**（**`api::fetch_workspace`**），结果缓存在 **`HashMap`** 中；**`workspace_child_rel`** 与后端相对路径规则一致。文件/目录行与列表空白处右键打开 **`workspace_context_menu`**；选择新建后在树内行内输入名称（VS Code 风格，Enter 确认 / Escape 或失焦取消），IDE 布局下新建文件打开编辑器标签，删除时关闭匹配标签。

### `frontend/src/storage.rs`

- 会话列表经 **`/user-data`** 落盘（按工作区分桶）。**`ChatSession`** 含 **`pinned` / `starred`**（侧栏排序与展示；旧数据缺省为 `false`）。
- **`DEFAULT_CHAT_SESSION_TITLE`**：新建会话默认标题，与 **`app/mod.rs`** 首条消息自动命名（**`session_ops::title_from_user_prompt`**）条件一致。
- 会话导出信封在 **`crabmate-chat-export`**（**`schema_version` `2.0.0`**）：必填 **`schema` / `schema_version` / `projection` / `version` / `messages`**；**`projection=raw`**（CLI/TUI 完整 **`Message`**）与 **`projection=display`**（Web/Tauri 展示投影）；**`tool-replay`** 只接受 raw；**不**兼容缺字段旧 JSON；见 **`docs/命令行与路由.md`** **`save-session`**。
...
- **前端用户级持久化（`/user-data` → `$XDG_DATA_HOME/crabmate`）**：
...
- **安全边界**：
  ...
  - **密钥与日志**：勿将真实 API key、token、`.env` 内容写入代码、示例配置、commit message 或日志；日志与错误回显须脱敏。Cursor 规则见 **`.trae/rules/secrets-and-logging.mdc`**。
  - 已知 HTTP 鉴权（共享密钥模型与网关分工）、监听地址、`workspace_set` 等安全与协议债见 [`docs/待办清单.md`](待办清单.md) 与 [`docs/未来规划功能.md`](未来规划功能.md)。
- **SSE 协议演进**：载荷形状以 **`sse::protocol::SseMessage` / `SsePayload`**（及 `sse/mod.rs` 再导出）为事实来源；**版本号**以 **`crates/crabmate-sse-protocol`** 与 Leptos 共享。Rust 侧行分类见 **`sse/line.rs`**；浏览器侧在 **`frontend/src/api/chat_stream/parser_v2.rs`**。**人读契约与错误码表**：**`docs/SSE协议.md`**。**AG-UI 契约测试**：`fixtures/sse_ag_ui_golden.jsonl` + `cd frontend && cargo test golden_ag_ui_v2_parser_matches_expected`。

## 延伸阅读（设计稿与专题）

工作流编排、上下文裁剪方案、Web 主题等**专题设计**仍以仓库内 **`docs/`** 下各专题文档为准；本文仅提供入口级索引，不复制全文。
