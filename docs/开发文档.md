...
| **`crabmate-chat-export`**（`crates/crabmate-chat-export`） | 会话导出 **schema 常量** / **`ChatSessionFile`**（完整 `Message`）与 Markdown 标题/组装；**`crabmate-runtime::chat_export`** 负责落盘；Web/Tauri **`session_export`** 复用常量与 MD 标题（展示投影消息体）。 |
...
- 与导出结构保持兼容：JSON 信封 schema 在 **`crabmate-chat-export`**（经 **`runtime/chat_export`** / 前端 **`session_export`** 共用），含 **`schema`**（`crabmate.chat_session`）、**`schema_version`**（SemVer 信封）、**`version`**（`messages` 约定）；CLI/TUI 落盘为完整 **`Message`**，Web 下载可为展示投影；见 **`docs/命令行与路由.md`** **`save-session`**。
