...
- **提交与质量**：根目录 **`.pre-commit-config.yaml`**（`cargo fmt`、`cargo clippy -D warnings`、复杂度棘轮等）；提交说明 **Conventional Commits**，见 **`.cursor/rules/conventional-commits.mdc`**。
...
**`observability`**：初始化 tracing；Web 任务可有 **`TracingChatTurn`**（**`chat_turn`** span：**`job_id`**、**`conversation_id`**、**`outer_loop_iteration`**、工具展示的短 **`tool_call_id`** 标签等）。JSON 日志 **`CM_LOG_JSON`**。
...
| 区域 | 职责 |
|------|------|
| **`agent/`** | 单轮与多轮编排：**`agent_turn`**（外层循环、分阶段规划 **staged**、**`staged_plan_intent_gate`**（含架构/重构咨询启发式下绕过滚动规划）、意图门控、工具执行 **execute_tools**）、**`context_window`** / **`message_pipeline`**、**`per_coord`**（终答与工作流协调）、**`workflow`**（DAG）、可选 **`hierarchy`**（分层 Manager/Operator）。 |
...
- **前端**：**`localStorage`** 存会话列表、主题、部分 LLM 草稿等（非服务端密钥）。
