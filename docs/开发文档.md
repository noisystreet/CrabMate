...
**R 三条轨（正交，勿混用名词）**：（1）**终答 Gate** — `per_coord/final_plan_gate.rs`，决策原因见 **`FinalPlanGateDecisionReason`**；（2）**工作流反思** — `workflow_reflection_controller` + `prepare_workflow_execute`，仅 `workflow_execute` 路径，经 **`PlanRequirementSource`** 与 Gate 衔接；（3）**外循环 Gate 前纠偏** — `outer_loop_reflect.rs`（build-idle / 终答缺失），原因见 **`OuterLoopReflectPreGateReason`**，**不**改写 Gate 判定。分层 Manager **`reflect_and_replan`** 为第四条独立路径，见 **`docs/design/per_state_machine_consolidation.md`** §3.2 注意。已删除未接线的 **`plan_rewrite_controller`**（策略与计数以 **`PerCoordinator`** + **`final_plan_gate`** 为准）。
