...
- **流式布局 v2 不变量**：`project_turn_projection` 分离定稿/active；Web **`projection_reconciler`** 落盘；正文写入点见 **`text_ownership`**。金样：`fixtures/turn_project_projection_golden.jsonl`（`cargo test -p crabmate-turn-layout golden_turn_project_projection`）。过渡债见 **`docs/Turn布局设计.md` §15**。
