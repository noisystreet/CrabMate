...
- **`agent_turn`** 将上述失败与编排早停映射为 **`RunAgentTurnError`**（**`agent_turn::errors`**），维护当前 **`sub_phase`**（**`planner` / `executor` / `reflect`**，与 P/R/E 心智模型对齐）；Web 经 **`TracingChatTurn::job_id`** 作为 **`turn_id`**（与 **`x-stream-job-id`** 一致）。**编排观测**：`target: crabmate::agent_turn` 下结构化字段 **`turn_orchestration_mode`**（见 **`turn_orchestration::TurnOrchestrationMode`**）；**`planner_executor_mode = hierarchical`** 时在同 target 下另有 **`hierarchical_phase`**（**`agent_turn/hierarchy.rs`** 内 `run_hierarchical_agent` / `handle_execution_result`），与顶层 `turn_orchestration_mode=hierarchical` 正交，便于对照「分层内走到哪一步」。
...
核心流程在 `src/lib.rs` 的 `run_agent_turn(RunAgentTurnParams { … })`：内部组装 **`RunLoopParams`** 后调用 **`run_agent_turn_common`**（实现见 **`src/agent/agent_turn/mod.rs`**；模式分发见 **`run_dispatch.rs`**；顶层编排形态枚举与解析见 **`turn_orchestration.rs`**，结构化日志目标 **`crabmate::agent_turn`**、字段 **`turn_orchestration_mode`**）。
