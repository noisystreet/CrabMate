...
- **流式布局 v2 不变量**：`crabmate-turn-layout::project_turn_projection` 分离定稿行与 active；`project_turn_web_v2` 仍为定稿旁注/工具序。Web 经 **`projection_reconciler`** 将定稿/锚定 active 旁白 upsert 为 `turn-commentary-*`；无锚点预览与终答增量常走 `stream_text_overlay` / loading 句柄。`ChatSession.layout_schema_version = 2` 随新会话/流式投影写入缓存，水合优先保留稳定投影键。过渡债与收敛阶段见 **`docs/Turn布局设计.md` §15**；I14 同帧移交见 **§12.10.1**。
