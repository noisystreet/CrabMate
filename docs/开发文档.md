...
| `dsml/` | DeepSeek **DSML** 解析、物化与展示剥离：**`DsmlToolCallAdapter`** / **`materialize_deepseek_dsml_tool_calls_in_message`**（执行轮兜底）、**`strip_deepseek_dsml_for_display`**、**`StreamingDsmlContentFilter`**（流式 SSE/CLI/TUI 下发前剥离）、**`parse_combined_assistant_text`**；分阶段无工具轮经 **`staged_policy`**（**`strip_staged_planner_message_tool_calls`**、**`staged_first_planner_tool_call_total_after_materialize`** 等）。**`llm::complete_chat_retrying`** 在每次成功 `stream_chat` 后按 **`materialize_deepseek_dsml_tool_calls`** 物化；**`llm::api::sse_parser`** 在流式正文增量经 **`dsml_stream_strip_enabled`** 过滤后下发。子模块：**`normalizer`**、**`parser`**、**`strip`** / **`strip_scan`**、**`stream`**、**`adapter`**、**`types`**、**`staged_policy`**。 |
...
| **步级验收** | `step_verifier.rs` | 分阶段**单步**内 `acceptance` 与最后一条 `tool` 比对（步界同上）；**不**替代会话级完成证据 heuristic。 |

**编排注入 user**（外循环纠偏、重复规划拦截、状态机流转、全局重规划等）须在 **`crabmate-display-rules`** 登记为 server-injected，构造时优先 **`Message::user_staged_orchestration_injection`**。**真实 user** 解析共用 **`crabmate-types::real_user_message`**。**完成后冗余探针**判定（只读工具 / `run_command` 验证命令 / 分阶段 verify 步）统一见 **`agent_turn/completion_suppression.rs`**。

抑制触发时 **`tracing`**（目标：`crabmate::agent_turn` / `crabmate::staged`）记录 `goal_preview`（截断）与冗余 tool 名列表，便于排障；尚未写入 SSE `reason_code`（见 **`docs/规划执行验证架构.md`** §2.4）。
...
- **意图门控（默认 L2，弃用 L0/L1 兜底，共用）**：
  - 管线构成：默认 **`intent_l2_enabled=true`** 时经 `intent_l2_classifier` 无工具 LLM（合并路由 + 当前句）直接产出 `IntentDecision`；`intent_l2_min_confidence` 仅作为日志/时间线观测阈值，低于阈值仍采纳 L2，不再回退 L1。`intent_l0` 仍在澄清/确认流中做**路由文本**与特征（可合并前序 user + 当前短续接、含 `role: tool` 近期失败位），旧 L1 规则与 L0 提级（`intent_l0_routing_boost_enabled`）只在 L2 关闭、调用失败或解析失败时作为弃用兜底；可将 **`intent_l2_enabled=false`** 临时关闭以省一次调用。**`IntentContext`** 由 **`crabmate-agent::agent_turn::intent::context`** 的 **`build_intent_routing_context`** 装配；根包 **`IntentL2ClassifierHost`**（**`l2_classifier_host.rs`**）注入 L2 LLM；**非分层**主路径在 **`intent/staged_planning_gate.rs`** 的 **`assess_staged_planning_gate_full_pipeline`** 与 **`intent/at_turn_start`** 共用 **`assess_intent_routing_full_pipeline`**（同步 **`assess_staged_planning_gate`** 仍仅规则层，供单测/离线探测）。**L2 缓存**：`intent_at_turn_start` 写入 **`turn_planner_hints.intent_routing_cache`**，同 effective task 下 `staged_plan_intent_gate` 复用判定、跳过第二次 LLM。**简单编译 fast path**（`simple_execute_fast_path`）对单步 cmake/构建类 Execute 拒绝分阶段、走 freeform。工具失败后用户短句「继续」在 L0 合并前序 user 并在 **`extract_effective_user_task`** 还原原任务。
...
非分层路径无 **`hierarchical_phase`**；其顶层形态见 **`turn_orchestration_mode`**（`freeform` / `planned_step_*` / `intent_at_turn_start_finished`）。分阶段 **`staged_turn_orchestrator_phase`** 见 **`staged/turn_driver`**，与上表三者并列、勿混名词。

**设计稿（渐进落地）**：见 **`docs/design/per_state_machine_consolidation.md`**（终答 Gate、**`staged/turn_driver`** 运行时顶层相位、**`prepared_route_reduce`** / **`staged_parse_terminal`** 两层解析路由、**`step_iteration_reduce`** 步循环表驱动；全局单表 PER 仍属后续）。
...
**顶层分支**（非分层：**`staged_plan_intent_gate`** 放行 → **`PlannedStep`** 滚动视界；拒绝 → **`Freeform`** 外循环。`logical_dual_agent` 在门控放行时走逻辑双代理规划消息构造）：

| planner_executor_mode | staged_plan_intent_gate | 顶层入口 | 首轮 P | 后续每步 |
|-----|-----|-----|-----|-----|
| `logical_dual_agent` | Allow | `run_logical_dual_agent_then_execute_steps` | 无工具 P 一轮；上下文 `build_logical_dual_planner_messages` + 规划 system（见上） | 每步：`run_agent_outer_loop` → 多轮 **P（带 tools）→ R → E** 直至该步终答 |
| `single_agent` | Allow | `run_staged_plan_then_execute_steps` | 无工具 P 一轮；上下文 `build_single_agent_planner_messages`（**保留** `role: tool`）+ 规划 system | 同上 |
| 任意（非分层） | Deny | `run_agent_outer_loop`（经 **`non_hierarchical_turn`** **`Freeform`**） | 无单独规划轮；每圈 **P（带 tools）→ R → E** | （同一循环直至本轮结束） |
...
- **单轮预算**（`crates/crabmate-agent/src/turn_budget.rs`，根包 **`agent::turn_budget`**）：**`TurnBudgetCounter`** 贯穿 **`complete_chat_retrying`**（墙钟 / LLM 次数 / Token 粗估）、**`agent_turn::outer_loop`**、**`staged/`** 步循环与 **分层 Operator ReAct**。Token 粗估经 **`tiktoken_prompt_tokens::estimate_chat_exchange_tokens`** 在每次成功 LLM 往返后累计。超限时 Operator 在有进展时返回 **部分完成**摘要；**`budget_degradation_enabled`** 时 **`GoalVerifier::verify_degraded`** 跳过分层非关键验收。配置与环境变量见 **`docs/配置说明.md`**（**`CM_MAX_TURN_*`** 等）。与会话 **`context_window` / `message_pipeline` 裁剪**正交。
