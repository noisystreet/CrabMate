...
- **Web 前端（`frontend/`）**：Leptos（CSR）+ WASM，**Trunk** 打包；静态产物由后端从 **`frontend/dist`** 提供。负责聊天 UI、工作区浏览/编辑、任务清单、状态栏展示，以及消费后端 SSE 流。**界面美化与样式分层、Material 主题关系、演进路线**见 **`docs/Web界面美化设计.md`**。
- **CLI（`runtime/cli` 等）**：当前交互入口为 **reedline + crossterm** 的行编辑 REPL；若要将交互改为 **全屏 TUI** 并与 Web 信息架构对齐，可按 **`docs/design/TUI_CLI改造实施步骤.md`** 分阶段实施（实施落地后须同步本节「代码模块索引」与 **`README.md`** / **`docs/命令行与路由.md`**）。
...
**D. Operator 实现细节**
- **目录**: `src/agent/hierarchy/operator/`（`mod.rs` 汇总；**`react_loop`**：`execute_with_tools`；**`agent_impl`**：`execute` / LLM 与产物上下文；**`operator_tool_analysis`**：工具执行结果启发式（任务完成判定）；**`prompt`**：系统提示与按任务类型的执行指导；**`compile`**：编译错误分类与收敛指标；**`inject`**：占位符注入与工作目录探测；**`text`**：轨迹截断；**`state`**：ReAct 内部状态；**`types`**：`OperatorAgent` / **`OperatorConfig`**（**`policy: OperatorPolicy`** + **`runtime: OperatorRuntimeHandles`**）/ `OperatorError` 等）
...
- **对外文档**：版本号、`error`/`code` 与 `tool_result.error_code` 枚举、控制面变体表、与 Leptos 的对齐清单见 **`docs/SSE协议.md`**（修改协议时须同步 **`crates/crabmate-sse-protocol`** 与 **`frontend/src/sse_dispatch.rs`**）。

### `src/sse/line.rs`

- **消费侧分类**：将单条 SSE `data:` 字符串分为工具状态、`tool_call`（含 `name/summary`）、审批请求、`tool_result`（含 `name/summary/ok/exit_code/error_code`）、工作区刷新、流错误、忽略或正文（`Plain`）；与 **`protocol`** 反序列化及若干历史裸 JSON 键名兼容。与 **`frontend/src/sse_dispatch.rs`** 语义对齐；当前 crate 根**不**再导出本模块符号。
...
- **宿主目标（默认）**：仓库根执行 **`cargo test -p crabmate-web-leptos`**（或 **`cd frontend && cargo test`**）。覆盖 `session_ops`、`session_search`、`markdown`、`message_render`（`#[test]`）、**`debounce_schedule`** 等纯逻辑与 HTML 净化断言。
...
- **`app_shell_effects/`**（**`mod.rs`** 再导出；实现按职责分子文件）：**`ShellEscapeSignals`** 与壳级 **`wire_*`**——**`session_storage`**（首启 **`localStorage`**、**`GET /web-ui`**、会话列表写回）、**`persist_prefs`**（侧栏视图 / 底栏 / 角色 / 宽度 / 左侧栏收起等）、**`sync_dom`**（**`data-theme`** / **`lang`** / **`data-bg-decor`**）、**`approval_follow`**（待审批会话切换时审批条展开态）、**`settings_llm_open`**（打开设置时填充 LLM 草稿；**`status_data`** 用 **`get_untracked`**）、**`escape`**（**全局 `Escape`**，在 **`input`/`textarea`/`contenteditable` 外**按层级关闭菜单 / 查找 / 模态——**`wire_escape_key_layered_dismiss`**）。**`mod.rs`** 文档说明各块与「勿隐式订阅」边界；注册顺序仍见 **`app_shell_init`**。
- **`chat_session_state.rs`**（`frontend/src/`，**非**仅 `app/`）：**`ChatSessionSignals`**——会话列表、活动 id、**`SessionSyncState`**、水合 nonce、流式 job / SSE 序等 **`RwSignal`** 聚合（**`Clone`/`Copy`**）；**`clear_stream_resume_handles`** 统一重置 **`stream_job_id` / `stream_last_event_seq`**（会话切换、流结束、错误、换角等）。置于 crate 根以便 **`session_modal_row`** 与 **`app/`** 共用，避免 `app` ↔ 根模块循环依赖。
...
- **`wire_chat_session_lifecycle.rs`**：**`wire_chat_session_lifecycle_effects`**——按顺序调用 **`app_shell_effects`** 的 **`wire_initial_sessions_from_storage`**、**`wire_web_ui_config_once_after_init`**、**`session_hydrate::wire_session_hydration`**、**`wire_persist_chat_sessions`**（阶段 B 从 **`App`** 迁入，顺序与原先一致）。
...
- **SSE 协议演进**：载荷形状以 **`sse::protocol::SseMessage` / `SsePayload`**（及 `sse/mod.rs` 再导出）为事实来源；**版本号**以 **`crates/crabmate-sse-protocol`** 与 Leptos 共享。Rust 侧行分类见 **`sse/line.rs`**；浏览器侧在 **`frontend/src/sse_dispatch.rs`**（由 `frontend/src/api.rs` 调用）。**人读契约与错误码表**：**`docs/SSE协议.md`**。**控制面分类契约测试**：`fixtures/sse_control_golden.jsonl` + `cargo test golden_sse_control`（**`crabmate-sse-protocol`** + **frontend** 对齐）。
