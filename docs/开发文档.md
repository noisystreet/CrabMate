...
- **Execution / DAG**：分层按依赖拓扑调度与子目标执行的核心编排见 **`execution.rs`**；**DAG 构建与拓扑分层、`trim_for_detail` / 证据摘要、`supplement_subgoal_required_tools`** 等纯逻辑见 **`execution_helpers.rs`**；**`ExecutionError`** 独立为 **`execution_error.rs`**（供 `execution` 与 `execution_helpers` 共用，避免与 `execution.rs` 形成模块依赖环）。
...
| `github_cli/` | GitHub CLI（`gh`）封装：`mod.rs` + **`api.rs`**（`gh_api`）；`gh_pr_*`（含 **`gh_pr_checks`**、**`gh_pr_create`**）/ `gh_issue_*` / `gh_run_*` / `gh_release_*` / `gh_search` 等列表类经 **`command::run`** |
