...
| **`crabmate-web-host`**（`crates/crabmate-web-host`） | Axum Web **宿主**面（与 Leptos 包 **`crabmate-web`** / `frontend/` 不同）：HTTP DTO、`limits`、**`GET /web-ui`**（handler + `routes::web_ui`）；后续承接更多 routes/handlers；**禁止**依赖 **`crabmate-internal`**。路线见 **`docs/design/web_host_extract.md`**。 |
