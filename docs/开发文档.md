...
- **`chat_job_queue`** 将 **`RunAgentTurnError`** 编码为 SSE **`SsePayload::Error`**（**`code`**、可选 **`turn_id` / `sub_phase`**），详见 **`docs/SSE协议.md`**（如 **`LLM_RATE_LIMIT`**、**`turn_aborted`**）。
