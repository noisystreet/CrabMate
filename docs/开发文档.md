...
| **`crabmate-agent`**（`crates/crabmate-agent`） | 规划产物 **`plan_artifact`**、意图 **`intent_*`**、**`turn_budget`**、工作流反思控制器等**领域层**；根包 **`agent`** 再导出并承载 **`agent_turn`** / **`hierarchy`** / **`workflow` 执行** 等编排（依赖 `tool_registry`、SSE）。 |
