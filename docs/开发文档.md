...
- **`chat/scroll_shell.rs`** / **`chat/scroll_follow.rs`** / **`chat/messages_list.rs`**：消息列滚动壳（**`ChatScrollShellSignals`**：跟底意图、`on:scroll` / `on:wheel`、prepend 锚点补偿）；跟底 **一条规则 + 两个入口**（[`wire_content_follow_scroll`] / [`engage_follow_and_scroll_bottom`]）；全量 **`ChatChunk`** 渲染；更早历史仅经顶部 **「加载更早」** 按钮触发（**`try_load_older_messages_for_active_session`**）。
