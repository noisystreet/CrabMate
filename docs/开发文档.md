...
- **流式布局 v2 不变量**：`crabmate-turn-layout::project_turn_web_v2` 将每条已关闭工具前旁注按 `tool_call_id` 投影为独立行；Web 以 `turn-commentary-*` upsert 到锚定工具前（可纠错序）；当前唯一可变预览常走 `stream_text_overlay` / loading 句柄。`ChatSession.layout_schema_version = 2` 随新会话/流式投影写入缓存，水合优先保留稳定投影键。过渡债与收敛阶段见 **`docs/Turn布局设计.md` §15**；I14 同帧移交见 **§12.10.1**。
