...
**单轮工具回合的消息展示顺序**（旁注在对应工具之前、post-tool 尾泡、晚到 plain delta）见 **`docs/Turn布局设计.md`**（共享 **`crates/crabmate-turn-layout`** + Web **`TurnLayout`** / **`TurnCanonicalState`**）。
...
| **`crabmate-turn-layout`**（`crates/crabmate-turn-layout`） | 单轮 canonical **`Turn`** + **`TurnEvent`** reducer 与 **`project_turn`** 投影；金样 **`fixtures/turn_project_golden.jsonl`**；Web **`TurnLayout`** 消费见 **`docs/Turn布局设计.md`**。 |
