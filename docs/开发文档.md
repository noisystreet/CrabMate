...
| `command.rs` | `run_command`：`RunCommandError`（JSON/白名单/参数/限流/启动失败等可判别）、`run`（兼容字符串输出）、`run_checked`（`Result`）；**`prepare_run_command_invocation`** / **`run_cargo_test_with_optional_cache`** / **`run_command_execute_workspace_binary`** 拆分执行路径；可选 `cargo test …` 路径经 **`test_result_cache`**；**`error_playbook::playbook_run_commands`** 对校验失败打 **`warn`**（含 `kind`） |
