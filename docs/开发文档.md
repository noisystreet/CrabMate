...
| `web_static_dir.rs` | **`resolve_web_static_dir`**：`serve`、**`config --dry-run`** 与 **`GET /health`** 的 **`frontend_static_dir`** 检查共用的静态资源根；开发时为仓库 **`frontend/dist`**；桌面 **`.deb`** 安装布局为 **`/usr/share/crabmate/frontend/dist`**（可用 **`CM_WEB_STATIC_DIR`** 覆盖）。 |
