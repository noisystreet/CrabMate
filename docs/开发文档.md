...
- **P / R / E**：**P** = 向模型要一轮输出；**R** = 助手消息后的反思/终答门控（**`reflect`**、**`per_coord`**）；**E** = **工具执行**（**`execute_tools`** → **`ToolExecutionHost`** 注入 **`tool_registry`** / **`workflow_execute`**；分层 **`hierarchy/tool_executor`** 复用同一 registry 宿主）。
...
| 区域 | 职责 |
|------|------|
| **`agent/`** | 单轮与多轮编排：**`agent_turn`**（外层循环、意图门控、工具执行 **execute_tools**）、**`context_window`** / **`message_pipeline`**、**`per_coord`**（终答与工作流协调）、**`workflow`**（DAG）、可选 **`hierarchy`**（分层 Manager/Operator）。 |
...
| **`crabmate-agent`**（`crates/crabmate-agent`） | 规划产物 **`plan_artifact`**、意图 **`intent_*`** / **`intent_routing`**、**`agent_turn::tool_execution`**（批模式、early-deny 等；**不含**根包 **`ToolExecutionHost` trait**）、**`agent_turn::outer_loop_*`**（外循环 FSM / reduce / driver / pre-gate reason）、**`agent_turn::turn_completion_decision`** / **`completion_suppression`** / **`task_level_evidence`** / **`run_command_dedupe`**（完成判定核）、**`step_executor_policy`**、**`turn_budget`**、工作流反思控制器等**领域层**；根包 **`agent`** 再导出并承载 **`agent_turn` 执行面**（**`ToolExecutionHost`** / **`tool_execution_host`**、SSE **`emit`**、**`TurnControlSink` / `TurnTerminalIo`**、**`outer_loop` IO**、**`turn_completion` 纠偏文案**）、**`hierarchy`** / **`workflow` 执行** 等编排。拆分进度见 **`docs/design/agent_turn_split.md`**；执行面解耦见 **`docs/design/turn_host_decouple.md`**。 |
...
改下列能力时，**除** `src/agent/hierarchy/` 外，请交叉核对 **`agent_turn/hierarchy.rs`**（分层入口、`hierarchical_phase` 日志）、**`agent_turn/task_level_evidence/`**（`common` / `verify` / `render`；任务级验收与「关键证据」Markdown）、**`per_coord::final_plan_gate`** / **`plan_rewrite`** 是否需同步文案或行为：
...
- 步级验收与 **`run_agent_outer_loop`** 回落（分层「话语型」仍可能走 **`run_agent_outer_loop`**）。

#### 真实 user、完成证据抑制与步级验收（`agent_turn`）

维护 **`task_level_evidence/`**、**`turn_completion.rs`**、**`completion_suppression.rs`**、**`plan_optimizer`**、**`crabmate-types::real_user_message`** 时，请区分三层语义：
...
| `web/` | Web（HTTP）专用 axum 模块：**`http_types/`**（仅 serde：`chat` / `workspace` / `tasks` / **`skills`**，供 handler 与 **`routes/*`** 并列引用，避免路由模块与 handler 环依赖）、**`routes/`**（**`chat`**：`/chat*`、**`POST /chat/async`**、**`GET /chat/jobs/{job_id}`**、`/upload*`、**`GET /conversation/messages`**（只读会话快照）；**`skills`**：**`GET /skills`**（工作区 skills JSON，composer `/` 浮层）；**`workspace`**：`/workspace*`（含 changelog handler 挂载）；**`tasks`**：`/tasks`；**`config`**：`/config/reload`、**`POST /config/session/conversation-store`**；**`system`**：`/health`、`/status`；各子模块导出对应 `router()`）、**`openapi/`**（**`GET /openapi.json`**：`mod.rs` 聚合 **`openapi_paths`** / **`openapi_components`** 生成 OpenAPI 3.0 摘要，与路由对齐；由 `server` 单独 `route` 挂载；SSE 行协议仍以 **`docs/SSE协议.md`** 为准）、**`async_chat_job.rs`**（**`POST /chat/async`** 任务状态与 Webhook payload）、**`app_state`**（组合式 **`AppState`**：**`http`**→**`AppStateHttpCore`**（**`cfg`** 为 **`Arc<RwLock<AgentConfig>>`**、**`config_path_for_reload`**、`client` / `api_key` / `tools`、工作区覆盖、上传目录）、**`chat`**→**`AppStateChatRuntime`**（**`chat_queue`** + **`WebChatQueueDeps`**）、**`conversation`**→**`AppStateConversationRuntime`**（**`conversation_backing`**：`Arc<tokio::sync::RwLock<ConversationBacking>>`；**`conversation_id_counter`**）、**`aux`**→**`AppStateWebAux`**（审批表、**`long_term_memory`**、**`web_tasks_by_workspace`**、模型探测缓存、**`sse_stream_hub`**、**`process_handles`**、**`async_chat_jobs`**）；**`MemoryConversationEntry`** / SQLite 行含 **`active_agent_role`**；**`ConversationTurnSeed`** 携带 **`persisted_active_agent_role`**；SQLite 路径下 **`save` / `truncate`** 经统一 **`sqlite_conversation_store_op`** 包装）、**`cron_scheduler`**（**`[[scheduled_agent_task]]`**：`tokio-cron-scheduler`；与 **`chat_handlers`** 共用 **`prepare_json_chat_enqueue`**，走 **`chat_job_queue` JSON** 路径）、**`audit`**（写副作用工具审计：客户端 IP、共享密钥 SHA-256 短指纹；**不**记密钥明文）、**`chat_handlers/`**（**`mod`** 再导出；子模块 **`parse`** / **`conflict`** / **`upload`** / **`auth`** / **`chat`**（**`enqueue`** / **`async_chat`** / **`stream`** / **`builtin_skills`** / **`turn_build`**） / **`workspace_changelog`** / **`health_status`** / **`config_reload`** / **`session_conversation_store`**；**`CONVERSATION_CONFLICT_*`** 与 **`conversation_conflict_sse_line`** 供 HTTP 与 SSE 冲突文案一致）、`server`（`merge` 各域 `router()` 与 **`/openapi.json`**；Web API 鉴权中间件（**`Authorization: Bearer`** / **`X-API-Key`**）是否在启动时挂载由 **`web_api_bearer_layer_enabled`** 决定）、**`workspace/`**（handler 实现；含 **`GET /workspace/profile`**）、`task`（`/tasks` handler）。`open_conversation_sqlite` 会 **`LongTermMemoryRuntime::migrate_on_connection`**；`AppState` 由 `lib.rs::run` 装配；`SaveConversationOutcome` 在 **`conversation_store`**，crate 根再导出供 `chat_job_queue` 等使用。**第三方 HTTP 集成（IM 桥接、自动化、多租户增强方向）设计备忘见 `docs/design/web_api_integration.md`。** |
| `web_static_dir.rs` | **`resolve_web_static_dir`**：`serve`、**`config --dry-run`** 与 **`GET /health`** 的 **`frontend_static_dir`** 检查共用的静态资源根；开发时为仓库 **`frontend/dist`**；桌面 **`.deb`** 安装布局为 **`/usr/share/crabmate/frontend/dist`**（可用 **`CM_WEB_STATIC_DIR`** 覆盖；Tauri 仅在**非源码树**启动 sidecar 时自动注入，**`cargo tauri dev`** 不注入以免误用本机已装 deb 的旧 UI）。 |
...
核心流程在 **`src/run_agent_turn.rs`** 的 **`run_agent_turn`**（入参 **`RunAgentTurnParams`**，类型定义在 **`lib.rs`**）：内部组装 **`RunLoopParams`** 后调用 **`run_agent_turn_common`**（实现见 **`src/agent/agent_turn/mod.rs`**；模式分发见 **`run_dispatch.rs`**；非分层统一 driver 见 **`non_hierarchical_turn.rs`**；回合阶段枚举与 **`NonHierarchicalTurnResolution`**（门控→**`Freeform`** / **`PlannedStep`**）见 **`turn_orchestration.rs`**；分层意图后路由见 **`hierarchical_intent_route.rs`**，结构化日志目标 **`crabmate::agent_turn`**、字段 **`turn_orchestration_mode`**（如 **`freeform`**、**`planned_step_single_agent`**）。
...
- **工具批执行（E，`ToolExecutionHost`）**：
  ...
  - **根包宿主**（**`agent_turn/execute/tool_execution_host.rs`**）：**`CrabmateToolExecutionHost`**（串行 + **`workflow_execute`**）、**`CrabmateParallelToolDispatch`** / **`CrabmateRegistryToolDispatch`**（并行只读 / 分层 Operator，仅 **`tool_registry`**）；并行 **`http_fetch`** 经 **`dispatch_parallel_http_fetch`**（`spawn_blocking` + **`run_direct`**）。
...
**R 三条轨（正交，勿混用名词）**：（1）**终答 Gate** — `per_coord/final_plan_gate.rs`，决策原因见 **`FinalPlanGateDecisionReason`**；（2）**工作流反思** — `workflow_reflection_controller` + `prepare_workflow_execute`，仅 `workflow_execute` 路径，经 **`PlanRequirementSource`** 与 Gate 衔接；（3）**外循环 Gate 前纠偏** — `outer_loop_reflect.rs`（build-idle / 终答缺失），原因见 **`OuterLoopReflectPreGateReason`**，**不**改写 Gate 判定。分层 Manager **`reflect_and_replan`** 为第四条独立路径（原因见 **`hierarchy/reflect_replan_reason::ManagerReflectReplanReason`**，`tracing` 字段 **`manager_reflect_replan_reason`**），**不**经终答 Gate；见下表与 **`docs/design/per_state_machine_consolidation.md`** §3.2。已删除未接线的 **`plan_rewrite_controller`**（策略与计数以 **`PerCoordinator`** + **`final_plan_gate`** 为准）。

**分层观测：`hierarchical_phase` 与 `TurnRouteDecisionV1.orchestration_mode`**（`agent_turn/hierarchy.rs` + **`turn_route_decision`**；二者**正交**——前者为分层入口内子阶段，后者为回合路由决策对外字段）：

| `hierarchical_phase`（`tracing`） | 典型 `orchestration_mode`（`TurnRouteDecisionV1`） | 说明 |
|-----------------------------------|-----------------------------------------------------|------|
| `intent_gate` | `hierarchical` | 分层意图门控进行中 |
| `intent_gate_finished_early` | `intent_at_turn_start_finished` | 意图已写入终答，本函数早退 |
| `discourse_fallback_outer` | **`freeform`**（路由决策） | 话语型/澄清/只读 QA 回落 **`run_agent_outer_loop`**；同条 `info!` 仍带 `turn_orchestration_mode=hierarchical` 表示**入口**为分层配置，实际执行面为外循环 |
| `router_manager_runner` | `hierarchical` | Router → Manager → Operator 主路径 |
| `execution_aborted_summary` | `hierarchical` | `run_hierarchical` 失败或回合中止 |
| `finalize_summary` | `hierarchical` | 正常收尾（任务级验收 + 终答） |
| `no_user_task` | `hierarchical` | 无有效 user 任务，返回 `Err` |

非分层路径无 **`hierarchical_phase`**；其顶层形态见 **`turn_orchestration_mode`**（`freeform` / `planned_step_*` / `intent_at_turn_start_finished`）。

**设计稿（渐进落地）**：见 **`docs/design/per_state_machine_consolidation.md`**（终答 Gate、运行时顶层相位、**`prepared_route_reduce`** 两层解析路由、**`step_iteration_reduce`** 步循环表驱动；全局单表 PER 仍属后续）。
...
**顶层分支**（非分层：意图门控 → **`PlannedStep`** 滚动视界 或 **`Freeform`** 外循环。`logical_dual_agent` 在门控放行时走逻辑双代理规划消息构造）：

| planner_executor_mode | 顶层入口 | 首轮 P | 后续每步 |
|-----|-----|-----|-----|
| `logical_dual_agent` | `run_logical_dual_agent_then_execute_steps` | 无工具 P 一轮；上下文 `build_logical_dual_planner_messages` + 规划 system（见上） | 每步：`run_agent_outer_loop` → 多轮 **P（带 tools）→ R → E** 直至该步终答 |
| `single_agent` | `run_staged_plan_then_execute_steps` | 无工具 P 一轮；上下文 `build_single_agent_planner_messages`（**保留** `role: tool`）+ 规划 system | 同上 |
| 任意（非分层） | `run_agent_outer_loop`（经 **`non_hierarchical_turn`** **`Freeform`**） | 无单独规划轮；每圈 **P（带 tools）→ R → E** | （同一循环直至本轮结束） |
...
- **单轮预算**（`crates/crabmate-agent/src/turn_budget.rs`，根包 **`agent::turn_budget`**）：**`TurnBudgetCounter`** 贯穿 **`complete_chat_retrying`**（墙钟 / LLM 次数 / Token 粗估）、**`agent_turn::outer_loop`** 与 **分层 Operator ReAct**。Token 粗估经 **`tiktoken_prompt_tokens::estimate_chat_exchange_tokens`** 在每次成功 LLM 往返后累计。超限时 Operator 在有进展时返回 **部分完成**摘要；**`budget_degradation_enabled`** 时 **`GoalVerifier::verify_degraded`** 跳过分层非关键验收。配置与环境变量见 **`docs/配置说明.md`**（**`CM_MAX_TURN_*`** 等）。与会话 **`context_window` / `message_pipeline` 裁剪**正交。
...
- **SSE 协议演进**：载荷形状以 **`sse::protocol::SseMessage` / `SsePayload`**（及 `sse/mod.rs` 再导出）为事实来源；**版本号**以 **`crates/crabmate-sse-protocol`** 与 Leptos 共享。Rust 侧行分类见 **`sse/line.rs`**；浏览器侧在 **`frontend/src/sse_dispatch/dispatch.rs`**（由 **`frontend/src/api/chat_stream/`** 等调用）。**人读契约与错误码表**：**`docs/SSE协议.md`**。**控制面分类契约测试**：`fixtures/sse_control_golden.jsonl` + `cargo test golden_sse_control`（**`crabmate-sse-protocol`** + **frontend** 对齐）。
