...
| **`crabmate-config`**（`crates/crabmate-config`） | **`AgentConfig`** 加载、`finalize`、热重载、CLI 定义、Skills/Cursor rules 合并；根包 **`pub use crabmate_config as config`**。 |
...
| **L5 按轮 Skills** | 工作区 Skills + **当前用户消息** top-k | Web **`build_messages_for_turn`** 与 CLI `chat`/REPL 刷新首条 `system` 时 | **`context_bootstrap/prompt_compose.rs`** · **`web/chat_handlers/chat/turn_build.rs`** · **`runtime/cli/chat.rs`** · **`runtime/cli/repl.rs`** |
