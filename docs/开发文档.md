...
> **说明**：原 `src/agent/hierarchy/`（Manager/Operator）与分阶段 `staged/` 编排入口已从树中移除；运行时固定为 **意图门控 → ReAct 外循环**。前端仍可能展示历史 `hierarchical_subgoal` / `staged_plan_*` 时间线条目（SSE 展示契约，非已删编排路径）。
