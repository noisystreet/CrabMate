...
- **范围**：**`smoke`**、分页、**`sse-*`**、**`prefs-*`**、**`settings-*`**、**`keyboard-shortcuts`**、**`session-list`** / **`session-crud`**、**`status-bar`** 等；依赖 **`CM_E2E_FIXTURES=1`** 与 **`POST /e2e/fixtures/conversation`**。辅助 **`e2e/tests/helpers/`**；本地 **`scripts/e2e.sh`**；CI job **`e2e`** 见 **`.github/workflows/ci.yml`**。关键 **`data-testid`**：**`settings-open`**、**`settings-llm-model`**、**`settings-client-api-key`**、**`nav-new-chat`**、**`approval-deny`**、**`chat-messages-scroller`** 等。
