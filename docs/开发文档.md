...
| `mcp/mod.rs` | **MCP**：**客户端**（stdio）`try_open_turn_handle` / `McpTurnSessions`（`rmcp` + `TokioChildProcess`），按 **user-data `mcp_servers.json`** 多 server（`id`+`command` 指纹）**进程内复用**；`slug` 由 **`name` 自动生成**；远端 `tools/list` → OpenAI `Tool`（`mcp__{slug}__{remote}`）合并；执行经 `tool_registry::dispatch_tool` → `tools/call`（超时见 user-data `tool_timeout_secs`）。**服务端**（`mcp/server.rs`）：**`crabmate mcp serve`**。**安全**：`command` 等价允许启动子进程。Streamable HTTP / 传输鉴权等待办。 |
...
- **`runtime`**：CLI 运行时逻辑，负责交互式 CLI、**`chat` 单次**与调用 `run_agent_turn`。
  ...
  - **`runtime/cli_mcp`**：**`mcp list`** 只读输出进程内 MCP 多服务器缓存（`mcp::mcp_servers_runtime_status`；配置真源 **`user_data` `mcp_servers.json`**）；可选 **`--probe`** 调用 `try_open_turn_handle` 刷新缓存。**`mcp serve`** 支持 stdio（默认）与 **TCP**（`--port N`）两种传输模式，均通过 `rmcp` 的 `TransportAdapterAsyncCombinedRW` 将 `TcpStream` 直接用作 transport。TCP 模式可配合 SSH 隧道实现远程开发。交互式 CLI **`/mcp`**、**`/mcp list`**、**`/mcp probe`** 走同一实现（`run_mcp_list`，`repl_context=true` 时提示语指向 **`/mcp probe`**）。Web：**`GET/PUT /user-data/mcp-servers`**（GET 仅 `has_command`、不返回 `command`）、**`POST …/import`**、**`…/status`**、**`POST …/probe`**；设置页 **`#settings/mcp`**（JSON 解析在服务端；草稿上提至设置页壳层，删除/改服务器计入顶栏 **「保存全部」** dirty，亦可点页内 **「保存 MCP 配置」**）。
