...
| 区域 | 职责 |
|------|------|
| **`agent/`** | 单轮与多轮编排：**`agent_turn`**（外层循环、分阶段规划 **staged**、意图门控、工具执行 **execute_tools**）、**`context_window`** / **`message_pipeline`**、**`per_coord`**（终答与工作流协调）、**`workflow`**（DAG）、可选 **`hierarchy`**（分层 Manager/Operator）。 |
