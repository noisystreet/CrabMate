...
- **`session_hydrate.rs`**：**`wire_session_hydration`**：订阅水合 nonce，拉取 **`GET /conversation/messages`** 并与本地 **`ChatSession`** 对齐；Effect 同步段用 **`try_hydration_wire_snapshot`** → **`HydrationWireSnapshot`**，异步段为 **`run_conversation_hydration_cycle`**；合并前守卫为 **`try_hydration_merge_precheck`**（`Result` 表达「可合并 / 跳过原因」），结果以 **`SessionHydrationMergeOutcome`** 命名；从 **`mod.rs`** 拆出以隔离同步语义；由 **`chat::wire_chat_session_lifecycle_effects`** 在首启 / Web UI 配置之后注册。
