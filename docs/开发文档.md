...
- **`tui_stream_view.rs`** / **`tui_transcript_sync.rs`**：TUI 默认聊天视图；每回合独立 `section[data-tui-msg-id]`，流式只对 live 按行 patch（半行 `textContent`、闭合行 append）；会话切换全量重建。演进见 **`docs/design/web_tui_stream_to_opencode_style.md`**。
