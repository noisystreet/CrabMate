...
- **流式布局 v2 不变量**：`crabmate-turn-layout::project_turn_projection` 分离定稿行与 active；Web **`projection_reconciler`** 负责定稿旁白、锚定 active、终答 flush 与工具占位插入；`TurnLayout` 编排 scratch / overlay / loading 句柄。无锚点预览与终答增量常走 `stream_text_overlay`。`ChatSession.layout_schema_version = 2` 随新会话/流式投影写入缓存。过渡债见 **`docs/Turn布局设计.md` §15**；I14 同帧移交见 **§12.10.1**。
