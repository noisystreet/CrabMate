...
- **`workspace_panel_state.rs`**：**`WorkspacePanelSignals`**：工作区目录树、根路径草稿、设置/选目录忙状态、**`recent_workspace_roots`** 等 **`RwSignal`** 聚合，供 **`make_refresh_workspace`**、**`side_column_view`** 传递。**`workspace_root_actions.rs`**：顶栏正中路径框与 **「文件 → 选择工作区目录 / 最近的工作区」（级联子菜单）**（Tauri 原生选夹 / 浏览器聚焦路径框 / 一键打开已记录路径）共用提交逻辑；成功后写入 **`prefs.last_workspace_root`** 与 **`prefs.recent_workspace_roots`**（最多 10 项）。
