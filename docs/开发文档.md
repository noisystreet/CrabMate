...
| **`crabmate-turn-layout`**（`crates/crabmate-turn-layout`） | 单轮 canonical **`Turn`** + **`TurnEvent`** reducer 与 **`project_turn`** / **`project_turn_web_v2`** 投影；金样 **`fixtures/turn_project_golden.jsonl`**；Web **`TurnLayout`** 与终端 TUI **`turn_project`** 消费见 **`docs/Turn布局设计.md`**。 |
...
| `mcp/mod.rs` | **MCP**：**客户端**（stdio）`try_open_turn_handle` → `McpTurnOpenResult` / `McpTurnSessions`（`rmcp` + `TokioChildProcess`），按 **user-data `mcp_servers.json`** 多 server（指纹含 `id`+`command`+`args`+`env`+`cwd`）**进程内复用**；结构化启动或 legacy 整行拆分；`slug` 由 **`name` 自动生成**；远端 `tools/list` → OpenAI `Tool`（`mcp__{slug}__{remote}`，`inputSchema` 经 `sanitize_mcp_json_schema`）合并；连接失败写入 `skipped`/`last_error` 并经回合 `timeline_log`/`eprintln` 提示。执行经 `tool_registry::dispatch_tool` → `tools/call`。**服务端**（`mcp/server.rs`）：**`crabmate mcp serve`**。**安全**：`command` 等价允许启动子进程。Streamable HTTP / 传输鉴权等待办。 |
...
- **`runtime`**：CLI 运行时逻辑，负责交互式 CLI、**`chat` 单次**与调用 `run_agent_turn`。
  ...
  - **`runtime/cli_mcp`**：**`mcp list`** 只读输出进程内 MCP 多服务器缓存（`mcp::mcp_servers_runtime_status`；配置真源 **`user_data` `mcp_servers.json`**）；可选 **`--probe`** 调用 `try_open_turn_handle` 刷新缓存（失败时打印 `skipped`/`last_error`）。**`mcp serve`** 支持 stdio（默认）与 **TCP**（`--port N`）两种传输模式，均通过 `rmcp` 的 `TransportAdapterAsyncCombinedRW` 将 `TcpStream` 直接用作 transport。TCP 模式可配合 SSH 隧道实现远程开发。交互式 CLI **`/mcp`**、**`/mcp list`**、**`/mcp probe`** 走同一实现（`run_mcp_list`，`repl_context=true` 时提示语指向 **`/mcp probe`**）。Web：**`GET/PUT /user-data/mcp-servers`**（GET 仅 `has_command`/`has_args`/`has_env`/`has_cwd`、不返回启动明文）、**`POST …/import`**（结构化 `command`/`args`/`env`/`cwd`）、**`…/status`**（含 `last_error`）、**`POST …/probe`**；设置页 **`#settings/mcp`**（JSON 解析在服务端；草稿上提至设置页壳层，删除/改服务器计入顶栏 **「保存全部」** dirty，亦可点页内 **「保存 MCP 配置」**）。
...
- **流式布局 v2 不变量**：`crabmate-turn-layout::project_turn_web_v2` 将每条已关闭工具前旁注按 `tool_call_id` 投影为独立行；`callbacks/turn_layout/bubble_queue.rs` 只执行 insert-if-absent，ready 行不得改文、删除或移动。当前唯一可变正文是 loading 尾泡的 `stream_text_overlay`；`ChatSession.layout_schema_version = 2` 随新会话/流式投影写入缓存，水合优先保留 `turn-commentary-*` / `turn-final-answer` 稳定投影，空缓存与无 segment key 的旧会话继续走 v1 adapter。旧 `turn-batch-narration` 仅作为普通历史 assistant 行读取，不再参与 Web 流式判断；crate 级 v1 projector/replay 待发布观察窗口结束后删除。工具前旁注 **I14 同帧原子移交**（flush commentary 与清 loading 同 `update_bound_session`）见 **`docs/Turn布局设计.md` §12.10.1**。
