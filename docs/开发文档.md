...
- **范围**：**`smoke.spec.ts`**（流式/工作区 route 桩）、**`user-data.spec.ts`**、**`conversation-messages-pagination.spec.ts`**（分页 API + 水合 UI；依赖 **`CM_E2E_FIXTURES=1`** 与 **`POST /e2e/fixtures/conversation`**）。公共逻辑在 **`e2e/tests/helpers/`**；主 CI job **`e2e`** 见 **`.github/workflows/ci.yml`**（先 **`trunk build`** + release 二进制，再 Playwright）。关键 DOM 使用 **`data-testid`**，与纯样式 class 解耦。
