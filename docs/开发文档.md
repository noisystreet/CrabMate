...
| `mcp/mod.rs` | **MCP**：**客户端**（stdio 与 **Streamable HTTP**）`try_open_turn_handle` → `McpTurnOpenResult` / `McpTurnSessions`（`rmcp`：`TokioChildProcess` / `StreamableHttpClientTransport`），按 **user-data `mcp_servers.json`** 多 server（指纹含 `id`+stdio 字段或 `url`+`headers`）**进程内复用**；`slug` 由 **`name` 自动生成**；远端 `tools/list` → OpenAI `Tool`（`mcp__{slug}__{remote}`，`inputSchema` 经 `sanitize_mcp_json_schema`）合并；连接失败写入 `skipped`/`last_error`。执行经 `tool_registry::dispatch_tool` → `tools/call`。**服务端**（`mcp/server.rs`）：**`crabmate mcp serve`**（stdio/TCP）。**安全**：`command` 等价启动子进程；`url` 仅 HTTPS 或 loopback HTTP；GET 公开体不回传启动明文。OAuth / 延迟 schema / Resources 等待办。 |
