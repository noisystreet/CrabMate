...
- **范围**：**`smoke`**、分页、**`sse-control`** / **`sse-clarification`** / **`sse-staged-plan`**、**`prefs-*`**、**`session-list`** 等；依赖 **`CM_E2E_FIXTURES=1`** 与 **`POST /e2e/fixtures/conversation`**。辅助 **`e2e/tests/helpers/`**（**`assertions`**、流式桩队列）；本地 **`scripts/e2e.sh`**；CI job **`e2e`** 见 **`.github/workflows/ci.yml`**（**`frontend/dist`** 缓存未命中时才 **`trunk build`**）。关键 **`data-testid`**：**`chat-load-older`**、**`composer-clarification-*`**、**`timeline-panel`** / **`timeline-panel-toggle`**、**`session-list-modal`** 等。
