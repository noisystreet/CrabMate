...
- **前端**：`frontend/src/sse_dispatch.rs`、`frontend/src/api.rs`。
- **修改控制面 JSON 分支顺序**时：同步 **`crates/crabmate-sse-protocol/control_classify.rs`**、**`frontend/src/sse_dispatch.rs`** 与 **`fixtures/sse_control_golden.jsonl`**，并执行：**`cargo test golden_sse_control`**。
...
将工作流或 HTTP 请求轨迹导出为 Chrome Trace JSON 时，可使用环境变量 **`CRABMATE_WORKFLOW_CHROME_TRACE_DIR`** / **`CM_WORKFLOW_CHROME_TRACE_DIR`** 等（与 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`** 的合并行为见 [docs/配置说明.md](配置说明.md) 与 [docs/开发文档.md](开发文档.md)）。
