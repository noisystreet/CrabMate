...
将工作流或 HTTP 请求轨迹导出为 Chrome Trace JSON 时，可使用环境变量 **`CM_WORKFLOW_CHROME_TRACE_DIR`**（工作流 DAG）与 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`**（整轮 `run_agent_turn`）；二者合并行为见 [docs/配置说明.md](配置说明.md) 与 [docs/开发文档.md](开发文档.md)。
