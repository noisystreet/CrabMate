...
- **Many built-in tools; the model picks as needed**:
  ...
  - `create_file` / `modify_file`; before **`create_file`** runs, the server normalizes common **`path`** aliases (e.g. **`file_path`**, **`filename`**, **`file`**, **`name`**, **`output_path`**) and **`content`** aliases (**`body`**, **`text`**, **`code`**, …); key matching is case-insensitive; if **`content`** is omitted it is treated as an empty string. If streamed `arguments` are invalid JSON, local execution **keeps the raw string** (tool reports JSON parse error) and only the vendor round-trip sanitizes to `{}`, so you do not get a misleading missing-`path` Schema error. `read_file` supports ranges, line caps, **`anchor_line`/`context_lines`** (symmetric window around a search hit; mutually exclusive with `start_line`/`end_line`), **`encoding`** (`utf-8` strict, `utf-8-sig`, `gb18030`/`gbk`/`big5`, `utf-16le`/`be`, `auto`, …; malformed → error). For broad code analysis, prefer shrinking the candidate set with `search_in_files` / `codebase_semantic_search` (index required) before segmented reads. `modify_file` can replace line ranges. **Per-turn** `read_file` cache (mtime+size; cleared on writes / workspace change), key includes **encoding**, cap **`read_file_turn_cache_max_entries`**. Web `GET /workspace/file` caps **1 MiB**, same decoding, optional **`encoding`**. Paths in outputs are **relative to workspace** (POSIX), not host absolutes.
