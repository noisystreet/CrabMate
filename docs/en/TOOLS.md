...
- **Many built-in tools; the model picks as needed**:
  ...
  - `web_search`: **Web search**. Default **`web_search_provider = worbrow`** ([worbrow](https://crates.io/crates/worbrow) **≥0.1.1** drives local **Firefox / Chrome/Edge** headless; engine chain **bing→duckduckgo**; **no API key**; alias `browser`). Optional **`brave`** ([Brave Search API](https://brave.com/search/api/)) or **`tavily`** ([Tavily](https://tavily.com/)) with `web_search_api_key` / `CM_WEB_SEARCH_API_KEY`. Prefer `search_in_files` for exact string/regex; use `codebase_semantic_search` for **hybrid FTS + vector** retrieval (default `retrieve_mode: hybrid`; needs `rebuild_index`; see below and **`docs/en/CONFIGURATION.md`**).
