...
**Compound shell (`&&` / `||` / `|` / `;`, …):** Both tools pass **`command`** as a bare executable name or workspace-relative path plus string **`args`**. Do **not** split a shell expression across **`args`** elements—plain **`execve`** does not parse shell metacharacters. Use **`command`: `bash` or `sh`, `args`: `["-c", "whole script one line"]`** (embedded defaults include **`bash`** and **`sh`** in **`allowed_commands`**; keep them if you narrow the list). For **`terminal_session`**, the first **`exec`** follows the same rule; if an interactive shell is already running in the PTY, you may send a full line (including operators) via **`input`**.
