...
**`run_command` timeout and cancel:** Wall clock defaults to **`command_timeout_secs`** (`CM_COMMAND_TIMEOUT_SECS`). On expiry, user cancel, or SSE close, CrabMate sends SIGTERM then SIGKILL to the child **process group** (Unix) and returns truncated partial stdout/stderr with envelope **`error_code`** `timeout` / `cancelled`. Workflow nodes currently reap this way only for **`run_command`**; other tools may still leave `spawn_blocking` children. Docker-sandbox processes are out of P0 scope. See **`docs/design/long_running_tool_execution_todo.md`**.
