...
- **`cargo install crabmate`** (crates.io **stable `0.4.0`**, default **`server`**; pre-release **`0.5.0-alpha.2`** via clone **`cargo install --path .`** or the GitHub Release): does **not** install man into `MANPATH` by default; copy `man/crabmate.1` to `.../share/man/man1/` and run `mandb` (distro-dependent), or prefer **`cargo deb`** / distro packages.
