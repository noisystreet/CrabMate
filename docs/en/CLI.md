...
Reads **`<workspace>/.crabmate/tui_session.json`** by default (`--workspace` and global `--config` before subcommand), writes timestamped **`chat_export_*.json`** / **`chat_export_*.md`** under **`<workspace>/.crabmate/exports/`** (schema / Markdown titles shared via **`crabmate-chat-export`**; on-disk messages are full OpenAI-shaped `Message`, Web download may be a display projection — see `crates/crabmate-chat-export`, `runtime/chat_export.rs`, `frontend/src/session_export.rs`). JSON top-level **`ChatSessionFile`**: `schema` (fixed `crabmate.chat_session`), `schema_version` (SemVer for the envelope), `version` (message-array contract, evolved with OpenAI-compatible `messages`), `messages`. Older files without `schema` / `schema_version` still deserialize; missing fields default to current constants. Each stdout line is the absolute path of a written file for scripts.
