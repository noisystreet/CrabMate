...
- **`cargo install crabmate`** (crates.io **stable `0.5.2`**, default **`server`**; git tag **`v0.5.2`** matches this package): does **not** install man into `MANPATH` by default; copy `man/crabmate.1` to `.../share/man/man1/` and run `mandb` (distro-dependent), or prefer **`cargo deb`** / distro packages.
...
| `sse-replay` | Replay AG-UI events from a **`sse-replay-events.jsonl`** recording and project them into Web block-layout rows (**no** `API_KEY`). **`--format`** takes `rows` (default, projected rows) or `canonical` (canonical Turn state); **`--job_id N`** limits output to one job (`0` = all). |
| `mcp list` | Read-only list of in-process MCP sessions from user-data `mcp_servers.json` (stdio / remote) and merged OpenAI tool names (**no** `API_KEY`). If no chat has run yet, **`mcp list --probe`** tries one connection. |
...
| `workflow compile` | Compile **`workflow_spec` v2** YAML (or a `.md` containing a `` ```crabmate-workflow `` block) into the `{"workflow":{…,"nodes":[…]}}` JSON used by **`workflow_execute`**, printing it to stdout (**no** `API_KEY`). Tutorial: **`docs/工作流编写教程.md`**; examples: **`examples/workflows/`**. |
...
| DELETE | `/conversation/{conversation_id}` | Delete a persisted conversation; **idempotent**: **204** whether it exists, is missing, or has expired; invalid `conversation_id` (empty / too long / bad chars) → **400** `INVALID_CONVERSATION_ID`. Server-side record only—does **not** cascade to the Client sidebar index (Client maintains it) |
