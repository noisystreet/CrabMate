...
| GET | `/conversation/messages` | Read persisted session: query **`conversation_id`**; optional **`limit`** / **`before_index`**. Response includes **`revision`**, **`messages`**, paging fields, optional **`active_agent_role`** / **`tiktoken_prompt_tokens`**, optional **`layout`** (B2; **session-scoped**, not sliced with the `messages` page; written on save; omitted on older rows). **404** `CONVERSATION_NOT_FOUND` |
