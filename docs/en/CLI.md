...
- **`cargo install crabmate`** (crates.io **stable `0.4.0`**, default **`server`**; pre-release **`0.5.0-alpha.3`** via clone **`cargo install --path .`** or the GitHub Release): does **not** install man into `MANPATH` by default; copy `man/crabmate.1` to `.../share/man/man1/` and run `mandb` (distro-dependent), or prefer **`cargo deb`** / distro packages.
...
| GET | `/conversation/messages` | Read persisted session: query **`conversation_id`**; optional **`limit`** / **`before_index`**. Response includes **`revision`**, **`messages`**, paging fields, optional **`active_agent_role`** / **`tiktoken_prompt_tokens`**, optional **`layout`** (session-scoped; not sliced with the `messages` page; written on save; omitted on older rows). Official Web dual-reads `layout` for fingerprints; persisted GET history keeps legacy row ids (does not stamp live stream keys). **404** `CONVERSATION_NOT_FOUND` |
