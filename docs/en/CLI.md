...
- **`cargo install crabmate`** (crates.io **stable `0.5.1`**, default **`server`**; git tag **`v0.5.1`** matches this package): does **not** install man into `MANPATH` by default; copy `man/crabmate.1` to `.../share/man/man1/` and run `mandb` (distro-dependent), or prefer **`cargo deb`** / distro packages.
