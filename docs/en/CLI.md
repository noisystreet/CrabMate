...
- **`cargo install crabmate`** (crates.io **`0.4.0`**, default **`server`**; from a clone also **`cargo install --path .`**): does **not** install man into `MANPATH` by default; copy `man/crabmate.1` to `.../share/man/man1/` and run `mandb` (distro-dependent), or prefer **`cargo deb`** / distro packages.
