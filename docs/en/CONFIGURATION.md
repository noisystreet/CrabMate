...
- **`API_KEY`**: Resolution order is the environment, then the system keyring shared by Web/Desktop/CLI (request body still wins on Web). Persisted LLM keys, Web API bearer (`web_api_bearer`), and remote MCP bearer (`mcp_bearer_{id}`) use macOS Keychain, Windows Credential Manager, or Linux Secret Service—not TOML or XDG plaintext files. Legacy files are deleted only after a successful keyring migration. CLI hot reload does not re-read the environment or keyring.
