...
**`api_base`** containing **`deepseek`** (e.g. **`https://api.deepseek.com/v1`**) selects the DeepSeek vendor adapter (after Kimi/MiniMax/Zhipu routing in **`config/llm_vendors.toml`**). Per [DeepSeek thinking mode](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode), CrabMate may send **`thinking: {"type":"enabled"|"disabled"}`** and, when explicitly enabling, **`reasoning_effort: "high"`** on **`chat/completions`** requests. The catalog **`models`** list includes **`deepseek-v4-flash`**, **`deepseek-v4-pro`**, and **`deepseek-v4-flash-vision-exp`** (the latter sets **`image_url_content_parts`**).
