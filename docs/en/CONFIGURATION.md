...
| Variable | Description |
| --- | --- |
| `CM_CLI_WAIT_SPINNER` | If truthy, show indicatif spinner on stderr before first stream chunk on **`plain_terminal_stream`** paths (e.g. `web_chat_json` stdout echo; needs TTY stderr, not **`NO_COLOR`**). In-process `chat`/`repl` entries removed (D2.2). |
