...
- **`API_KEY`**: Optional process fallback for **`serve`** / ops. Resolution: environment → system keyring (shared with Web/Desktop) → request body **`client_llm.api_key`** (official Client). In-process **`repl` / `chat` / `tui` and `/api-key` entries are removed** (D2.1). Hot reload does **not** re-read **`API_KEY`** / LLM keyring; **`web_api_bearer_token`** may still fall back to the system keyring when TOML/env are empty (same as **`serve`** startup).
...
Then **`API_KEY`** is not required for `serve`. Prefer request body **`client_llm.api_key`** (official Client) or keyring / env fallback. In-process **`repl` / `chat` / `tui` entries are removed**. Function-calling quality depends on model/Ollama; try **`--no-tools`** to validate chat. `crabmate config` does **not** need **`API_KEY`**.
