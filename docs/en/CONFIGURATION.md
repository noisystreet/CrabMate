...
- **CLI**: Type **`/config reload`** (Tab completes). Re-reads the same config path as startup (**`--config`**, or default cwd **`config.toml`** / **`.agent_demo.toml`**, else **`$XDG_CONFIG_HOME/crabmate/config.toml`** (seeded from **`/etc/crabmate`** on first launch when present; skipped inside the source tree unless **`CM_CRABMATE_CONFIG_DIR`** is set)), merges with **current process env**, writes hot fields into in-memory [`AgentConfig`](DEVELOPMENT.md); clears MCP stdio cache; next turn uses the new MCP fingerprint.
...
| `CM_WEB_API_BEARER_TOKEN` | Shared secret for protected Web APIs (server-side check). Clients must send **`Authorization: Bearer …`** or **`X-API-Key: …`**. In the browser, also save the same value under **Settings → Web API shared secret** (see “Web API auth layer” above); **not** the LLM `API_KEY`. |
...
| `CM_WEB_CORS_ALLOWED_ORIGINS` | Comma-separated Origin allowlist (trim each; skip empty). Maps to TOML **`web_cors_allowed_origins`**. Empty/omitted (default): no CORS layer (same-origin). Non-empty: exact Origin allowlist at **`serve`** startup (no `*`); also **`Access-Control-Expose-Headers`** for **`x-conversation-id` / `x-stream-job-id` / `x-request-id`**, and **`/uploads`** CORP **`cross-origin`** (default same-origin keeps **`same-site`**). Restart **`serve`** after changes. |
...
The shell **does not** spawn **`crabmate serve`**. Start the backend yourself (local or remote), then connect via the connect page (see **`desktop-tauri/README.md`**). Before packaging: **`cd frontend && trunk build --release`** and **`bash desktop-tauri/scripts/prepare-sidecar.sh`** (syncs connect/splash and optional **`frontend/dist`**; **no** sidecar binary).
...
| `CM_TUI_PANEL_BG` | **`tui` only**: pane fill. Default / `solid` → **solid blocks** (no border lines; adjacent panes differ slightly in shade); `transparent` / `off` / `none` / `0` → transparent; `dim` → darker fills; `focus` → fill only the focused pane. TOML themes may follow. |
