...
- **`API_KEY`**: Optional process fallback for **`serve`** / ops. Dialogue keys: request body **`client_llm.api_key`** (official Client). **No** server keyring / `saved_model_*` backfill via `merge_client_llm_body`. In-process **`repl` / `chat` / `tui` and `/api-key` entries are removed** (D2.1). Hot reload does **not** re-read env **`API_KEY`**.
