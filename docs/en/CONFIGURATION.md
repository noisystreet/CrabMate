...
| `CM_PLANNER_EXECUTOR_MODE` | **Removed**: the run mode is fixed to **`single_agent`** (ReAct); writing TOML `planner_executor_mode` under `[agent]` fails load via **`deny_unknown_fields`** (legacy values `logical_dual_agent` / `hierarchical` no longer work). **Removed (TOML)**: `intent_mode_bias_enabled`, `llm_fold_system_into_user`, **`intent_at_turn_start_enabled`**, **`intent_l2_*`**, **`intent_execute_*_threshold`**, **`intent_non_hier_execute_*`**, **`intent_l0_routing_boost_enabled`** — writing them under `[agent]` fails load via **`deny_unknown_fields`**. Matching legacy env vars (**`CM_INTENT_AT_TURN_START_ENABLED` / `CM_INTENT_L2_*` / `CM_INTENT_EXECUTE_*` / `CM_INTENT_NON_HIER_*` / `CM_INTENT_L0_ROUTING_BOOST_ENABLED` / `CM_INTENT_MODE_BIAS_ENABLED`**, etc.) are **no longer read** (silently ignored; do not fail startup). |
...
| `CM_LONG_TERM_MEMORY_SCOPE_MODE` | **Removed**: scope is fixed to `conversation_id`; writing TOML `long_term_memory_scope_mode` under `[agent]` fails load via **`deny_unknown_fields`**. |
...
**`api_base`** containing **`deepseek`** (e.g. **`https://api.deepseek.com/v1`**) or a model ID starting with **`deepseek-`** selects the DeepSeek vendor adapter (after Kimi/MiniMax/Zhipu routing in **`config/llm_vendors.toml`**). Per [DeepSeek thinking mode](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode), CrabMate may send **`thinking: {"type":"enabled"|"disabled"}`** and, when explicitly enabling, **`reasoning_effort: "high"`** on **`chat/completions`** requests. The catalog **`models`** list includes **`deepseek-v4-flash`**, **`deepseek-v4-pro`**, and **`deepseek-v4-flash-vision-exp`** (the latter sets **`image_url_content_parts`**). Chat attachments stay as **`/uploads/<file>`** in the session (files on disk in **`chat_uploads/`** beside the session SQLite, not the current workspace root; **`GET /uploads/{filename}`** uses the same auth as other APIs). Workspace **`@image.png` / `file:///image.png`** are not expanded as source text. On the wire, text models drop image parts (avoids HTTP 400), while vision models inline `/uploads/` and workspace raster files as **`data:`** URLs per [Vision](https://api-docs.deepseek.com/guides/vision). Set **`model`** to **`deepseek-v4-flash-vision-exp`** to actually see images.
...
Trimming, summarization, and tool-output compression now modify only a per-turn **`ModelContextView`**. A successful turn appends its new messages to the complete canonical history instead of overwriting SQLite with the compacted view. Hidden `ModelContextArtifact` sidecars retain summary text and canonical ranges; **`GET /conversation/messages.context_artifacts`** exposes their optional replay recipes. Its **`tiktoken_prompt_tokens`** may also include `used_input_tokens`, `max_input_tokens`, reserved output, message/tools/attachment components, `counting_source`, and `provider_input_tokens`. Provider usage wins when present; otherwise tokenizer/conservative estimates remain. All additions are optional for backward compatibility.
