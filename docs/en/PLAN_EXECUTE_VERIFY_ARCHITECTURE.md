...
| **Workflow DAG reflection** | **`WorkflowReflectionController`**, `INSTRUCTION_WORKFLOW_REFLECTION_PLAN_NEXT` | DAG runs inside **`workflow_execute`**; Manager has its own reflection path | If adding prompt-like instructions parallel to reflection injects, document side-by-side in **`docs/工作流编排架构.md`** / **`docs/en/WORKFLOW_ORCHESTRATION_ARCHITECTURE.md`** and shared constants (keep SSE/control-plane rules aligned via **`.cursor/rules/api-sse-chat-protocol.mdc`**) |
