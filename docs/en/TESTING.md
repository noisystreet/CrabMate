...
- **`lizard-rust`**: Rust cyclomatic complexity (requires **`pip install lizard`**; **`scripts/lizard-rust.sh`** / **`scripts/lizard_rust_metrics.py`**: per-module count of functions with **CCN>10** (configurable **`high_ccn_threshold`**); per-module count caps in **`scripts/lizard_module_ccn_caps.toml`**, unlisted modules use **`default_over_max`** (default **0**); full scans also ratchet **`global_over_ccn_sum_cap`** (sum of CCN over all over-threshold functions); measured values must **exactly equal** caps — if they drop, lower the caps). Optional **`--module`**, **`--list-modules`**, **`--list-above N`**, **`--write-caps`** (rewrites module counts and the global sum; `--module` keeps the existing global sum)
