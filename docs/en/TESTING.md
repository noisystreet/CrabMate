...
- **`fn-nloc-ratchet`**: Rust function body line counts (lizard **`nloc`**; **`scripts/fn-nloc-ratchet.sh`** / **`scripts/fn_nloc_rust_metrics.py`**); ratchet files **`scripts/fn_nloc_max_baseline.txt`**, **`scripts/fn_nloc_top10_sum_baseline.txt`**; optional hard cap **`FN_NLOC_CAP`**; runs in **`.github/workflows/code-complexity.yml`**
