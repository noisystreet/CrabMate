...
- **`lizard-rust`**: Rust cyclomatic complexity (requires **`pip install lizard`**; **`scripts/lizard-rust.sh`** / **`scripts/lizard_rust_metrics.py`**: per-module count of functions with **CCN>10** (configurable **`high_ccn_threshold`**); per-module count caps in **`scripts/lizard_module_ccn_caps.toml`**, unlisted modules use **`default_over_max`** (default **0**); ratchet requires measured count **exactly equal** to the cap — if the count drops, lower the cap). Optional **`--module`**, **`--list-modules`**, **`--list-above N`**, **`--write-caps`** (rewrite caps from measured over-threshold counts)
