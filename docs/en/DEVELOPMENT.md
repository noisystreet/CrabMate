...
| `health.rs` | **`build_health_report`** for **`GET /health`** (optional CLI checks include **`dep_gh`** via **`gh version`**). |
...
| `tool_result.rs` | **`crabmate_tool`** v1 envelope + compression helpers. |
...
| `git.rs`, `grep.rs`, `symbol.rs`, `code_nav.rs`, `code_metrics.rs` | VCS, search, metrics |
