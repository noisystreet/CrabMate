...
- **Root `crabmate` Cargo features**: **`default = ["mcp", "docker_sandbox"]`** (matches full product builds). **`mcp`** gates **`rmcp`** (MCP client + `mcp serve`); **`docker_sandbox`** gates **`bollard`** (`sync_default_tool_sandbox_mode = docker`). Use **`cargo build --no-default-features`** or **`--no-default-features --features mcp`** to trim; if **`docker_sandbox`** is off, **`finalize`** rejects **`sync_default_tool_sandbox_mode = docker`**. See **`docs/BACKEND_CORE_FRAMEWORK_DESIGN.md`**.
