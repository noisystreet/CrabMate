...
| `runtime/` | CLI: `chat`, interactive REPL, **`save-session`**, **`tool-replay`**, slash commands, doctor/probe/models, reedline completion, **`CliExitError`**, **`CliToolRuntime`**, transcripts, benchmark, export. Benchmark planning/testing: **`docs/BENCHMARK_PLANNING.md`**. |
...
- **`RunAgentTurnParams`**: split into **transport** (**`AgentTurnTransport`**: `out`, `cancel`, `per_flight`, Web/CLI tool runtime, `plain_terminal_stream`, **`llm_backend`**) and **LLM overrides** (**`AgentTurnLlmOverrides`**: temperature/model/api_base/api_key, `seed_override`); see Chinese **`docs/DEVELOPMENT.md`**.
