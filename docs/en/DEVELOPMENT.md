...
- **Official UI** (Client repo `../crabmate-client/frontend`): Leptos + WASM (Trunk); this repo’s `serve` is **API-only by default**; host SPA with **`--with-web`** + **`CM_WEB_STATIC_DIR`** (**`--no-web`** is a compat no-op).
