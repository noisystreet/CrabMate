...
- [ ] **Shared-secret HTTP auth limits**: embedded default **`web_api_require_bearer=false`** ( **`serve`** may start without a token); set **`web_api_require_bearer=true`** to force non-empty **`web_api_bearer_token`** before **`serve`**; non-empty token still enables Bearer/`X-API-Key` on protected routes; there is still **no per-user identity** or quotas; compromise of the shared secret equals full API access. Optional structured audit for write-side-effect tools (**`target=crabmate::audit_write_tool`**, **`web_audit_*`**). **`API_KEY`** remains LLM-only.
...
> **Done**: **A3/A1/A2** (`44b13fd2`); **B1 / Turn §16 E1** (this branch: `stream_draining` → persist → `conversation_saved` → last `RUN_FINISHED`; dual-order Web; soft `terminal_order`).  
> **Next**: **B2**. Items below need a compatibility window or are optional.
