...
**B. 壳与域分离** | `app/mod.rs` 仅保留布局组合 + 全局 `Effect`，会话/工作区/任务各自的 `Effect` 块可迁到对应子模块的 `wire_*`；**长 `view!` 实参**（如大块 `SettingsPageFormSignals { … }`）收拢到 **`AppShellCtx`** 的组装方法，避免在 `App` 内联重复字段 | `mod.rs` 行数持续下降或由脚本统计不再增长
