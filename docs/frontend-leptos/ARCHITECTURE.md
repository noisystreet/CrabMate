...
**B. 壳与域分离** | `app/mod.rs` 仅保留布局组合 + 全局 `Effect`，会话/工作区/任务各自的 `Effect` 块可迁到对应子模块的 `wire_*`；**`init_app_shell()`** 直接返回 **`AppShellCtx`**（不再向 `App` 暴露 **`AppSignals`**）；**长 `view!` 实参**收拢到 **`AppShellCtx`** 的组装方法（如 **`mobile_shell_header_signals()`**、**`chat_find_bar_signals()`**、**`approval_modal_signals()`**、**`settings_page_view_input()`**、**`settings_page_form_signals()`**） | `mod.rs` 行数持续下降或由脚本统计不再增长
