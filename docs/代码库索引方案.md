...
**修订（实现进展）**：`codebase_semantic_search` 所用 SQLite 已增加 **`crabmate_codebase_files`** 文件目录表（`size` / `mtime_ns` / `content_sha256`）；**整库** `rebuild_index` 默认**增量**跳过未改文件，**子目录** `path` 仍为子树替换；`.rs` 嵌入文本附带轻量符号提示。schema **v4** 起增加 **FTS5**（**`crabmate_codebase_chunks_fts`** 外挂块表），**`query`** 默认 **hybrid**（BM25 + 向量加权，见 **`codebase_semantic_hybrid_alpha`** 等）。详见 `docs/开发文档.md` 与 `docs/工具说明.md`。

本文描述「把整个仓库源码 + 元数据做成**持久、可增量更新**的统一索引，以加速代码浏览与检索」的方向性规划。实现时须与 **工作区路径安全**（`resolve_for_read` / 白名单根、`..` 与符号链接策略）、**密钥与日志脱敏**、以及 **对外多租户身份与网关隔离**（见 **`docs/未来规划功能.md`**）对齐；详见 `.cursor/rules/security-sensitive-surface.mdc` 与 `docs/待办清单.md`。
...
- 实时亚秒级与 IDE 相当的 FS 监听（可作为后续增强）。
   - 在 **未** 由网关完成租户隔离前，勿为 Web 提供跨用户共享的全局索引路径（默认应按 **工作区 + 租户键** 隔离；见 **`docs/未来规划功能.md`**）。
...
- Web 多用户场景：**索引与查询必须绑定**「会话/工作区/租户」键，与网关侧身份/隔离方案同时交付或默认仅本地 CLI 启用（**`docs/未来规划功能.md`**）。
...
- 文档：`README.md` / `docs/开发文档.md` / `docs/工具说明.md` 占位说明（实现各 phase 时同步补全）。
...
- 实现某一阶段后：在 **`docs/待办清单.md`** 中删除已完成的细项或合并重复项，并在本文件顶部「修订记录」中简述（或依赖 Git 历史）。  
