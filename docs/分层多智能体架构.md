...
**状态（2026-08 对齐现实）**：**历史设计 / 归档**。曾落地的 `src/agent/hierarchy/`（Manager/Operator）与相关入口已从树中移除；现行运行时为 **session_mode →（Act 句启发式）→ ReAct 外循环**。本文**不再**描述可运行路径；若将来重做多 Agent，须另开设计稿并对齐当前 `agent_turn`。  
**受众**：维护者（考古/对比用）。  
**语言**：中文。  
