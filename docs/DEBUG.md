...
本文汇总 CrabMate 常用的**调试手段**（环境变量、日志、HTTP 探针、内置工具、协议与测试），与 [docs/CONFIGURATION.md](CONFIGURATION.md)（配置与 `CM_*` 全表）、[docs/CLI.md](CLI.md)（子命令与路由）、[docs/DEVELOPMENT.md](DEVELOPMENT.md)（架构与可观测性细节）互补。
...
| 变量 | 说明 |
| --- | --- |
| **`CM_CLI_WAIT_SPINNER=1`** | 在等待模型首包流式输出（或非流式整段 body）时，于 **TTY stderr** 显示等待动效（默认关；见 [docs/CONFIGURATION.md](CONFIGURATION.md)） |
| **`CM_STAGED_PLAN_CLI_SHOW_PLANNER_STREAM=0`** | 关闭交互式 CLI 下无工具规划轮的模型原文打印（仍保留步骤摘要等） |

---

## 8. 工作流 / 请求 Chrome Trace（可选）

将工作流或 HTTP 请求轨迹导出为 Chrome Trace JSON 时，可使用环境变量 **`CRABMATE_WORKFLOW_CHROME_TRACE_DIR`** / **`CM_WORKFLOW_CHROME_TRACE_DIR`** 等（与 **`CRABMATE_REQUEST_CHROME_TRACE_DIR`** 的合并行为见 [docs/CONFIGURATION.md](CONFIGURATION.md) 与 [docs/DEVELOPMENT.md](DEVELOPMENT.md)）。
...
| 文档 | 内容 |
| --- | --- |
| [CONFIGURATION.md](CONFIGURATION.md) | `CM_*`、热重载、Web 鉴权与安全开关 |
