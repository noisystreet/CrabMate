...
- [ ] **浏览器侧多轮状态（续）**：可选**加密**本地缓存；与 P1 会话模型其它项同向。已实现：`ChatSession` 持久化 **`server_conversation_id` / `server_revision`**，流式回合写入后触发 **`GET /conversation/messages`** 水合；标签页内仍用 **`frontend/src/session_sync.rs`** 的 **`SessionSyncState`**。
