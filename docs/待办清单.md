...
本文仅保留**未完成**待办。**某项完成后必须从本文件删除该条目**（勿长期保留 `[x]`）；空的小节可删掉标题。追溯完成时间请查 Git。维护约定见 `docs/开发文档.md`「TODOLIST 与功能文档约定」。
...
- [ ] **HTTP 身份模型**：嵌入默认 **`web_api_require_bearer=false`**（可无密钥启动 **`serve`**）；生产建议 **`web_api_require_bearer=true`** 并配置 **`web_api_bearer_token`** / **`CM_WEB_API_BEARER_TOKEN`**（共享密钥 + Bearer / X-API-Key）；仍**无**按用户账号的鉴权与配额；共享密钥泄露等同整体沦陷。写副作用工具可选结构化审计（**`target=crabmate::audit_write_tool`**，见 **`web_audit_*`**）。`API_KEY` 仍仅用于上游 LLM。
- [ ] **多角色与人设切换**：支持创建/配置多种**角色**（各角色可绑定不同系统提示、工具可见性、温度等策略）；**CLI**（如 `repl` 的 `/` 命令）与 **Web** 提供**内建命令或等价控件**切换当前会话生效角色，且与会话持久化、导出、`POST /config/reload` 可重载字段的边界在 `README` / `docs/开发文档.md` 中写清。若对外多租户部署，身份与隔离宜放在 **CrabMate 前的网关/BFF**，共识见 **`docs/未来规划功能.md`**。
...
- [ ] **多 Agent 协作**：多角色实例、消息路由与监督式编排（对标 AutoGen / CrewAI / MetaGPT）；与当前单 `agent_turn`、会话与配额模型如何共存须在设计与 `docs/开发文档.md` 中预案。
...
- [ ] **编排层结构化（阶段模型、入参收敛、可测与可观测）**：
  - **严守分层**：对外模型调用仅经 `llm::complete_chat_retrying`；会话侧语义变换仅在 `message_pipeline` / `context_window`；失败语义分层为 `LlmCompleteError` → `RunAgentTurnError`（含 `sub_phase`）→ SSE `code`（权威说明见 `docs/开发文档.md`「`agent_turn` 与 `llm`」）。
  - **显式阶段机**：用枚举/小状态表表达 P/R/E、分阶段 `staged`、工作流嵌套等迁移，与 `AgentTurnSubPhase` 及 SSE 对齐；意图与执行/澄清阈值尽量 **单一决策入口**，并与 `plan_rewrite` / `intent_execute_*` 的边界在实现或文档中对齐（**`docs/配置说明.md`**「意图门控与 `plan_rewrite` 边界」；可与本章「意图识别」条目协同落地）。
  - **收敛入参**：**已实现**：**`RunLoopParams`** 拆为 **`RunLoopCtx`**（整场固定上下文；其内再分为 **`RunLoopCore` / `RunLoopIo` / `RunLoopAttach` / `RunLoopObs`**）与 **`RunLoopTurnState`**（`messages` / `sub_phase` / 模型与温度覆盖 / **`TurnPlannerHints`**（**`src/agent/agent_turn/params.rs`**））；库入口 **`RunAgentTurnParams`** 拆 **`transport`**（**`AgentTurnTransport`**）与 **`llm`**（**`AgentTurnLlmOverrides`**）。后续仍可收紧 **`PerCoordinator`** 与会话侧可变状态的归属。
   - **用例层薄封装**：`agent_turn/run_dispatch.rs` 承载 **分层** / **非分层**（`LogicalDualAgent` / `staged` / `single_agent`外循环）顶层分发；非分层分阶段路由门控见 **`agent_turn/intent/staged_planning_gate.rs`**（**`assess_staged_planning_gate_full_pipeline`** → **`StagedPlanningGateOutcome`**）；**`mod.rs`** 的 **`run_agent_turn_common`** 仅日志、`insert_separator`、`PerCoordinator::new` 与 **`dispatch_*`** 接线；更深层的 P/E/R 仍以 **`plan`** / **`execute_tools`** / **`outer_loop`** / **`staged`** 等子模块为真源，可继续下钻抽「命名步骤」。
...
- [ ] **多入口多门控 — 回合路由决议收敛（高优先级）**：**`intent_at_turn_start`**、**`staged_plan_intent_gate`**、**`planner_executor_mode`** 分层分支与 R 阶段 **`plan_rewrite`** 彼此独立，同类任务可能走分阶段 / 外循环 / 分层，排障难。现有 **`TurnOrchestrationTransition`** / **`freeform_because`** / **`turn_orchestration_mode`** 日志仍缺「单条决议快照」。分三阶段（行为变更由金样兜住）：
  - **阶段 1 · 可观测（零行为变更）**：新增 **`TurnRouteDecision`**（v1 JSON 形状）在门控链结束后一次性记录 `top_level`、`intent_gate`、`staged_gate`、`turn_phase`、`freeform_because`、`plan_requirement_source`；双写 **`tracing`** + SSE **`timeline_log`**（`kind=orchestration_route`）；金样 **`fixtures/turn_route_decision_golden.jsonl`** + **`cargo test golden_turn_route_decision`**（与 **`golden_orchestration_sse`** / **`golden_fsm_orchestrator`** 并列）。
  - **阶段 2 · 决策收敛**：**`assess_turn_routing`** 纯函数 + 显式优先级表（P0 安全/取消 → P1 intent early exit → P2 hierarchical → P3 staged deny → P4 planned_step）；**`run_dispatch`** / **`hierarchy.rs`** 仅解析决议并调 driver；分层与非分层共用决议类型子集；**(intent × staged_gate × planner_mode)** 笛卡尔积金样回归。
  - **阶段 3 · 编排档位（产品简化）**：TOML/Web 三档 **`freeform` / `staged` / `auto`** 映射现有配置键（不增第四套实现）；**`doctor`** / **`GET /status`** 输出本进程「有效编排路径」摘要。边界说明仍见 **`docs/配置说明.md`**「意图门控与 `plan_rewrite` 边界」。

- [ ] **意图识别：设计稿独立阈值键（可选）**：当前沿用 **`intent_execute_*` / `intent_non_hier_*`**，与 **`plan_rewrite_max_attempts`** 的职责边界见 **`docs/配置说明.md`**「意图门控与 `plan_rewrite` 边界」；**`fixtures/intent_regression.jsonl`** 金样由 **`cargo test golden_intent_regression`** 校验（**`.github/workflows/code-complexity.yml`** 的 **`intent-golden`** job）。若后续要引入与现有键**并列**的独立 `execute_direct`/澄清 TOML 键，再拆 merge/finalize；扩展种子见 **`docs/design/intent_regression_seed_cases.md`**。
...
- [ ] **规划与反思策略可插拔**：在现有 `FinalPlanRequirementMode` 与已落地的 `final_plan_require_*` / `final_plan_semantic_check_*` 之上，继续按场景细化（如按工具类型门控、非 workflow 路径策略等）；与可选「观众」侧向点评的边界见 **`docs/design/audience_critic_role.md`**。
- [ ] **观众角色（侧向点评）**：按 **`docs/design/audience_critic_role.md`** 在编排层实现可选、有界、脱敏的无工具侧向调用（触发锚点、输出契约、与 `plan_rewrite` / 分阶段补丁计数、`final_plan_semantic_check` 的互斥或合并策略、tracing/SSE）；默认 fail-open；CLI/TUI/Web 可观测性对齐 **`docs/未来规划功能.md`** 索引。
...
- [ ] **上下文窗口管理 — ReAct 循环消息裁剪（中优先级）**：Operator 与主路径已共用 **`prepare_session_messages_shared`** + **`context_budget_pressure`**（预算 ≥70%/90% 时收紧 char 预算与摘要触发）；**仍可增强**：按 ReAct 轮次更细粒度保留策略。
- [ ] **执行预算控制 — 增强项（中优先级）**：**已实现 MVP**：全链 **`TurnBudgetCounter`**、预算联动裁剪（见 **`context_budget_pressure`**）、超限时 Operator **部分结果**收尾、**`budget_degradation_enabled`** 时跳过分层非关键验收。**仍待办**：子目标级独立预算、更细降级（简化后续子目标等）。
- [ ] **渐进式结果汇报 — Operator 中间状态（中优先级）**：**已实现**：ReAct 每轮 **`TimelineLog`（`hierarchical_subgoal`）** 推送轮次/工具次数/预算用量（Web 子目标气泡 upsert）。**仍待办**：关键里程碑（编译/测试开始）、预估剩余时间。
...
- [ ] **MCP 扩展（续）**：客户端支持 Streamable HTTP / SSE、多 server 与鉴权；可选将本 agent 以 **HTTP**（streamable）暴露并与 Web 鉴权策略对齐；stdio server 已提供 **`crabmate mcp serve`**（见 **`docs/命令行与路由.md`**）；与 `run_command` / 工作区策略的边界在文档中继续细化。
...
**职责摘要**：根入口 `frontend/src/lib.rs`；HTTP/SSE 与本地存储见 `api.rs`、`sse_dispatch/dispatch.rs`、`storage.rs`、`app_prefs.rs`；主界面 `app/mod.rs`（`chat_column`、`chat_composer`、`message_row` / `message_group_views`、`sidebar_nav`、`side_column`、`workspace_panel`、各 `*_modal` 等）；会话与导出见 `session_ops.rs`、`session_export.rs`、`session_search.rs`；Markdown 与展示见 `markdown.rs`、`assistant_body/`、`message_format/`；样式与打包见 `frontend/styles/*.css`、`index.html`、`Trunk.toml`。
...
- [ ] **Web 底栏上下文用量（续）**：已实现底栏「上下文」芯片、新会话 system 基线（`~` + 虚线进度条）、**`stream_ended` / `conversation_saved` 附带 `tiktoken_prompt_tokens`** 与水合实测。后续可完善：
  ...
  - **三端对齐**：**`tui`** 状态行、**`repl`** `/context` 或 **`doctor`** 子项复用 `/status` 与水合字段（见 **`.cursor/rules/cli-tui-web-shared-logic.mdc`**）。
  - **契约与测试**：OpenAPI/`docs/配置说明.md` 文档化新字段；`GET /status` 金样 + 前端「无 `server_conversation_id` → 基线」单测。
- [ ] **国际化与可访问性**：已集中 **`frontend/src/i18n/`**（按域拆分子模块，设置内语言切换）与 **`a11y.rs`**（主要模态焦点 + Tab 陷阱、全局 Esc 关闭弹层）；分层子目标气泡与流式回调中的**协议行解析**已迁至 **`i18n/hierarchical_subgoal.rs`**、**`i18n/intent_timeline.rs`**（中英键兼容，芯片/指标/横幅随 `Locale`）；**剩余**：其它 `app/*` 若有零星硬编码、全量 grep 复核、以及各 `*_modal` / `session_message_search` 等待办条目逐项收口。
