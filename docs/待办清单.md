...
- [ ] **HTTP 身份模型**：默认已 **`web_api_require_bearer=true`**，启动 **`serve`** 须配置 **`web_api_bearer_token`** / **`CM_WEB_API_BEARER_TOKEN`**（共享密钥 + Bearer / X-API-Key）；仍**无**按用户账号的鉴权与配额；共享密钥泄露等同整体沦陷。写副作用工具可选结构化审计（**`target=crabmate::audit_write_tool`**，见 **`web_audit_*`**）。`API_KEY` 仍仅用于上游 LLM。
