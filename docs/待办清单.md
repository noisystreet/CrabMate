...
- [ ] **编排层结构化（阶段模型、入参收敛、可测与可观测）**：
  ...
  - **收敛入参**：**已实现**：**`RunLoopParams`** 拆为 **`RunLoopCtx`**（整场固定上下文）与 **`RunLoopTurnState`**（`messages` / `sub_phase` / 模型与温度覆盖 / **`TurnPlannerHints`**（**`src/agent/agent_turn/params.rs`**））；库入口 **`RunAgentTurnParams`** 拆 **`transport`**（**`AgentTurnTransport`**）与 **`llm`**（**`AgentTurnLlmOverrides`**）。后续仍可收紧 **`PerCoordinator`** 与会话侧可变状态的归属。
