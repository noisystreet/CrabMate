...
- [ ] **MCP 扩展（续）**：~~客户端 Streamable HTTP~~（已支持 `url`/`headers`，HTTPS 或 loopback HTTP）；多 server 鉴权 UX / OAuth、延迟 schema、能力矩阵与审批仍待；可选将本 agent 以 **HTTP**（streamable）暴露并与 Web 鉴权策略对齐；stdio server 已提供 **`crabmate mcp serve`**（见 **`docs/命令行与路由.md`**）。
