...
- [ ] **编排层结构化（阶段模型、入参收敛、可测与可观测）**：
  ...
  - **收敛入参**：**已实现**：**`RunLoopParams`** 拆为 **`RunLoopCtx`**（整场固定上下文；其内再分为 **`RunLoopCore` / `RunLoopIo` / `RunLoopAttach` / `RunLoopObs`**）与 **`RunLoopTurnState`**（`messages` / `sub_phase` / 模型与温度覆盖 / **`TurnPlannerHints`**（**`src/agent/agent_turn/params.rs`**））；库入口 **`RunAgentTurnParams`** 拆 **`transport`**（**`AgentTurnTransport`**）与 **`llm`**（**`AgentTurnLlmOverrides`**）。后续仍可收紧 **`PerCoordinator`** 与会话侧可变状态的归属。
   - **用例层薄封装**：`agent_turn/run_dispatch.rs` 承载 **分层** / **非分层**（`LogicalDualAgent` / `single_agent`外循环）顶层分发；**`mod.rs`** 的 **`run_agent_turn_common`** 仅日志、`insert_separator`、`PerCoordinator::new` 与 **`dispatch_*`** 接线；更深层的 P/E/R 仍以 **`plan`** / **`execute_tools`** / **`outer_loop`** 等子模块为真源，可继续下钻抽「命名步骤」。
