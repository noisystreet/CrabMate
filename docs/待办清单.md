...
- [ ] **Web 底栏上下文用量（续）**：已实现底栏「上下文」芯片、新会话 **`GET /status`** 的 **`tiktoken_new_session_baseline_by_agent_role`**（仅 `system` 粗估）与水合后 tiktoken 实测。后续可完善：
  - **SSE 回合结束推送**：`stream_ended` / `conversation_saved` 附带 `tiktoken_prompt_tokens`，减少仅依赖水合的延迟。
  - **预估 vs 实测区分**：基线显示 `~` / 虚线进度条，水合后为实数；水合失败短提示而非静默。
  - **分项明细**：tooltip 或悬停展示 system / 历史消息 /（可选）工具定义 JSON 近似 token。
  - **基线补全**：工作区已设置时纳入首轮项目画像 `user`、skills 合并后的 system（宜缓存，随工作区/`config/reload` 失效）。
