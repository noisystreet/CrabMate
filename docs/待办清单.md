...
- [ ] **HTTP 身份模型**：默认已 **`web_api_require_bearer=true`**，启动 **`serve`** 须配置 **`web_api_bearer_token`** / **`CM_WEB_API_BEARER_TOKEN`**（共享密钥 + Bearer / X-API-Key）；仍**无**按用户账号的鉴权、配额与审计，共享密钥泄露等同整体沦陷。`API_KEY` 仍仅用于上游 LLM。
- [ ] **多角色与人设切换**：支持创建/配置多种**角色**（各角色可绑定不同系统提示、工具可见性、温度等策略）；**CLI**（如 `repl` 的 `/` 命令）与 **Web** 提供**内建命令或等价控件**切换当前会话生效角色，且与会话持久化、导出、`POST /config/reload` 可重载字段的边界在 `README` / `docs/开发文档.md` 中写清；若对多租户开放须与上条鉴权同盘。
