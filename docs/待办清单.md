...
- [ ] **上下文窗口管理 — ReAct 循环消息裁剪（中优先级）**：Operator 已在每次 **`call_llm`** 前调用 **`prepare_messages_for_hierarchical_operator`**（同步裁剪 + 可选 LLM 摘要，#571）；**仍可增强**：按 ReAct 轮次更细粒度保留策略、与 **`max_turn_tokens`** 预算联动触发压缩。
- [ ] **执行预算控制 — 增强项（中优先级）**：**已实现 MVP**：全链 **`TurnBudgetCounter`**（墙钟 / LLM 次数 / Token 粗估 tiktoken 累计）、超限时 Operator **部分结果**收尾、**`budget_degradation_enabled`** 时跳过分层非关键 **`GoalVerifier`** 与 Manager 反思 LLM。**仍待办**：子目标级独立预算、更细降级（简化后续子目标等）。
