...
- [ ] **国际化与可访问性**：已集中 **`frontend-leptos/src/i18n/`**（按域拆分子模块，设置内语言切换）与 **`a11y.rs`**（主要模态焦点 + Tab 陷阱、全局 Esc 关闭弹层）；分层子目标气泡与流式回调中的**协议行解析**已迁至 **`i18n/hierarchical_subgoal.rs`**、**`i18n/intent_timeline.rs`**（中英键兼容，芯片/指标/横幅随 `Locale`）；**剩余**：其它 `app/*` 若有零星硬编码、全量 grep 复核、以及各 `*_modal` / `session_message_search` 等待办条目逐项收口。
