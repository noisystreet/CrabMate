...
**关联**：运行时行为以 **`docs/工具说明.md`** 中 **`workflow_execute` / `workflow_validate`**、**`docs/开发文档.md`** 中 **`agent::workflow`** 与 **`agent_turn`** 为准；本文定义**能力边界**与**推荐演进方向**，避免与现有 DAG 语义 silently 分叉。  
**Markdown 作者层**（确定性围栏编译 vs 大模型解析、混合策略与 `workflow_spec` v2 草案）见 **`docs/工作流Markdown作者层设计.md`**。  
**与 P-E-V 闭环**：会话级「规划—执行—验证」、**`plan_rewrite` 与验收闸门正交**等见 **`docs/规划执行验证架构.md`**（本文侧重 **DAG / FSM**；彼文侧重 **`plan_artifact` + 分阶段 + PER**）。
...
- **会话级多步**：**`agent_turn`** 外环（P/R/E）、**终答规划**（**`agent_reply_plan` v1**）与 **工作流反思**（**`workflow_reflection_controller`**、`workflow_node_id` 与 DAG 节点对齐规则等）。
...
1. **轮内编排（Intra-turn）**  
   单次模型 **`tool_calls`** 触发的 **`workflow_execute`**：在 **`run_agent_turn` 的 E 步**内跑完；**`WorkflowNodeSpec`** 见 **`src/agent/workflow/model.rs`**，解析见 **`parse.rs`**，调度见 **`execute/schedule.rs`**。可选 **`nodes[].node_tool_role`**（与分阶段 **`executor_kind`** 同策略表，**`src/agent/step_executor_policy.rs`**）仅约束**该节点**的 **`tool_name`**，**不**承担分阶段「滚动步 + 步级 `tools` 列表」职责（后者仍在 **`agent_turn` / `staged`**）。

2. **会话级编排（Inter-turn）**  
   多轮 **P → E → P…**：由 **`agent_turn::outer_loop`**、分阶段 **`staged`**、**`per_coord`** 等驱动；**不**等同于单次 DAG。

3. **声明式规划（Declarative plan）**  
   **`agent_reply_plan` v1**：步骤 **`id` / `workflow_node_id` / `executor_kind`** 等与 DAG 或 validate-only 结果对齐（规则见 **`plan_artifact`**、**`开发文档.md`** 相关小节）。
...
3. **Chrome Trace / 回合 Trace 合并**：与 **`request_chrome_trace`** 的合并规则是否仍适用（见 **`开发文档.md`**）？  
4. **工作区变更集**：DAG 内工具仍走独立 **`ToolContext`**，**不**写入 **`workspace_changelist`**——FSM 路径若引入新执行器，须 **显式继承或放弃**该语义并在 **`工具说明.md`** 写明。
...
| 阶段 | 内容 | 产出 |
|------|------|------|
| **Phase 0** | 文档与示例：DAG **`id` 命名规范**、典型「条件链」模板、外圈循环示例 | **`docs/工具说明.md` 片段 + 本设计稿修订** |
...
| 日期 | 摘要 |
|------|------|
| 2026-04-12 | 初稿：分层（轮内 DAG / 会话级 / 规划）、FSM 编译优先、条件/循环边界与非目标；增补与 **`规划执行验证架构.md`** 的职责交叉引用。 |
| 2026-05-01 | 轮内节点可选 **`node_tool_role`**：与分阶段 **`executor_kind`** 共用 **`step_executor_policy`** 工具允许表，正交于 **`staged`** 的步级 `tools` 收窄。 |
