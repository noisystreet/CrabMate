...
1. **轮内编排（Intra-turn）**
   单次模型 **`tool_calls`** 触发的 **`workflow_execute`**：在 **`run_agent_turn` 的 E 步**内跑完；**`WorkflowNodeSpec`** 见 **`src/agent/workflow/model.rs`**，解析见 **`parse.rs`**，调度见 **`execute/schedule.rs`**。可选 **`nodes[].node_tool_role`**（策略表见 **`src/agent/step_executor_policy.rs`**）仅约束**该节点**的 **`tool_name`**。

2. **会话级编排（Inter-turn）**
   多轮 **P → E → P…**：由 **`agent_turn::outer_loop`**、**`per_coord`** 等驱动；**不**等同于单次 DAG。
...
2. **外圈循环（Agent 级）**  
   ...
   - 与 **`plan_rewrite`** 的「重试故事」一致，文档上易解释。
...
| 2026-05-01 | 轮内节点可选 **`node_tool_role`**：**`step_executor_policy`** 工具允许表约束节点级 `tool_name`。 |
