...
**工具结果 stdout**：**交互式 CLI** / **`chat`**（无 SSE）下每轮工具执行后会打印 **`### 工具 · …`** 标题与正文。**`read_file`** 在终端打印**摘要**：保留编码/行范围等元数据，正文仅前若干行（每行可截断），并注明完整输出在对话历史；**`read_dir`** 与 **`list_tree`** 仍仅标题与一行省略说明、不回显正文（目录树易刷屏）；完整结果均写入历史并供模型使用。若某工具结果判定为**失败**（如 `run_command` 非零退出、`错误：` 前缀等），终端会额外打印 **「自愈提示 · 诊断命令包」**：一行 JSON，可交给模型调用工具 **`playbook_run_commands`**（与 **`error_output_playbook`** 同源启发式，但会**真实执行**经白名单过滤的 `run_command`；**须先脱敏** `error_text`）。**不会**自动执行命令。
