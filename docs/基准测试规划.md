...
- 若修改 **`fixtures/turn_route_decision_golden.jsonl`** 或回合路由启发式，遵循现有 hook / `code-complexity` 的 `fsm-golden`；**benchmark 专用 fixture** 建议放在 **`fixtures/benchmark/`**（新建目录时在**本文**登记即可；勿堆回开发文档）。  
...
```bash
cargo run -- --no-tools bench --benchmark human_eval --batch humaneval_crabmate_tasks.jsonl --batch-output results.jsonl
...
`human_eval` 适配器会在官方 `prompt`（函数桩）前附加续写指令，并设置 **`system_prompt_suffix`**（不要追问、不要调工具）。全局 **`--no-tools`** 才会真正关掉工具表；**`--max-tool-rounds 0` 表示不限制轮次**。抽取走 **`extract_humaneval_completion`**：若模型复述桩，会剥掉与 `prompt` 重叠的前缀，再交给官方 `prompt + completion`。

默认写出汇总 JSON 到 stdout，逐条判分写入 `--output`。冒烟夹具：`fixtures/benchmark/humaneval_tiny_*.jsonl`（可用上列命令本地验证判分链路）。

### 5.4 已落地 / 仍待跟进

- **官方子集与全量基线**：已在 **`docs/BENCHMARK_RESULTS.md`** 记录「前 30 题」与「全量 164 题」基线（`deepseek-v4-flash`，pass@1）。
- **pass@k**：`bench --samples N` 每任务跑 N 次采样；`scripts/humaneval_score_benchmark_results.py --k K` 用无偏估计（`1 - C(n-c,k)/C(n,k)`）聚合输出 `pass@k`（默认 `--k 10`）。
- [ ] **内置判分**（可选）：`bench --evaluate-human-eval` 等，与本文 §5.3 外挂并存时须更新 CLI 与本文。  

**tiny 冒烟基线**（2026-08-13，`tiny/add` 1 题）：见 **`docs/BENCHMARK_RESULTS.md`**。抽取已用 `extract_humaneval_completion` 剥离重复桩前缀。
...
| `crates/crabmate-benchmark/src/types.rs` | `BenchmarkKind`、`BenchmarkTask`、`BenchmarkResult`、`BatchRunConfig` |
| `src/runtime/benchmark/runner.rs` | `run_batch`、加载 JSONL、写结果 |
| `crates/crabmate-benchmark/src/adapter.rs` | 各 benchmark 适配器 |
| `crates/crabmate-benchmark/src/artifact.rs` | 从模型输出抽取 patch / 答案 / HumanEval `completion` |
| `crates/crabmate-benchmark/src/metrics.rs` | 单任务与批次指标 |
