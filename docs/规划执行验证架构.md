...
- **分阶段失败策略**：**`staged_plan_feedback_mode`**（**`fail_fast` / `patch_planner`**）等（见 **`docs/配置说明.md`**）。
...
**双轨**指：会话轮末 **`per_coord::final_plan_gate`** + **`plan_rewrite`** +（可选）**`per_plan_semantic_check`** 与 **`reflect_semantic`**，与 **Manager / Operator** 在 **`src/agent/hierarchy/`** 内的分解—执行—子目标验收—**`reflect_and_replan`** **不共用同一套类型与计数器**，但产品语义上同属 P-E-V。**收敛目标**是「合同与可观测一致」，**不是**把 Manager 硬塞进 `AfterFinalAssistant`。

| 主题 | PER / `outer_loop` / `staged` | `hierarchy`（分层） | 维护时注意 |
|------|-------------------------------|---------------------|-------------|
| **终答 `agent_reply_plan` v1 与 `plan_rewrite`** | **`after_final_assistant`** → **`final_plan_gate`**；计数 **`plan_rewrite_attempts`** | Manager 产出 **`SubGoal`** 等结构化 JSON，**不经**终答门控；**话语型 / 澄清确认** 回落 **`run_agent_outer_loop`** 后才进入 PER | 勿把 **`ManagerAgent::reflect_and_replan`** 称为 `plan_rewrite`；改终答规则只动 **`per_coord` / `reflect_semantic`** |
...
| **侧向语义 LLM（规划 vs 工具摘要）** | **`per_plan_semantic_check`** + **`reflect_semantic::map_plan_semantic_llm_outcome_to_reflect_ctl`** | **无**默认等价路径 | 不把分层子目标反思误当成 **`final_plan_semantic_check`** |
...
| **SSE** | 新增控制面事件或 **`reason_code`** 须在 **`docs/SSE协议.md`**、**`crabmate-sse-protocol`**、**`frontend-leptos`** 同步（见仓库 **api-sse-chat-protocol** 规则）。 |
