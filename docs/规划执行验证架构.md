...
- **分阶段失败策略**：**`staged_plan_feedback_mode`**（**`fail_fast` / `patch_planner`**）等（见 **`docs/配置说明.md`**）。在 **`patch_planner`** 下，补丁规划注入的 **user** 正文对**验证失败**附带 `step_verifier` 的确定性原因；对**执行子循环 `run_agent_outer_loop` 返回 `Err`** 附带**截断后的** `RunAgentTurnError::to_string()` 摘要（与工具失败路径的固定指引并列），便于规划器下一轮 JSON 对齐真实失败态。
