...
- **验收缺省与双轨对齐**（**`crabmate-agent::acceptance::executor_defaults`**）：解析 `agent_reply_plan` 后，**`test_runner`** 步若模型未填 `acceptance`，自动注入 **`expect_exit_code: 0`**；分层子目标描述像构建/测试且未填 `acceptance` 时，`GoalVerifier` 复用同一缺省退出码。步末 **`empty_execution`** 对 **`review_readonly` / `patch_write`** 要求出现对应探查/写文件类工具，避免「仅有 `--version` 探针或只读却无实质进展」误判通过。
