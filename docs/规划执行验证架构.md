...
**语言**：中文。  
**关联文档**：**`docs/工作流编排架构.md`**（轮内 DAG / FSM 扩展边界）、**`docs/开发文档.md`**（**`agent_turn` / `per_coord` / `plan_artifact`**）、**`docs/工具说明.md`**（**`workflow_execute` / `agent_reply_plan`** 字段约定）、**`docs/配置说明.md`**（**`final_plan_*` / `reflection_*`**）。

### 仓库嵌入默认（把闭环「用满」而不改代码）

- **`steps[].executor_kind` / `acceptance` / `max_step_retries`**：由模型在无工具规划轮产出的 **`agent_reply_plan` v1** 填写；典型范式为 **读（`review_readonly`）→ 写（`patch_write`）→ 跑测（`test_runner` + `acceptance`）**。单步滚动规划下每轮通常 1 条 `steps`，验收在步窗口内**自后向前**聚合全部 `role: tool`，**任一**满足 `acceptance` 即 Pass（见 **`docs/工具说明.md`**）。
...
- **`workflow_execute`**：同轮 **DAG**；可由规划 **`workflow_node_id`** 对齐责任。

### 2.3 验证（Verify）— **已实现**

- **确定性验证闸门**（**`step_verifier.rs`**）：在分阶段路径下，对「**当前分步**」步界内全部 `role: tool` 与 **`steps[].acceptance`** 比对（**步界**为：本分步 `user` 注入之后、**下一条**普通 `user` 或下一条分步注入之前；步内编排注入 `user` **不**截断窗口；工具枚举见 **`crabmate-types::tool_messages_in_staged_step_window`**）。自后向前尝试，**任一** tool 满足即 Pass；单条 tool 判定共用 **`acceptance::verify_plan_step_acceptance_for_tool_message`**。支持以下验收规则：
...
- **验收缺省与双轨对齐**（**`crabmate-agent::acceptance::executor_defaults`** + **`hierarchy/goal_acceptance`**）：解析 `agent_reply_plan` 后，**`test_runner`** 步若模型未填 `acceptance`，自动注入 **`expect_exit_code: 0`**（**`effective_plan_step_acceptance`**）；分层子目标经 **`effective_goal_acceptance`** 复用同一构建类描述缺省；退出码文本解析共用 **`parse_exit_code_from_combined_output`**。步末 **`empty_execution`** 对 **`review_readonly` / `patch_write`** 要求出现对应探查/写文件类工具，避免「仅有 `--version` 探针或只读却无实质进展」误判通过。
...
- **计数与上限语义（与 `plan_rewrite` 分离）**：**`PerCoordinator`** 维护 **`plan_rewrite_attempts`**（终答 **`after_final_assistant`** 路径）；**`GET /status`** 的 **`per_active_jobs[*]`** 同步暴露。
...
**终答 Gate 金样**：`fixtures/final_plan_gate_golden.jsonl` + **`per_coord/final_plan_gate_golden.rs`**（`cargo test golden_final_plan_gate`）；与 **`FinalPlanGateRoute`** / **`FinalPlanGateDecisionReason`** 对齐。
...
- 分层意图门控（与 `intent_at_turn_start` 共用 L2 优先管线，旧 L0/L1 规则层兜底）：**`src/agent/agent_turn/intent/at_turn_start.rs`**（**`run_intent_for_hierarchical`**）。
...
| 外循环 reflect **`BreakOuter`** 单轮停 | **`cargo test -p crabmate run_agent_turn_outer_loop_reflect_break_single_planner_mock`** | 钉 **`OuterLoopDriver`** reflect → stop |
| 冗余探针早停 | **`cargo test -p crabmate golden_turn_completion`**（`redundant_tools`）+ **`outer_loop_iteration_reduce`** 金样 | 全链路 mock 见 **`turn_completion_golden`** |
| 外循环工具后继续再终答 | **`cargo test -p crabmate run_agent_turn_outer_loop_post_tools_continue_then_stop_mock`** | 钉 **`ContinueNextIteration`** ×2 + 终答 |
| 外循环完成证据后终答 | **`cargo test -p crabmate run_agent_turn_outer_loop_post_tools_early_stop_mock`** | 预置证据 + reflect 单轮停 |
...
| **分阶段 `no_task` → 外循环** | **`cargo test -p crabmate run_agent_turn_staged_no_task_degrades_to_outer_loop`** | 规划 `no_task=true` 后 PER 外循环 |
| **分阶段解析失败 → 外循环** | **`cargo test -p crabmate run_agent_turn_staged_invalid_plan_degrades_to_outer_loop`** | 首轮无 `agent_reply_plan` 时降级 |
| **分阶段步验收失败 → patch_replanner** | **`cargo test -p crabmate run_agent_turn_staged_step_verify_fail_patch_replanner_mock`** | 验收失败触发补丁规划后重试步外循环 |
| **终答 Gate 纯函数矩阵** | **`cargo test golden_final_plan_gate -p crabmate`** | **`fixtures/final_plan_gate_golden.jsonl`** |
| **工作流反思控制器** | **`cargo test golden_workflow_reflection_regression -p crabmate`** | **`fixtures/workflow_reflection_regression.jsonl`** |
| 回落路由纯函数 | **`cargo test -p crabmate hierarchical_intent_route`**（模块内测试） | 变更 **`resolve_hierarchical_post_intent_route`** 时必跑 |

变更 **`final_plan_requirement` / `plan_rewrite` / `after_final_assistant` / `WorkflowReflectionController`**：至少跑 **PER 轨** 触点 + **`cargo test golden_final_plan_gate -p crabmate`**；若动意图管道再加 **`cargo test -p crabmate golden_intent_regression`**（**`fixtures/intent_regression.jsonl`**）。变更 **Manager 反思 / 子目标验收**：跑 **分层** 触点 + 必要时 **`hierarchy`** 下单元测。变更 **话语型回落或意图阈值**：跑 **交汇** 触点 + **`hierarchical_intent_route`**。变更 **分阶段 post-parse / 降级路径**：跑 **`run_agent_turn_staged_*_degrades_to_outer_loop`**。变更 **分阶段 patch / 步验收**：跑 **`run_agent_turn_staged_step_verify_fail_patch_replanner_mock`** + **`golden_fsm_orchestrator`**（含 **`step_patch_recover_reduce`**）。
...
| 2026-06-30 | **`hierarchy/goal_acceptance`**：`effective_goal_acceptance` / `verify_goal_acceptance_spec`；**`acceptance::exit_code_parse`** 共用退出码解析。 |
| 2026-06-30 | **`acceptance::executor_defaults`**：`test_runner` / 构建类子目标缺省 `expect_exit_code`；`review_readonly` / `patch_write` 步末空执行检测强化。 |
