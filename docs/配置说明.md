...
| `CM_CRABMATE_CONFIG_DIR` | 覆盖本机用户**配置**根（绝对路径）；默认 **`$XDG_CONFIG_HOME/crabmate`**（多为 **`~/.config/crabmate`**）。无 **`--config`** 时发现顺序：**cwd** **`config.toml`** / **`.agent_demo.toml`**（可并存合并）→ 否则本目录下的 **`config.toml`**。若尚无用户 **`config.toml`** 且系统有 **`/etc/crabmate/config.toml`**（桌面 deb 模板），首次会**种子拷贝**运行时所需子集（**`config.toml`**、**`agent_roles.toml`**、**`prompts/`**、**`config/`**；**不**拷贝嵌入分片如 **`tools.toml`**；**不覆盖**已有文件）。在 CrabMate **源码树内**默认**不**自动采用本机 XDG（避免开发机装过 deb 后污染嵌入默认）；设本变量则可强制走 XDG。运行时以用户副本为准（桌面仅在种子失败时只读回退 `/etc`）。 |
...
| `CM_SKILLS_ENABLED` | 是否启用 skills 注入（默认 `true`；L2 索引与 L5 按轮正文、以及 **`/<skill-id>` 强制选用** 都受其控制，读取 `skills_dir/*.md` 与 `skills_dir/<id>/SKILL.md`）。 |
