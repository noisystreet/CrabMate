...
`/chat` 与 `/chat/stream` 经有界队列调度；满时 **503**、`QUEUE_FULL`。`/status` 返回队列与 **`per_active_jobs`** 等字段；**`per_active_jobs[*]`** 含 **`plan_rewrite_attempts`**（终答重写）等，见 **`docs/开发文档.md`**「`GET /status`」。
