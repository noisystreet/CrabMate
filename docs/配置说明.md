...
| `CM_MCP_TOOL_TIMEOUT_SECS` | MCP 工具超时（秒）。同一进程按指纹复用连接：stdio 含 `command`/`args`/`env`/`cwd`；远程 Streamable HTTP 含 `url`/`headers`。**`crabmate mcp list`** 不要求 `API_KEY`；**`mcp list --probe`** 会连接（stdio 启动子进程 / 远程 HTTP）。导入 MCP JSON 时支持 **`command`** 或 **`url`**（`https://`；`http://` 仅 loopback）；连接失败进入 status 的 **`last_error`**，并在对话回合以 `timeline_log` / 终端提示。 |
