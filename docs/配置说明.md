...
| `CM_INTENT_NON_HIER_EXECUTE_LOW_THRESHOLD` | **非分层**意图路由「确认后执行」低阈值；省略时回退 `CM_INTENT_EXECUTE_LOW_THRESHOLD`。TOML：`intent_non_hier_execute_low_threshold`。 |
| `CM_INTENT_NON_HIER_EXECUTE_HIGH_THRESHOLD` | **非分层**意图路由「直接执行」高阈值，且不小于 non-hier low；省略时回退 `CM_INTENT_EXECUTE_HIGH_THRESHOLD`。TOML：`intent_non_hier_execute_high_threshold`。 |
...
| **`intent_non_hier_execute_low_threshold` / `intent_non_hier_execute_high_threshold`** | 同上，但仅当**非分层模式**时覆盖上一行两阈值；省略则回退 `intent_execute_*` | **无关** |
