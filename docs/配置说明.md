...
| `CM_ORCHESTRATION_PROFILE` | 编排档位 **`freeform`** / **`staged`** / **`auto`**（默认 **`auto`**；映射 `staged_plan_intent_gate`，不增第四套实现）。TOML **`[agent] orchestration_profile`**。`doctor` / **`GET /status`** 的 **`effective_orchestration_path`** 为静态摘要。 |
...
| **`staged_plan_intent_gate`（门控 ②）** | 在门控 ① **通过之后**再评一次（同 L2 优先管线；旧规则层兜底）；不通过则**不进入**滚动分阶段，改走单 Agent 外循环（**`Freeform`**） | **无关** |
...
| `CM_MAX_TURN_DURATION_SECONDS` | 单轮墙钟上限（秒）；**`0`** 不限制；默认 **600**。与 **`agent_turn` / `staged` / 分层 Operator** 共用 **`TurnBudgetCounter`**。 |
...
| `CM_MCP_ENABLED` | 是否启用 MCP。需 **`cargo build --features mcp`**；未编译该 feature 时 `mcp list` / 进程内 MCP 工具代理不可用。 |
