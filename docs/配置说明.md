...
**分阶段意图门控（`staged_plan_intent_gate`）**：与开局 **`intent_at_turn_start`** 一致走 L0+L1+可选 L2。除「非 `IntentAction::Execute`」不进入分阶段外，当管线已判定 **`Execute`** 但用户正文命中**架构/重构类咨询**启发式（例如同时出现「重构/架构/隐式状态…」与「哪里/哪些/如何/建议/分析…」等征询语气，且不含明确的改代码/跑测/提交等落地指令）时，**不进入**滚动分阶段规划，改走**单 Agent 外循环**；日志与排障中拒绝原因为 **`advisory_execute_bypass_staged`**。无工具规划默认说明见源码 **`staged_plan_phase_instruction_default`**（可用 **`staged_plan_phase_instruction`** 覆盖）。
