...
- **优先级**：本轮请求 JSON **`session_mode`** → 会话持久化 **`active_session_mode`**（内存与 SQLite 同源）→ 配置默认。
- **Web**：`POST /chat*` 可选 **`session_mode`**: `"ask"` \| `"plan"` \| `"act"`。Ask/Plan 在意图门控之后挂只读约束（即使默认关闭意图门控也生效）。回合成功落盘时写入/保留 **`active_session_mode`**（与 `agent_role` 同 revision 路径）。
