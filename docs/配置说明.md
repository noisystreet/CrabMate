...
- **交互式 CLI（`repl` / `tui` / `chat` / `serve`）与桌面对齐**：无 **`--config`** 且 cwd 无本地 `config.toml` / `.agent_demo.toml` 时，采用 **`$XDG_CONFIG_HOME/crabmate/config.toml`**（可从 `/etc` 种子；源码树内也采用）；并合并 **`$XDG_DATA_HOME/crabmate/llm_overrides.json`**（api_base / model 等）与 **`prefs.json`** 的最近工作区 / `cm_role`。密钥见上条。
...
| `CM_DSML_STREAM_STRIP_ENABLED` | 流式 SSE/CLI/TUI 下发前剥离 DSML 标记；省略时与物化开关同默认（`true`）。TOML：`dsml_stream_strip_enabled`。 |
