...
| `CM_INTENT_AT_TURN_START_ENABLED` | 是否启用回合起点意图门控（默认同 TOML **`intent_at_turn_start_enabled=false`**）。**`false`**：跳过门控启发式。**`true`** 且会话为 **Act**：仅跑用户句**关键词执行约束**（见下「门控工具收窄」）；**不**再额外 `chat`、**不**发 `intent_analysis`。Ask/Plan 时跳过门控启发式（只读由 mode 在门控后挂载）。 |
| `CM_PLANNER_EXECUTOR_MODE` | 有效值仅为 **`single_agent`**（ReAct）。省略则同默认。TOML：`planner_executor_mode`。**已移除（TOML）**：`logical_dual_agent` / `hierarchical`、`intent_mode_bias_enabled`、`llm_fold_system_into_user`、**`intent_l2_*`**、**`intent_execute_*_threshold`**、**`intent_non_hier_execute_*`**、**`intent_l0_routing_boost_enabled`**——写入 `[agent]` 会因 **`deny_unknown_fields` 加载失败**。对应旧环境变量 **`CM_INTENT_L2_*` / `CM_INTENT_EXECUTE_*` / `CM_INTENT_NON_HIER_*` / `CM_INTENT_L0_ROUTING_BOOST_ENABLED` / `CM_INTENT_MODE_BIAS_ENABLED`** 等**不再读取**（静默忽略，不导致启动失败）。 |
...
| **`plan_rewrite_max_attempts`** | **已有** `agent_reply_plan` v1（或等价的终答规划）产出后的校验失败、侧向语义不一致反馈等触发的「请重写」循环 | 与意图门控**独立**；超限见 SSE `plan_rewrite_exhausted`（**`docs/SSE协议.md`**） |

**门控工具收窄（L2 退役 R2/R3）**：开门控且 **Act** 时，仅当用户句同时命中「不要改 / 不要跑」类词**与**分析/解释类词时，才设置 **`step_executor_constraint = ReviewReadonly`** 并注入短 hint。L2 分类、阈值键与 Clarify/Confirm 早退路径已**删除**（旧 TOML 写入对应键会配置加载失败）。需要默认只读请用 **Ask/Plan**。**`ReviewReadonly` / `PatchWrite` / `TestRunner` 均允许用户已启用的 `mcp__*` 代理工具**（仍不参与并行只读批）。
...
- **与意图分类**：L2 与相关配置键已拆除（退役 **R3**）；能力边界靠 Ask/Plan/Act。
