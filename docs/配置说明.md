...
- **`API_KEY`**：进程内 **`serve` / `repl` / `chat` / `tui`** 启动时读取密钥并保存在内存（可为空）。优先级：**环境变量 `API_KEY`** → **系统钥匙串**（macOS Keychain、Windows Credential Manager、Linux Secret Service；与 Web/桌面侧栏保存同源）→ 启动后再用 **REPL/TUI `/api-key set`**。默认 `/api-key set` 写钥匙串；加 **`--no-persist`** 才仅本进程。**热重载不**重读环境或钥匙串。旧 **`$XDG_DATA_HOME/crabmate/secrets/client_llm`** / **`executor_llm`** 及 `llm_overrides.saved_models[*].api_key` 会在钥匙串写入成功后自动迁移并删除明文；钥匙串不可用时保留旧文件并明确报错，避免丢失。**`crabmate models` / `probe`** 在 **`bearer`** 下仍要求启动前环境变量 **`API_KEY`** 非空。
