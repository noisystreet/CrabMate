...
| `CM_MCP_TOOL_TIMEOUT_SECS` | MCP 工具超时（秒）。同一进程按指纹（含 `command`/`args`/`env`/`cwd`）复用 stdio；**`crabmate mcp list`** 不要求 `API_KEY`；**`mcp list --probe`** 会启动子进程。导入 MCP JSON 时写入结构化字段（不再强制 `sh -c`）；连接失败会进入 status 的 **`last_error`**，并在对话回合以 `timeline_log` / 终端提示。 |
