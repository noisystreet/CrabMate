...
- **`API_KEY`**：进程内 **`serve` / `repl` / `chat` / `tui`** 启动时读取密钥并保存在内存（可为空）。优先级：**环境变量 `API_KEY`** → **系统钥匙串**（macOS Keychain、Windows Credential Manager、Linux Secret Service；与 Web/桌面侧栏保存同源）→ 启动后再用 **REPL/TUI `/api-key set`**。默认 `/api-key set` 写钥匙串；加 **`--no-persist`** 才仅本进程。**热重载不**重读环境或钥匙串。Web 请求合并与 CLI 一致：请求体有密钥优先，否则若进程环境 **`API_KEY` 非空则不用钥匙串填补**。旧 **`$XDG_DATA_HOME/crabmate/secrets/`** 下的 **`client_llm`** / **`executor_llm`** / **`web_api_bearer`** / **`mcp_bearer_{id}`** 及 `llm_overrides.saved_models[*].api_key` 会在钥匙串写入成功后自动迁移并删除明文；钥匙串不可用时保留旧文件并明确报错，避免丢失。**`crabmate models` / `probe`** 在 **`bearer`** 下仍要求启动前环境变量 **`API_KEY`** 非空。
...
| `CM_INTENT_MODE_BIAS_ENABLED` | **已忽略的历史键**（分层 runner 偏置；路径已移除）。TOML `intent_mode_bias_enabled` 仍可出现以免旧配置解析失败，但**不生效**。 |
| `CM_PLANNER_EXECUTOR_MODE` | 运行时固定 **`single_agent`**（ReAct）。`logical_dual_agent` / `hierarchical` 为废弃别名，解析/`finalize` 均映射为 `single_agent`。TOML：`planner_executor_mode`。 |
...
| `CM_MCP_COMMAND` | legacy 单条 MCP stdio 启动命令；仅当 user-data 尚无服务器时**一次性**导入。 |
