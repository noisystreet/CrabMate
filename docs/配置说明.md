...
| `CM_INTENT_L2_ENABLED` | 是否启用默认 L2 无工具语义意图分类（**额外** `chat` 一次；不可用时 **fail-open 进主模型** 且门控开启时 **工具只读收窄**；**默认开**）。同义 TOML 键 **`intent_l2_enabled`**。 |
| `CM_INTENT_L2_MIN_CONFIDENCE` | L2 `confidence` 阈值（0.0–1.0，默认 0.7）。**≥ 阈值**：采纳 L2。**低于阈值**：仍采纳 Greeting / Qa / Ambiguous（及需澄清的 Execute）；**低置信直执 Execute** 则 fail-open + 保守只读。TOML：`intent_l2_min_confidence`。 |
...
**澄清 / 待确认与工具收窄**：当 **`intent_at_turn_start`** 将本回合定为 **`ClarifyThenExecute`** 或 **`ConfirmThenExecute`** 时，在进入主循环前会设置 **`step_executor_constraint = ReviewReadonly`**（与 **`qa.readonly`** 等只读收窄一致）。**L2 不可用或低置信 Execute fail-open** 时同样收窄为只读并注入短 hint（确认续接 / 工具失败后续跑除外）。**`ReviewReadonly` / `PatchWrite` / `TestRunner` 均允许用户已启用的 `mcp__*` 代理工具**（仍不参与并行只读批）。若 **`intent_at_turn_start_enabled=false`**，意图管线仍可打时间线，但会**清除**上述工具收窄副作用。
