...
| `CM_WEB_WORKSPACE_POOL` | Web 远程「项目池」根目录（如 `/workspace`）；配置后浏览器可用项目名切换/新建子工作区，无需手输绝对路径。须与 `workspace_allowed_roots` 配合收窄范围（见下文）。 |
...
**Web 项目池（`web_workspace_pool` / `CM_WEB_WORKSPACE_POOL`）**：面向 VPS / 远程浏览器。将固定目录（如 **`/workspace`**）作为项目池；用户通过 **`GET/POST /workspace/projects`** 或 Web「文件 → 选择工作区」弹窗按**项目名**（字母数字及 `._-`）打开或新建子目录（如 `/workspace/my-app`），无需在浏览器里输入服务器绝对路径。推荐同时设置 **`workspace_allowed_roots = ["/workspace"]`**（或环境变量 **`CM_WORKSPACE_ALLOWED_ROOTS`**）并配置 **Web Bearer 鉴权**。目录不存在时 finalize 会尝试 **`mkdir -p`** 创建池根。
