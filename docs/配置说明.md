...
| `CM_INTENT_L2_ENABLED` | 是否启用 L2（**额外**无工具 `chat` 一次）。**默认关**（L2 退役 R1；见 `agent_space/l2-retirement-plan.md`）。仅当本项为 **`true`** 且 **`intent_at_turn_start_enabled=true`** 且本回合 **`session_mode=act`** 时才会实际调用；输出 JSON **`suggested_mode`**（仅时间线/观测，**不**自动改用户 mode；Web 已不渲染气泡）。不可用或置信不足时 **fail-open 进主模型**，门控开启时 **临时只读**。TOML：`intent_l2_enabled`。 |
| `CM_INTENT_L2_MIN_CONFIDENCE` | L2 `confidence` 阈值（0.0–1.0，默认 0.7）。**≥ 阈值**：采纳建议（进主路径、不因 L2 收窄）。**低于阈值**或 L2 不可用：fail-open + 保守只读（Act 下文案提示 `/mode act` 确认后重试）。TOML：`intent_l2_min_confidence`。 |
| `CM_INTENT_L2_MAX_TOKENS` | L2 分类请求的 `max_tokens`（32–1024，默认 384）。TOML：`intent_l2_max_tokens`。 |
| `CM_INTENT_AT_TURN_START_ENABLED` | 是否启用回合起点意图门控（默认同 TOML **`intent_at_turn_start_enabled=false`**）。为 **`false`** 时**不调用 L2**（省一次 chat）。为 **`true`** 且会话为 Ask/Plan 时同样跳过 L2（mode 已决定只读档）。 |
| `CM_INTENT_L0_ROUTING_BOOST_ENABLED` | **已无决策作用**（L1 提级已移除）；仍可读配置以免旧 TOML 报错。TOML：`intent_l0_routing_boost_enabled`。 |
| `CM_INTENT_EXECUTE_LOW_THRESHOLD` | **已无决策作用**（L1 关键词路由已移除）；仍可读。TOML：`intent_execute_low_threshold`。 |
| `CM_INTENT_EXECUTE_HIGH_THRESHOLD` | **已无决策作用**；仍可读，且 finalize 仍保证 ≥ low。TOML：`intent_execute_high_threshold`。 |
| `CM_INTENT_NON_HIER_EXECUTE_LOW_THRESHOLD` | **已无决策作用**；省略时回退 `CM_INTENT_EXECUTE_LOW_THRESHOLD`。TOML：`intent_non_hier_execute_low_threshold`。 |
| `CM_INTENT_NON_HIER_EXECUTE_HIGH_THRESHOLD` | **已无决策作用**；省略时回退 `CM_INTENT_EXECUTE_HIGH_THRESHOLD`。TOML：`intent_non_hier_execute_high_threshold`。 |
| `CM_PLANNER_EXECUTOR_MODE` | 有效值仅为 **`single_agent`**（ReAct）。省略则同默认。TOML：`planner_executor_mode`。**已移除**：`logical_dual_agent` / `hierarchical`（写入将导致配置加载失败）。`intent_mode_bias_enabled` / `CM_INTENT_MODE_BIAS_ENABLED`、`llm_fold_system_into_user` 亦已移除（`deny_unknown_fields`）。 |

### 意图门控与 `plan_rewrite` 边界（速查）

| 配置 / 机制 | 生效阶段 | 与 `plan_rewrite_max_attempts` 的关系 |
| --- | --- | --- |
| **`intent_execute_low_threshold` / `intent_execute_high_threshold`** | **历史键**，L1 路由已移除后**不再**划分确认/直执；L2 失败时 fail-open | **无关** |
| **`intent_non_hier_execute_low_threshold` / `intent_non_hier_execute_high_threshold`** | 同上覆盖键；省略则回退 `intent_execute_*` | **无关** |
| **`intent_at_turn_start`（门控）** | 回合调度入口**最先**执行；可因澄清 / 待确认 / QA 等**提前结束本回合**或写入 `intent_turn_gate_hint` 等 | **无关** |
...
**澄清 / 待确认与工具收窄**：当 **`intent_at_turn_start`** 将本回合定为 **`ClarifyThenExecute`** 或 **`ConfirmThenExecute`** 时，在进入主循环前会设置 **`step_executor_constraint = ReviewReadonly`**（与 **`qa.readonly`** 等只读收窄一致）。**L2 不可用或低置信 fail-open** 时同样收窄为只读并注入短 hint（确认续接 / 工具失败后续跑除外；Act 下文案提示可用 **`/mode act`** 确认后重试写操作）。**`ReviewReadonly` / `PatchWrite` / `TestRunner` 均允许用户已启用的 `mcp__*` 代理工具**（仍不参与并行只读批）。若 **`intent_at_turn_start_enabled=false`**（默认）或本回合为 **Ask/Plan**，**不调用 L2**。
...
- **Web**：`POST /chat*` 可选 **`session_mode`**。底栏 **Ask / Plan / Act** 三段切换；偏好 **`prefs.session_mode`** 记忆上次选择；`GET /status` 暴露 **`default_session_mode`** 与 **`agent_role_default_session_modes`**；`GET /conversation/messages` 返回 **`active_session_mode`**。Ask/Plan 在意图门控之后挂只读约束。回合成功落盘时写入/保留 **`active_session_mode`**。
- **与 L2**：L2 **默认关**（退役 R1）。门控关或不显式开 `intent_l2_enabled` 时不跑 L2；仅显式开启且为 Act 时 L2 才建议 `suggested_mode`（观测用，不自动改 mode；Web 不渲染气泡）。
