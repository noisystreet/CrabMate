...
| `CM_SKILLS_ENABLED` | 是否启用 skills 注入（默认 `true`；L2 索引与 L5 按轮正文、以及 **`/<skill-id>` 强制选用** 都受其控制，读取 `skills_dir/*.md`）。 |
