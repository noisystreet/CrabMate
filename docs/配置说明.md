...
| `CM_INTENT_L2_ENABLED` | 是否启用默认 L2 无工具语义意图分类（**额外** `chat` 一次；失败回退弃用规则层，**默认开**）。同义 TOML 键 **`intent_l2_enabled`**；设为 `false` 可关。失败时 `intent_analysis` 会记录脱敏原因（如 `api_key_missing` / `transport_error` / `json_parse_failed`）。 |
| `CM_INTENT_L2_MIN_CONFIDENCE` | L2 分类 `confidence` 的观测阈值（0.0–1.0，默认 0.7）；低于该值仍采纳 L2，只在日志/时间线标记低置信。TOML：`intent_l2_min_confidence`。 |
| `CM_INTENT_L2_MAX_TOKENS` | L2 分类请求的 `max_tokens`（32–1024，默认 384）。TOML：`intent_l2_max_tokens`。 |
| `CM_INTENT_L0_ROUTING_BOOST_ENABLED` | 是否按 L0 特征（路径/错误/cargo 等）对弃用 L1 兜底的模糊句做**保守**提级（默认开）。TOML：`intent_l0_routing_boost_enabled`。 |
| `CM_INTENT_EXECUTE_LOW_THRESHOLD` | 弃用规则兜底路由「确认后执行」低阈值（0.0–1.0，默认 0.2）。TOML：`intent_execute_low_threshold`。 |
| `CM_INTENT_EXECUTE_HIGH_THRESHOLD` | 弃用规则兜底路由「直接执行」高阈值，且**不小于**低阈值（默认 0.45）。TOML：`intent_execute_high_threshold`。 |
| `CM_INTENT_NON_HIER_EXECUTE_LOW_THRESHOLD` | 意图路由「确认后执行」低阈值覆盖；省略时回退 `CM_INTENT_EXECUTE_LOW_THRESHOLD`。TOML：`intent_non_hier_execute_low_threshold`。 |
| `CM_INTENT_NON_HIER_EXECUTE_HIGH_THRESHOLD` | 意图路由「直接执行」高阈值覆盖，且不小于 non-hier low；省略时回退 `CM_INTENT_EXECUTE_HIGH_THRESHOLD`。TOML：`intent_non_hier_execute_high_threshold`。 |
...
| **`intent_non_hier_execute_low_threshold` / `intent_non_hier_execute_high_threshold`** | 同上两阈值的覆盖键；省略则回退 `intent_execute_*` | **无关** |
...
**澄清 / 待确认与工具收窄**：当 **`intent_at_turn_start`** 将本回合定为 **`ClarifyThenExecute`** 或 **`ConfirmThenExecute`** 时，在进入主循环前会设置 **`step_executor_constraint = ReviewReadonly`**（与 **`qa.readonly`** 等只读收窄一致），降低该阶段误触写类工具的概率。**`ReviewReadonly` / `PatchWrite` / `TestRunner` 均允许用户已启用的 `mcp__*` 代理工具**（仍不参与并行只读批）。若 **`intent_at_turn_start_enabled=false`**，意图管线仍可打时间线，但会**清除**上述工具收窄副作用。
