...
若 **`api_base`** 指向火山 **`*.volces.com`** 域名（例如 **`https://ark.cn-beijing.volces.com/api/coding/v3`**），CrabMate **不按 Moonshot 官方 Kimi 规则**组装出站请求，避免向方舟写入其不支持的 **`thinking`** 等扩展字段而引发 HTTP **400 InvalidParameter**。**`model`** 仍以控制台/文档为准（如 **`Kimi-K2.6`**）。**`API_KEY`** 使用方舟发放的密钥（**`llm_http_auth_mode = bearer`**）。
