...
- **`API_KEY`**：进程内 **`serve` / `repl` / `chat` / `tui`** 启动时读取密钥并保存在内存（可为空）。优先级：**环境变量 `API_KEY`** → 本机 **`$XDG_DATA_HOME/crabmate/secrets/client_llm`**（与 Web/桌面侧栏保存同源）→ 启动后再用 **REPL/TUI `/api-key set`**（仅本进程，**`/config reload` 不会清除**）。**热重载不**重读环境 **`API_KEY`**，也**不**重读磁盘 secrets。**`crabmate models` / `probe`** 在 **`bearer`** 下仍要求启动前环境变量 **`API_KEY`** 非空。
...
| 环境变量 | 说明 |
| --- | --- |
| `API_KEY` | 云厂商 / OpenAI 兼容后端的 Bearer token；`llm_http_auth_mode=bearer`（默认）时用于发往 `chat/completions` 等。**不写 TOML**。**`serve` / `repl` / `chat` / `tui` 可在未设置时启动**；对话前须有关密钥：**本环境变量**、本机 **`secrets/client_llm`**（Web 侧栏保存）、或 **REPL/TUI `/api-key`**。**`models` / `probe` 子命令**在 bearer 下仍要求此处非空。`llm_http_auth_mode=none`（如本地 Ollama）时可不设。 |
...
| `CM_CRABMATE_USER_DATA_DIR` | 覆盖本机用户数据根（绝对路径）；默认 **`$XDG_DATA_HOME/crabmate`**（Linux 多为 **`~/.local/share/crabmate`**）。存放 Web 侧栏会话、壳层偏好（含 **`prefs.recent_workspace_roots`** / **`last_workspace_root`**）、LLM 非机密覆盖与 **`secrets/`** 密钥；经 **`/user-data/*`** 读写。见 **`docs/design/user_data_dir.md`**。 |
...
然后可不设环境变量 **`API_KEY`** 即启动 `serve` / `repl` / `chat` / `tui`。若使用默认 **`llm_http_auth_mode=bearer`**（云端网关）而未 export **`API_KEY`**，进程仍可启动；CLI/TUI 会回退本机 **`secrets/client_llm`**（Web 侧栏保存），或使用 **`/api-key set …`**；皆无时对话会报缺密钥（Web 为 **`LLM_API_KEY_REQUIRED`** 等）。**工具调用（function calling）**依赖模型与 Ollama 版本；若不稳定可先 **`--no-tools`** 验证对话。`crabmate config`（自检）**不要求** **`API_KEY`**。
