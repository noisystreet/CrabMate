...
仓库内默认正文含**指令优先级**（安全 → 用户当轮明确指令 → 可核对事实 → 本提示与角色 → Cursor 式规则）、**非代码问题勿强行使用开发工具**、**用户未授权则默认不擅自改代码**、工具与任务拆分等约定（例如**同一工作区路径在未被修改前不要重复 `read_file`**），并约定用户以中文为主时助手**解释性文字用简体中文、减少中英夹杂**（代码与专有名词等除外）。**沟通规范**中含**问题类型对齐**（如「哪里/对比/排序」类先给简短清单再展开）、**收窄范围时须标明**、**元对话须复述用户原话**（勿改写成自拟技术标题）等。若规则合并段出现**截断提示**，不得假定未见条文。完全自定义时可改 `config/prompts/default_system_prompt.md` 或换用自有路径；各角色 **`config/prompts/*_system_prompt.md`** 的「沟通规范」与默认稿保持同类约定。
