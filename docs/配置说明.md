...
在全局 **`system_prompt`**（通用 L0，默认 **`config/prompts/base_system_prompt.md`**）之外，可为**命名 id** 配置不同的首条 `system` 正文。每条角色的 **`system_prompt_file`** 仅需写**增量**；**`finalize`** 会先叠加通用 L0，再按全局 **`coding_workbench_enabled`** 与角色 **`prepend_coding_workbench`**（默认 true）决定是否叠加 **L0b 编程工作台层**，再经 **`cursor_rules_*`** 与 **`skills_*`** 合并。
