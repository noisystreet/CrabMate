...
| `CM_INTENT_L2_ENABLED` | **历史键**（L2 退役 **R2**：生产路径已拆除，**不再**发起额外 `chat`）。仍可读入配置；R3 再 deprecate/删除。TOML：`intent_l2_enabled`（默认 `false`）。见 `agent_space/l2-retirement-plan.md`。 |
| `CM_INTENT_L2_MIN_CONFIDENCE` | **历史键**（R2 起无决策作用）。TOML：`intent_l2_min_confidence`。 |
| `CM_INTENT_L2_MAX_TOKENS` | **历史键**（R2 起无请求发出）。TOML：`intent_l2_max_tokens`。 |
| `CM_INTENT_AT_TURN_START_ENABLED` | 是否启用回合起点意图门控（默认同 TOML **`intent_at_turn_start_enabled=false`**）。**`false`**：跳过门控启发式。**`true`** 且会话为 **Act**：仅跑用户句**关键词执行约束**（见下「门控工具收窄」）；**不**再额外 `chat`、**不**发 `intent_analysis`、**不**因「无 L2」一律临时只读。Ask/Plan 时跳过门控启发式（只读由 mode 在门控后挂载）。 |
...
| --- | --- | --- |
| **`intent_execute_low_threshold` / `intent_execute_high_threshold`** | **历史键**，L1 路由已移除后**不再**划分确认/直执（金样 fail-open 管线仍可读） | **无关** |
| **`intent_non_hier_execute_low_threshold` / `intent_non_hier_execute_high_threshold`** | 同上覆盖键；省略则回退 `intent_execute_*` | **无关** |
| **`intent_at_turn_start`（门控）** | 回合调度入口**最先**执行；开门控 + Act 时仅关键词启发式（可写 `intent_turn_gate_hint` / `ReviewReadonly`）；**不会**提前结束本回合 | **无关** |
...
**门控工具收窄（L2 退役 R2）**：开门控且 **Act** 时，仅当用户句同时命中「不要改 / 不要跑」类词**与**分析/解释类词时，才设置 **`step_executor_constraint = ReviewReadonly`** 并注入短 hint；普通写/修/编译请求**不**再因「无 L2」一律临时只读。旧 Clarify / Confirm / DirectReply 早退与 L2 fail-open 只读路径已拆除。需要默认只读请用 **Ask/Plan**。**`ReviewReadonly` / `PatchWrite` / `TestRunner` 均允许用户已启用的 `mcp__*` 代理工具**（仍不参与并行只读批）。
...
- **与 L2**：L2 调用已拆除（退役 **R2**）；能力边界靠 Ask/Plan/Act。配置中 `intent_l2_*` 为历史键，待 R3 清扫。
