...
| `CM_PLANNER_EXECUTOR_MODE` | 有效值仅为 **`single_agent`**（ReAct）。省略则同默认。TOML：`planner_executor_mode`。**已移除**：`logical_dual_agent` / `hierarchical`（写入将导致配置加载失败）。`intent_mode_bias_enabled` / `CM_INTENT_MODE_BIAS_ENABLED`、`llm_fold_system_into_user`、**`intent_l2_*` / `CM_INTENT_L2_*`**、**`intent_execute_*_threshold` / `CM_INTENT_EXECUTE_*`**、**`intent_non_hier_execute_*` / `CM_INTENT_NON_HIER_*`**、**`intent_l0_routing_boost_enabled` / `CM_INTENT_L0_ROUTING_BOOST_ENABLED`** 亦已移除（`deny_unknown_fields`）。 |
