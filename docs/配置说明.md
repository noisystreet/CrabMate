...
**`worbrow`**：链接 [worbrow](https://crates.io/crates/worbrow) **≥0.1.1** 库，优先使用本机 **Firefox**，否则 **Chrome/Edge/Chromium**；引擎链默认 **`bing,duckduckgo`**（验证码/低产时降级）；结果含解包装后的落地 URL 与 `domain`。无浏览器时请改用 `brave`/`tavily` 或安装浏览器。Docker SyncDefault 沙盒内通常**没有**宿主浏览器，请改 API 供应商或勿在沙盒内依赖联网搜索。
