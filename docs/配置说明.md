...
| `CM_INTENT_L2_ENABLED` | 是否启用 L2（**额外**无工具 `chat` 一次）。仅当 **`intent_at_turn_start_enabled=true`** 且本回合 **`session_mode=act`** 时才会实际调用；输出 JSON **`suggested_mode`**（`ask`/`plan`/`act`，仅时间线/UI 提示，**不**自动改用户 mode）。不可用或置信不足时 **fail-open 进主模型**，门控开启时 **临时只读**。**默认开**。TOML：`intent_l2_enabled`。 |
...
- **与 L2**：默认关门控时不跑 L2；门控开且为 Act 时 L2 仅建议 `suggested_mode`（时间线展示），不自动改 mode。Act 下 fail-open 临时只读时提示 `/mode act` 确认后再写。
