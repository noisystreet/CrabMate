...
- **默认**：嵌入的 **`config/default_config.toml`** 使用 **`system_prompt_file = "config/prompts/base_system_prompt.md"`**（**领域无关**的通用 L0；运行时读盘，**修改该 Markdown 无需重新编译**）。**`finalize`** 另叠加 **`config/prompts/coding_workbench_increment.md`**（编程 / Git / 开发工具；默认全局会话与 engineer / scientist / code_reviewer 等角色；**不**叠加于 companion / philosopher / literary）。
...
在全局 **`system_prompt`**（通用 L0，默认 **`config/prompts/base_system_prompt.md`**）之外，可为**命名 id** 配置不同的首条 `system` 正文。每条角色的 **`system_prompt_file`** 仅需写**增量**；**`finalize`** 会先叠加通用 L0（及除 companion / philosopher / literary 外的**编程工作台层**），再经 **`cursor_rules_*`** 与 **`skills_*`** 合并。
