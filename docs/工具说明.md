...
- `workflow_execute`（DAG 工作流执行：并行/串行、审批、SLA、失败补偿）：
  - **内置模板（减少手写 `nodes`）**：在 **`workflow`** 对象中设 **`workflow_template`**（当前仅 **`rust_ci_light`**）可展开为 **`cargo_fmt_check` → `cargo_check`（`all_targets`）→ `cargo_clippy`（`all_targets`）→ `cargo_test`** 的串行 DAG；可与手写 **`nodes`** 二选一。若同时提供 **`workflow_template`** 与其它键（如 **`fail_fast`**、**`max_parallelism`**），则先展开模板再被其余键覆盖；若 **`workflow`** 中含 **`nodes`**，则覆盖模板中的节点列表。
