...
- `modify_file`（**局部改**优先 `mode=replace_lines`：行号区间 + `content`，流式改写不落整文件到内存；**`dry_run=true`** 时仅返回 diff、不写盘）：
  ```json
  {"path":"src/huge.rs","mode":"replace_lines","start_line":120,"end_line":135,"content":"// 新片段\n"}
  ```
  **`mode=full`（默认）或 `mode=overwrite`：整文件覆盖**（二者等价，`overwrite` 仅强调「整盘替换」语义）：`content` 即为磁盘上的**全部**新正文；若不完整将**永久截断**原文件。若命中高危启发式（显著缩短、大量删行、清空非空文件等），须 **`confirm_full_overwrite=true`** 才执行写入；可先 **`dry_run=true`** 预览 unified diff。局部改动请继续优先 `replace_lines` 或 `search_replace`。
- `read_dir`（列出目录内容）：
  ```json
