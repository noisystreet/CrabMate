...
- `modify_file`（**局部改**优先 `mode=replace_lines`：行号区间 + `content`，流式改写不落整文件到内存）：
  ```json
  {"path":"src/huge.rs","mode":"replace_lines","start_line":120,"end_line":135,"content":"// 新片段\n"}
  ```
  **`mode=full`（默认）= 整文件覆盖**：`content` 即为磁盘上的**全部**新正文；若模型只贴了片段或 JSON 被截断，会**永久截断**原文件，工具侧无法自动恢复（请依赖 Git/备份）。仅在你已确认 `content` 为完整新文件时使用 `full`。
- `read_dir`（列出目录内容）：
  ```json
