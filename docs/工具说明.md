...
- `workflow_execute`（DAG 工作流执行：并行/串行、审批、SLA、失败补偿）：
  - **按工作区文件执行**：参数顶层 **`workflow_file`**（相对工作区路径，如 **`.crabmate/workflows/ci.yaml`**；支持 `.yaml`/`.yml` 与含 `` ```crabmate-workflow `` 的 Markdown）。服务端编译作者层 `steps`（含 `when` / `for_each`）后执行；可选 **`workflow`** 对象覆盖 `fail_fast` 等（不覆盖 `nodes`/`steps`）。CLI 等价命令：**`crabmate workflow run <path>`**（不要求 `API_KEY`）。示例文件见仓库 **`.crabmate/workflows/ci.yaml`**。
