...
**`run_command` 超时与取消**：墙上时钟默认 **`command_timeout_secs`**（可用 **`CM_COMMAND_TIMEOUT_SECS`** 覆盖）。到期、用户取消或 SSE 关闭时，对子进程**进程组**发送 SIGTERM，短等待后 SIGKILL（Unix）；工具正文带已截断的部分 stdout/stderr，信封 **`error_code`** 为 **`timeout`** / **`cancelled`**（`timeout` 启发式 `retryable`）。workflow 节点目前仅 **`run_command`** 在工具内 reap；其它节点超时仍可能留下 `spawn_blocking` 子进程。Docker 沙盒内进程不在本刀保证范围。实现见 **`docs/design/long_running_tool_execution_todo.md`**。
