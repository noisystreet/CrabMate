...
- `diagnostic_summary`（脱敏排障：工具链、`target/`、关键 env 是否设置，**不输出任何 env 取值**）：
  ...
  ```
- `present_clarification_questionnaire`（**澄清问卷**：参数合法且工具成功时，Web 流式路径在 **`tool_result` SSE 之后**再下发控制面 **`clarification_questionnaire`**，用户下一条 **`POST /chat`** / **`POST /chat/stream`** 可带 **`clarify_questionnaire_answers`**；**`crabmate tui`** 弹出全屏表单收集答案并并入下一轮用户消息（语义与 Web 字段对齐）；**`repl` / `chat`** 尚无表单 UI，仅见工具正文）：
  ```json
  {
