...
**复合 shell（`&&` / `||` / `|` / `;` 等）**：二者均按「**`command` 仅为可执行名或工作区内相对路径** + **`args` 字符串数组**」传参。**禁止**把含上述操作符的表达式拆成多个 `args`（普通 `execve` 不会按 shell 解析）。应使用 **`command`: `bash` 或 `sh`，`args`: `["-c", "整段脚本一行"]`**（嵌入默认 **`allowed_commands`** 含 **`bash`**、**`sh`**；生产收窄白名单时请按需保留）。**`terminal_session`** 首轮 **`exec`** 同理；若 PTY 内已是交互式 shell，也可把整行命令写入 **`input`**。

**`run_command` 先换工作区子目录**：可用 **`command` 为 `cd`**、**`args` 为 `["相对子目录","&&",后续命令名,…]`** 的形式（`相对子目录` 须已存在；路径穿越形 `..`/`../` 与工作区外绝对路径默认经 **`allow_external_path_with_approval`** 人工审批后放行，设 `false` 则仍拒绝；**git** `A..B` 不算穿越）。实现会换目录后执行后续参数，**不经 shell**，避免把 `cd` 当独立可执行时报「不存在」。更复杂的 shell 逻辑仍须 **`bash`/`sh` + `-c`**。
...
- **内置多种工具，由模型按需调用**：
  ...
  - `crate_contract_map`：只读**契约地图**：对工作区内与 **HTTP/SSE/配置/前后端对齐** 相关的固定锚点（如 `src/lib.rs`、`docs/SSE协议.md`、`frontend/src/api/mod.rs`、`frontend/src/api/chat_stream/parser_v2.rs`、`crates/crabmate-sse-protocol/control_classify.rs`、`fixtures/sse_control_golden.jsonl`、`config/default_config.toml`、`docs/配置说明.md`、`docs/命令行与路由.md`）输出 **Markdown** 小节；每节含**文件头预览**与少量**关键字匹配行节选**（不执行命令、不联网、不读 `.env`）。可选 `head_lines_per_file`、`keyword_hits_per_file`、`extra_paths`（相对路径，仅头预览）、`max_extra_paths`。与 `repo_overview_sweep` 互补：本工具偏「协议与路由入口」，后者偏「仓库骨架与画像」。
