...
**终答 `agent_reply_plan` v1**（在工作流反思等路径下由服务端校验）：`steps[].id` 须唯一且符合稳定字符规则（见 **`docs/开发文档.md`**）；可选 **`workflow_node_id`** 用于与最近一次 **`workflow_execute`** 工具结果里的 **`nodes[].id`** 对齐（子集校验），便于规划与 DAG 节点一一对应。分阶段规划下还可选 **`steps[].executor_kind`**（`review_readonly` / `patch_write` / `test_runner`）以步级收窄可见工具并拒绝越权调用（越权时工具结果含本步允许名摘要）；**`[tool_registry]`** 可用 **`sub_agent_*`** 键扩展默认补丁/测试名单或只读步拒绝表，详见 **`docs/配置说明.md`**（分阶段规划与 `tool_registry`）。**架构扩展**：显式「规划—执行—验证」闭环、与 **`plan_rewrite` / 工作流反思** 的边界及演进阶段见 **`docs/规划执行验证架构.md`**（设计稿，**不**改变本节字段约定）。
...
验收失败时触发**补丁规划轮**（按 `staged_plan_feedback_mode` 配置；仓库嵌入默认 **`patch_planner`**，见 **`config/planning.toml`**），可通过 **`max_step_retries`** 控制本步重试次数，通过 **`transitions`** 定义 `on_verify_fail` 等条件跳转。失败文案常见机器可读前缀包括 **`exit_code_mismatch:`**、**`stdout_missing:`** / **`stderr_missing:`**、**`combined_output_missing:`**（分层子目标合并输出场景）、**`file_not_found:`**、**`json_path_mismatch:`**、**`http_status_mismatch:`** 等，补丁 **user** 中会随 `step_verifier` 结果一并回灌。详见 **`docs/规划执行验证架构.md`** 附录示例与文首「可复制 JSON 片段」。

**分阶段闭环默认范式（配置 + 规划 JSON，占位符）**：嵌入 **`staged_plan_feedback_mode = "patch_planner"`**、**`staged_plan_patch_max_attempts = 3`**；模型在规划轮为每步填写 **`executor_kind`**（读→写→测）、可选 **`acceptance`** 与 **`max_step_retries`**。服务端默认规划 system（`staged_plan_phase_instruction_default`）与 **`plan_artifact::PLAN_V1_EXAMPLE_JSON`** 已偏向 **`test_runner` + `acceptance`（退出码 + stdout 锚点）** 范式，与下列片段一致思路。下列片段可逐轮粘贴（单步滚动规划下每轮通常 1 条 `steps`）。
...
```json
{
  ...
  "steps": [
    {
      "id": "read-only-survey",
      "description": "只读扫描占位路径 YOUR_ANCHOR_FILE.md",
...
```json
{
  ...
  "steps": [
    {
      "id": "run-checks",
      "description": "运行占位命令并验收 stdout/退出码",
...
**`run_command` 先换工作区子目录**：可用 **`command` 为 `cd`**、**`args` 为 `["相对子目录","&&",后续命令名,…]`** 的形式（`相对子目录` 须已存在且仍在工作区内，**禁止** `..` 与绝对路径；实现会换目录后执行后续参数，**不经 shell**，避免把 `cd` 当独立可执行时报「不存在」）。更复杂的 shell 逻辑仍须 **`bash`/`sh` + `-c`**。
...
- **内置多种工具，由模型按需调用**：
  ...
  - `web_search`：**联网网页搜索**（[Brave Search API](https://brave.com/search/api/) 或 [Tavily](https://tavily.com/)），需在配置中填写 `web_search_api_key` 并设置 `web_search_provider`（`brave` / `tavily`）；未配置 Key 时工具会返回说明性错误。仓库内**精确**字符串/正则匹配优先用 `search_in_files`；**混合全文+语义**检索用 `codebase_semantic_search`（SQLite **FTS5** + 向量，默认 `retrieve_mode: hybrid`；须先 `rebuild_index`，见下文与 **`docs/配置说明.md`**）。
  ...
  - `crate_contract_map`：只读**契约地图**：对工作区内与 **HTTP/SSE/配置/前后端对齐** 相关的固定锚点（如 `src/lib.rs`、`docs/SSE协议.md`、`frontend/src/api/mod.rs`、`frontend/src/sse_dispatch/dispatch.rs`、`crates/crabmate-sse-protocol/control_classify.rs`、`fixtures/sse_control_golden.jsonl`、`config/default_config.toml`、`docs/配置说明.md`、`docs/命令行与路由.md`）输出 **Markdown** 小节；每节含**文件头预览**与少量**关键字匹配行节选**（不执行命令、不联网、不读 `.env`）。可选 `head_lines_per_file`、`keyword_hits_per_file`、`extra_paths`（相对路径，仅头预览）、`max_extra_paths`。与 `repo_overview_sweep` 互补：本工具偏「协议与路由入口」，后者偏「仓库骨架与画像」。
...
- `gh_pr_create`（须已 push 源分支；**在 GitHub 创建 PR**；省略 `body` 时默认 `auto_body`）：
  ...
  ```
- `gh_pr_merge` / `gh_pr_review` / `gh_pr_comment`（**写远端**）：
  ```json
...
- `workflow_execute`（DAG 工作流执行：并行/串行、审批、SLA、失败补偿）：
  ...
  - **节点工具角色（可选）**：在节点对象上可设 **`node_tool_role`**（**`review_readonly` / `patch_write` / `test_runner`**），与分阶段规划 JSON 的 **`executor_kind`** 共用 **`agent::step_executor_policy`** 的允许表（亦可写 **`executor_kind`** 作为别名）；若设置，DAG 执行该节点前会校验 **`tool_name`** 是否属于该角色允许集合，越权则节点失败（**`error_code`**：`workflow_node_tool_role_denied`）。**不**替代 **`validate_only`** 的结构规划，也**不**替代分阶段在 **`outer_loop`** 中对模型可见 **`tools`** 列表的收窄。
