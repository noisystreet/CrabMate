...
- **内置多种工具，由模型按需调用**：
  ...
  - `web_search`：**联网网页搜索**。默认 **`web_search_provider = worbrow`**（经 [worbrow](https://crates.io/crates/worbrow) **≥0.1.1** 驱动本机 **Firefox / Chrome/Edge** headless；引擎链 **bing→duckduckgo**；**无需 API Key**；亦接受别名 `browser`）。可选 **`brave`**（[Brave Search API](https://brave.com/search/api/)）或 **`tavily`**（[Tavily](https://tavily.com/)），此时须配置 `web_search_api_key` / `CM_WEB_SEARCH_API_KEY`。无可用浏览器且未配置 API 路径时工具返回说明性错误。仓库内**精确**字符串/正则匹配优先用 `search_in_files`；**混合全文+语义**检索用 `codebase_semantic_search`（SQLite **FTS5** + 向量，默认 `retrieve_mode: hybrid`；须先 `rebuild_index`，见下文与 **`docs/配置说明.md`**）。
