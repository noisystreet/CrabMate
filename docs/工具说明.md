...
**分阶段闭环默认范式（配置 + 规划 JSON，占位符）**：嵌入 **`staged_plan_feedback_mode = "patch_planner"`**、**`staged_plan_patch_max_attempts = 3`**；模型在规划轮为每步填写 **`executor_kind`**（读→写→测）、可选 **`acceptance`** 与 **`max_step_retries`**。下列片段可逐轮粘贴（单步滚动规划下每轮通常 1 条 `steps`）。
