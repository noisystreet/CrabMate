...
验收失败时触发**补丁规划轮**（按 `staged_plan_feedback_mode` 配置；仓库嵌入默认 **`patch_planner`**，见 **`config/planning.toml`**），可通过 **`max_step_retries`** 控制本步重试次数，通过 **`transitions`** 定义 `on_verify_fail` 等条件跳转。详见 **`docs/规划执行验证架构.md`** 附录示例与文首「可复制 JSON 片段」。
