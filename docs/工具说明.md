...
- **内置多种工具，由模型按需调用**：
  ...
  - **GitHub CLI 封装**（须本机 **`gh`** 且 **`allowed_commands` 含 `gh`**）：`gh_pr_list` / `gh_pr_view` / **`gh_pr_checks`**（CI 检查状态）/ **`gh_pr_create`**（创建 PR，**写远端**；正文经工作区内临时 `--body-file`）/ **`gh_pr_diff`**、`gh_issue_*`、`gh_run_list` / **`gh_run_view`**（`log`+`job` 拉日志，受输出长度截断）、**`gh_release_list` / `gh_release_view`**、**`gh_search`**（**`scope` 仅 `issues`|`prs`|`repos`**，`query` 长度与字符受限；`repos` 时勿传 `repo`）、`gh_api` 等。**退出码 0** 且 **stdout 整段为合法 JSON** 时，输出末尾附加**缩进 JSON**（不必依赖是否传入 `fields`）。`gh_api` 写方法、`gh_pr_create` 为非只读。通用 `run_command` 仍可拼其它 `gh` 子命令。
...
- `gh_pr_checks`（当前分支关联 PR 或指定编号）：
  ...
  ```
- `gh_pr_create`（须已 push 源分支；**在 GitHub 创建 PR**）：
  ```json
  {"title":"fix: 修复空指针 / fix null deref","body":"## 摘要\n- …\n\n## 测试\n`cargo test`"}
  ```
  ```json
  {"repo":"octocat/Hello-World","title":"feat: API","body":"详见 commits","base":"main","head":"my-feature","draft":true}
  ```
