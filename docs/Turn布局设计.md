...
TUI **`sse_mirror`**：`turn_segment_*` 与已投影的工具/`timeline_log`（`intent_analysis` 等）**不**写入 `[SSE 控制面]` 附录，但经 **`turn_project`** 喂入同一套 `TurnReducer`（见 **`docs/design/tui_align_tauri_display.md`** Phase 3）。附录默认仅错误、思维迹与其余 timeline。
