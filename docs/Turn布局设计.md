...
- hydration：same-revision 守卫仍保留（E4 前不删）。回合保存已把 **`layout`** 写入 SQLite / 内存（由落盘 `Message[]` 派生）；官方 Web 在 B3 前仍走 legacy hydration。旧库行若 `layout_meta_json` 为空则 GET 省略该键。
