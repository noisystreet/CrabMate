...
| `workflow compile` | 将 **`workflow_spec` v2** YAML（或含 `` ```crabmate-workflow `` 的 `.md`）编译为 **`workflow_execute`** 用的 `{"workflow":{…,"nodes":[…]}}` JSON 并打印到 stdout（**不要**求 `API_KEY`）。MVP 支持 `steps` + `after`；`when` / `for_each` 见 **`docs/工作流Markdown作者层设计.md`**。 |
| `workflow validate` | 同上编译后做 DAG 拓扑与工具参数 schema 校验；默认人读摘要，`--json` 输出层与节点表。 |
| `workflow run` | 编译并**执行**工作区内工作流文件（等同 Agent `workflow_execute` + **`workflow_file`**；**不要**求 `API_KEY`）。例：`cargo run -- workflow run .crabmate/workflows/ci.yaml`。 |
