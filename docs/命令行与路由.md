...
查看帮助：`crabmate --help`、`crabmate help`、`crabmate help <子命令>`（与 `--help` 等价）。根级与 **`chat --help`** 文末含 **`docs/命令行契约.md`**、**`docs/SSE协议.md`** 交叉引用。**全局选项**写在子命令**之前**：`--config`、`--workspace`、`--agent-role`、`--no-tools`、`--log`。
