...
`RUST_LOG=crabmate=debug` 时每次调用模型前打印 **`message_pipeline session_sync`** 汇总；更细：`RUST_LOG=crabmate::message_pipeline=trace`。说明与 **`GET /status`** 计数见 **`docs/开发文档.md`**「**架构设计** → **上下文管道（观测）**」；实现见 `src/agent/message_pipeline.rs`。
