...
| 能力 | Web（`serve`） | CLI |
|------|----------------|------------------------|
| **会话持久** | 可选 SQLite（`conversation_store_sqlite_path`）+ `conversation_id`，多会话、进程重启可续聊（受 TTL/条数上限等约束，见 `docs/开发文档.md`）。 | **部分等价**：**交互式 CLI** 可选从工作区 **`.crabmate/tui_session.json`** 启动时加载/退出时保存（`tui_load_session_on_start` / `tui_session_max_messages`），为**单条会话链**文件，**不是** Web 的按 `conversation_id` 多会话库。`chat` 单次或批跑**不**自动跨命令持久化；需自行用 `--messages-json-file` 等传入上下文。另：**`repl_initial_workspace_messages_enabled`**（默认 false，见 `docs/配置说明.md`）为 true 时 **CLI** 才在后台构建 **`initial_workspace_messages`**（项目画像、依赖摘要，并参与上述磁盘恢复逻辑）；为 false 时启动仅一条 `system`，不跑 tokei / `cargo metadata` 等。 |
