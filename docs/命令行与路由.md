...
未设置 `RUST_LOG` 时：`serve` 默认 **info**；`repl` / `chat` / `tui` / `bench` / `config` / `mcp` / `save-session`（及别名 `export-session`）/ `tool-replay` / `plugin` 默认 **warn**。可用 `RUST_LOG` 或 `--log <FILE>`。
...
未写子命令时仍可用 `--serve`、`--query`、`--benchmark`、`--dry-run` 等，内部映射为对应子命令。若参数中**任意位置**出现显式子命令名（如 `serve` / `doctor` / `tui` / `save-session` / `export-session` / `tool-replay` / `plugin`），则整段 argv 不再插入默认 `repl`（与 `tests/fixtures/cli/legacy_normalize.json` 契约一致）。
