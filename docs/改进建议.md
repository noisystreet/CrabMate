...
| 改进项 | 说明 | 状态 |
|--------|------|------|
| **Web API 共享密钥与网关分工** | 进程内为 **`web_api_bearer_token`** 共享密钥；按用户账号、配额、审计放在 **CrabMate 前的网关/BFF** | **已文档化**；`doctor` 含 serve 部署检查；写工具审计默认开启 |
| **工作区路径 TOCTOU** | Linux 上 **`openat2` + `RESOLVE_IN_ROOT`** 贯通读/写/删；写路径父目录 **`mkdirat`** | **已落地**（残余风险见 **`src/workspace/path.rs`**） |

> **驱动因素**：对外暴露前须明确「进程内只做服务级密钥、身份在网关」的边界；路径安全与工具白名单仍是本仓库内的硬要求。
...
| **MCP Server 暴露** | stdio server 已提供（`crabmate mcp serve`，见 `src/cm_mcp/mcp_impl/server.rs`）；剩余：streamable HTTP 出口与传输鉴权对齐（见 `docs/待办清单.md` tools/ 章） | 低-中（stdio 已落地） |
...
| **Token / 费用估算** | token 侧已部分落地（`tiktoken_prompt_tokens` 随 `stream_ended` / `conversation_saved` 暴露、Web 底栏上下文芯片）；剩余：上游 `usage` 元数据透出与费用估算 | 低（计算层改动） |
...
| 改进项 | 说明 | 实现难度 |
|--------|------|---------|
| **外部向量库支持** | Qdrant / pgvector adapter；解耦现有 `long_term_memory_store`，抽象为 trait；多租户键与网关身份方案同向（**`docs/未来规划功能.md`**） | 中（需存储层抽象） |
...
聚焦 **Web 鉴权与部署边界** + **Benchmark 评测落地**。

- 前者明确共享密钥 + 网关职责（见 **`docs/未来规划功能.md`**），后者建立能力基线为后续迭代提供客观度量。
- 具体行动：
  1. 确认生产路径已启用 **`web_api_require_bearer=true`** 与非空密钥；对外服务时在网关完成用户身份与配额，并审计所有写操作接口（`/chat/stream`、文件操作、Git 操作等）在「仅持服务密钥的调用方」模型下是否仍满足预期。
  2. 运行 `crabmate bench --benchmark swe_bench`，产出基线分数并记录到 `docs/BENCHMARK_RESULTS.md`。

### 中期（3–6 个月）

推进 **MCP Server 暴露** + **多 Agent 协作框架**设计。

- MCP stdio server 已随 `crabmate mcp serve` 落地；剩余 HTTP 传输形态可继续复用 `tool_registry` 导出能力。
- 多 Agent 协作可先做单进程内的逻辑角色分离扩展（如增加 `role` 字段到 Agent 配置），再演进为独立实例。
- 具体行动：
  1. 扩展 MCP 出口：在既有 stdio server（`crabmate mcp serve`，`src/cm_mcp/mcp_impl/server.rs`）之上，新增 streamable HTTP 传输与传输鉴权选项（对齐 Web bearer 策略）。
  2. 设计 Agent 实例抽象（组合面 `src/agent/`、领域核 `src/cm_agent/`），支持独立配置模型/工具/系统提示，引入共享消息队列供协作。
...
| MCP 双向 | MCP 生态 | ⏳ client 侧 + server stdio（`mcp serve`）已通；streamable HTTP 出口待实现 |
| Token/费用估算 | LangChain Usage Tracking | ⏳ token 预算/上下文芯片已落地；费用估算未实现 |
...
| API 鉴权（按用户账号） | — | ⏳ 进程外（网关/BFF）；见 **`docs/未来规划功能.md`** |
