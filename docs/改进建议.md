...
| 改进项 | 说明 | 实现难度 |
|--------|------|---------|
| **Web API 共享密钥与网关分工** | 进程内为 **`web_api_bearer_token`** 共享密钥（`serve` 启动校验、受保护路由 Bearer/`X-API-Key`）；**按用户账号、配额、审计**宜放在 **CrabMate 前的网关/BFF**，共识见 **`docs/未来规划功能.md`** | 低（运维/架构选型；进程内不实现账号体系） |
