...
**HTTP JSON（非 SSE `data:`）**：`POST /chat` 失败时 **`ApiError`** 含 **`code`**、**`message`**（用户可读）；可选 **`reason_code`** 仅在 **`code` 为 `INTERNAL_ERROR`** 时出现（截断内部摘要）。**SSE** 仍可在多种 `code` 下带 **`reason_code`**（见 **`docs/SSE协议.md`**）。握手阶段其它码以 **`web/chat_handlers`** 与 OpenAPI 为准；与 **SSE 协议版本**相关的补充码见 **[`docs/SSE协议.md`](SSE协议.md)**。
