...
- [ ] **编排层结构化（阶段模型、入参收敛、可测与可观测）**：
  ...
  - **收敛入参**：**已实现**：**`RunLoopParams`** 拆为 **`RunLoopCtx`**（整场固定上下文）与 **`RunLoopTurnState`**（`messages` / `sub_phase` / 模型与温度覆盖 / `intent_turn_gate_hint` / `step_executor_constraint`）；库入口 **`RunAgentTurnParams`** 拆 **`transport`**（**`AgentTurnTransport`**）与 **`llm`**（**`AgentTurnLlmOverrides`**）。后续仍可收紧 **`PerCoordinator`** 与会话侧可变状态的归属。
...
- [ ] **跨层级依赖传递增强（中低优先级）**（已实现：`SubGoal.consumes_from_dependencies` + 默认/白名单**注入裁剪**、`{ref:...}` 展开、步摘要、分解 prompt 的 I/O 契约与 `build_requirements` 解析；**仍待办/可增强**）：
  - 更细别名（如「仅 `goal_1` 的 executable 名 `foo`」而不仅是 `only_kinds` 子串）
  - 并行同层**可见**上一步同层产物的更一致策略（当前为 snapshot + 不合并同层彼此产物，符合 DAG 同层无依赖的假设）
