...
- [ ] **跨层级依赖传递增强（中优先级）**（`get_dependencies` 已改为每个前置子目标**全部**登记产物，并由 `HierarchicalExecutor` 按子目标分节注入 Operator；**仍待办**）：
  - 后续目标可能只需**某类**产物（`ArtifactKind` 筛选、避免上下文过长）
  - 需要支持依赖别名（如 `goal_5` 显式需要 `goal_1` 的 `executable` 而非全部登记项）
