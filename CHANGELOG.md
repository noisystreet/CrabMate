...
- **`run_command` approval SSE**: `command_approval_request.args` is argv **tail** only (not the full script that already includes argv0), so Client `command + " " + args` no longer shows `curl curl …`. Timeline detail skips re-prefixing when `args` already starts with `command`.
